...
FILE=`basename $DESTINATION`
  if [ ! -f $src_dir/audit/$FILE ] || [ $(grep -s -c $(shasum $src_dir/audit/$FILE | awk -F " " '{print $1}') $SUMFILE) -eq 0 ]; then
...
if [ ! -f /etc/apt/sources.list ] || [ $(grep -s -c $(shasum /etc/apt/sources.list|awk -F " " '{print $1}') $SUMFILE) -eq 0 ]; then

    echo "Creating backup of the sources.list" 
    mv -v /etc/apt/sources.list /etc/apt/sources.list.installbk.$(date +%d%b%Y_%T)

  fi

  echo "files is missing or out of date. Updating file"
  $XFER --output-document=/etc/apt/sources.list $REPLICA_OS_SRC/etc/apt/sources.list
