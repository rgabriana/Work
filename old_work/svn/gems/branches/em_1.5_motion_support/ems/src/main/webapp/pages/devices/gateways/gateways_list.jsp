		...
		<div id="gateway-dialog-form">
			...
			<c:if test="${dhcpEnable == 'true'}">		
				<form id="discoverGatewayButton" name="discoverGateway" method="post" action="${actionDiscoverURL}">
					<input type="submit" value="<spring:message code='gatewayForm.label.discoverBtn'/>" style="float: left; margin-right:5px" />
				</form>
			</c:if>
			<c:if test="${dhcpEnable == 'false'}">		
				<form id="addGatewayButton" name="addGateway" method="post" action="${actionAddGatewayURL}">
					<input type="button" onClick="return addGatewayDialog('${actionAddGatewayURL}')" value="<spring:message code='gatewayForm.label.addGatewayBtn'/>" style="float: left; margin-right:5px" />
				</form>
			</c:if>
			<form id="bulkGatewayCommissionButton" name="bulkCommissionGateway">
				<input type="button" onclick="parent.parent.showGatewayCommissioningForm(0)" value="<spring:message code='gatewayForm.label.bulkCommissionBtn'/>" style="float: left;" />
			</form>
			<div id="gwMessage">
				${message}
			</div>
			...
			colNames:["id", "Name", "Channel", "Network ID", "Version", "Commissioned", "No. of Fixtures", "Upgrade Status", "Action"],
		   	colModel:[
				{name:'id', index:'id', hidden: true},
		   		...
		   		{name:'channel', index:'channel', sortable:false, width:"11%"},
		   		{name:'wirelessNetworkId', index:'wirelessNetworkId', sortable:false, width:"11%"},
		   		{name:'app2Version', index:'app2Version', sortable:false, width:"11%"},
		   		{name:'commissioned', index:'commissioned', sortable:false, width:"11%"},
		   		{name:'noOfActiveSensors', index:'noOfActiveSensors', sortable:false, width:"11%"},
