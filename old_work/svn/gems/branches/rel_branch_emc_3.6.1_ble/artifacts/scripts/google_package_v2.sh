...
then
	echo "***********************setting ENL_APP_HOME hardcoded to /var/lib/tomcat6 as it is not still set************"
        export ENL_APP_HOME="/opt/tomcat"
fi
...
mkdir -p debian_ems_google$ENL_APP_HOME
mkdir -p debian_ems_google$ENL_APP_HOME/webapps
mkdir -p debian_ems_google$ENL_APP_HOME/conf
cp -r debian_ems_google/var/lib/tomcat6/Enlighted debian_ems_google$ENL_APP_HOME/
