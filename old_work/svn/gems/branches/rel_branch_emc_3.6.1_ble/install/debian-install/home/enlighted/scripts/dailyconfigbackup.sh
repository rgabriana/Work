...
sudo tar -cvpzf $backupfile  $OPT_ENLIGHTED/communicator /etc/network/interfaces /etc/timezone /etc/default/ntpdate /etc/ntp.conf /etc/default/dhcp3-server /etc/iptables.rules /etc/sudoers $ENL_APP_HOME/Enlighted --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/cloudServerInfo.xml --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/emsmode --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/ems_log4j --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/UpgradeImages --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/bacnet/bacnetLighting --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/tempExtract --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/bkRestoreFolderTemp --exclude=opt/enLighted/DB/DBBK --exclude=$ENL_APP_HOME_RELATIVE/Enlighted/adminlogs  --exclude=opt/enLighted/communicator/syncdata --exclude=opt/enLighted/communicator/last_communication_time --exclude=**/*.sh --exclude=**/*.log --exclude=**/*.log.* --exclude=**/*.jar --exclude=**/*.deb  --exclude=**/*.pl --exclude=**/*.bin
