...
public final class ArgumentUtils {
	...
	static public <E extends Enum<E>> boolean isInEnum(String value, Class<E> enumClass) {
	 ...
	 public static int isDurationInPeriod(Integer duration, Date startTime)
    {
		int interval = -1;
		boolean drActive =false;
		long currentTimeinMiliSec = Calendar.getInstance().getTime().getTime();
		long startTimeinMiliSec = startTime.getTime();
		long endTimeInMillis = startTimeinMiliSec + (duration*1000);
		
		if(duration == 0)
		{
			if(currentTimeinMiliSec >= startTimeinMiliSec)
				return 10 * 60;	// 600 seconds
			else 
				return 0;
		}
		//in seconds
		interval = (duration) - Integer.parseInt(new Long((currentTimeinMiliSec -startTimeinMiliSec)/1000).toString());
		if(currentTimeinMiliSec >= startTimeinMiliSec && currentTimeinMiliSec < endTimeInMillis)
		{
			drActive= true;
			//System.out.println(new Date() + ": DR ACtive " + interval);
		}
		if(drActive==false && interval>0)
		{
			//System.out.println("DR will Schedued in " + interval);
			return 0;
		}
    	return interval;
    }
