...
$().ready(function() {
	...
	$.validator.addMethod("arearegx", function(value, element, regexpr) {          
	    return regexpr.test(value);
	}, "Please enter a valid Area Name. Allowed Special Characters are @ # - _ : . ,");
	
	$.validator.addMethod("areadescregx", function(value, element, regexpr) {          
		if (value != "") {
			return regexpr.test(value);
		}
		return true;
	}, "Please enter a valid Area description. Allowed Special Characters are , @ # - _ : \\ / . * &");
	...
	<c:if test="${mode == 'admin'}">
		$(".maindiv").css("height", $(window).height() - 330);
		$(".maindiv").css("overflow", "auto");
	</c:if>
});
		...
		success: function(msg) {
			...
			$(msg).find('area').each(function() {
				areas[$(this).find('id').text()] = new Array($(this).find('name').text(), 
						$(this).find('description').text(),$(this).find('zonesensorenable').text(),$(this).find('occupancyState').text());
				/*
				$("<tr></tr>").html( 
						'<td id=' +
			...
			var buffer="";
			for (var key in areas) {
						buffer+="<tr><td id=" +	areas[key][0]  + "name >" + areas[key][0] + "</td> <td>" + areas[key][1] + "</td>";
						var occStatus = Number(areas[key][3]);
						if(occStatus >= Number("1")){ 
							buffer+='<td>Occupied</td>';
						}else if(occStatus == Number("0"))
						{
							buffer+='<td>Not Occupied</td>';
						}else
						{
							buffer+='<td>Not Available</td>';
						}
						buffer+='<td>'+areas[key][2] ;
						buffer+="</td><td class='btnsettings'> <input id=" +key + " type='button' onclick='edit(this);' value=<spring:message code='action.edit'/> />&nbsp;<input id="+ key +" type='button' onclick='deleteOrg(this);' value=<spring:message code='action.delete'/> /></td>";
						buffer+="</tr>";
			}
			$('#areaList').html(buffer);
		},
		error: function() {
			$("#submit").val("<spring:message code='action.create'/>");
			$("#tableContainer").hide();
...
function edit(obj) {
	...
	for (var key in areas) {
		if(areaid == key) {
			...
			<c:if test="${zoneSensorLicenseEnabled == 'true'}">
			if(areas[key][2] == "true"){
				$('#zoneSensorEnable').prop('checked', true);
			}else{
				$('#zoneSensorEnable').prop('checked', false);
			}
			</c:if>
		}
	}
...
<div class="maindiv">
					...
					<div class="field">
						...
						<div class="formValue">
							<form:input id="name" name="name"
								path="name" />
							<c:if test="${zoneSensorLicenseEnabled == 'true'}"><form:checkbox id="zoneSensorEnable" name="zoneSensorEnable" path="zoneSensorEnable" />  Enable as Zone Sensor</c:if>
						</div>
					</div>
					<div class="field">
						<div class="formPrompt">
			...
			<table cellpadding="0" cellspacing="0" class="entable"
				...
				<thead>
					<tr>
						...
						<th><spring:message code="areaSetup.header.floor.occupancystatus" /></th>
						<th><spring:message code="areaSetup.header.floor.zonesensorenabled" /></th>
	...
	<div class="navdiv" align="center">
		<c:if test="${mode != 'admin'}">
			...
			<spring:url value="/createPricing.ems" var="nextURL" scope="request" />
