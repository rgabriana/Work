...
<spring:url value="/services/org/fixtureclassservice/assignfixturetypetolocatordevice" var="assignFixtureTypeToLocatorDevicesUrl" scope="request" />
...
var totalFixtureRecordUpdated = 0;
...
function assignFixtureTypeToFixture(){
	...
	if(SELECTED_FIXTURES_TO_FIXTURE_TYPE.length == 0 && SELECTED_LOCATOR_DEVICES_TO_FIXTURE_TYPE.length > 0){
		assignFixtureTypeToLocatorDevice();	
	}else{
		...
		$.ajax({
			...
			success: function(data){
				totalFixtureRecordUpdated = data.status;
				if(SELECTED_LOCATOR_DEVICES_TO_FIXTURE_TYPE.length > 0){
					assignFixtureTypeToLocatorDevice();
				}else{
					if(totalFixtureRecordUpdated = 0){
						setFixtureTypeMessage("Assignment Failed.", "red");
					}else{
...
function assignFixtureTypeToLocatorDevice(){
		
	var selectedFixtureTypeId = $("#aft-fixturetype-combo").val();
	
	var locatorDeviceXML = "";
	for(var j=0; j<SELECTED_LOCATOR_DEVICES_TO_FIXTURE_TYPE.length; j++){
		var locatorDeviceJson = SELECTED_LOCATOR_DEVICES_TO_FIXTURE_TYPE[j];
		locatorDeviceXML += getlocatorDeviceXML(locatorDeviceJson.id);
	}
	var postData = getlocatorDeviceXML_LIST(locatorDeviceXML);
	
	setFixtureTypeMessage("Processing...", "black");
	
	$.ajax({
		type: 'POST',
		url: "${assignFixtureTypeToLocatorDevicesUrl}/"+selectedFixtureTypeId,
		data: postData,
		success: function(data){
			if(data.status == 0){
				if(totalFixtureRecordUpdated == 0){
...
function getlocatorDeviceXML(locatorDeviceId){
	return "<locatorDevice><id>"+locatorDeviceId+"</id></locatorDevice>";
}

function getlocatorDeviceXML_LIST(locatorDeviceXML){
	return "<locatorDevices>"+locatorDeviceXML+"</locatorDevices>";
}
