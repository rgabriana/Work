...
import com.ems.types.PlugloadProfileOverrideType;
...
import com.ems.vo.EMPlugloadProfile;
...
public class PlugloadProfileController {
	...
	public String plugloadProfileSetting(Model model,@RequestParam(value="plugloadId",required=false) Long plugloadId,@RequestParam(value = "groupId", required = false) Long groupId) {
		...
		list.add(PlugloadProfileOverrideType.No_Override.getName());
		list.add(PlugloadProfileOverrideType.Override1.getName());
		list.add(PlugloadProfileOverrideType.Override2.getName());
		list.add(PlugloadProfileOverrideType.Override3.getName());
		list.add(PlugloadProfileOverrideType.Override4.getName());
		model.addAttribute("overridelist", list);	
		
		ArrayList<String> list2 = new ArrayList<String>();
		list2.add(PlugloadProfileOverrideType.No_Override.getName());
		list2.add(PlugloadProfileOverrideType.Override5.getName());
		list2.add(PlugloadProfileOverrideType.Override6.getName());
		list2.add(PlugloadProfileOverrideType.Override7.getName());
		list2.add(PlugloadProfileOverrideType.Override8.getName());
		...
		}else if(plugloadGroup.isDefaultProfile()==true && derivedGrp==null)
		{
			...
			derivedGrp = plugloadGroupManager.getPlugloadGroupByName("Default");
			model.addAttribute("groups",derivedGrp);
		}else
		{
	...
	@RequestMapping(value = "/plugloadprofilesetting.ems", method = {
			RequestMethod.POST, RequestMethod.GET })
	public String plugloadProfileSetting(Model model,@CookieValue(FacilityCookieHandler.selectedPlugloadProfileCookie) String cookie) throws EmsValidationException {
		
		if (cookie == null || "".equals(cookie)) {
			return null; // TODO show internal error page.
		}

		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "plugloadProfileController.jstree_plugload_profile_select", cookie);
	...
	public String plugloadProfilesDevicesSettings(
			Model model,
			@CookieValue(FacilityCookieHandler.selectedPlugloadProfileCookie) String cookie) throws EmsValidationException {
	...
	public String profileAddEditSetting(Model model,@RequestParam("groupId") Long groupId,@RequestParam("templateId") long templateId,@RequestParam("type") String type,
        ...
        nameValMap.put("plugloadProfileController.profileType", type);
        nameValMap.put("plugloadProfileController.oldProfileName", oldProfileName);
    ...
    public String createNewProfileData(@RequestParam("type") String type, @RequestParam("typeid") Long typeid, @RequestParam("profilename") String profilename,
            @RequestParam("weekdays") String weekdays, @ModelAttribute("profilehandler") PlugloadProfileHandler ph, @RequestParam("templateId") long templateId, Locale local) throws EmsValidationException {
	
		Map<String,Object> nameValMap = new HashMap<String, Object>();
		nameValMap.put("plugloadProfileController.phType", type);
        nameValMap.put("plugloadProfileController.oldProfileName", profilename);
        nameValMap.put("plugloadProfileController.weekdays", weekdays);
        nameValMap.put("plugloadProfileController.morningTime", ph.getPlugloadProfileConfiguration().getMorningTime());
	    nameValMap.put("plugloadProfileController.dayTime", ph.getPlugloadProfileConfiguration().getDayTime());
	    nameValMap.put("plugloadProfileController.eveningTime", ph.getPlugloadProfileConfiguration().getEveningTime());
	    nameValMap.put("plugloadProfileController.nightTime", ph.getPlugloadProfileConfiguration().getNightTime());
	    CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, nameValMap);
       ...
       // }
        ...
        ph.setProfileGroupId(profileNo);
    ...
    public String updatePlugloadProfileData(@RequestParam("type") String type, @RequestParam("typeid") Long typeid, @RequestParam(value="profilename",required=false) String profileName,
            @RequestParam("weekdays") String weekdays, @ModelAttribute("profilehandler") PlugloadProfileHandler ph, Locale local) throws EmsValidationException {
       ...
       if (weekdays == null || "".equals(weekdays)) {
		...
		if ("plugloadgroup".equals(type)) {
            // 1. Copy the profile configuration of the current group on to each plugloads within the group with this
            // profile configuration
            // and update the group_id to the current one.
            // 2. Enable push flag for the list of plugloads(s) within the group to set to true, The profile will be
            // sync'd in the next PM stat
            plugloadGroupManager.updatePlugloadGroupProfile(ph1, typeid);
            plugloadGroupManager.updateAdvancePlugloadGroupProfile(ph1, typeid);
        } else if ("plugload".equals(type)) {
            //
        } else {
    		if (type == null || "".equals(type)) {
		...
		userAuditLoggerUtil.log("Update plugload profile configuration for " + profileName + profileLog + " There might be more plugload profile changes.", UserAuditActionType.Plugload_Profile_Update.getName());
	...
	@RequestMapping(value = "/filterPlugloadProfile.ems", method = {
			RequestMethod.POST, RequestMethod.GET })
	public String filterPlugloadProfile(Model model, @RequestParam(value = "plugloadAssociatedProfile", required = false) String plugloadAssociatedProfile) throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "plugloadProfileController.plugloadAssociatedProfile", plugloadAssociatedProfile);
    ...
    public String setVisibilityOfPlugloadGroups(@RequestParam("selectedPlugloadTemplateProfiles") String selectedPlugloadTemplateProfiles) throws EmsValidationException{
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "plugloadProfileController.selectedPlTemplateProfiles", selectedPlugloadTemplateProfiles);
		String visibility[] = selectedPlugloadTemplateProfiles.split(",");
		String  plugloadAssociatedProfile = "";
		plugloadAssociatedProfile = plugloadGroupManager.updatePlugloadGroupVisibility(visibility);
    ...
    @ResponseBody
    public String realTimePlugloadProfilePush(@RequestParam("type") String type, @RequestParam("typeid") Long plugloadId, Locale local) throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "plugloadProfileController.phType", type);
    ...
    @RequestMapping("/importplugloadprofiledialog.ems")
    public String importPlugloadProfileDialog(Model model) {
           return "plugloadprofile/importPlugloadprofile";
    }
	
	@RequestMapping("/uploadplugloadprofile.ems")  
	@ResponseBody
	    public String uploadPlugloadProfile(HttpServletRequest request,@RequestParam("upload") MultipartFile file,Model model,Locale locale) throws JAXBException, XMLStreamException, IOException, EmsValidationException {
	        String content="";
	        String message="";
	        if(file!=null)
	        {
                ...
                final boolean isValid = ImportExportFileUtil.validateXMLSchema(Constants.XSD_PLUGLOAD, content);
                ...
                PlugloadProfileHandler phObj = WebServiceUtils.convertPlPhStringToModel(content);
                EMPlugloadProfile emPlugloadProfile = new EMPlugloadProfile();
                String fileNameStr = file.getOriginalFilename();
                String[] fileNameArr = fileNameStr.split(".xml");
                String fileName = fileNameArr[0];
                CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "plugloadProfileController.fileName", fileName);
                PlugloadGroups plugloadGroup = plugloadGroupManager.getPlugloadGroupByName(fileName);
                Long index= 1l;
                if(plugloadGroup!=null)
                {
                	 fileName=fileName+"_"+index;
                }
                emPlugloadProfile.setName(fileName);
                emPlugloadProfile.setPlugloadProfileHandler(phObj);
                request.getSession().setAttribute("importplugloadprofileobj",emPlugloadProfile);
	        }
	...
	@RequestMapping(value = "/importplugloadprofilesetting.ems", method = {
            RequestMethod.POST, RequestMethod.GET })
    public String importPlugloadProfileSetting(HttpServletRequest request,Model model,@RequestParam("groupId") Long groupId,@RequestParam("templateId") long templateId) throws JAXBException, XMLStreamException {
	    model.addAttribute("type", "group");
	    EMPlugloadProfile emPlugloadProfile = (EMPlugloadProfile)request.getSession().getAttribute("importplugloadprofileobj");
	    PlugloadProfileHandler plphObj = emPlugloadProfile.getPlugloadProfileHandler();
	    List<PlugloadGroups> plugloadProfileList = new ArrayList<PlugloadGroups>();
	    PlugloadGroups plugloadGroup = plugloadGroupManager.getPlugloadGroupByName("Default");
	    model.addAttribute("oldProfileName",emPlugloadProfile.getName());
	    plugloadProfileList.add(plugloadGroup);
        model.addAttribute("groups", plugloadProfileList);
        PlugloadGroups oPlugloadGroup = null;
        if(groupId==-1)
        {
        	oPlugloadGroup = plugloadGroupManager.getGroupById(plugloadProfileList.get(0).getId());
        }
        PlugloadProfileHandler plph = plphObj;
        model.addAttribute("profilehandler", plph);
        model.addAttribute("management","new");
      
      //Template ID is >1, it means we are creating new plugload profile first time for user defined template. Therefore retaining same template ID.
      if(groupId==-1 || templateId>1)
      {
          model.addAttribute("templateId", templateId);
      }
      ...
      {
          model.addAttribute("templateId", oPlugloadGroup.getPlugloadProfileTemplate().getId());
      }
      model.addAttribute("typeid", oPlugloadGroup.getId());
      model.addAttribute("plugloadgroup", oPlugloadGroup);
      ...
      ArrayList<String> list2 = new ArrayList<String>();
		...
		list2.add(PlugloadProfileOverrideType.Override8.getName());
		model.addAttribute("override2list", list2);
     return "plugloadProfile/addeditsetting";
	}
