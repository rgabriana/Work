...
var SCENE_ID ;
var iDimVal = 0;
var appliedSceneToSwitch = false;
var clearSetTimer;

var plugloadLightLevelMap = {}; 

$(document).ready(function() {		
	<c:if test="${not empty plugloadlightlevels}">
		<c:forEach var="entry" items="${plugloadlightlevels}" varStatus="status">
				plugloadLightLevelMap["${entry.sceneId}"] = "${entry.lightLevel}";
	    </c:forEach>
	 </c:if>
	//Create Slider
...
function setDimmerState(dimmerValue){
	...
	{		
		...
		success: function(data){
			if(data != null){
				clearSetTimer = setTimeout(dblclickedSwitch, 300);
			}
		},
		error: function(){
			clearSetTimer = setTimeout(dblclickedSwitch, 300);
...
function dblclickedSwitch(){
	appliedSceneToSwitch = false;
	clearTimeout(clearSetTimer);
}

function applySceneToSwitch(sceneName , sceneId, sceneOrder, obj){
	if(appliedSceneToSwitch == false){
		appliedSceneToSwitch = true;
		...
		if(sceneName=="auto"){
			...
			var plugloadlightlvl = plugloadLightLevelMap[sceneId];
			if(plugloadlightlvl != undefined || plugloadlightlvl != null){
				if(plugloadlightlvl == 0){
					//operatePlugLoad("${plugloadUrl}"+"turnOnOff/0/"+n);
				}else{
					//operatePlugLoad("${plugloadUrl}"+"turnOnOff/100/"+n);
				}
