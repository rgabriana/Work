...
// 			setGatewayLocation();
				...
				$(msg).find('gateway')
						.each(
								function() {									
									...
									var gatewayWirelessNetworkId = 6854;
									<c:forEach items="${gatewayWirelessNetworkIdVOList}" var="gatewayWirelessNetworkIdVO">
										if("${gatewayWirelessNetworkIdVO.gatewayName}" == $("#gatewayName").val()){
											gatewayWirelessNetworkId = "${gatewayWirelessNetworkIdVO.wirelessNetworkId}";
										}
									</c:forEach>
									$("#wirelessNetworkId").val(gatewayWirelessNetworkId);
	...
	function commissionDevice() {
		...
		if (checkForDuplicateNetworkId($("#wirelessNetworkId").val()) == false) {
			resetDevicePositionOnFloorplan();
			alert("<spring:message code='error.duplicate.gateway.networkid'/>")
			return false
		}
...
// 		alert(titleEL.html());
	...
	function checkNetworkIdFormat(field) {
        ...
        var lowerlimit = convertHexToDecimal("0001");
        var upperlimit = convertHexToDecimal("fffe");
        var inetworkId = convertHexToDecimal(networkIdstr);
        if (inetworkId < lowerlimit || inetworkId > upperlimit || isNaN(inetworkId) ) {
        	return false;
        }
        return true;
	}
	
	function checkForDuplicateNetworkId(field){
		var networkIdstr = convertHexToDecimal(field);
		
		<c:forEach items="${commissionedGatewayWirelessNetworkIdList}" var="commissionedGatewayWirelessNetworkId">
		    if("${commissionedGatewayWirelessNetworkId.wirelessNetworkId}" == networkIdstr){
		    	return false;
		    }
		</c:forEach>
			...
			<div class="form-content_wrapper">
				...
				<div class="form-column-right-wrapper">
					...
					<div class="fieldWrapper">
						...
						<div class="fieldInput"><input class="text" id="wirelessNetworkId" name="wirelessNetworkId" size="10" /></div>
						<br style="clear:both;"/>
					</div>
					...
					<div class="fieldWrapper">
						...
						<div class="fieldInputCombo">
							<select name="wirelessEncryptType" id="wirelessEncryptType" class="text" disabled="disabled">
								<option value="0">none</option>
								<option value="1">aes56</option>
								<option value="2" selected="selected">aes128</option>
							</select>
						</div>
						<br style="clear:both;"/>
					</div>
