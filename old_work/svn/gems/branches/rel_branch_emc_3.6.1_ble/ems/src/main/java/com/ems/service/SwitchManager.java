...
public class SwitchManager {
    ...
    public List<SwitchScenes> loadAllSwitchScenes() {
    	List<Switch> switchList = loadAllSwitches();
    	List<SwitchScenes> switchScenesList = new ArrayList<SwitchScenes>();
    	if(switchList != null && !switchList.isEmpty()) {
    		for(Switch sw : switchList){
    			List<Scene> sceneList = loadSceneBySwitchId(sw.getId());
    			SwitchScenes swScenes = new SwitchScenes();
    			swScenes.setId(sw.getId());
    			swScenes.setName(sw.getName());
    			swScenes.setSceneList(sceneList);
    			switchScenesList.add(swScenes);
    		}
    	}
    	
    	return switchScenesList;
    }
    ...
    public void deleteSwitchByGroupId(long gemsGroupId) {
        
        Response response = new Response();
        
        Switch sw = getSwitchByGemsGroupId(gemsGroupId);
        if (sw != null) {
            
        	List<GemsGroupFixture> gemsGroupFixtureList = gemsGroupManager.getGemsGroupFixtureByGroup(gemsGroupId);
            List<GemsGroupPlugload> gemsGroupPlugloadList = gemsPlugloadGroupManager.getGemsGroupPlugloadByGroup(gemsGroupId);
            List<Wds> wdsList = wdsManager.loadCommissionedWdsBySwitchId(sw.getId());
        ...
        if (response.getStatus() == 0) {
            ...
            deleteSwitch(sw.getId());
        }
        
    }
    
    public void removeAllSwitchGroupFixtures(Long gemsGroupId)
    {
    	String resp = "";
        Switch sw = getSwitchByGemsGroupId(gemsGroupId);
        Long groupId = sw.getGemsGroup().getId();
        SwitchGroup sg = getSwitchGroupByGemsGroupId(groupId);
        int groupNo = Integer.parseInt(sg.getGroupNo().toString(), 16);    
        Set<Long> currentSceneFixtures = loadSwitchSceneFixtures(sw.getId());      
        ...
        for (Long fixtureId : currentSceneFixtures) {
            ...
            oGGFxList.clear();
            if (iStatus == 0) {
                deleteSceneLevelsForSwitch(sw.getId(), fixtureId);
    ...
    public void removeAllSwitchGroupPlugloads(Long gemsGroupId)
    { 
        ...
        SwitchGroup sg = getSwitchGroupByGemsGroupId(groupId);
        int groupNo = Integer.parseInt(sg.getGroupNo().toString(), 16);    	
        Set<Long> currentScenePlugloads = loadSwitchScenePlugloads(sw.getId());
        ...
        for (Long plugloadId : currentScenePlugloads) {
            ...
            oGGFxList.clear();
            if (iStatus == 0) {
               deletePlugloadSceneLevelsForSwitch(sw.getId(), plugloadId);
