...
<spring:url value="/services/org/gateway/getdiscoveredgatewaycount/" var="getDiscoveredGatewayCountUrl" scope="request" />
<spring:url value="/services/org/plugload/getcountbygateway/" var="getPlugloadCountByGatewayUrl" scope="request" />
...
<div id="gateway-list-topPanel" style="background:#fff">
	<c:if test="${page == 'floor'}">		
		<div id="gateway-dialog-form">
			...
			<c:if test="${isMaintenanceMode}">
			<form id="deleteAllDevicesButton" name="deleteAllDevicesButton">
				<input type="button" id="deleteAllDevicesButton" onclick="onDelete('${floorId}');" value="Purge devices" style="float: right;margin-left: 4px;" />
			</form>
			</c:if>
			
			<form id="bulkGatewayCommissionButton" name="bulkCommissionGateway">
				<input type="button" id="bulkGatewayCommissionBtn" onclick=" return beforeBulkgatewayCommissionCallBack(0)" value="<spring:message code='gatewayForm.label.bulkCommissionBtn'/>" style="float: right;margin-left: 4px;" />
			</form>
...
var PAGE = "${page}";
	...
	$(document).ready(function() {
		...
		jQuery("#gatewayTable").jqGrid({
			...
			forceFit: true,
			hoverrows: false,
			colNames:["id", "Name", "Channel", "Network ID", "Version", "Commissioned", "No. of Devices","No. of ERCs", "Upgrade Status", "Action"],
		   	colModel:[
				{name:'id', index:'id', hidden: true},
		   		{name:'gatewayName', index:'gatewayName', sorttype:"string", width:"22%"},
		   		{name:'channel', index:'channel', sorttype:"number", width:"10%"},
		   		{name:'wirelessNetworkId', index:'wirelessNetworkId', sorttype:"number", width:"10%"},
		   		...
		   		{name:'noOfActiveWds', index:'noOfActiveWds', sorttype:"number", width:"11%"},
		   		{name:'upgradeStatus', index:'upgradeStatus', sorttype:"string", width:"11%"},
		   		{name:'action', index:'action', align:"right", sortable:false, hidden:(PAGE=="area"), width:"18%"}
		   	],
...
// 		   	pager: '#gatewayPagingDiv',
		...
		<c:forEach items="${gateways}" var="gateway">
			...
			localData.noOfActiveWds = "${gateway.noOfActiveErc}";
	...
	function beforeBulkgatewayCommissionCallBack(GwId)
	{
		var floorId = "${floorId}";
		if(floorId==null)
		{
		...
		var urlOption = "floor/"+floorId;
		// get discovered gateway count
		$.ajax({
			type: 'GET',
			url: "${getDiscoveredGatewayCountUrl}"+urlOption+"?ts="+new Date().getTime(),
			success: function(data){
				if(data != null){
					if((1*data.status) == 0){
						alert("There is no gateway in discovered state. Please discover at least 1 Gateway.");
					}else
					{
						parent.parent.showGatewayCommissioningForm(GwId);						
					}
				}
			},
			dataType:"json",
								...
								success: function(data){
									if(data != null){
										...
										if(xml != undefined && xml.length != 0)
											alert("This Gateway has ERC(s) associated with it. It cannot be deleted");
										else{
											$.ajax({
												type: 'GET',
												url: "${getPlugloadCountByGatewayUrl}"+gatewayId+"?ts="+new Date().getTime(),
												success: function(data){
													if(data != null){
														if((1*data.status) == 0){	
															...
															alert("This Gateway has plugload(s) associated with it. It cannot be deleted");
														}
													}
												},
												dataType:"json",
	...
	function reloadGatewayListFrame(){
  	...
  	function onDelete(floorId){
		if(confirm("Are you sure you want to delete all the devices from the floor?") == true)
		{
			parent.parent.deleteAllDevicesByFloor(floorId);			
		}
	}
