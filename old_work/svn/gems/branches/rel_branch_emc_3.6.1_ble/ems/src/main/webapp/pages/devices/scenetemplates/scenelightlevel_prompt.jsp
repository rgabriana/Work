...
{	var isValid = true;
    ...
    var sel_row_id = jQuery("#scene-level-table").jqGrid('getGridParam','selrow');
	var sel_row_index = jQuery("#scene-level-table").jqGrid('getInd', sel_row_id);
	...
	if(gridRowCount>0){		
		...
		for(var i=0;i<data.length;i++){
			//case: edit mode, scene name modified and light level unchanged 
			if((mode == 'edit') 
				&& (lightLevel == data[sel_row_index-1].lightlevel) 
				&& (chkName != data[sel_row_index-1].name))
				isValid = true;
			else{
				...
				if(chkName == data[i].name)
				{
					isValid = false; 
				    displayLightLevelPromtMessage("Specified scene name already exists", COLOR_FAILURE);
				}
			}
		}
	}
	...
	<form:form id="create_scenelightleveldialog" commandName="sceneLightLevelTemplate" method="post" action="" onKeyPress="return disableEnterKey(event)">
		...
		<table>
			<tr>
				...
				<td class="fieldValue"><form:input class="inputField" id="sllp_name" name="sllp_name" path="name" onkeypress="clearMessage();" onmousedown="clearMessage();"/></td>
			</tr>
			<tr>
