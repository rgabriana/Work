...
public class GatewayService {
	...
	/**
	 * Fetch gateway Outage list
	 */
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin','Auditor')")
	@Path("list/report/gatewayOutage/{property}/{pid}")
	@GET
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public List<GatewayOutageVO> getGatewayOutageList(
        ...
        if(resp!= null && resp.getStatus()!=200){
        	m_Logger.error("Validation error"+resp.getMsg());
    		return new ArrayList<GatewayOutageVO>();
    	}
		return gatewayManager.getGatewayOutageList(property, pid);
	}
