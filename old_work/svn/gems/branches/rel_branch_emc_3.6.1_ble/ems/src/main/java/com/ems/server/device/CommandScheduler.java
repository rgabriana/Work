...
public class CommandScheduler {
  ...
  private static final int NO_BROADCAST_MODE = 0;
  private static final int BROADCAST_MODE = 1;
  private static final int BROADCAST_MODE_IN_PROCESS = 2;
  ...
  public long addCommand(int[] fixtArr, byte[] dataPacket, int msgType, String deviceType, boolean retryReq,
    ...
    */
	...
	public long addBroadcastSuCommand(int[] fixtArr, byte[] dataPacket,
			int msgType, boolean retryReq, int pktDelay,String deviceType) {

		
		CommandJob job = new CommandJob(pktDelay, deviceType, true);
		long seqNo = job.cmdSeq;
		switch (msgType) {
		case ServerConstants.SET_PROFILE_MSG_TYPE:
		case ServerConstants.SET_PROFILE_ADV_MSG_TYPE:
		...
		if (fixtureLogger.isInfoEnabled()) {
			fixtureLogger.info("cmd(txId=" + job.cmdSeq + ",msg=" + msgType+", deviceType = "+deviceType
					+ ",prio=" + job.priority + ") on " + fixtArr.length
					+ " fixtures " + ServerUtil.getLogPacket(dataPacket));
		}
		job.fixtureArr = fixtArr;
		job.dataPacket = dataPacket;
		job.msgType = msgType;
  ...
  public class GwCmdJob implements Runnable {
    ...
    private void sendCommands() {
		...
		if (cmdJob.broadcastMode == BROADCAST_MODE) {
			cmdJob.broadcastMode = BROADCAST_MODE_IN_PROCESS;
			...
			while (suIter.hasNext()) {
				...
				if (cmdJob.noOfRetries > 0) {
					if (oDevice.getType().equals(
							DeviceType.Fixture.getName())
							|| oDevice.getType().equals(
									DeviceType.Plugload.getName())) {
						SuJobDetails suDetails = new SuJobDetails(oDevice,
								cmdSentTime, cmdJob.retryInterval);
			...
			byte[] header = DeviceServiceImpl.getInstance().getHeader(
					cmdJob.dataPacket.length, cmdJob.msgType, "1.3", cmdJob.cmdSeq.intValue(), null);
			byte[] pkt = new byte[header.length + cmdJob.dataPacket.length
					+ 1];
			System.arraycopy(header, 0, pkt, 0, header.length);
			pkt[pkt.length - 1] = ServerConstants.FRAME_END_MARKER;
			System.arraycopy(cmdJob.dataPacket, 0, pkt, header.length,
					cmdJob.dataPacket.length);
			// now broadcast this packet first
      		if(fixtureLogger.isInfoEnabled()) {
      			fixtureLogger.info(cmdJob.cmdSeq + ": sending broadcast...");
      		}
			GatewayComm.getInstance().sendNodeDataToGateway(gwId,
					gw.getIpAddress(), pkt);
			cmdJob.broadcastMode = NO_BROADCAST_MODE;
			  // Give more time for broadcast reply to come in.
		      	setLastCmdSentTime(System.currentTimeMillis() + BROADCAST_DELAY);
      		if(fixtureLogger.isInfoEnabled()) {
      			fixtureLogger.info(cmdJob.cmdSeq + ": broadcast scheduled.");
      		}
		} else {
      ...
      if(mPkt.getNoOfTargets() < DeviceServiceImpl.NO_OF_MULTICAST_TARGETS) {
	      ...
	      if(mPkt.getNoOfTargets() > 0) {
	      	...
	      	if (oDevice.getType().equals(DeviceType.Fixture.getName())) {
	            ...
	            fixtureLogger.debug("sending to gateway comm");
  ...
  public class CommandJob implements Runnable {
    ...
    private int broadcastMode = NO_BROADCAST_MODE; // 0: not in broadcast mode, 1: broadcast mode, 2: broadcast in process... 
    ...
    public CommandJob(int pktDelay, String deviceType, boolean broadcast) {
        this.deviceType = deviceType;
        if(priority == PRIORITY_LOW) {
  	pauseAllowed = true;
        }      
	...
	for(int i = 0; i < noOfFixtures; i++) {	
	    ...
	    }else if(this.deviceType.equals(DeviceType.Device.getName())){
	    	Long gwId = null;
	    	List<Device> deviceList = plugloadMgr.getDeviceById(fixtureArr[i]);	 	    
	    	if(deviceList != null && deviceList.size() > 0){	    		
	    		Object object = deviceList.get(0);
	    		if(object instanceof Fixture){	  	    			
	    			Fixture fixtureObject = (Fixture) object;
	    	    	gwId = fixtureObject.getSecGwId();  
	    	    	if(!gwMap.containsKey(gwId)) {
	    	    		gwMap.put(gwId, new GwCmdJob(gwId, interPktDelay, this));
	    	    	}
	    	    	if(!gwThreadPoolMap.containsKey(gwId)) {
	    	    		PriorityBlockingQueue<GwCmdJob> gwWorkQueue = new PriorityBlockingQueue<GwCmdJob>(100, cmdComparator);
	    	    	    PausableThreadPoolExecutor gwCmdThreadPool = new PausableThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, gwWorkQueue);
	    	    	    gwThreadPoolMap.put(gwId, gwCmdThreadPool);
	    	    	 }
	    	    	gwMap.get(gwId).addSU(fixtureObject);	    	    	
	    		}else if(object instanceof Plugload){	
	    			Plugload plugloadObject = (Plugload) object;	
	  	    	  	gwId = plugloadObject.getSecGwId();
	  	    	  ...
	  	    	  if(!gwThreadPoolMap.containsKey(gwId)) {
	  	    	    ...
	  	    	    gwThreadPoolMap.put(gwId, gwCmdThreadPool);
	  	    	  	...
	  	    	  	gwMap.get(gwId).addPlugload(plugloadObject);	  	    	  	
	    		}	    		
	    	}
