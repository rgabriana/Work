...
$(document).ready(function() {
	...
	$.ajax({
		type:'POST',
		data: "",
		url: "${getExistingSoftMeteringUrl}"+"/"+"perf.pmStatsMode"+"?ts="+new Date().getTime(),
		success: function(data){
			if(data == null){
				$("#softMeteringStatus").html("Unable to find softmetering flag status");
			}else{
				pmStatsModeId = data.id;
				existingPmStatsMode = data.value;
				switch(data.value){
				case "1":
...
function updateSoftMeteringFlag(updatedPmStatsMode){	
	...
	$.ajax({
			...
			success: function(data){
				existingPmStatsMode = updatedPmStatsMode;
