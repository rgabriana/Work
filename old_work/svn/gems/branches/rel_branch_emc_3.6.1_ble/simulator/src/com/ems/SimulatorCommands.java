...
public final class SimulatorCommands {
    ...
    public static GWInterface createGateway() {
    ...
    public static void turnGatewayRangeOff(ArrayList<GWInterface> c) throws NumberFormatException, IOException {
    ...
    public static void turnGatewayRangeOn(ArrayList<GWInterface> c) throws NumberFormatException, IOException {
    ...
    public static ArrayList<GWInterface> CreateGatewayRange() throws IOException {
        int pktCount = 0;
        String gwInterface = "";
        String ipAddress = "";
        String startRange;
        ...
        ArrayList<GWInterface> oGateway = new ArrayList<GWInterface>();
    ...
    public static synchronized void turnAllGatewayOn(ArrayList<GWInterface> su) {
        Iterator<GWInterface> i = su.iterator();
        while (i.hasNext()) {
            i.next().on();
        }
    ...
    public static void turnAllGatewayOff(ArrayList<GWInterface> su) {
        Iterator<GWInterface> i = su.iterator();
        while (i.hasNext()) {
            i.next().off();
        }
    ...
    public static void doSenosrGatewayAssignment(ArrayList<SensorUnit> su, ArrayList<GWInterface> gu) {
        int j = 0;
    ...
    public static void doWDSGatewayAssignment(ArrayList<WDSUnit> wds, ArrayList<GWInterface> gu) {
        int j = 0;
    ...
    public static void currentState(ArrayList<SensorUnit> su, ArrayList<GWInterface> gw) {
        int count = 0;
        ...
        Iterator<GWInterface> iterator = gw.iterator();
        while (iterator.hasNext()) {
            GWInterface gateway = iterator.next();
            if (gateway.getKeepRunningreciever() != null && gateway.getKeepRunningreciever().isAlive())
    ...
    public static void doPlugloadGatewayAssignment(ArrayList<PlugloadUnit> pl,
            ArrayList<GWInterface> gu) {
        int j = 0;
        for (int i = 0; i < pl.size(); i++) {
            if (j == gu.size() && gu.size() != 1) {
                j = 0;
            }
            pl.get(i).assignPlugloadtoGatewy(gu.get(j));
            if (gu.size() == 1)
                continue;
            else
                j++;
        }
        System.out.println("Plugloads are Assigned and ready to start....");
    }

    public static void turnAllPlugloadsOn(ArrayList<PlugloadUnit> pl) {
        Iterator<PlugloadUnit> i = pl.iterator();
        while (i.hasNext()) {
            i.next().on();
        }
        System.out.println("All plugloads are switched on");
    }
