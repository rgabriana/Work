...
<script type="text/javascript">
	...
	var createUserValidatorObj = {
		rules: {
			firstName: {required: true, nameregx: /^[^|\\!?^<>~$%&*\/()+=[\]{}`\'\";]+$/,},
			lastName: {required: true, nameregx: /^[^|\\!?^<>~$%&*\/()+=[\]{}`\'\";]+$/,},
			email: { required: true, email: true},
			confirmPassword: { equalTo: "#password" }
		},
		messages: {
			firstName: {required: '<spring:message code="error.above.field.required"/>',},
			lastName: {required: '<spring:message code="error.above.field.required"/>',},
	...
	<form:form id="create_user" commandName="user" method="post" 
		...
		<table>
			...
			<tr>
				...
				<td class="fieldValue"><input class="inputField" type="password" autocomplete="off" id="password"
					name="password" size="40" /></td>
			</tr>
			<tr>
				<td class="fieldLabel">Confirm Password*</span></td>
				<td class="fieldValue"><input class="inputField" type="password" autocomplete="off" id="confirmPassword"
