...
public class ReplicaServerService {
     ...
     * @return boolean is sync paused
     */
    @Path("get/state/migration/{mac}")
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public String getLatestMigrationStatus(@PathParam("mac") String mac) {
    	String state = null;
    	EmInstance emInstance = emInstanceManager.loadEmInstanceByMac(mac);
    	if(emInstance.getLatestEmStateId()!=null)
		{
			EmState emState = emStateManager.loadEmStateById(emInstance.getLatestEmStateId());
			state = emState.getDatabaseState().getName();
		}	
    	return state;
    }
    ...
    @Path("getDataPullRequestForReplica")
	@POST
	@Consumes({ MediaType.TEXT_PLAIN})
	@Produces({ MediaType.TEXT_PLAIN})
	public String getDataPullRequestForReplica(String request) {
		...
		ReplicaServer replicaServer = replicaServerManager.getReplicaServersbyUid(map.get(CloudParamType.ReplicaServerUID));
		DataPullRequestVO dprVO = dataPullRequestManager.getTopDataPullRequestVOByReplicaId(replicaServer.getId());
		if(dprVO == null) {
			dprVO =  new DataPullRequestVO();
		}
		return JsonUtil.getJSONString(dprVO);
	}
    
    
    @Path("updateDataPullRequest/requestId/{requestId}/state/{newState}")
	@POST
	@Produces({ MediaType.TEXT_PLAIN})
	public String updateDataPullRequest(@PathParam("requestId") Long requestId , @PathParam("newState") String newState) {
		return dataPullRequestManager.updateDataPullRequestState(requestId, newState);
	}
