...
alter table em_instance add column em_state bigint default 0 ;
update em_instance set em_state = 0 where em_state is null;
...
ALTER TABLE em_instance ADD COLUMN em_os_arch character varying DEFAULT 'NA';
ALTER TABLE em_instance ADD COLUMN em_os_version character varying DEFAULT 'NA';
update em_instance set em_os_arch='NA' where em_os_arch is null;
update em_instance set em_os_version='NA' where em_os_version is null;

ALTER TABLE em_tasks ADD COLUMN em_tasks_uuid_id bigint;
...
ALTER TABLE ONLY em_tasks
    ADD CONSTRAINT em_tasks_em_tasks_uuid_fk FOREIGN KEY (em_tasks_uuid_id) REFERENCES em_tasks_uuid (id);
...
CREATE TABLE em_tasks_uuid
(
  id bigint NOT NULL,
  uuid character varying(128) NOT NULL,
  active boolean NOT NULL,
  CONSTRAINT em_tasks_uuid_pkey PRIMARY KEY (id)
);

ALTER TABLE em_tasks_uuid OWNER TO postgres;

CREATE SEQUENCE em_tasks_uuid_seq
  INCREMENT 1
  MINVALUE 1
  NO MAXVALUE
  START 1
...
ALTER TABLE em_tasks_uuid_seq OWNER TO postgres;
...
UPDATE em_instance set open_tunnel_to_cloud = 'FALSE' where open_tunnel_to_cloud is null;
UPDATE em_instance set browse_enabled_from_cloud = 'FALSE' where browse_enabled_from_cloud is null;
UPDATE em_instance set tunnel_port = 0 where tunnel_port is null;
UPDATE em_instance set ssh_tunnel_port = 0 where ssh_tunnel_port is null;
UPDATE em_instance set open_ssh_tunnel_to_cloud = 'FALSE' where open_ssh_tunnel_to_cloud is null;
...
INSERT INTO system_configuration (id, name, value) values (nextval('system_configuration_seq'), 'flag.ecloud.apply.validation', 'true');

UPDATE system_configuration SET value = '3.1.0' where name = 'browsing.supported.version';
...
ALTER TABLE em_instance add column latest_em_health_monitor_id int;
...
ALTER TABLE organization_energy_consumption_daily
  ADD CONSTRAINT organization_energy_consumption_daily_pkey PRIMARY KEY (cust_id, level_id, capture_at);
    ...
    ALTER TABLE floor_energy_consumption_15min
    ALTER COLUMN avg_temp TYPE double precision,
    ALTER COLUMN min_temp TYPE double precision,
    ALTER COLUMN max_temp TYPE double precision,
    ALTER COLUMN avg_amb TYPE double precision, 
    ALTER COLUMN min_amb TYPE double precision, 
    ALTER COLUMN max_amb TYPE double precision;
	ALTER TABLE floor_energy_consumption_hourly
    ALTER COLUMN avg_temp TYPE double precision,
    ALTER COLUMN min_temp TYPE double precision,
    ALTER COLUMN max_temp TYPE double precision,
...
ALTER TABLE facility DROP CONSTRAINT "facility_name_customer_id_unique";
...
SELECT setval('facility_seq', (SELECT max(id)+1 FROM facility));
SELECT setval('facility_em_mapping_seq', (SELECT max(id)+1 FROM facility_em_mapping));
...
alter table em_instance add column secret_key character varying(64);
alter table em_instance add column ip_address character varying(64);
alter table em_instance add column api_key character varying(128);
alter table em_instance add column no_of_floors bigint default 0;
...
INSERT INTO system_configuration (id, name, value) VALUES (nextval('system_configuration_seq'), 'SYNC_PROFILE_CRON_SETTING', '0 0 0/1 * * ?');

ALTER TABLE em_profile_mapping ADD COLUMN sync_status smallint;
ALTER TABLE em_profile_mapping ADD COLUMN sync_start_time timestamp without time zone;
...
alter table em_instance add COLUMN last_successful_sync_time timestamp without time zone;

update em_instance set last_successful_sync_time = current_timestamp where last_successful_sync_time is null;

alter table em_instance add column pause_sync boolean default false;
update em_instance set pause_sync = false where pause_sync is null;
...
DECLARE 
    ...
    override_id bigint;
BEGIN
	FOR ph_id IN (SELECT id FROM profile_handler)
	LOOP
      override_id:=null;
...
drop index em_stats_capture_at_index;

--------------------------------------------       sachink -----------------------------------------------
--
-- Name:app_instance; Type: TABLE; Schema: public; Owner: postgres; Tablespace: 
--
CREATE SEQUENCE app_instance_seq
    START WITH 1
    INCREMENT BY 1
    NO MAXVALUE
    NO MINVALUE
...
ALTER TABLE public.app_instance_seq OWNER TO postgres;
...
CREATE TABLE app_instance (
    ...
    app_commissioned_date timestamp without time zone,
...
ALTER TABLE ONLY app_instance
    ADD CONSTRAINT app_instance_pk PRIMARY KEY (id);

ALTER TABLE ONLY app_instance
    ADD CONSTRAINT app_instance_customer_fk FOREIGN KEY (customer_id) REFERENCES customer(id);

--------------------------------------------       sachink -----------------------------------------------

    
CREATE TABLE data_pull_request (
    id bigint NOT NULL,
    em_id bigint not null,
...
ALTER TABLE public.data_pull_request OWNER TO postgres;

ALTER TABLE ONLY data_pull_request
    ADD CONSTRAINT data_pull_request_pk PRIMARY KEY (id);

ALTER TABLE ONLY data_pull_request
    ADD CONSTRAINT data_pull_request_em_fk FOREIGN KEY (em_id) REFERENCES em_instance(id);
    
ALTER TABLE ONLY data_pull_request
    ADD CONSTRAINT data_pull_request_users_fk FOREIGN KEY (requested_by) REFERENCES users(id);
    
CREATE SEQUENCE data_pull_request_seq
    START WITH 1
    INCREMENT BY 1
    NO MAXVALUE
    NO MINVALUE
...
ALTER TABLE public.data_pull_request_seq OWNER TO postgres;

INSERT INTO system_configuration (id, name, value) values (nextval('system_configuration_seq'), 'data.pull.request.cleanup.days', '30');