...
public class ProfileTemplateDao extends BaseDaoHibernate {
    ...
    {    	
    	try {
            List<ProfileTemplate> results = null;
            String hsql = "Select new ProfileTemplate(pt.id,pt.name) from ProfileTemplate pt where name = ?";
            Query q = getSession().createQuery(hsql.toString());
    ...
    /**
     * Load All Global ProfileTemplate
     * 
     * @return com.ems.model.ProfileTemplate collection
     */
    @SuppressWarnings("unchecked")
    public List<ProfileTemplate> loadAllGlobalProfileTemplate() {
    	List<ProfileTemplate> results= null;
    	try {
    		results = getSession().createQuery("select pt from ProfileTemplate pt where pt.id not in (select et.uemTemplateId from  EmTemplateMapping et) order by pt.id").list();
    	 } catch (HibernateException hbe) {
             throw SessionFactoryUtils.convertHibernateAccessException(hbe);
         }
     	return results;
	...
	public ProfileTemplate getProfileTemplateByName(String profileTemplateName) {
        List<ProfileTemplate> groupsList = getSession()
                .createQuery("Select new ProfileTemplate(pt.id,pt.name) from ProfileTemplate pt where pt.name = :name")
    ...
    public void updateGroupProfile(ProfileHandler profileHandler, Long groupId) {
        try {
            Session session = getSession();
            ProfileGroups group = (ProfileGroups) session.get(ProfileGroups.class, groupId);
	...
	public List<Object> getFixtureCountForTemplate() {
		...
		try {
			...
			String hsql = "select t.id as templateId,count(f.id) as fixtureCount from Fixture f,Groups g,ProfileTemplate t where f.state!='DELETED' and f.groupId=g.id AND g.profileTemplate.id=t.id group by t.id";
	...
	public List<Object> getFixtureCountForProfileTemplate(Long profileId) {
		try {
			Session s = getSession();
			String hsql = "SELECT p.id,count(f.id) from Fixture f, ProfileTemplate p where f.state!='DELETED' and f.groupId=:profileId group by p.id";
			Query query = s.createQuery(hsql);
    ...
    public Long getNextProfileTemplateNo() {
        Long result = null;
        try {
            String hsql = "select max(templateNo) from ProfileTemplate";
            Query q = getSession().createQuery(hsql.toString());
