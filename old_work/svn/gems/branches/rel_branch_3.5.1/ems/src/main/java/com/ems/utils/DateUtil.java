...
public class DateUtil {
	...
	public static Date addTimeToDate(Date origDate, long incrementTimeInMillis) {
		final Date date = new Date(origDate.getTime() + incrementTimeInMillis);
		//Calendar calendar = Calendar.getInstance();		
		return date;
	}
	...
	/**
	 * pass the input time with timezone in timeZoneStr and also pass the destination timeZoneDestStr and get the time in timeZoneDestStr
	 * pattern is defaulted to be "yyyy-MM-dd:hh:mm:ss"
	 * @param input
	 * @param timeZoneStr
	 * @param timeZoneDestStr
	 * @param pattern
	 * @return
	 */
	public static String getTimeInSpecifiedTimeZone(final String input, final String timeZoneStr, final String timeZoneDestStr, String pattern){
		...
		try {
			TimeZone destZ = TimeZone.getTimeZone(timeZoneDestStr);
			TimeZone t = TimeZone.getTimeZone(timeZoneStr);
			TimeZone.setDefault(t);
			formatter.setTimeZone(t);
			final Date inputDate = formatter.parse(input);
			calI.setTimeZone(destZ);
			calI.setTime(inputDate);
			
			TimeZone.setDefault(destZ);
			formatter.setTimeZone(destZ);
	...
	/**
	 * Compare 2 dates in different timezones.. If inputDateStr is ahead of referenceDateStr then 1 will be returned else 0 if equal or -1 if it is less 
	 * pattern is defaulted to be "yyyy-MM-dd:hh:mm:ss"
	 * @param inputDateStr
	 * @param tzi - timezone of inputDateStr date
	 * @param referenceDateStr
	 * @param tzo - timezone of referenceDateStr date
	 * @param pattern
	 * @return
	 * @throws Exception
	 */
	public static int compareDate(final String inputDateStr, final String tzi, final String referenceDateStr,  final String tzo, String pattern) throws Exception {
		if(StringUtils.isEmpty(pattern)){
			pattern = "yyyy-MM-dd:hh:mm:ss";
		}
		final TimeZone defaultTZ = TimeZone.getDefault();
		final SimpleDateFormat formatter = new SimpleDateFormat(pattern);
		final Calendar calI = Calendar.getInstance();
		final Calendar calR = Calendar.getInstance();
		try {
			TimeZone currZone = TimeZone.getTimeZone(tzi);
			TimeZone.setDefault(currZone);
			formatter.setTimeZone(currZone);
			final Date input = formatter.parse(inputDateStr);
			calI.setTimeZone(currZone);
			calI.setTime(input);
			currZone = TimeZone.getTimeZone(tzo);
			TimeZone.setDefault(currZone);
			formatter.setTimeZone(currZone);
			final Date reference = formatter.parse(referenceDateStr);
			calR.setTimeZone(currZone);
			calR.setTime(reference);
			if(calI.getTimeInMillis() > calR.getTimeInMillis()){
				//Input Date is ahead than the referenceDate
				return 1;
			}else{
				if(calI.getTimeInMillis() == calR.getTimeInMillis()){
					return 0;
			...
			e.printStackTrace();
			throw new Exception("Can not compare the 2 dates with input as "+inputDateStr+" and "+referenceDateStr,e);
		}finally{
			TimeZone.setDefault(defaultTZ);
			formatter.setTimeZone(defaultTZ);
		}
