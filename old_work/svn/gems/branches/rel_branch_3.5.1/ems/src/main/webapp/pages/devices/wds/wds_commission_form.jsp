...
function rmaWdsConfirm(wdsId, wdsName, wdsVersion) {
	...
	if(ver1 != ver2)
	{
		alert("RMA does not work for wds with different version");
		return false;
	}
	...
	var wdsRmaConfirm = confirm("Selected Wds : " + wdsName + "  Replace with : " + $("#wdsname").val()
			+ "\n				Do you want to continue?");
	
	if(wdsRmaConfirm){
		displayFxCommissionMessage("Performing Wds RMA...", COLOR_SUCCESS);
		$.ajax({
...
function rmaWdsFinish(towdsId) {
	...
	$.ajax({
			...
			error: function() {
				displayFxCommissionMessage("RMA failed", COLOR_ERROR);
			}
	});
}
