...
public class FacilitiesController {
	...
	public String getFacilities(
		...
		// To get plugload profile tree populated
		model.addAttribute("plugloadProfileTreeHierarchy", getPlugloadProfileTreeHierarchy());
		...
		model.addAttribute("showBacnet", bacnetManager.isBacnetEnabled());
	...
	@RequestMapping("/plugloadprofiletree.ems")
	public String getPlugloadProfileTree(
        Model model,
        @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie,
        HttpServletResponse httpResponse) {
...
//		CommonUtils.isParamValueAllowed(messageSource, "tenantId", tenantId);
		TreeNode<FacilityType> facilityTreeHierarchy = facilityTreeManager
...
//		if(selectedFacilities.contains(",")){
//			assignedFacilities = selectedFacilities.split(",");
//		} else {
//			CommonUtils.isParamValueAllowed(messageSource, "assignedFacilities", selectedFacilities);
//		}
//		if(assignedFacilities!=null){
//			for(String assignedFacilitiesStr : assignedFacilities){
//				CommonUtils.isParamValueAllowed(messageSource, "assignedFacilities", assignedFacilitiesStr);
//			}
//		}
	...
	private TreeNode<FacilityType> getPlugloadProfileTreeHierarchy() {
		TreeNode<FacilityType> PlugloadProfileTreeHierarchy = null;
		// If visibility check is applied, tree will load all plugload profiles which are in visible state
		boolean visibilityCheck =true;
		PlugloadProfileTreeHierarchy = plugloadGroupManager.loadPlugloadProfileHierarchy(visibilityCheck);
		return PlugloadProfileTreeHierarchy;
	}
...
//		CommonUtils.isParamValueAllowed(messageSource, "userId", userId);
		Long tenantId = 0L;
		TreeNode<FacilityType> facilityTreeHierarchy = null;
