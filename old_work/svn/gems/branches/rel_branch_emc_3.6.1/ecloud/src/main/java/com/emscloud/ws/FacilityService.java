...
public class FacilityService {
	...
	public String getFacilityTree(@PathParam("custId") Long custId) {
  	...
  	if(metaDataServerIp.equalsIgnoreCase("localhost")) {
  		...
  		if(facility != null) {
  			getReplicaIdForFacility(facility);
  		}
	...
	private void getReplicaIdForFacility(Facility facility) {
		FacilityEmMapping emMapping = facilityEmMappingManager.getFacilityEmMappingOnFacilityId(facility.getId());
		if(emMapping != null) {
			EmInstance emInstance = emInstanceManager.getEmInstance(emMapping.getEmId());
			facility.setReplicaServerId(emInstance.getReplicaServer().getId());
		}
		if(facility.getChildFacilities() != null && facility.getChildFacilities().size() > 0) {
			for(Facility f: facility.getChildFacilities()) {
				getReplicaIdForFacility(f);
			}
		}
 	}
