...
import com.emscloud.vo.EmConnectivityDataVO;
...
public class InternalService {
	...
	@Resource
	EMEnergyConsumptionManager ecManager;
	
	@Path("getEmConnectivity/{emId}")
	@GET
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public EmConnectivityDataVO getEmConnectivity(@PathParam("emId") Long emId) throws UnsupportedEncodingException {
		
		EmInstance em = emInstMgr.getEmInstance(emId);
		EmConnectivityDataVO vo = new EmConnectivityDataVO();
		if(em != null) {
			vo.setId(emId);
			vo.setLastEmSync(em.getLastSuccessfulSyncTime());
			vo.setLastEmConnectivity(em.getLastConnectivityAt());
			vo.setLast5MinCaptureAt(ecManager.getMaxCaptureAtForEM(em.getDatabaseName(), em.getReplicaServer().getInternalIp()));
		}
		return vo;
