...
public class EmInstanceController {
	...
	public String loadEmTaskListByEmInstanceId(Model model, @RequestParam("emInstanceId") Long emInstanceId) {
		model.addAttribute("emInstanceId", emInstanceId);
		model.addAttribute("emInstanceVersion", emInstanceManager.loadEmInstanceById(emInstanceId).getVersion());
	...
	@RequestMapping("/scheduleEmCertificateUpgrade.ems")
    String scheduleEmCertificateUpgrade(Model model, @RequestParam("emInstanceId") Long emInstanceId) {
		...
		return "emScheduleCertificateUpgrade/details";
    }
