...
public class CallHomeManager {
	...
	private OSDetails os_cached = null;
	public void sendDataCallHome() throws Exception{
		...
		try {
			if (os_cached == null){
				...
				String[] osversionCmd = {"/bin/bash", System.getenv().get("OPT_ENLIGHTED")+"/communicator/getOSDetail.sh"};
				final String osversionjson = CommonUtils.executeScriptAndGetReply(osversionCmd);
				final ObjectMapper mapper = new ObjectMapper(); 
				OSDetails os = mapper.readValue(osversionjson, OSDetails.class);
				os_cached = os;
			}
			if (os_cached != null){
				cloudrequest.setOs(os_cached);
			}
		} catch (Exception e1) {
			logger.error("ERROR: Getting osversion of the EM", e1) ;
		}
