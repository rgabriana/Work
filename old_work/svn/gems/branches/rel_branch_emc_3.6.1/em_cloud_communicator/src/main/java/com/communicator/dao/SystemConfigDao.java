...
public class SystemConfigDao{
	...
	public void emptyMaxIdValues() {
		...
		updateSysConfigValue("plugload_energy_consumption.sync.max.id", "");
		updateSysConfigValue("plugload_energy_consumption_hourly.sync.max.id", "");
		updateSysConfigValue("plugload_energy_consumption_daily.sync.max.id", "");
	...
	public void setMaxIdValues() {
		...
		updateSysConfigValue("plugload_energy_consumption.sync.max.id", getMaxIdToBeSynced(CommonStateUtils.plugloadEnergyTableName).toString());
		updateSysConfigValue("plugload_energy_consumption_hourly.sync.max.id", getMaxIdToBeSynced(CommonStateUtils.plugloadEnergyHourlyTableName).toString());
		updateSysConfigValue("plugload_energy_consumption_daily.sync.max.id", getMaxIdToBeSynced(CommonStateUtils.plugloadEnergyDailyTableName).toString());
	...
	public boolean dataExistsBetweenIds(String tableName, Long minId, Long maxId) {
		SqlRowSet rs = this.jdbcTemplate.queryForRowSet("select min(id) from " + tableName + " where id > " + minId + " and id <= " + maxId);
		if (rs.next()) {
