...
public class UsersController {
    ...
    String getListOfUsers(Model model, @RequestParam(value = "tenantId", required = false) Long tenantId, @RequestParam(value = "facilityAssignmentStatus", required = false) String facilityAssignmentStatus,
    		@RequestParam(value = "newUserId", required = false) Long newUserId, @RequestParam(value = "status", required = false) String status) throws EmsValidationException {

    	//fix for EM-967 : when request is forwarded from save.ems to list.ems, latest status is appended with previous status. 
    	//Hence splitting the status at ',' in order to get latest status.
    	if(status != null){
    	...
    	Map<String,Object> nameValMap = new HashMap<String, Object>();
     	nameValMap.put("users.facilityAssignmentStatus", facilityAssignmentStatus);
     	nameValMap.put("usersController.status", status);
    ...
    public String unlockUser( User user,  @RequestParam("password") String password) throws EmsValidationException {

    	CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "users.password", password);
    ...
    @PreAuthorize("permitAll()")
    public String resetUserPassword(User user,  @RequestParam("password") String password) throws EmsValidationException {
    ...
    @PreAuthorize("permitAll()")
    public String toLoginPage(Model model,@PathVariable("userid") Long userid, @PathVariable("result") String result) throws EmsValidationException {
    	CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "users.tologinpage.result", result);
    	...
    	final User dbUser = userManager.loadUserById(user.getId());
    	 ...
    	 if("admin".equalsIgnoreCase(dbUser.getEmail())){
    		 try {
          		...
          		bReturn = false;
          		m_Logger.error("ERROR: while updating Password ",e);
          		e.printStackTrace();
          	}
         	...
         	try {
         		...
         		bReturn = false;
         		m_Logger.error("ERROR: while update Admin Type User's Password ",e);
         	}
    	 }
...
//    	 userManager.updateUserDetails(Constants.IDENTIFIER_FORGOT_PASS, null, String.valueOf(user.getId()));
    ...
    String saveUser(Model model,User user, @RequestParam("tenantId") Long tenantId, @RequestParam("password") String password) throws EmsValidationException {
    	...
    	if(user.getEmail() == null || "null".equals(user.getEmail()) || "".equals(user.getEmail())){
    		userAuditLoggerUtil.log("User email should not be null or Empty",
                    UserAuditActionType.Unauthorised_Access.getName());
    		m_Logger.error("ERROR: User email should not be null or Empty ");
            throw new IllegalArgumentException("User email should not be null or Empty");
    	}
    	 
    	Map<String,Object> nameValMap = new HashMap<String, Object>();
     	nameValMap.put("usersController.email", user.getEmail());
     	nameValMap.put("usersController.fname", user.getFirstName());
     	nameValMap.put("usersController.lname", user.getLastName());
     	nameValMap.put("usersController.contact", user.getContact());
     	nameValMap.put("usersController.form.status", user.getStatus().getName());
 	    CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, nameValMap);
        ...
        if ( user.getId()!=null && user.getId() ==1 ) {
            userAuditLoggerUtil.log("Super Admin User not permitted to change",
                    UserAuditActionType.Unauthorised_Access.getName());
            m_Logger.error("ERROR: Super Admin User not permitted to change ");
            throw new IllegalArgumentException("Super Admin User not permitted to change");
        }
        ...
        if (user.getId() == null || user.getId() == 0) {
            ...
            if(role.getRoleType() == RoleType.Admin){
            	try {
					...
					m_Logger.error("FileNotFoundException: while adding Admin Type User ",e);
				} catch (IOException e) {
					m_Logger.error("IOException: while adding Admin Type User ",e);
				}
            }
            ...
            if(role.getRoleType() == RoleType.Admin){
            	try {
            		...
            		m_Logger.error("FileNotFoundException: while updating Admin Type User ",e);
				} catch (IOException e) {
					m_Logger.error("IOException: while updating Admin Type User ",e);
				}
            }
    ...
    public String deleteUser(@RequestParam("userId") Long userId,
        ...
        if ( userId!=null && userId ==1 ) {
            userAuditLoggerUtil.log("Super Admin User not permitted to change",
                    UserAuditActionType.Unauthorised_Access.getName());
            m_Logger.error("ERROR: Super Admin User not permitted to delete ");
            throw new IllegalArgumentException("Super Admin User not permitted to change");
        }

        if(Constants.BACNET.equals(userName)){
        	userAuditLoggerUtil.log("Bacnet User should not be deleted",
                    UserAuditActionType.Unauthorised_Access.getName());
        	m_Logger.error("ERROR: Bacnet User should not be deleted ");
            throw new IllegalArgumentException("Bacnet User should not be deleted");
        }
        ...
        if(user.getRole().getRoleType() == RoleType.Admin){
        	try {
				...
				m_Logger.error("FileNotFoundException: While deleting Admin Type User ");
			} catch (IOException e) {
				m_Logger.error("IOException: While deleting Admin Type User ");
			}
        }
