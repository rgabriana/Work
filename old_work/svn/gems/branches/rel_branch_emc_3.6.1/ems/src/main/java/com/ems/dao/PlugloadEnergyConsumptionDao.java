...
public class PlugloadEnergyConsumptionDao extends BaseDaoHibernate{
	...
	public Map<String,Map<Long,List<Object[]>>> getAllPlugloadEnergyConsumptionZBRecords(Long plugloadId){
		...
		if(l != null){
			for(Object [] zbObjects : l){
				sql2 = "select min(capture_at) as capt_at,plugload_id,COALESCE(managed_energy_cum,0) as managed_energy_cum,COALESCE(unmanaged_energy_cum,0) as unmanaged_energy_cum from plugload_energy_consumption where zero_bucket = 0 "
						+ "and capture_at > '"+zbObjects[0].toString()+"' and managed_energy_cum is not null and plugload_id="+plugloadId
						+ " group by capture_at,plugload_id,managed_energy_cum,unmanaged_energy_cum order by capt_at limit 1";
				sql3 = "select max(capture_at) as capt_at,plugload_id,COALESCE(managed_energy_cum,0) as managed_energy_cum,COALESCE(unmanaged_energy_cum,0) as unmanaged_energy_cum from plugload_energy_consumption where zero_bucket = 0 "
						+ "and capture_at < '"+zbObjects[0].toString()+"' and managed_energy_cum is not null and plugload_id="+plugloadId
						+ " group by capture_at,plugload_id,managed_energy_cum,unmanaged_energy_cum order by capt_at desc limit 1";
				logger.info("=========  sql2 is "+sql2);
				logger.info("=========  sql2 is "+sql3);
	...
	public void updateZeroBuckets(Long id, Double spreadManagedEnertyCumValues,
			Double spreadUnManagedEnertyCumValues, Date nzbCaptat, Date zbCaptat) {
...
//    	System.out.println("================= pec"+pec);
	...
	/**
     * load latest PlugloadEnergyConsumption details for given plugload id.
     * 
     * @param id
     *            database id(primary key)
     * @return com.ems.model.PlugloadEnergyConsumption
     */
    @SuppressWarnings("unchecked")
    public PlugloadEnergyConsumption loadLatestPlugloadEnergyConsumptionByPlugloadId(Long id) {
        try {
            List<PlugloadEnergyConsumption> results = null;
            String hsql = "from PlugloadEnergyConsumption as ec where ec.plugload.id=? "
                    + "and ec.captureAt=(Select max(ec2.captureAt) from PlugloadEnergyConsumption as ec2 "
                    + "where ec2.plugload.id=?)";
            Query q = getSession().createQuery(hsql.toString());
            q.setParameter(0, id);
            q.setParameter(1, id);
            results = q.list();
            if (results != null && !results.isEmpty()) {
                return (PlugloadEnergyConsumption) results.get(0);
            }
