...
<spring:url value="/services/org/plugload/rma/" var="rmaPlugloadUrl" scope="request" />
...
function validatePlugloadForm(){	
	...
	else if(!IsDoubleValue($.trim($('#predefinedBaselineload').val()))){
		$("#predefinedBaselineload").addClass("invalidField");
		displayPlCommissionMessage("Predefined Baseline load should be a valid Number", COLOR_FAILURE);
		return false;
	}else {
		clearPlCommissionMessage();		
...
function getPlugloadXML(){
	...
	var xmldata = "<plugload>"+
						...
						"<predefinedBaselineLoad>"+$("#predefinedBaselineload").val()+"</predefinedBaselineLoad>"+
					"</plugload>";
...
var confirmPlugloadRmaMonoShotTimer;

function rmaPlugload(plugloadId, plugloadName, plugloadVersion) {
	if(plugloadRmaTimerInProgress == 0){
		confirmPlugloadRmaMonoShotTimer = setTimeout(rmaPlugloadConfirm, 1000, plugloadId, plugloadName, plugloadVersion);
		plugloadRmaTimerInProgress = 1;
	}
}

function rmaPlugloadConfirm(plugloadId, plugloadName, plugloadVersion){
	
	clearTimeout(confirmPlugloadRmaMonoShotTimer);
	plugloadRmaTimerInProgress = 0;
	
	if(!validatePlugloadForm()){
		return false;
	}

	var ver1 = plugloadVersion.charAt(0);
	var ver2 = CURRENT_PL_OBJECT.version.charAt(0);
	
	if(ver1 != ver2)
	{
		alert("RMA does not work for plugloads with different version");
		return false;
	}

	enableDisableUIControls(false);

	var bConfirm = confirm("Selected Plugload : " + plugloadName + "  Replace with : " + $("#plugloadname").val()
			+ "\n				Do you want to continue?");
	
	if(bConfirm) {
		displayPlCommissionMessage("Performing Fixture RMA...", COLOR_SUCCESS);
		$.ajax({
			url: "${rmaPlugloadUrl}" + plugloadId + "/" + CURRENT_PL_OBJECT.id,
			type: "POST",
			async: false,
			dataType:"xml",
			contentType: "application/xml; charset=utf-8",
			success: function(data){
			displayPlCommissionMessage("RMA successful", COLOR_SUCCESS);
				// In case of RMA the old plugload id is retained so assign it accordingly
			CURRENT_PL_OBJECT.id = plugloadId;
...
function getCommissionPlacementStatus_Success(status){
	if(status.toUpperCase() == COMMISSIONED) {
		...
		try {
			...
			getCommissionPlanObj("c_plugload_floorplan").addPlugload('floor', '${floorId}', $("#plugloadId").val(), $("#plugloadname").val(), "", bRMAInProgress);
		} catch(e){
			resetDevicePositionOnFloorplan();
								...
								<form id="pl-basic-form" class="plcd-form-body">
									...
									<fieldset class="form-column-left">
										<div class="fieldWrapper">
											...
											<br style="clear:both;"/>
											<br/>
											<div class="fieldlabel"><label for="plugloadname">Baseline Load:</label></div>
											<div class="fieldInput"><input id="predefinedBaselineload"  size="10" maxLength="10" onkeypress="clearMessage(this.id);"/></div>											
