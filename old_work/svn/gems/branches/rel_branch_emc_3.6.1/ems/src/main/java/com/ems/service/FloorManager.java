...
public class FloorManager {
            ...
            for (int i = 0; i < floors.size(); i++) {
                if (floors.get(i).getPlanMap() != null) {
                    if (floors.get(i).getPlanMap().getId() != null) {
                        ...
                        floors.get(i).setByteImage(planMap.getPlan());
                    }
                }
            }
        }
    ...
    public void updateFloor(Floor floor) {
        try {
            ...
            floor1.setSiteId(floor.getSiteId());
            if (floor.getPlanMap() != null && floor.getPlanMap().getFileData() != null && !floor.getPlanMap().getFileData().isEmpty()) {
                floor1.setFloorPlanUrl(floor.getPlanMap().getFileData().getOriginalFilename());
                PlanMap planMap = null;
                if (floor1.getPlanMap() != null) {
    ...
    public List getAllFloorsOfCompanyWithNames() throws SQLException, IOException {
    	
      List floors = floorDao.getAllFloorsOfCompanyWithNames();
      // Use loadFloorByBuildingId in case you want plan map for all the floors.
      /*
       * if(floors!=null){ for(int i=0;i<floors.size();i++){ if(floors.get(i).getPlanMap()!=null){
       * if(floors.get(i).getPlanMap().getId()!=null){ PlanMap planMap=new PlanMap(); planMap=
    ...
    public String getApplySettingsStatus(){
		SystemConfiguration sc = systemConfigurationManager.loadConfigByName("deleteDeviceStatus");
		String status = "";
		if(sc != null){
			status = sc.getValue();
		}
    ...
    public Floor getFloorByUid(String uid) {
    	return floorDao.getFloorByUid(uid);
    }
