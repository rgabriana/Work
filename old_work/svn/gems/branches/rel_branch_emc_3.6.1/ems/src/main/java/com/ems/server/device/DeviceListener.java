...
public class DeviceListener {
   ...
   */
    ...
    public class GwResponseWork implements Runnable {
        ...
        public GwResponseWork(byte[] pkt, String mac, boolean poolPkt, boolean ipBased) {
        ...
        public void run() {
          ...
          GatewayResponsePacket gwRespPkt = null;
            try {
                ...
                gwRespPkt = new GatewayResponsePacket(packet, gwIp, ipBased);
    ...
    // this method is used to add the response packet from the gateway to the thread pool
    // it will be called from this class and SSL
    public void addGatewayKafkaResponse(byte[] pkt, String mac, boolean poolPkt) {

        GwResponseWork work = new GwResponseWork(pkt, mac, poolPkt, false);
        threadPool.addWork(work);
        logger.debug(mac + ": queue size - " + threadPool.getQueueSize());
        work = null;
