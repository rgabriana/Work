...
public class GatewayComm {
  ...
  private void sendDataPktToGateway(Gateway gw, byte[] gwPacket) {
  	...
  	//if it is EMC then send the message to Kafka
  	if(ServerMain.getInstance().isEmcMode()) {
  		//System.out.println("sending over kafka");
  		KafkaProxyEndpoint.getInstance(ServerConstants.KAFKA_GW_NETWORK).sendMessageToGateway(ByteBuffer.wrap(gwPacket), gw.getMacAddress().replaceAll(":", ""));
  		return;
  	}
  	//System.out.println("sending over ssl");
    ...
    if(sslEnabled) {
      if(gw.getApp2Version() == null || gw.getApp2Version().equals("")) {
      	try {
      		...
      		//e.printStackTrace();
      		System.err.println("could not send the packet "+ (gw!=null?" ( GW having IP "+gw.getIpAddress()+" with gateway id "+ gw.getId()+ ") " : "(GW Null) ")+"over SSL: " + e.getMessage());
      ...
      try {
      	...
      	System.err.println("could not send the gateway "+ (gw!=null?" (having IP "+gw.getIpAddress()+" with gateway id "+ gw.getId()+")" : " (Not Available)")+" packet: " + ex.getMessage());
      }
    } else {
      sendDataToGwUDP(gw.getIpAddress(), gwPacket);
    }
  ...
  public void sendNodeDataToGateway(long gwId, String gwIp, byte[] packet) {
    ...
    byte[] gwPacket = gwPktObj.getGatewayPacket();
    System.out.println("sending in node data to gateway");
    sendDataPktToGateway(gw, gwPacket);   
