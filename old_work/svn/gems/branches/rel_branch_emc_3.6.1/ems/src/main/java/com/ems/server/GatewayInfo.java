    ...
    public void run() {
      ...
      while(operationalMode != GW_DELETE_MODE) {
      	...
      	try {	  
      		...
      		fixtureLogger.error(gw.getMacAddress() + "- not able to send time", ex);
      	}
      	...
      	if(emcMode) {
      		...
      		//in local mode, time command should be sent every 5 minutes instead of waiting for cloud interval
      		cloudTime = true;
      	}
      }      
