...
<spring:url value="/scripts/enlighted/em.js" var="emjs"></spring:url>
<script type="text/javascript" src="${emjs}"></script>
...
<c:if test="${emcMode != null && emcMode != ''}">
	<c:set var='emcMode' value="${emcMode}" scope="session" />
</c:if>
...
<c:if test="${enableProfileFeature != null && enableProfileFeature != ''}">
	<c:set var='enableProfileFeature' value="${enableProfileFeature}" scope="session" />
</c:if>
<c:if test="${enablePlugloadProfileFeature != null && enablePlugloadProfileFeature != ''}">
	<c:set var='enablePlugloadProfileFeature' value="${enablePlugloadProfileFeature}" scope="session" />
</c:if>

<c:if test="${title24 != null && title24.compliance != null && title24.compliance.flag != 'false'}">
	<c:set var='enableTitle24' value="${title24.compliance.flag}" scope="session" />
</c:if>
		    	     ...
		    	     if (xhr.status == 200){
		        	   ...
		        	   if($(data).find("#maintenance").html() == "Y") {
                          ...
                          clearInterval(networkCheck);
                    	   ...
                    	   if($.cookie('accordian_select') == "facility"){
							   if(drstatuschangedtime == ""){
								   drstatuschangedtime = $(data).find("#drstatuschangedtime").html();
			            	   }else if (drstatuschangedtime != $(data).find("#drstatuschangedtime").html()){
			            		   drstatuschangedtime = $(data).find("#drstatuschangedtime").html();
			            		   try{
				            		   refreshFacilitiesTree();
				            	   }catch(e){
	   			...
	   			if($(data).find("#duration").html() != "")
	   			{
		        	...
		        	var type = $(data).find("#drtype").html(); 
	   				...
	   				if($(data).find("#duration").html() != "-1" && (durationinsec == 0 || timeremaininginsec > 0))
	   				{
	   					...
	   					var spamNodetext1 = document.createTextNode("Override in progress. Initiate type: ");
	...
	<table id="tblcompcreated" border="0" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<td align="right">
				<table border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						<td>
							<div class="nav" style="display:table-cell;">
								<ul id="cssdropdown">
									...
									<security:authorize
										...
										<li class="headlink" onmouseover="showMenu();" onmouseout="hideMenu();"><a id="adminMenu" href="#" class="link" >
											...
											<ul id="subMenu">
												...
												<spring:url value="/settings/network_settings.ems" var="networkSettings" />
												<li><a href="${networkSettings}"
												onclick="highLightTab(2);" id="networkSettingsMenu"><spring:message
												code="menu.networkSettings" /></a></li>
												</security:authorize>
												...
												<li><a href="${drTarget}" onclick="highLightTab(2);" id="drTargetMenu"><spring:message
															code="menu.overrideschedules" /></a></li>
												<security:authorize access="hasAnyRole('Admin')">			
												...
												<spring:url value="/bacnet/bacnetConfiguration.ems" var="bacnetConfig" />
												...
												<li><a id="gemsmanagement" target="_blank" onClick="hideMenu();"><spring:message
															code="menu.gemsmanagement" /></a></li>
												</security:authorize>
												...
												<spring:url value="/plugloadProfileTemplateManagement/management.ems" 
													var="plugloadTemplateManagement"/>
													
												<c:if test="${enablePlugloadProfileFeature == 'true'}">
												
												<li><a href="${plugloadTemplateManagement}" id="plugloadProfileTemplateMainMenu"
													onclick="highLightTab(2);"><spring:message
													code="menu.plugloadTemplateManagement" /></a></li>
												</c:if>
												...
												<security:authorize access="hasAnyRole('Admin')">
												<spring:url value="/licenseSupport/management.ems" 
													var="showLicenseSupportUrl"/>
												<li><a href="${showLicenseSupportUrl}" id="showLicenseSupportMainMenu"
													onclick="highLightTab(2);"><spring:message
													code="menu.showLicenseSupport" /></a></li>
												</security:authorize>
												
												<security:authorize access="hasAnyRole('Admin')">
												<spring:url value="/emailNotification/management.ems" 
													var="emailNotificationUrl"/>
												<li><a href="${emailNotificationUrl}" id="emailNotificationMenu"
													onclick="highLightTab(2);"><spring:message
													code="menu.emailNotification" /></a></li>
												</security:authorize>
												 
												
												<security:authorize access="hasAnyRole('Admin')">
												<spring:url value="/contactClosure/management.ems" 
													var="contactClosureUrl"/>
												<c:if test="${emcMode != 'true'}">
												<li><a href="${contactClosureUrl}" id="contactClosureMenu"
													onclick="highLightTab(2);"><spring:message
													code="menu.contactClosure" /></a></li>
												</c:if>
												</security:authorize>
												
												<security:authorize access="hasAnyRole('Admin')">
													<c:if test="${enableTitle24 == 'true'}">
														<spring:url value="/title24/compliance.ems" 
															var="title24complianceUrl"/>
														<li><a href="${title24complianceUrl}" id="title24ComplianceMenu"
															onclick="highLightTab(2);"><spring:message
															code="menu.title24compliance" /></a></li>
													</c:if>
												</security:authorize>
									...
									<security:authorize
										...
										<li class="headlink"><spring:url
											...
											<ul id="reportsSubMenu">
												...
												<li><spring:url value="/reports/gwoutage.ems"
														var="gwoutage" /> <a href="${gwoutage}"
													onclick="highLightTab(3);" id="gwoutageMenu"><spring:message
															code="menu.gwoutage" /></a>
												</li>		
												<security:authorize
												...
												<security:authorize
													access="hasAnyRole('Admin','Auditor','FacilitiesAdmin', 'TenantAdmin')">
													<li><spring:url value="/reports/inventoryreport.ems"
															var="inventoryreport" /> <a href="${inventoryreport}"
														onclick="highLightTab(3);" id="inventoryReportMenu"><spring:message
																code="menu.inventoryreport" /></a></li>
												</security:authorize>
												<security:authorize
												access="hasAnyRole('Admin','Auditor','FacilitiesAdmin', 'TenantAdmin')">
												<li><spring:url value="/reports/ercbatteryreport.ems"
														var="ercbatteryreport" /> <a href="${ercbatteryreport}"
													onclick="highLightTab(3);" id="ercbatteryreportMenu"><spring:message
															code="menu.ercbatteryreport" /></a></li>
												</security:authorize>
											</ul></li>
									</security:authorize>
		...
		<tr>
			...
			<td align="right"><ems:showAppVersion />&nbsp;<c:set var="now"
					...
					pattern="yyyy-MM-dd HH:mm z" value="${now}" /></td>
		</tr>
	</table>
	...
	<c:if test="${apacheInstalled != null}">
		<script type="text/javascript">
			...
			var gemsupgrade_url = location.protocol + '//' + location.host
					+ '/em_mgmt/upgrade/';
			$("#gemsupgrade").attr("href", gemsupgrade_url);
			var gemsmanagement_url = location.protocol + '//' + location.host
			+ '/em_mgmt/history/';
			$("#gemsmanagement").attr("href", gemsmanagement_url);
		</script>
	</c:if>
	<spring:url value="/users/list.ems" var="usersList" />
					...
					<div id="firstStep">
						<table cellpadding="0" cellspacing="0" width=100%>
							<tr>
								...
								<td class="next"><em><spring:message
											code="setup.step6.header" /></em><br /> <span><spring:message
											code="setup.step6.description" /></span></td>
							</tr>
						</table>
					</div>
					<div id="secondStep">
