...
public class AreaServiceV1 {
	...
	@PreAuthorize("hasAnyRole('Admin','Bacnet')")
  	...
  	public OccupancyStatus getOccupancyStateOfArea(@PathParam("area_id") Long area_id) {
  		...
  		if(!licenseSupportManager.isZoneSensorsEnabled()){
  			occStatus.setOccupancyState("-2");
  			return occStatus;
  		}
  		
  		Object[] area = areaManager.getOccupancyStatusOfArea(area_id);
  		if(area == null || area.length == 0) {
  			//there is no matching areas
  			occStatus.setOccupancyState("-3");
  			return occStatus;
  		}
  
  		//0 index is lighting_occ_count
  		//1 index is percentage_faulty_sensors
			//2 index is zone_sensor_enable
			//3 index is total no. of sensor
			//4 index is no. of faulty sensors
  		
  		if(!((Boolean)area[2]).booleanValue() ){
  			occStatus.setOccupancyState("-2");
  			return occStatus;
  		}
  		...
  		if(area[1] != null) {
  			percentageSensorsFaulty = ((Short)area[1]).intValue();
  		}
  		
  		if(area[3] != null && area[4] != null) {
  			long totalCount = ((BigInteger)area[3]).longValue();
  			long faultyCount = ((BigInteger)area[4]).longValue();
  			if((100 * faultyCount/totalCount) > percentageSensorsFaulty){
  				occStatus.setOccupancyState("-1");
  				return occStatus;
  			}
  			
  			if(area[0] != null && ((Short)area[0]).intValue() > 0) {
  				occStatus.setOccupancyState("1");
  			} else {
  				occStatus.setOccupancyState("0");
  			}
  		}		
  		return occStatus;		
	...
	public Response setEMEmergency(@PathParam("area_id") Long area_id,@QueryParam("time") Integer time) {
		...
		fixtures = fixtureManager.loadFixtureByAreaId(area_id);
		if(fixtures==null)
		{
		...
		fixtureManager.dimFixtures(fixtureList, 100, time);
		List<Plugload> plugloadArr = new ArrayList<Plugload>();
		plugloadArr = plugloadManager.loadPlugloadByAreaId(area_id);
		if(plugloadArr==null)
		{
			plugloadArr = new ArrayList<Plugload>();
		}
	...
	/** This API allows getting the emergency mode of an area 
	 * @param  area_id
	 * 			  unique identifier of the area
	 * @return Response
	 * 			-1- error
	 ...
	 */
	@PreAuthorize("hasAnyRole('Admin','Bacnet')")
	@Path("getEmergencyMode/{area_id}")
	@GET
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response getEMEmergencyMode(@PathParam("area_id") Long area_id) {
		
		Response res = new Response();
		if(area_id <= 0) {
			res.setStatus(-1);
		} else {
			List<Fixture> fixtures = fixtureManager.getDimmedFixturseByAreaId(area_id);
			if(fixtures == null ){
				res.setStatus(0); // Area has no fixtures
			}else if(fixtures.size() == 0) {
				res.setStatus(1); // Area has no emergency fixtures
			} else {
				res.setStatus(0);
			}
		}
