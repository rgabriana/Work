...
<spring:url value="/services/org/fixture/op/setdualchannelled/"
	var="updateDCLfixtureUrl" scope="request" />
	
<spring:url value="/services/org/fixture/op/mode/" var="autofixtureUrl"
	scope="request" />
<spring:url value="/services/org/fixture/op/colortemperatureauto" var="colorTemperatureAutofixtureUrl"
	scope="request" />
...
<spring:url	value="/services/org/fixture/reboot/" var="rebootfixtureurl" scope="request" />

<spring:url	value="/services/org/fixture/op/initreaddress" var="readdressdaliUrl" scope="request" />
...
var orgDCLSlider1Value = 0;

//var currUptime = '<c:out value="${gateway.currUptime}"/>';
var currUptime = '<c:out value="${fixtureUpTime}"/>';

$("#fixture_currUptime").text(getLastCommunicationsString(currUptime).replace("ago", ""));

function fixtureReboot()
{	
	var fixtureId = "${fixture.id}";
	var proceed = confirm("Are you sure you want to reboot the fixture?");
	if(proceed){
		$.ajax({
			type: 'GET',
			url: "${rebootfixtureurl}"+fixtureId+"?ts="+new Date().getTime(),			
...
// 	$("#updateButton").button();
	...
	$('#enableFixtureType').click(function() {
		$('#fixtureType').val(this.checked?"1":"0");
	});
	...
	function loadFormValues(){
		...
		var fixtureType = '<c:out value="${fixture.fixtureType}"/>';
		var dualControlSlider1 = '<c:out value="${fixture.dualChannelLedValue}"/>';
		
		$("#enableHopper").attr('checked', isHopper==1);
		$("#enableFixtureType").attr('checked', fixtureType==1);
		...
		$("#dualChannelCurrentstate").html((dualControlSlider1<=0?"OFF":"ON") + " ("+dualControlSlider1+"%)");
		...
		//Create DUAL CHANNEL LED Slider 1
		var defaultDCLSlider1Value = dualControlSlider1;
		$( "#slider_dualchannel1" ).slider({
			value: defaultDCLSlider1Value,
			min: 0,
			max: 100,
			step: 1,
			change: function( event, ui ) {
				updateDualSliderValueSpan(ui.value);
			}
		});
		$( "#slider_dualchannel1" ).bind( "slidestop", function(event, ui) {
			updateDualSliderValueSpan(ui.value);
			setDCLDimmerState(ui.value);
		});
		$( "#slider_dualchannel1" ).bind( "slidestart", function(event, ui) {
			orgDCLSlider1Value = ui.value;
		});
		updateDualSliderValueSpan(defaultDCLSlider1Value);
	...
	function setDualChannelSliderValue(value)
	{
		$("#slider_dualchannel1").slider("value", value);
		setDCLDimmerState(value) ;
	}
	function setDCLDimmerState(value){
		var dualchlightControlMinutes = $("#dualchlightControlMinutes").val();
		
		if(dualchlightControlMinutes == "") {
			displayLabelMessage('<spring:message code="fixtureForm.message.validation.time_blank"/>', COLOR_FAILURE);
			resetDCLSliderValue();
		}else if(!(/^([0-9]+)$/).test(dualchlightControlMinutes)){
			displayLabelMessage("Manual Override Time should be a Number", COLOR_FAILURE);
			resetDCLSliderValue();
			return;
		}else if(dualchlightControlMinutes > 99999){
			displayLabelMessage("Manual Override Time should be less than or equal to 99999", COLOR_FAILURE);
			resetDCLSliderValue();
			return;
		}
		else if(1*dualchlightControlMinutes == 0) {
			displayLabelMessage('<spring:message code="fixtureForm.message.validation.time_zero"/>', COLOR_FAILURE);
			resetDCLSliderValue();
		}else {
			clearLabelMessage();
			setDCLCurrentState(value);

			$.ajax({
				type: 'POST',
				url: "${updateDCLfixtureUrl}"+value+"/"+(1*dualchlightControlMinutes),
				data: "<fixtures><fixture><id>${fixture.id}</id></fixture></fixtures>",
				success: function(data){
	 					},
				dataType:"xml",
	...
	function updateDualSliderValueSpan(value)
	{
		$("#slider_dualchannel1_value").html(value+"%");
		$("#slider_dualchannel1_value").css("left", (value*3 - (value<10?15:20)) + "px");
	}
	function resetDCLSliderValue(){
		$("#slider_dualchannel1_value").slider("value", orgDCLSlider1Value);
		updateSliderValueSpan(orgDCLSlider1Value) ;
	}
	function setDCLCurrentState(dimmerControlValue) {
		$("#dualChannelCurrentstate").html((dimmerControlValue<=0?"OFF":"ON") + " ("+dimmerControlValue+"%)");
	}
	...
	function saveFixture(){
		...
		if ($("#fixtureClassCombo option:selected").text() == "Select Fixture Type"){
			displayLabelMessage("Select a Fixture Type from the list", COLOR_FAILURE);
			return false;
		}
		...
		if(FixtureName == ""){
			...
			var new_current_profile_id = $("#currentProfile").val();
			var new_current_profile = getSelectedProfile(new_current_profile_id);
			var fixGroupId = '<c:out value="${fixture.groupId}"/>';
			//get groupId from profile name
			$("#groupId").val(new_current_profile_id);
			if(fixGroupId != new_current_profile_id){
				...
				if(FX_CURRENT_PROFILE_BEFORE_UPDATE == "Custom"){
					...
					for (var i = 0; i < modifiedCurrentProfile.options.length; i++) {
					    if (modifiedCurrentProfile.options[i].text == "Custom") {
					    	//alert("i "+i);
					    	modifiedCurrentProfile.remove(i);
					        break;
					    }
					}
	...
	function sendColorTemperatureAuto(){
		$.ajax({
			type: 'POST',
			url: "${colorTemperatureAutofixtureUrl}",
			data: "<fixtures><fixture><id>${fixture.id}</id></fixture></fixtures>",
			success: function(data){
 				},
			dataType:"xml",
	...
	function daliReaddressing() {
		$("#readdressdali_message").html("Please wait...");
		$.ajax({
			type: 'POST',
			url: "${readdressdaliUrl}",
			data: "<fixtures><fixture><id>${fixture.id}</id></fixture></fixtures>",
			success: function(data){
				if (data.status == 0) {
					$("#readdressdali_message").html("Success");
				}else {
					$("#readdressdali_message").html("Please try again!");
				}
 			},
			dataType:"json",
			contentType: "application/xml; charset=utf-8",
...
<body>
	<div id="fixture-dialog-form">
		...
		<form:form id="fixture-form" commandName="fixture" method="post"
			...
			<form:hidden id="fixtureType" name="fixtureType" path="fixtureType" />
			<div class="upperdiv divtop">
				<fieldset class="column50">
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.ModelNo" />
							:
						</div>
						<div class="fieldValue">${fixture.modelNo}</div>
					</div>
				</fieldset>
				<fieldset class="column50">
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.GatewayName" />
							:
						</div>
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="gatewayForm.label.lastCommunication" />
							:
						</div>
						<div class="fieldValue">
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="gatewayForm.label.uptime" />
							:
						</div>
						<div id="fixture_currUptime" class="fieldValue"></div>
					</div>
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="gatewayForm.label.radioChannelID" />
							:
						</div>
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="gatewayForm.label.radioNetworkID" />
							:
						</div>
			...
			<div class="upperdiv">
				<fieldset class="column33">
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.bleFirmwareversion" />
								:
						</div>
						
						<c:if test="${fixture.bleFirmwareVersion!=null}">
						<div id="blefirmVersion" name="blefirmVersion" class="fieldValue">${fixture.bleFirmwareVersion}</div>
						</c:if>
						<c:if  test="${fixture.bleFirmwareVersion==''}">	
							<div class="fieldValue">NA</div>
						</c:if>	
				...
				<fieldset class="column33 longlabel">
					...
					<div class="fieldWrapper">
						...
						<div class="fieldValue">
							<ems:breakSecondsInString seconds="${fixture.lastOccupancySeen}" />
						</div>
					</div>
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.CurrentAmbientThreshold" />
							:
						</div>
						<div class="fieldValue">${fixture.currentAmbientValue}</div>
					</div>
					
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.bleMode" />
								:
						</div>
						
						<c:if test="${fixture.currBleMode!=null}">
						<div id="blefirmVersion" name="blefirmVersion" class="fieldValue">${fixture.currBleMode}</div>
						</c:if>
						<c:if  test="${fixture.currBleMode==''}">	
							<div class="fieldValue">Unknown</div>
						</c:if>	
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.FixtureOccupancyStatus" />
							:
						</div>
						<div class="fieldValue">
							<label id="fixtureOccupancyStatus">${fixtureOccupancyStatus}</label>
						</div>
					</div>
...
<!-- 					</div> -->
				...
				<fieldset class="column33">
					...
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.ManuallySetAmbientThreshold" />
							:
						</div>
						<c:if test="${fixture.manualAmbientValue>-1}">
							<div class="fieldValue">${fixture.manualAmbientValue}</div>
						</c:if>
						<c:if  test="${fixture.manualAmbientValue==-1}">	
							<div class="fieldValue">Not set</div>
						</c:if>	
					</div>					
					<div class="fieldWrapper">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.CircRatio" />
							:
						</div>
						<div class="fieldValue">${circRatio}</div>
					</div>
					<div class="fieldWrapper">
						<div class="fieldValue">&nbsp;&nbsp;</div>
					</div>
					...
					<div class="fieldWrapper"
						style="border-bottom: thin solid #AAAAAA; border-right: thin solid #AAAAAA; border-bottom-right-radius: 10px">
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.DaliDriverCnt" />
							:
						</div>
						<c:choose>
							<c:when test="${fixture.daliDriverCount != null}">
								<div class="fieldValue">${fixture.daliDriverCount}</div>
							</c:when>
							<c:otherwise>
								<div class="fieldValue">NA</div>
							</c:otherwise>
					...
					<div class="fieldWrapper">
						<!-- 			<div class="fieldLabel"></div> -->
						<div style="padding-top: 5px;">
							<input type="checkbox" name="enableFixtureType" id="enableFixtureType"
								style="width: 12px;" /> <label for="enableFixtureType"
								style="font-weight: bold;">Emergency Fixture</label>
						</div>
					</div>
					<div class="fieldWrapper">
						<div style="padding-top: 5px;"></div>
						<div class="fieldLabel">
							<spring:message code="fixtureForm.label.HeartbeatStatus" />
							:
						</div>
						<div class="fieldValue">
							<label id="HeartbeatStatus">${heartbeatStatus}</label>
						</div>
					</div>
				</fieldset>
				<div class="fieldWrapper">
							...
							<table cellspacing="0" cellpadding="0">
								<tr>
									...
									<td>&nbsp; <span><spring:message
												code="fixtureForm.label.manualfor" />:</span>&nbsp; 
										<form:input class="text" id="lightControlMinutes" style="width: 30px;"
											name="manualModeDuration" path="manualModeDuration" />		
										&nbsp; <span><spring:message
					...
					<tr>
						<td width=33%>
							<table cellspacing="0" cellpadding="0">
								<tr>
									<td><div class="fieldLabel">
											<spring:message code="fixtureForm.label.colorTemperature" />
											:&nbsp;
										</div></td>
									<td>
										<button onclick="javascript:sendColorTemperatureAuto(); return false;">
											<spring:message code="fixtureForm.label.Auto" />
										</button>
									</td>
									<td>&nbsp; <span><spring:message
												code="fixtureForm.label.manualfor" />:</span>&nbsp; 
										<form:input class="text" id="dualchlightControlMinutes" style="width: 30px;"
											name="dualChannelLedDuration" path="dualChannelLedDuration" />		
										&nbsp; <span><spring:message
												code="fixtureForm.label.min" /></span>
									</td>
									<td>
						...
						<table cellspacing="0" cellpadding="0">
								<tr>
									<td>
										<button onclick="javascript:setDualChannelSliderValue(0); return false;">
											<spring:message code="fixtureForm.label.Off" />
										</button>
									</td>
									...
									<td style="width: 300px; padding: 2px 25px;">
										...
										<div id="slider_dualchannel1"></div>
										<div id="slider_dualchannel1_info" style="padding-top: 7px;">
											<span id="slider_dualchannel1_value"
												style="position: relative; background-color: #DDDDDD; border: thin solid #AAAAAA; padding: 0 2px;">0%</span>
										</div>
									</td>
									...
									<td>
										<button
											onclick="javascript:setDualChannelSliderValue(100); return false;">
											<spring:message code="fixtureForm.label.Fullon" />
										</button>
									</td>
								</tr>
						...
						<td width=33%>
						<div class="fieldLabel">
											&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="fixtureForm.label.dualChannelStatus" />&nbsp;:
										</div>
										<div class="fieldValue" id="dualChannelCurrentstate">${fixture.dualChannelLedValue}</div>
						</td>
					</tr>
				</table>
			</div>
			...
			<div class="upperdiv" style="padding-left: 5px;">
						...
						<tr>
							<td>
								<button id="rebootButton"
									onclick="javascript:return fixtureReboot();">
									<spring:message code="fixtureForm.label.rebootBtn" />
								</button> Reboot. &nbsp;
							</td>
							<td>
					...
					<table cellpadding="0" cellspacing="0">
						<tr>
							<td>
								<button id="readdressdaliBtn"
									onclick="javascript:return daliReaddressing();">
									<spring:message code="fixtureForm.label.readdressdaiBtn" />
								</button>Rediscover the number of drivers&nbsp;
							</td>
							<td>
								<div id="readdressdali_message"
									style="font-size: 14px; font-weight: bold; padding: 5px 0 0 10px;"></div>
							</td>
						</tr>
					</table>
