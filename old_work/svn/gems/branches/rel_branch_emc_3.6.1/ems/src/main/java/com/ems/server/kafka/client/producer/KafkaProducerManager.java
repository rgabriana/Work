...
public class KafkaProducerManager {
	 ...
	 * @param the id of the gateway for which this message is produced
	 * @param the partition on which this message is to be produced
	 * @param the byte array representing the binary message
	 */
	public void produceGatewayBinary(String gatewayId, 
			Integer partition,
			byte[] bytes) {
		binaryProducer.send(new ProducerRecord<String, byte[]>(generateOutboundTopic(gatewayId),bytes),
							createCallback());
		if(logger.isDebugEnabled()) {
			logger.debug(gatewayId + "- exiting produceBinary");
		}
	}
	
	/***
	 * Produces the passed binary message on the topic
	 * @param the topic for which this message is produced
	 * @param the partition on which this message is to be produced
	 * @param the byte array representing the binary message
	 */
	public void produceBinary(String topic,
			Integer partition,
			byte[] bytes) {
		binaryProducer.send(new ProducerRecord<String, byte[]>(topic,bytes),
							createCallback());
		if(logger.isDebugEnabled()) {
			logger.debug(topic + "- exiting produceBinary");
		}
	}
