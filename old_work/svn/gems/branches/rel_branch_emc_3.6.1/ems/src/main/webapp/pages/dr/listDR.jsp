...
<spring:url value="/dr" var="assignFacilityDrUrl" scope="request"/>

<fmt:setLocale value="en_US"/>
<div id="drDetailsDialog"></div>
<div id="assignFacilityDRDialog"></div>
...
function startUpperTable(pageNum, orderBy, orderWay) {
	...
	jQuery("#weekdayTable").jqGrid({
		...
		colNames:["id", "Event Type", "Identifier", "Start Time", "Duration(min)", "Pricing(per kWh)", "Override Level", "Status", "Action"],
	   	colModel:[			
			{name:'id', index:'id', hidden:true},			
	   		{name:'drtype', index:'drtype', align:"center", sorttype:"string", width:"12%", searchoptions:{sopt:['cn']}, formatter:eventTypeRenderer},
	   		{name:'dridentifier', index:'dridentifier', align:'center', sorttype:"string", width:"12%", search:false, },
	   		{name:'starttime', index:'starttime', align:"center", sortable:true, width:"15%", formatter:datetimeRenderer},
	   		{name:'duration', index:'duration', align:"center", sortable:true, sorttype:"number", width:"12%", search:false, formatter:durationRenderer},
	   		{name:'pricing', index:'pricing', align:"center", sortable:true, width:"12%", search:false, formatter:pricingRenderer},
	   		{name:'pricelevel', index:'pricelevel', align:"center", width:"12%", sortable:true, sorttype:"string", search:false, },
	   		{name:'drstatus', index:'drstatus', align:"center", sortable:true, sorttype:"string", width:"12%", search:false, },
	   		{name:'action', index:'action', align:"center", sortable:false, width:"28%", search:false, formatter:actionRenderer},	   		
	   	],
	   	jsonReader: { 
	        root:"drtarget", 
	        page:"page", 
...
function actionRenderer(cellvalue, options, rowObject){	
	...
	action += "&nbsp;<button onclick=\"assignFacilityDR("+drId+")\">"
	+ "<spring:message code='action.assignFacility' />" + "</button>";
...
function assignFacilityDR(id){	
	$("#assignFacilityDRDialog").load("${assignFacilityDrUrl}/"+id+"/assignFacilityDR.ems?ts="+new Date().getTime()).dialog({
...
function closeAssignFacilityDRDialog(){
	$("#assignFacilityDRDialog").dialog('close');        	
}
