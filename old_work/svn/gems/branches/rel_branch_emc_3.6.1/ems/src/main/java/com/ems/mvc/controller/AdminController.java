...
public class AdminController {
	...
	public String orgSetup(Model model, @RequestParam(value = "dhcpSettingStatus", required = false) String dhcpSettingStatus, @RequestParam(value = "serverReboot", required = false) Boolean serverReboot) throws EmsValidationException {
		...
		if (emcMode != null) {
			if("1".equals(emcMode.getValue())){
				company.setEmcMode(true);
			}else{
				company.setEmcMode(false);
			}
        }
		
		SystemConfiguration emcKafkaServer = systemConfigurationManager.loadConfigByName("kafka.bootstrap.servers");
		
		if (emcKafkaServer != null) {
        	company.setEmcKafkaServer(emcKafkaServer.getValue());
        }
        ...
        if(serverReboot != null){
        	model.addAttribute("serverReboot", serverReboot);
        	if(serverReboot){
        		try {
					Thread.sleep(5000);
	...
	public String updateCompany(@ModelAttribute("company") Company company) throws EmsValidationException {
        ...
        if (emcMode != null) {
			if(company.getEmcMode()){
				if(!"1".equals(emcMode.getValue())){
					serverReboot = true;
				}
				emcMode.setValue("1");
			}else{
				if(!"0".equals(emcMode.getValue())){
					serverReboot = true;
				}
				emcMode.setValue("0");
			}
			systemConfigurationManager.save(emcMode);
        }
		
		SystemConfiguration emcKafkaServer = systemConfigurationManager.loadConfigByName("kafka.bootstrap.servers");
		
		if (emcKafkaServer != null) {
			emcKafkaServer.setValue(company.getEmcKafkaServer());
			systemConfigurationManager.save(emcKafkaServer);
        }
	
		company.setId(companyManager.getCompany().getId());
		companyManager.updateCompany(company);
		try {
			//Update the tile24 key in system_configuration
			String title24Flag = company.getTitle24().getCompliance().getFlag();
			title24Flag = StringUtils.isEmpty(title24Flag)?Constants.FALSE:title24Flag.equalsIgnoreCase(Constants.FALSE)?Constants.FALSE:Constants.TRUE;
			final Title24 title24  = title24Manager.loadTitle24Details();
			title24.getCompliance().setFlag(title24Flag);
			title24Manager.saveTitle24Details(title24);
		} catch (Exception e) {
			syslog.info("ERROR: Not able to update/retrieve the title24 json objet", e);
		}
		...
		return "redirect:/admin/organization/setup.ems?serverReboot="+serverReboot;
	}

	@RequestMapping("/organization/setting.ems")
	public String orgSetting(
			Model model,
			@CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie,  @RequestParam(value = "refreshTree", required = false) String refreshTree
			,  @RequestParam(value = "upload", required = false) String upload,@RequestParam(value = "uploadStatus", required = false) String uploadStatus,@RequestParam(value = "message", required = false) String message) throws EmsValidationException {
		Map<String,Object> nameValMap = new HashMap<String, Object>();
        nameValMap.put("admin.org.settings.refreshTree", refreshTree);
        nameValMap.put("admin.org.settings.upload", upload);
		...
		case CAMPUS: {
			...
			model.addAttribute("companyLatitude", comp.getLatitude());
			model.addAttribute("companyLongitude", comp.getLongitude());
			return "createSite";
		}
		case BUILDING: {
	...
	public String addFloor(@ModelAttribute("floor") Floor floor) throws EmsValidationException {
        ...
        CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, nameValMap);
		if (floor.getPlanMap() != null && floor.getPlanMap().getFileData() != null && !floor.getPlanMap().getFileData().isEmpty()) {
			SystemConfiguration floorPlanImageSizelimitConfig = systemConfigurationManager.loadConfigByName("floorplan.imagesize.limit");
	...
	public String updateFloor(@ModelAttribute("floor") Floor floor) throws EmsValidationException {
        ...
        nameValMap.put("floorName", floor.getName());
        nameValMap.put("floorDesc", floor.getDescription());
        nameValMap.put("floorSiteId", floor.getSiteId());
        CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, nameValMap);
		SystemConfiguration floorPlanImageSizelimitConfig = systemConfigurationManager.loadConfigByName("floorplan.imagesize.limit");
	...
	public String editBuildingName(@ModelAttribute("org") Building building) throws EmsValidationException {
		...
		savedBuilding.setUseOrgLocation(building.getUseOrgLocation());
		savedBuilding.setLatitude(building.getLatitude());
		savedBuilding.setLongitude(building.getLongitude());
	...
	public String editFloorName(@ModelAttribute("org") Floor floor) throws EmsValidationException {
		...
		savedFloor.setSiteId(floor.getSiteId());
	...
	String saveDhcpSetting(String dhcp) throws EmsValidationException {

    	boolean retVal = false;
		CommonUtils.isParamValueAllowedAndThrowException(messageSource,
				systemConfigurationManager, "dhcp", dhcp);
		if(dhcp.contentEquals("true"))
			retVal = ServerMain.getInstance().startDHCP();
		else if (dhcp.contentEquals("false"))
