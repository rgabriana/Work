...
public class SSLSessionManager {
   ...
   */
	...
	public void initNwInterface() {
		...
		ServerMain.getInstance().initializeNetworkSettings();
		...
		sslLogger.info("nimCorporate  "+nimCorporate+" nimBuilding mapping "+nimBuilding);
		sslLogger.info("corporate Mapping "+corporateMapping+" building mapping "+buildingMapping);
	...
	private SSLConnection establishConnection(Gateway gw) throws SSLException {
		...
		try {
			...
			sslLogger.error(gwIp + "(createConnection): ", ex);
