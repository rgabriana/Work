...
public class ServerUtil {
  ...
  public static byte computeChecksum(byte checksum, byte[] array) {
...
//        
  ...
  public static String getLogPacket(byte[] packet, boolean spaceDelim) {
  ...
  } //end of method getCurrentSecond
  ...
  public static String getCommandString(int msgType) {
    ...
    case ServerConstants.SET_COLOR_TEMP:
  ...
  public static String getCurrentState(int currState) {
    ...
    case ServerConstants.CURR_STATE_PWRCURVE:
    	return ServerConstants.CURR_STATE_AUTO_STR;
    case ServerConstants.CURR_PL_STATE_TO_SAFETY:    	
    	return ServerConstants.CURR_STATE_AUTO_STR;
    case ServerConstants.CURR_PL_STATE_SAFETY:
    	return ServerConstants.CURR_STATE_SAFETY_STR;      
    case ServerConstants.CURR_PL_STATE_GOTO_MANUAL_NORMAL_HIGH:
    	return ServerConstants.CURR_STATE_MANUAL_STR;
    default:
  ...
  public static String removeLeadingZerosFromMac(String mac) {
  	
		StringTokenizer st = new StringTokenizer(mac, ":");
		StringBuffer finalMac = new StringBuffer();
		boolean first = true;
		while(st.hasMoreTokens()) {
			if(!first) {
				finalMac.append(":");
			...
			if(token.startsWith("0")) {
				finalMac.append(token.substring(1));
			} else {
				finalMac.append(token);
			}
		}
		return finalMac.toString();
		
  } //end of method removeLeadingZerosFromMac
  
  public static String generateMACAddress(String snapAddr)
	{
		String strMacAddress = "";
      	String[] macSplitArray;
      	macSplitArray = snapAddr.split(":");
      	for (int i = 0; i < macSplitArray.length; i++) {
  ...
  public static void main(String args[]) {
    ...
    */
  	//System.out.println("checksum -- " + checksum(0, "1234678950hjfufufugggoufuggufufukfiyfyvufyvvycyvcycyvvvc".getBytes()));
    byte data[] = { 0x14, 0, 0, 6, 0, 0, 0, 0x1e, 0, 0, 0, 0, 0, 3, 0x64, 0, 0x19, 1, 0x16, 0, 0, 0x10, 1, 0x2};
    
    byte[] bm = longToByteArray(1164762395524923647L);
    
    byte[] pkt = new byte[data.length + bm.length];
    System.arraycopy(data, 0, pkt, 0, data.length);
    System.arraycopy(bm, 0, pkt, data.length, bm.length);
  
    System.out.println("checksum -- " + checksum(0, pkt));
  ...
  /*
   * this is a two byte checksum
   */
  public static int checksum(int cksum, byte[] args) {
  	...
  	for(int i = 0; i < length; i++) {  		
  		...
  		sum += (sum >> 16); //add overflow bit
  		sum &= 0xFFFF;
  	}
  	//System.out.println("checksum -- " + sum);
  	return sum;
    
  } //end of method checksum
  
  public static short convertProfileTimeToShort(String time) {
