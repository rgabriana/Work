...
foreach($all_interfaces as $val) {
        ...
        $obj->default_gateway = getShelloutput('netstat -rn  | grep '.$val.' | grep UG | head -1 | awk \'{print $2}\'');
        ...
        $obj->physical_state = getShelloutput('cat /var/run/network/ifstate | grep '.$val.' | wc -l');
