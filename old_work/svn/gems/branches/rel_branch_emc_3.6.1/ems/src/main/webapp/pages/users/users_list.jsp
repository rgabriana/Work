...
<spring:url value="/services/org/user/genapikey" var="genApiUrl" scope="request"/>
...
<div class="outermostdiv">
	<script type="text/javascript">
        ...
        function generateApiKey(email,uId){
        	
        	if(!confirm('Are you sure you want to generate new api key for user '+email+'?')){
    			return false;
    		}
        	...
        	$.ajax({
                ...
                {
                	displayLabelMessage('<spring:message code="user.genapi.success" arguments="'+email+'"/>', COLOR_SUCCESS);
                	$("#"+uId+"_secretKeyId").text(data);
                	$("#"+uId+"_secretKeyId").prop('title',data);
                },
...
<div style="height:5px;"></div>
	...
	<table id="all_users_list" class="entable" style="width: 100%">
		<thead>
			<tr>
				<th align="center" style="width: 15%" ><spring:message code="users.email" /></th>
				<th align="center" style="width: 13%" ><spring:message code="users.firstName" /></th>
				<th align="center" style="width: 13%" ><spring:message code="users.lastName" /></th>
				<th align="center" style="width: 8%" ><spring:message code="users.contact" /></th>
				<th align="center" style="width: 8%" ><spring:message code="users.role" /></th>
				<th align="center" style="width: 7%" ><spring:message code="users.status" /></th>
				<th align="center" style="width: 8%" ><spring:message code="users.apikey" /></th>
				<th align="center" style="width: 28%" >Action</th>
			</tr>
		</thead>
		...
		<c:forEach items="${usersList}" var="user">
			<tr>
				...
				<td title = ${user.role.roleType}>${user.role.roleType}</td>
				<td title = ${user.status.name}>${user.status.name}</td>
				<td title = "${user.secretKey}" id="${user.id}_secretKeyId">${user.secretKey}</td>
				<td align="right" style="padding-right:5px;width:25%;">					
					<div>
						<form id="user-details" method="post" onsubmit="return beforeDeleteUser();" action="${actionURL}">
							...
							<c:if test="${user.role.roleType != 'Bacnet'}">
							...
							<input type="button" id="${user.id}_userAPIKeyBtn" onclick="return generateApiKey('${user.email}',${user.id});" value="Gen API Key"/>
