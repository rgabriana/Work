...
public class NetworkSettingsManager {
	...
	public boolean saveInitialNetworkSettings() {
		Map<String,String> physicalStatusMap = new HashMap<String,String>();
		...
		try {
			...
			if(interfacesJson != null){
				...
				for(NetworkSettings ni : networkInterfaces.getInterfaces()){					
					...
					physicalStatusMap.put(ni.getName(), ni.getPhysical_state());
				}
				networkSettingsDao.saveInitialNetworkSettings(networkSettingsList);		
				networkSettingsDao.saveInitialNetworkSettingsMappings(networkSettingsList,physicalStatusMap);
...
//						networkSettingsDao.saveNetworkSettings(ns);
	...
	public void cleanupDefunctNetworkSettings() {
		boolean remappingFlag = false;
		Map<String,String> physicalStatusMap = new HashMap<String,String>();
		List<NetworkSettings> networkSettingsList = new ArrayList<NetworkSettings>();	
		try {
			networkSettingsList = networkSettingsDao.loadAllNetworkInterfacesSettings();
			List<NetworkSettings> networkSettingsList1 = new ArrayList<NetworkSettings>();	
			List<NetworkInterfaceMapping> currentMapping =  networkSettingsDao.loadCurrentNetworkInterfaceMapping();
			Map<String,String> currentMappingMap = new HashMap<String,String>();
			for(NetworkInterfaceMapping nim : currentMapping){
				currentMappingMap.put(nim.getNetworkTypes().getName(), nim.getNetworkSettings().getId().toString());
				//System.out.println("current mapping is "+nim.getNetworkTypes().getName()+" "+nim.getNetworkSettings().getId());
			}
			Map<Long,String> dbEthMap = new TreeMap<Long,String>();
	    	for(NetworkSettings ns : networkSettingsList){
	    		dbEthMap.put(ns.getId(),ns.getName());
	    	}
			...
			ObjectMapper mapper = new ObjectMapper();
			if(interfacesJson != null){
				List<NetworkSettings> networkSettingsList2 = new ArrayList<NetworkSettings>();
				InterfacesList networkInterfaces = null;			
				networkInterfaces = mapper.readValue(interfacesJson, InterfacesList.class);
				for(NetworkSettings ni : networkInterfaces.getInterfaces()){					
					physicalStatusMap.put(ni.getName(), ni.getPhysical_state());
					networkSettingsList1.add(ni);						
					if(!dbEthMap.containsValue(ni.getName())){	
						networkSettingsList2.add(ni);						
					}
				}
				networkSettingsDao.saveInitialNetworkSettings(networkSettingsList2);
			}			
			for(NetworkSettings ns : networkSettingsList){
				if(!physicalStatusMap.containsKey(ns.getName())){					
					if(currentMappingMap.containsValue(ns.getId().toString())) {						
						remappingFlag = true;
					}
					networkSettingsDao.deleteNetworkSettingsAndMapping(ns);					
				}
			}			
			if(remappingFlag) {
				syslog.info("remapping network interfaces");
				networkSettingsDao.remapNetworkSettings(networkSettingsList1,currentMappingMap,physicalStatusMap);
				//Change the cloudSErverInfo.xml Update EM mac for the remap case
				CloudServerInfo cloudInfo = cloudServerManager.getCloudServerInformation();
		        final String prevEMMac = cloudInfo == null ? "" : cloudInfo.getEmMac();
				cloudServerManager.actualPopulateEMMac();
		        cloudInfo = cloudServerManager.getCloudServerInformation();
		        if (cloudInfo != null){
		        	final String emmac = cloudInfo.getEmMac();
		        	syslog.info("******************** EM MAC for the cloud has been updated from "+prevEMMac+" to " + emmac);
		        }else{
		        	syslog.error("Some issue in configuring cloudSErverInfo.xml as cloudInfo is null");
		        }
			}else{
				syslog.info("No change in mapped interfaces.");
			}
...
eth1		eth0		NO					NO						YES
	...
	public void enableDisableDHCPServer(String dhcp) {
		
		networkSettingsDao.enableDisableDHCPServer(dhcp);
	...
	public boolean isDhcpServerRunning(){
		...
		try {
			...
			Process dhcpProcess = Runtime.getRuntime().exec("/bin/bash "+ServerMain.getInstance().getTomcatLocation()+"adminscripts/getDHCPStatus.sh");
            ...
            if(dhcpProcess.waitFor() == 0){
                ...
                while ((dhcpProcessString = outReader.readLine()) != null) {
                	if(dhcpProcessString.contains("is running")){
                		syslog.info("dhcpd is running");
                		return true;
                	}
                	if(dhcpProcessString.contains("is not running")){
                		syslog.info("dhcpd is not running");
                		return false;
                	}
                }
                //AdminUtil.readStreamOfProcess(dhcpProcess);
	...
	public void mapGatewayNetworkToNA(){
		String gatewayMapping = networkSettingsDao.loadCurrentMappingByNetworkType(NetworkType.Building.getName());		
		if(gatewayMapping != null && gatewayMapping != "NA"){
			NetworkSettings ns = networkSettingsDao.loadNetworkSettingsByInterfaceName(gatewayMapping);
			if(ns != null &&  !"DHCP".equalsIgnoreCase(ns.getConfigureIPV4())){
				String ipAdrress = ns.getIpaddress();
				if(ipAdrress == null || ipAdrress.length()==0 || ipAdrress.contains("169.254.0.1")){
					syslog.info("ipadrress for interface is "+gatewayMapping+" "+ipAdrress+ ns.getConfigureIPV4());
					networkSettingsDao.mapGatewayNetworkToNA(ns);
				}
			}
		}
	}
