...
$().ready(function() {
	...
	$.validator.addMethod("campusregx", function(value, element, regexpr) {          
	    return regexpr.test(value);
	}, "Please enter a valid Campus Name. Allowed Special Characters are @ # - _ : . ,");
	
	$.validator.addMethod("locationregx", function(value, element, regexpr) {          
		value = $.trim(value);
		if (value != "") {
			return regexpr.test(value);
		}
		return true;
	}, "Please enter a valid Location. Allowed Special Characters are @ # - _ \\ / & * : . ,");
	...
	<c:if test="${mode == 'admin'}">
		$(".maindiv").css("height", $(window).height() - 265);
		$(".maindiv").css("overflow", "auto");
	</c:if>
});
        ...
        if(result) {
        	$.ajax({
        		...
        		success: function(msg) {
        			if(msg == "F") {
        				$("#deleteStatus").html("Campus '" + name  + " cannot be deleted as there is at least one building attached to it." );
        			}
        			else if(msg == "S") {
        				<c:if test="${mode == 'admin'}">
		...
		<div>
			...
			<form:form id="create-campus" commandName="campus" method="post"
				...
				<div class="field">
					...
					<div class="formValue">
						<form:input id="name" name="name" size="24" 
							path="name" />
					</div>
				</div>
				<div class="field">
					...
					<td align="left">
						<c:if test="${!empty campuses}">
							<form method="post" action="${nextURL}">
								<input id="campusNext" class="navigation" type="submit"
