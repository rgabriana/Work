...
public class FixtureImageUpgradeWorker extends Thread {
  ...
  private Long[] devices; //array of fixtures which are upgraded by this worker
   ...
   * 
   */
  public FixtureImageUpgradeWorker(ImageUpgradeDBJob job) {
  ...
  public void startImageUpload(Long[] fixtureIds, String fileName) {
      
    logger.info("fixture upgrade request on " + fixtureIds.length + " using " + fileName);
    ...
    } else if(fileName.toLowerCase().indexOf("ble") != -1) {
    	//ble upgrade
    	appType = ServerConstants.ISP_BLE_APP_INIT_OPCODE;    	
    }
  ...
  class GwFixtImgUpgThread extends Thread {
      	...
      	for(int i = 0; i < noOfFixtures; i++) {
      		...
      		if(details.status.equals(ServerConstants.IMG_UP_STATUS_ABORTED)) {
      			...
      			if(details.description == null || details.description.equals("")) {
      				...
      				if(appType == ServerConstants.ISP_APP1_INIT_OPCODE) {
      					description = "Image upgrade of Firmware with image " + fileName + " failed";
      				}
      			} else {
      		...
      		if(!aborted) {
      			...
      			fixtureMgr.setImageUpgradeStatus(fixture.getId(), ServerConstants.IMG_UP_STATUS_ABORTED);
      			firmUpMgr.finishDeviceUpgrade(dbJob.getId(), fixture.getId(), 
    ...
    private void initiateImageUpgrade() {
      ...
      if(aborted) {
      	Iterator<Long> inProgressIter = fixtureInProgressMap.keySet().iterator();
      	while(inProgressIter.hasNext()) {
      		Long fixtId = inProgressIter.next();
      		FixtImgUpgrDetails details = fixtureInProgressMap.get(fixtId);
      		details.status = ServerConstants.IMG_UP_STATUS_ABORTED;
      	}
      	return;
      }
      
      //image upload is complete. give 90 (was 45) seconds for fixtures to send node boot info
    ...
    public void startMulticastImageUpload(ArrayList<Fixture> fixtureList, int imgType) {
      ...
      //if the end time of the job has reached stop the upgrade rest of the sensors will be upgraded next day
...
//  	  EventsAndFault.FIXTURE_IMG_UP_STR, EventsAndFault.MAJOR_SEV_STR);
    ...
    logger.debug("Inside updateUpgradeCompleteStatus -> {fx:"
                + fixture.getSnapAddress() + ", targetver: "
                + details.targetVersion + ", curver: " + currVer
                + ", otherver: " + otherVer + ", appId: " + appId
                + ", version: " + version + ", imgupstatus: "
                + imgUpgrStatus + ", currRetryAttempts: "
                + currRetryAttempts + ", maxAttempts: " + maxAttempts
                + ", blever: " + fixture.getBleFirmwareVersion()
                + ", appUpgraded: " + details.appUpgraded + "}");
      ...
      //new image
      if(details.appUpgraded == ServerConstants.ISP_BLE_APP_INIT_OPCODE){
    	  logger.debug("SU is in BLE Image Upgrade workflow " + details.targetVersion  + " == " + fixture.getBleFirmwareVersion());
    	  if(details.targetVersion == Integer.parseInt(fixture.getBleFirmwareVersion())) {   
    		  logger.debug("BLE Image Upgrade is now successful");
				imageUpgradeSuccess = true;
			} else {
				details.description += " failed as SU rebooted with old image(" + currVer + ")";
			}
      }else if(details.appUpgraded == ServerConstants.ISP_APP2_INIT_OPCODE) { //app2 is upgraded
