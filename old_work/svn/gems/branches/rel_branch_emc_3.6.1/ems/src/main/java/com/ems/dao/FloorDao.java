...
public class FloorDao extends BaseDaoHibernate {
    ...
    public List getAllFloorsOfCompanyWithNames() {
      
      try {       
        String hsql = "select f.id, f.name as floor_name, b.name as build_name, c.name as camp_name, o.name as org_name, " +
            "f.description, floorplan_url from floor f, building b, campus  c, company o " +
            " where f.building_id = b.id and b.campus_id = c.id order by f.id";
        Query q = getSession().createSQLQuery(hsql.toString());       
        List results = q.list();
        if (results != null && !results.isEmpty()) {
    ...
    public Floor getFloorByUid(String uid) {
  		...
  		List<Floor> floorList = getSession().createCriteria(Floor.class)
