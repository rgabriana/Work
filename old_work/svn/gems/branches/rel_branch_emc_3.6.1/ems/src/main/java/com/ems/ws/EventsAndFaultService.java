...
public class EventsAndFaultService {
	...
	public String getEventsList(
		...
		for (int i = 1; i < eventsAndFaults.size(); i++) {
			...
			output.append("{ \"id\": \""
					...
					+ (String) each[1]
					+ "\", \""
					+ (each[7] != null ? (String) each[8] + " -> "
							+ (String) each[12] : "") + "\", \"" + each[3]
					+ "\", \"" + each[2] + "\", \"" + each[4] + "\", \""
					+ "View" + "\", \"" + each[5] + "\" ]}");
		}
		output.append(" ]");
	...
	public Response getCompleteEventsList(
		...
		for (int i = 1; i < eventsAndFaults.size(); i++) {
			...
			location = (location != null && location != "") ? location.replace(
					",", "-") : location;
			output.append("\r\n");
		...
		return Response
				...
				.header("Content-Disposition",
						"attachment;filename=Events_And_Faults.csv").build();
	}
	...
	public String resolve(@PathParam("ids") String ids,
		...
		userAuditLoggerUtil.log("Resolved event: " + ids,
				UserAuditActionType.Events_Update.getName());
	...
	public String updateEvent(@PathParam("id") String id,
		...
		try {
			...
			nameValMap.put("resolve", resolve);
			...
			userAuditLoggerUtil.log("Update event: " + id,
					UserAuditActionType.Events_Update.getName());
    ...
    public EMEventList getEMEventsList(@RequestParam("data") String userdata,@PathParam("facilityId") Long facilityId)
		...
		if (params != null && params.length > 0) {
			...
			eventsAndFaults = eventAndFaultManager.getEventsAndFaults(orderBy,
					orderWay, filter, role.getName(),
	 ...
	 * add a BACnet event
	 *
	 * @param eventsAndFault
	 *            <eventsAndFault>
	 *            <severity>Critical</severity>
	 *            <eventType>Bacnet</eventType>
	 *            <description>Bacnet Service Started</description>
	 *            </eventsAndFault>
	 */
	...
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public com.ems.ws.util.Response addBacnetEvent(EventsAndFault eventsAndFault) {

		com.ems.ws.util.Response response = new com.ems.ws.util.Response();

		Map<String,Object> nameValMap = new HashMap<String,Object>();
        nameValMap.put("bacnetEventDesc", eventsAndFault.getDescription());
        nameValMap.put("bacnetEventType", eventsAndFault.getEventType());
        nameValMap.put("bacnetEventSeverity", eventsAndFault.getSeverity());
        ...
        if(resp!= null && resp.getStatus()!=200){
        	m_Logger.error("Validation error: "+resp.getMsg());
        	response.setMsg("Validation error: "+resp.getMsg());
        	response.setStatus(1);
        	return response;
    	}
        // Since we are allowing arbitrary severity, we must treat this as an alarm: Only one instance of this event type in the table
        eventAndFaultManager.addAlarm(eventsAndFault.getDescription(), eventsAndFault.getEventType(), eventsAndFault.getSeverity());
