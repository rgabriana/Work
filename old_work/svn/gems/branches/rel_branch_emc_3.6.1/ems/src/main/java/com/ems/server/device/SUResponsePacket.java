  ...
  public void processResponse() {
    ...
    if(fixture == null) {
    	if(logger.isInfoEnabled()) {
    		logger.info(snapAddr + ": There is no fixture, ignoring the node pkt - " + ServerUtil.getLogPacket(pkt));	
    	}
      pkt = null;
      return;
    }
    ...
    switch (msgType){      	
    	case ServerConstants.REPT_PM_DATA_MSG_TYPE:	  	  
      	...
      	case ServerConstants.ACK_TO_MSG:
            ...
            case ServerConstants.CMD_LAMP_CALIBRATION_REQ:
            	...
            	break;
            case ServerConstants.BLE_MODE_SET:
          		int noOfDevicesUpdated = fixtureMgr.updateCurrentBleMode(fixture.getId());
          		if(logger.isDebugEnabled()) {
          			logger.debug(fixture.getId() + ": curr ble mode updated - " + noOfDevicesUpdated);
          		}
          		break;
        ...
        case ServerConstants.NACK_TO_MSG:
            ...
            if ((pkt[msgStartPos] & 0xFF) != ServerConstants.ISP_INIT_ACK_OPCODE) {
                ...
                case ServerConstants.CMD_MOTION_GRP_APPLY_ACTION:
                	if(logger.isInfoEnabled()) {
                		fixturelogger.info(fixture.getId() + " " + fixture.getSnapAddress() + " motion group configuration not support!");
                	}
                	break;
                }
            }
        ...
        case ServerConstants.SU_TRIGGER_MOTION_BIT_MSG_TYPE:
            ...
            if (device != null && device.getLastMBitSeqNo() == seqNo) {
                ...
                if(logger.isDebugEnabled()) {
                	logger.debug(fixture.getId() + " seq: " + seqNo + ", motion_bit_info: " + (motion_bit_info & 0xFF) + 
                			", mask: " + (bitmask & 0xFF) + ", motion bit recived " + ServerUtil.getLogPacket(pkt));
                }
                ...
                if(logger.isDebugEnabled()) {
                logger.debug(fixture.getId() + " seq: " + seqNo + ", Processed motion bit pkt: " + 
        ...
        case ServerConstants.CMD_LAMP_CALIBRATION_RES:
            ...
            break;
            /* Found that this message type is not used on the sesnor. so commenting out
        ...
        case ServerConstants.SU_CMD_HB_STATS_MSG_TYPE:
            if (!fixture.getState().equals(ServerConstants.FIXTURE_STATE_COMMISSIONED_STR)) {
                // fixture is not yet commissioned. So, ignore the stats
            	if(logger.isInfoEnabled()) {
                logger.info(fixture.getFixtureName() + ": Fixture is not yet commissioned");
            	}
                return;
            }
