...
(
  ...
  override7 bigint,
  override8 bigint,
  circadian_option smallint default 0,
  color_fx smallint default 0,
  pwmbehaviour smallint default 0,
  sunrise_temperature_ratio smallint default 0,
  noon_temperature_ratio smallint default 0,
  sunset_temperature_ratio smallint default 0,
  night_temperature_ratio smallint default 0,
...
(
  ...
  ntp_enable character varying DEFAULT 'Y',
  ntp_server_list  character varying DEFAULT '0.us.pool.ntp.org,1.us.pool.ntp.org,2.us.pool.ntp.org,3.us.pool.ntp.org',
  latitude float(4),
  longitude float(4),
...
(
  ...
  latitude float(4),
  longitude float(4),
  use_org_location boolean DEFAULT true,
  visible boolean DEFAULT true,
...
(
  ...
  plan_map_id bigint,
  sweep_timer_id bigint,
  lighting_occ_count smallint DEFAULT 0,
  sensor_faulty_time smallint DEFAULT 15,
  percentage_sensors_faulty smallint DEFAULT 75,
  visible boolean DEFAULT true,
...
(
  ...
  CONSTRAINT unique_gateway_ip_address UNIQUE (ip_address)
);
...
(
  ...
  avg_temperature numeric (7,2) DEFAULT 0,
  ...
  lighting_occ_status smallint DEFAULT 0,
  current_trigger_type integer not null default 0,
  change_trigger_type integer not null default 0,
  occ_level_trigger_time integer not null default 90,
  heartbeat_status integer,
  dali_driver_cnt smallint,
  manual_mode_duration smallint not null default 60,
  ble_firmware_version character varying(20),
  ble_mode character varying,
  dual_channel_led_value integer,
  dualchannel_led_duration smallint not null default 60,
  circ_ratio smallint,
  curr_ble_mode character varying,
...
INSERT INTO users (id, email, "password", first_name, last_name, created_on, role_id, contact, old_passwords, location_id, location_type, approved_location_id, approved_location_type,identifier_forgot_password) VALUES (nextval('user_seq'), 'admin', '21232f297a57a5a743894a0e4a801fc3', 'Administrator', '', '2009-12-12', 1, NULL, NULL, NULL, NULL, NULL, NULL, 'RESET_PASSWORD');
...
CREATE TABLE switch (
	...
	force_auto_mode smallint DEFAULT 0
);
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'db_pruning.5min_table', '45');
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'imageUpgrade.no_multicast_retransmits', '1');
-- DR related
...
BEGIN
	...
	SELECT value INTO no_days
	FROM system_configuration
	WHERE name = 'db_pruning.gw_stats_table';

	no_days_text = no_days || ' day';
	no_days_time = tm - no_days_text::interval;

	DELETE FROM gw_stats WHERE capture_at < no_days_time;
...
BEGIN
	    ...
	    -- Clean up excess alarms due to bug in older code. If there are more than 10,000 alarms for a specific event type,
	    -- remove all of those alarms. An alarm is an event whose severity is NOT "Info".
	    FOR alarm IN (SELECT count(*), event_type FROM events_and_fault WHERE severity != 'Info' GROUP BY event_type) LOOP
	    	IF alarm.count > 10000 THEN
	    		DELETE FROM events_and_fault WHERE event_type = alarm.event_type and severity != 'Info';
	    	END IF;
	    END LOOP;
END;
$$
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'cloud.communicate.type', '0');
...
(
  id bigint NOT NULL,
  profile_id bigint,
  managed_load numeric(19,2),
  unmanaged_load numeric(19,2),
  profile_handler_id bigint,
  current_profile character varying(255),
  original_profile_from character varying(255),
  current_state character varying,
  ...
  last_stats_rcvd_time timestamp without time zone,
  last_zb_update_time timestamp without time zone,
  pre_defined_baseline_load numeric(19,2) default 0,
...
(
  ...
  dr_high_level smallint DEFAULT 0,
  dr_special_level smallint DEFAULT 0,
  heartbeat_interval bigint DEFAULT 30,
  heartbeat_linger_period bigint,
  no_of_missed_heartbeats bigint DEFAULT 3,
  ...
  CONSTRAINT plugload_profile_handler_profile_configuration_id_fkey FOREIGN KEY (profile_configuration_id)
      ...
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT override5_plugload_profile_fk FOREIGN KEY(override5) REFERENCES plugload_profile(id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT override6_plugload_profile_fk FOREIGN KEY(override6) REFERENCES plugload_profile(id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT override7_plugload_profile_fk FOREIGN KEY(override7) REFERENCES plugload_profile(id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT override8_plugload_profile_fk FOREIGN KEY(override8) REFERENCES plugload_profile(id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION
)
WITH (
  OIDS=FALSE
);
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'upgrade.plugload_pattern', 'pl.bin');
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'iptables.rules.static.ports', '22,8085,7,443,8443,80,389,12302,9191,2332,9092,2181');
...
--Sree 07/05
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'kafka.bootstrap.servers', 'localhost:9092');
...
--Sree 08/10
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'kafka.dispatcher.threads', '1');

INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'db_pruning.gw_stats_table', '30');
