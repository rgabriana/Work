...
public class UserAuditDao extends BaseDaoHibernate {
	...
	public List<Object> getUserAudits(String order, String orderWay, List<Object> filter, int offset, int limit) {
		...
		rowCount = getSession().createCriteria(UserAudit.class, "audit")
    		    			.setProjection(Projections.rowCount());
		
		data = getSession().createCriteria(UserAudit.class, "audit");
		
    	if(filter != null) {

    		String username = (String)filter.get(0);
    		if(username != null) {
    			rowCount.add(Restrictions.ilike("audit.userName", username, MatchMode.ANYWHERE));
    			data.add(Restrictions.ilike("audit.userName", username, MatchMode.ANYWHERE));
    		}
