...
# need to set pgpass for postgres 9+
sudo cp /home/enlighted/.pgpass /root/.pgpass
...
then
  ...
  then
    ...
    if [ `psql -q -U postgres -h localhost -p 5433 ems -t -c "select count(*) from pg_tables where tablename = 'switch_fixtures'"` -eq 1 ]
    then
      #check no. of groups for each sensor
      if [ `psql -q -U postgres -h localhost  -p 5433 ems -t -c "select count(*) from (select fixture_id, count(*) from (select id, fixture_id, group_id from gems_group_fixture union select id, fixture_id, switch_id from switch_fixtures) AS sq group by fixture_id having count(*) > 10) AS sq2"` -gt 0 ]
      then
