...
public class SwitchDao extends BaseDaoHibernate {
	...
	public Switch update(Switch switchval) {
		...
		query.setLong("areaId", switchval.getAreaId());
		return switchval;
	}
...
/*	public Switch updateSceneId(Long id, Long scene_id) {
	...
	public List loadSwitchDetailsByUserId(String uId) {
		List results = null;
		String hsql = " Select switch.id, switch.name, count(scene) , scene.id , scene.name  from Switch switch , UserSwitches us, "
				+ " Scene scene where switch.id = us.switchId and switch.id = scene.switchId and  us.userId = :uid group by switch.id, switch.name,scene.id,scene.name  ";
		Query q = getSession().createQuery(hsql.toString());
	...
	public List loadSwitchDetailsForAdmin() {
		List results = null;
		String hsql = " Select switch.id, switch.name, count(scene) ,scene.id , scene.name  from Switch switch ,"
				+ " Scene scene where switch.id = scene.switchId group by switch.id, switch.name,scene.id,scene.name  ";
		Query q = getSession().createQuery(hsql.toString());
		results = q.list();
		return results;
	}
