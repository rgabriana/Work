...
public class SwitchService {
   ...
   private static Long VALIDATION_INACTIVITY_TIME = 15 * 60 * 1000l; //15 minutes it is in sec
    ...
    public Response sendSwitchGroupMsgToFixtures(@PathParam("switchid") Long switchId,
        ...
        if(switchId!=null && switchManager.getSwitchById(switchId).getName()!=null)
        {
        ...
        if(wdsManager.loadCommissionedWdsBySwitchId(switchId).size()>0)
        {	        	
        		//For scene toggle/change , dimup , dimdown
        	   if(action.equalsIgnoreCase("dimup"))
        	   {
        		   argument = 10;
        		   m_Logger.debug("Switch through Wds/dimup: " + switchId + ", Percentage: " + argument + ", Time: " + 60);
        		   userAuditLoggerUtil.log("Dimup switch with wds " + mSwitchName
        	   ...
        	   {
        		   argument = 10;
        		   m_Logger.debug("Switch through Wds/dimdown: " + switchId + ", Percentage: " + argument + ", Time: " + 60);
        		   userAuditLoggerUtil.log("Dimdown switch with wds " + mSwitchName
                           + " by " + 10 + "% for " + 60 + " minutes",
                           UserAuditActionType.Switch_Fixture_Dimming.getName());
        	   }
        	   else if(action.equalsIgnoreCase("scene"))
        	   {
        		   m_Logger.debug("Switch through Wds/scene: " + switchId + ", Percentage: " + argument + ", Time: " + 60);
        		   userAuditLoggerUtil.log("Scene Change switch with wds " + mSwitchName
                           + " by scene id" + argument + "% for " + 60 + " minutes",
                           UserAuditActionType.Switch_Fixture_Dimming.getName());
        	   }
        	   else
        	   {
        		   m_Logger.debug("Switch through Wds/auto: " + switchId + ", Percentage: " + argument + ", Time: " + 60);
        		   userAuditLoggerUtil.log("Auto switch with wds " + mSwitchName
        ...
        {        	
        	m_Logger.debug("Dimup with Switch: " + switchId + ", Percentage: " + argument + ", Time: " + 60);
        ...
        {
        	m_Logger.debug("Dimdown with Switch: " + switchId + ", Percentage: " + argument + ", Time: " + 60);
        	DeviceServiceImpl.getInstance().dimFixturesBySwitch(switchId, argument, 60);
        	userAuditLoggerUtil.log("Dim fixture with switch " + mSwitchName
                    + " by " + argument + "% for " + 60 + " minutes",
                    UserAuditActionType.Switch_Fixture_Dimming.getName());
        ...
        {        
        	Long sceneId = null;        	        	
        	List<Scene> sceneList = switchManager.loadSceneBySwitchId(switchId);
        	for (Scene scene : sceneList) {
				if(argument == scene.getSceneOrder())
				{
					sceneId = scene.getId();
					break;
				}
