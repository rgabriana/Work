...
CREATE OR REPLACE FUNCTION restructure_schema1() returns void
as $$
DECLARE
  rec switchrecord;  
  sGroupId bigint;
...
LOOP
  ...
  SELECT group_id , fixture_id INTO groupId,fId from gems_group_fixture where group_id = sGroupId limit 1;
  SELECT f.id, f.firmware_version INTO fixtureId, firmwareVersion from Fixture f where f.id=fId;
  
  fxVersion = SUBSTR(firmwareVersion,0,2);
  --Raise Notice 'For Group% => Fixture Version %', sGroupId,fxVersion ;
  IF fxVersion = '2' THEN
		UPDATE switch_group set fixture_version = '2.x' where gems_group_id = sGroupId;	
							
  ELSIF fxVersion = '1' THEN
...
SELECT restructure_schema1();

UPDATE switch_group set fixture_version = '2.x' where fixture_version is null;
