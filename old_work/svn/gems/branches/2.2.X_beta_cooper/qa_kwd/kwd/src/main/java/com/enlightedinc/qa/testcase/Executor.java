...
public class Executor {
	String Datafile ="D:/enlighted_qa_automation/Trunk/qa_kwd/test_cases/em/test_files/ALL/Data.xls";
        ...
        while (rowIter.hasNext()) {
            ...
            try {
                ...
                if (type.equalsIgnoreCase(Keyword.BUTTON)) {
                	...
                	switchBack(testRow);
                }else if (type.equalsIgnoreCase(Keyword.DataDriven)){
                	DataDriven(testRow);
    ...
    private void DataDriven(XSSFRow testRow)
    {
    	String accessType = testRow.getCell(0).getStringCellValue();
    	if(accessType.equalsIgnoreCase("data") )
    	{
    		try{
    			
    			XSSFWorkbook data = new XSSFWorkbook(Datafile);
    			XSSFSheet suiteSheet = data.getSheet("Data");
    		    Iterator rowIter1 = ((Sheet) suiteSheet).rowIterator();
    		    while (rowIter1.hasNext())
    		    {
    	            XSSFRow dataRow = (XSSFRow) rowIter1.next();
    	            XSSFCell testCaseCell = dataRow.getCell(0);
    	            String dataname = testCaseCell.getStringCellValue();
    	             
    	            webDriver.findElement(By.id("profilename")).sendKeys(dataname);
    	            webDriver.findElement(By.id("newbtn")).click(); // click on Save button
    	            webDriver.findElement(By.xpath("html/body/div[13]/div[1]/a/span")).click(); // close the pop up window
    	            Thread.sleep(2000);
    	                      
    	            if (dataname == null || dataname.equalsIgnoreCase(Keyword.FINISH)) {
    	                break;
    	            }
    		    }
     		} catch (Exception e) {
