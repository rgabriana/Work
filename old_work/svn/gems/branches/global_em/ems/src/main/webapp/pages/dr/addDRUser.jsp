		...
		<div class="formContainer">
			...
			<form:form id="drUserRegistration" commandName="druser" method="post" enctype="multipart/form-data">
				...
				<div class="field">
					...
					<div class="formValue"><input id="keystoreCertificate" type="file" name="keystoreCertificate" onChange="checkKeyStoreCertificateSize(this);" />
				...
				<div class="field">
					...
					<div class="formValue"><input id="truststoreCertificate" type="file" name="truststoreCertificate" onChange="checkTrustStoreCertificateSize(this);" />
...
$().ready(function() {
	...
	else if(status == 'keystoreFail') {
		$("#error").html("KeyStore Certificate file size should be less than 1 MB.");
	}
	else if(status == 'truststoreFail') {
		$("#error").html("TrustStore Certificate file size should be less than 1 MB.");
	}
	else if(status == 'E') {
...
function checkKeyStoreCertificateSize(obj) { 
	$("#error").html("");
	if(obj.files[0].size > 1048576) {
		$('#keystoreCertificate').val('');
		$("#error").html("KeyStore Certificate file size should be less than 1 MB.");
	}
}

function checkTrustStoreCertificateSize(obj) { 
	$("#error").html("");
	if(obj.files[0].size > 1048576) {
		$('#truststoreCertificate').val('');
		$("#error").html("TrustStore Certificate file size should be less than 1 MB.");
	}
}
