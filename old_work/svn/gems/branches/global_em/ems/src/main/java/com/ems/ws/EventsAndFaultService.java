...
public class EventsAndFaultService {
	...
	public String updateEvent(@PathParam("id") String id,
	 ...
	 /**
     * All the scheduled events like DR, SweepTimer, Image Upgrade currently running in the system is sent to Sticker component.
     * @return List of Scheduled Events currently active
     */
	@Path("list/scheduledEvent")
    @GET
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public RunningJobStatus getScheduledEventList() {
		RunningJobStatus runningJobStatus = new RunningJobStatus();
		
		List<DRTarget> drTargets = drTargetManager.getEnabledDRTargets();
		DRStatus drInfo = new DRStatus();
		drInfo.setStatus(false);
		
		if(drTargets != null && drTargets.size() > 0) {
			DRTarget drTarget = drTargets.get(0);
			drInfo.setStatus(true);
			drInfo.setType(drTarget.getDrType());
			drInfo.setLevel(DrLevel.valueOf(drTarget.getPriceLevel().toUpperCase()));
			int drTimeRem = -1;
			if(drTarget.getDuration() != 0) {
				drTimeRem = DeviceServiceImpl.getInstance().getDRTimeRemaining();
			}
			drInfo.setTimeremaning(drTimeRem);
		}
		
		runningJobStatus.setDrStatus(drInfo);
		//IMAGE :TODO
