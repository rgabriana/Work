...
import com.emscloud.dao.ProfileGroupDao.ProfileComparator;
import com.emscloud.dao.ProfileGroupDao.TemplateComparator;
...
public class ProfileGroupManager {
    ...
    static final Logger logger = Logger.getLogger(ProfileGroupManager.class
            .getName());
	/**
     * Load all group
     * 
							...
							while (floorListItr.hasNext()) {
								...
								if(emProfileMapping!=null)
								{
									...
									while (profileItr.hasNext()) {
										...
										TreeNode<FacilityType> profileNode = createNode(profileObj.getId(), profileObj.getName(), FacilityType.PROFILEGROUP,templateNode.getNodeId());
    ...
    public Short getMaxProfileNo(Long tenantId,Long emInstanceId) {
        return profileGroupDao.getMaxProfileNo(tenantId,emInstanceId);
    }
			...
			if (FacilityType.FLOOR.ordinal() == floor.getType()) {
				...
				}else
				{
		            ...
		            if (emProfileMapping != null)
		            {
		                ...
		                {
		                    logger.info("PROFILE ASSIGNMENT : Profile Group Id : " + profileGroups.getId()+ " marked for push to Fixtures");
