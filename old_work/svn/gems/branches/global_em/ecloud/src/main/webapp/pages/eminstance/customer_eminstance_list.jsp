...
$(document).ready(function() {
	...
	function startCustomerEmInstance(inputdata, pageNum, orderBy, orderWay) {
		jQuery("#CustomerEmInstanceTable").jqGrid({
			...
			colNames: ["Eminstance Id", "Name", "Mac Id","Version","Last Connectivity","Time Zone","Action"],
		       colModel: [		       
	...
	function startCustomerEmInstanceMapping(inputdata, pageNum, orderBy, orderWay) {
		jQuery("#CustomerCloudEmInstanceTable").jqGrid({
			...
			colNames: ["Eminstance Id", "Customer Cloud Facility Path","Eminstance Facility Id","Eminstance Facility Path","Action"],
		       colModel: [		       
	...
	function viewCloudEmInstanceMappingFormatter(cellvalue, options, rowObject) {
		var rowId = rowObject.id;
		var deleteFacilityEmMappingHtml = "<button onclick=\"javascript: deleteFacilityEmMapping('"+rowId+"');\">UnMap</button> ";
...
<div style="width: 100%; height: 100%; background: #fff; padding: 0px 5px 0px 0px" >
		...
		<div id="outerDiv" style="padding: 5px 5px 5px 5px">
				...
				<div style="float:right;padding: 0px 0px 0px 10px;font-weight: bolder; ">
	 			...
	 			<div style="font-weight: bolder; ">Active EM Instances of Customer "${customer.name}"</div>
	 	...
	 	<div id="outerDiv2" style="padding: 5px 5px 5px 5px;">
			...
			<div style="float:right;padding: 0px 0px 0px 10px">
				<select id="searchCustomerCloudColumn">
				  <option value="emId">Eminstance Id</option>
				  <option value="emFacilityId">Eminstance Facility Id</option>
				  <option value="emFacilityPath">Eminstance Facility Path</option>
				</select>
			</div>
			<div style="float:right;padding: 0px 0px 0px 10px;font-weight: bolder; ">
				<label>Search by</label>
			</div>
				<div style="font-weight: bolder; ">Cloud to EM Instances Mappings of Customer "${customer.name}"</div>
			</div>
