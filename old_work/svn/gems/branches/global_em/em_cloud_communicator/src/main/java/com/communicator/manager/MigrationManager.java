	...
	public  Boolean setReplicaMigrationFlagDirectECloud(String migrationStatus )
	{
		try{
				synchronized (CommunicatorConstant.setMigrationFlagService)
				{
					...
					CloudHttpResponse response = securecloudConnectionTemplate.executePost(CommunicatorConstant.setMigrationFlagOnEcloudService+serverInfoManager.getMacAddress()+"/"+migrationStatus ,"Dummy post",serverInfoManager.getHost(), MediaType.TEXT_PLAIN);
					int state= response.getStatus();
					if(state!=200)
					{
