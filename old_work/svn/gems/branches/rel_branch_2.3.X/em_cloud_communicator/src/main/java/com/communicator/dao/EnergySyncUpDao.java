...
public class EnergySyncUpDao  {
	...
	public ByteArrayOutputStream getBulkData(String tableName , Long lastDataId , Long minDataId   ) 
	{
		...
		try {
			...
			cm.copyOut("COPY (SELECT *"
				       + " from "+tableName+" where id >= " + minDataId + " and id < " + lastDataId + " order by id) "
