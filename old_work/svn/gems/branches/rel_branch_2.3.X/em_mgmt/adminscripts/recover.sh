...
then
    ...
    then
        ...
        then
            auditinsert=$($AUDITLOGSCRIPT "add" "EM Management Upgrade/Restore" "EC upgrade process was interrupted because of system failure. System failure recovery process has tried to rollback the em to previous stable state." "127.0.0.1")
        fi
        ...
        if [ -f "$AUDITLOGSCRIPT" -a "$recovermode" != "FROM_UPGRADE" ]
        then
            auditinsert=$($AUDITLOGSCRIPT "add" "EM Management Upgrade/Restore" "EC Database restore process was interuppted due to system failure. It is re-initiated on system start-up using file $filename." "127.0.0.1" )
        fi
        ...
        if [ -f "$AUDITLOGSCRIPT" -a "$recovermode" != "FROM_UPGRADE" ]
        then
            auditinsert=$($AUDITLOGSCRIPT "add" "EM Management Upgrade/Restore" "EC Database restore process was interuppted due to system failure. It was re-initiated on system start-up using file $filename." "127.0.0.1" )
        fi
    ...
    then
        if [ -f "$AUDITLOGSCRIPT" -a "$recovermode" != "FROM_UPGRADE" ]
        then
            auditinsert=$($AUDITLOGSCRIPT "add" "EM Management" "EC database backup process was interrupted because of system failure." "127.0.0.1")
        fi
    fi
