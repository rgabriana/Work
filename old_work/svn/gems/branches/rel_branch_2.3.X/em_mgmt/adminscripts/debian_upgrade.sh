...
upgradeAll() {
    ...
    echo "*** Preparing all EC components for upgrade ***" >> $LOGS_ERR
    ...
    then
        echo "step1;"
	    echo "*** Upgrading individual EC components ***" >> $LOGS_ERR
    else
	    echo "UPGRADE ERROR: Failed to prepare all EC components. Exit upgrade process."
        status="2"
        return
    fi
...
startTomcat() {
    ...
    if [[ "$startStatus" =~ "done" ]]
    then
        echo "*** Tomcat6 service is running. EC application should be up again ***" >> $LOGS_ERR
        echo "*** NOTE: If you are not able to access EC application in another 2-3 minutes, please raise an alarm ***" >> $LOGS_ERR
        return
    fi
    status="1"
...
then
    ...
    then
        ...
        auditinsert=$($AUDITLOGSCRIPT "add" "EM Management Upgrade/Restore" "EC upgrade process is initiated using $DEBIAN_FILE_NAME. Processing..." "$IP_ADDRESS")
    ...
    then
        ...
        then
            ...
            then
                ...
                auditinsert=$($AUDITLOGSCRIPT "add" "EM Management Upgrade/Restore" "EC upgrade process is initiated using $DEBIAN_FILE_NAME. Processing... Successful." "$IP_ADDRESS" "" "$upgradedate" )
            fi
        else
            if [ -z "$upgradedate" ]
            then
                ...
                auditinsert=$($AUDITLOGSCRIPT "add" "EM Management Upgrade/Restore" "EC upgrade process is initiated using $DEBIAN_FILE_NAME. Processing... Failed." "$IP_ADDRESS" "" "$upgradedate" )
            fi
        fi
    fi
fi
