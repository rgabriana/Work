    ...
    public String validateEmsMgmtSession(@PathParam("key") String key, @Context HttpServletRequest req) {
		...
		if(session != null && session.getAttribute("securityKey") != null && key.equals(session.getAttribute("securityKey").toString().substring(0, 15))) {
			try {
				...
				userAuditLoggerUtil.log("EC management session validated successfully.", UserAuditActionType.EM_Management_Validation.getName());
				return retKey;
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		userAuditLoggerUtil.log("EC management session validation failed.", UserAuditActionType.EM_Management_Validation.getName());
		return "F";
    }
