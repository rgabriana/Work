...
$(document).ready(function() {
	...
	jQuery("#discwds-wds-list-grid").jqGrid({
		...
		forceFit: true,
		scrollOffset: 0,
	   	colNames:["id", "Discovered (<span id='othr-wds-cnt'>"+othr_fx_cnt+"</span> EWS)", "",""],
	   	colModel:[
	...
	if(status.toUpperCase() == COMMISSIONED) {
		...
		try {
			...
			if(SWITCH_ID != undefined && SWITCH_ID != "")
				getCommissionPlanObj("c_wds_floorplan").addWds('floor', '${floorId}', $("#wdsId").val(), $("#wdsname").val(), SWITCH_ID, GATEWAY_ID);
			else
				getCommissionPlanObj("c_wds_floorplan").addWds('floor', '${floorId}', $("#wdsId").val(), $("#wdsname").val(), "", "");
		} catch(e){
			resetDevicePositionOnFloorplan();
		...
		success: function(data) {
			...
			switch(bStatus) {
				case DISC_STATUS_SUCCESS:
					displayFxCommissionMessage("EWS discovered.", COLOR_SUCCESS);
					stopDiscoveryTimer();
					break;
...
function validateWDSName()
{
	...
	var invalidFormatStr = 'EWS name must contain only letters, numbers, or underscore';
