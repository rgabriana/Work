...
//        }
    ...
    public void dimFixtures(int groupId, int percentage, int time) {
        DeviceServiceImpl.getInstance().dimFixturesByGroup(groupId, percentage, time, ServerConstants.DIM_TYPE_RELATIVE);
    }
		...
		for (String facility : selectedFacility) {
			...
			if ("template".equalsIgnoreCase(facilityType)) {			
				...
				if(!(visibilityFlag==false && list.size()>=1))	
				{
					...
					userAuditLoggerUtil.log("Update Profile Template Visibility , id : " + facilityId + " Flag :"+visibilityFlag, UserAuditActionType.Profile_Update.getName());
				}
