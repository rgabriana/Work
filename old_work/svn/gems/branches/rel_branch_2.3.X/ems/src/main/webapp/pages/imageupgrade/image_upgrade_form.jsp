...
function loadWdsImageCombo(){
	...
	WDS_IMG_COMBO.empty();
	WDS_IMG_COMBO.append($('<option></option>').val("-1").html("Select a EWS image"));
...
function validateUpgradeForm(device,gw,fx,wds) {
...
function setSelectedDevices(selFixtures, selGateways, selWds) {
	...
	if (selGwRows.length == 0 && GW_IMG_COMBO_VALUE == -1 && selFxRows.length == 0 && FX_IMG_COMBO_VALUE == -1 && selWdsRows.length == 0 && WDS_IMG_COMBO_VALUE == -1 ) {
