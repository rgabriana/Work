...
public class EcUtil {
	...
	public static AggregatedSensorData convertToAggregatedData(AggregatedStatsData data, String timeSpan, String target) {
		...
		sensorData.setSavedTaskTunedEnergy(new BigDecimal(data.getTuneupSavings().doubleValue() + 
				data.getManualSavings().doubleValue()));
		return sensorData;
	...
	public static AggregatedSensorData convertToAggregatedData(Object[] data, String timeSpan, String target, long levelId,
		...
		sensorData.setSavedTaskTunedEnergy(new BigDecimal((Double)data[6] + (Double)data[7]));
		if(data.length == 20) {
			sensorData.setMotionEvents(((Long)data[19]).intValue());
		}
		return sensorData;
	}
