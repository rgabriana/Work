...
upgradeAll() {
    ...
    then
        ...
        then
            echo "ERROR: Cannot upgrade to lower version"
            status="2"
            return
        fi
        ...
        then
            echo "ERROR: The debian package is not valid."
            status="2"
            return
        fi	
    ...
    then
	    ...
	    echo "ERROR: Failed to prepare all EM components. Exit upgrade process."
        status="2"
        return
    fi
        ...
        then
            ...
            then
                ...
                then
                    ...
                    if [ -z "$backupexists" ]
                    then
                        echo "ERROR: Database backup was not successful. Cannot continue with upgrade."
                        status="2"
                        return
                    fi
                else
            ...
            echo "ERROR: Postgres not running. Cannot take database backup. Stopping upgrade process."
            status="2"
            return
        fi
    ...
    then
        ...
        echo "ERROR: Failed to stop tomcat6 service. Exit upgrade process."
        status="1"
	    return
    fi
    ...
    then
	    ...
	    echo "ERROR: EM App upgrade failed. Exit upgrade process and bringing up tomcat server."
        status="1"
	    return
    fi
    ...
    then
	    ...
	    echo "ERROR: EM System upgrade failed. Exit upgrade process and bringing up tomcat server."
        status="1"
        return
    fi

    echo "*** Upgrading EM Communicator... ***" >> $LOGS_ERR
    sudo dpkg -i --force-overwrite ./*_ems_communicator.deb
    if [ $? -eq 0 ]
    then
        echo "step5;"
	    echo "*** EM Communicator upgraded successfully ***" >> $LOGS_ERR
    else
	    echo "ERROR: EM Communicator upgrade failed. Exit upgrade process and bringing up tomcat server."
        status="1"
	    return
    fi
    ...
    if [[ "$check" =~ "python2.6-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install python2.6-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "python-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install python-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libapr1" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libapr1 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libaprutil1" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libaprutil1 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libaprutil1-dbd-sqlite3" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libaprutil1-dbd-sqlite3 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libaprutil1-ldap" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libaprutil1-ldap successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "apache2.2-bin" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install apache2.2-bin successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "apache2-utils" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install apache2-utils successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "apache2.2-common" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install apache2.2-common successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "apache2-mpm-worker" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install apache2-mpm-worker successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "apache2" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install apache2 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "1.8.1+dfsg-2ubuntu0.2" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libk5crypto3 successfully."
                status="1"
	            return
            fi
	    else
    ...
    if [[ "$check" =~ "1.8.1+dfsg-2ubuntu0.2" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libkrb5support0 successfully."
                status="1"
	            return
            fi
	    else
    ...
    if [[ "$check" =~ "1.8.1+dfsg-2ubuntu0.2" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libkrb5-3 successfully."
                status="1"
	            return
            fi
	    else
    ...
    if [[ "$check" =~ "1.8.1+dfsg-2ubuntu0.2" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libgssapi-krb5-2 successfully."
                status="1"
	            return
            fi
	    else
    ...
    if [[ "$check" =~ "2.4.21-0ubuntu5.3" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libldap-2.4-2 successfully."
                status="1"
	            return
            fi
	    else
    ...
    if [[ "$check" =~ "8.4.6-0ubuntu10.04" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libpq5 successfully."
                status="1"
	            return
            fi
	    else
    ...
    if [[ "$check" =~ "libldap2-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libldap2-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libexpat1-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libexpat1-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libdb4.8-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libdb4.8-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libpcrecpp0" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libpcrecpp0 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libpcre3-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libpcre3-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "uuid-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install uuid-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libapr1-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libapr1-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libsqlite3-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libsqlite3-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "zlib1g-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install zlib1g-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libssl-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libssl-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libgssrpc4" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libgssrpc4 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libkdb5-4" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libkdb5-4 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libkadm5srv-mit7" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libkadm5srv-mit7 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libkadm5clnt-mit7" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libkadm5clnt-mit7 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "comerr-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install comerr-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "krb5-multidev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install krb5-multidev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libkrb5-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libkrb5-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libpq-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libpq-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "mysql-common" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install mysql-common successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libmysqlclient16" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libmysqlclient16 successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libmysqlclient-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libmysqlclient-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "libaprutil1-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install libaprutil1-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    if [[ "$check" =~ "apache2-threaded-dev" ]]
        then
            ...
            if [ $? -ne 0 ]
            then
	            echo "ERROR: EM Management upgrade failed. Could not install apache2-threaded-dev successfully."
                status="1"
	            return
            fi
    fi
    ...
    then
            ...
            then
                ...
                if [ $? -eq 0 ]
                        then
                                ...
                                echo "ERROR: Database was not upgraded succesfully. Could not continue with the upgrade."
                                status="1"
	                            return
                fi
            else 
                ...
                echo "ERROR: database server is not running. Could not continue with the upgrade."
                status="1"
	            return
        fi
        ...
        sleep 10
        startapache=$(sudo /etc/init.d/apache2 force-reload)
        sleep 10
        startapache=$(sudo /etc/init.d/apache2 restart)
        if [[ "$startapache" =~ "done" ]]
        then
	        ...
	        echo "ERROR: Failed to start Apache2 service. Contact enlighted admin."
            status="1"
	        return
        fi
    else
	    echo "ERROR: EM Management upgrade failed. Exit upgrade process and bringing up tomcat server."
        status="1"
	    return
    fi
}
...
startTomcat() {
    ...
    status="1"
    echo "*** ERROR: TOMCAT6 RESTART FAILED. Please do it manually ***" >> $LOGS_ERR
}
...
then
   ...
   status="1"
   echo "ERROR: Cannot continue with upgrade. There is already an ongoing process doing some critical work. Please try again after some time."
fi
