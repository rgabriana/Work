...
<div id="switch-list-topPanel" style="background: #fff">
	<c:if test="${page == 'floor' or page == 'area'}">
		<button id="newSwitchflow" onclick="javascript:parent.parent.showWidgetDialog();">Create</button>		
		<button id="newSwitchflow" onclick="javascript:parent.parent.showWidgetDialog();">Create Switch Wizard</button>
	</c:if>
	<br style="clear:both;"/>
...
// 		   	pager: '#switchPagingDiv',
		...
		<c:forEach items="${switches}" var="switch">
			...
			<c:if test="${page == 'floor' and switch.areaId == undefined }">
				localData.action += "<button onclick=\"javascript: parent.parent.showWidgetDialog(${switch.id});\">Edit</button>";
			</c:if>
			<c:if test="${page == 'area'}">
				localData.action += "<button onclick=\"javascript: parent.parent.showWidgetDialog(${switch.id});\">Edit</button>";
			</c:if>
