...
<spring:url value="/services/org/switch/getcommissionstatus" var="getCommissionStatusUrl" scope="request" />
<spring:url value="/services/org/switch/list/" var="getAllWdsBySecondaryGatewayUrl" scope="request" />
<spring:url value="/services/org/switch/list/" var="getAllWdsByWdsId" scope="request" />
<spring:url value="/services/org/switch/details/" var="getWdsDetailsUrl" scope="request" />
<spring:url value="/services/org/switch/updateduringcommission" var="getWdsUpdateUrl" scope="request" />
<spring:url value="/services/org/switch/startcommission" var="startWdsUpdateUrl" scope="request" />
<spring:url value="/services/org/switch/validatewds" var="validateWdsUrl" scope="request" />
<spring:url value="/services/org/switch/changeprofile/" var="changeWdsProfileUrl" scope="request" />
<spring:url value="/services/org/switch/exitcommission/" var="exitCommissionUrl" scope="request" />
<spring:url value="/services/org/switch/rma/" var="rmaUrl" scope="request" />
...
<title>Commission and Place WDS</title>
...
$(document).ready(function() {
	...
	var othr_fx_cnt = 0;
	jQuery("#discwds-fx-list-grid").jqGrid({
		datatype: "local",
		autowidth: true,
		forceFit: true,
		scrollOffset: 0,
	   	colNames:["id", "Others (<span id='othr-fx-cnt'>"+othr_fx_cnt+"</span> WDS)", "",""],
	   	colModel:[
	...
	forceFitJQgridHeight(jQuery("#discwds-fx-list-grid"), otherGridEL.offsetHeight);
...
function loadAllWdsBySecondaryGatewayId(){
	var params = "secondarygateway/"+WDS_ID+"/";
	$.ajax({
		url: "${getAllWdsByWdsId}"+params+"?ts="+new Date().getTime(),
		dataType:"xml",
		contentType: "application/json; charset=utf-8",
		success: function(data){
			if(data!=null){
				...
				if(switchData != undefined){
					...
					if(switchData.length == undefined){ //FIXME : JSON response issue : For only 1 record it returns Object instead of an Array
						...
						loadAllWDSBySecondaryGatewayId_Success(switches);
...
function parseXml(xml) { 
    ...
    $(xml).find("switch").each(function() { 
    	switches.push({ 
    		...
    		dimmercontrol: $(this).find("dimmercontrol").text(),
    		activecontrol: $(this).find("activecontrol").text(),
    		switchtype: $(this).find("switchtype").text(),
...
function loadAllWDSBySecondaryGatewayId_Success(data){
	...
	var uncommissionJGrid = jQuery("#discwds-fx-list-grid");
...
function updateWdsCount(){
	$("#othr-fx-cnt").html(wds_data_uncommissioned.length);
}
var currentWds = {};
	...
	var uncommissionJGrid = jQuery("#discwds-fx-list-grid");
	 if(gridIndex == 2) { // Other Grid
		if(rowid > 0){
			...
			displayFxCommissionMessage("Select a WDS from Others WDS list", COLOR_FAILURE);
			return;
		}
	}
...
function validateWDSForm(){
	if(CURRENT_WDS_OBJECT.id == undefined) {
		displayFxCommissionMessage("Select a WDS from the list", COLOR_FAILURE);
		return false;
	}
	var wdsname = $("#wdsname").val();
	WDSName = $.trim(wdsname);
	if(WDSName == ""){
		displayFxCommissionMessage("Enter WDS name", COLOR_FAILURE);
		return false;
	} else {
		clearFxCommissionMessage();
...
function getWDSXML(){
	...
	var xmldata = "<switch>"+
						...
						"<campusid>"+CURRENT_WDS_OBJECT.campusid+"</campusid>"+
						"<xaxis>"+CURRENT_WDS_OBJECT.xaxis+"</xaxis>"+
						"<yaxis>"+CURRENT_WDS_OBJECT.yaxis+"</yaxis>"+
						"<dimmercontrol>"+CURRENT_WDS_OBJECT.dimmercontrol+"</dimmercontrol>"+
						"<activecontrol>"+CURRENT_WDS_OBJECT.activecontrol+"</activecontrol>"+
						"<switchtype>"+CURRENT_WDS_OBJECT.switchtype+"</switchtype>"+
						"<state>"+CURRENT_WDS_OBJECT.state+"</state>"+
						"<gatewayid>"+CURRENT_WDS_OBJECT.gatewayid+"</gatewayid>"+
						"<gemsgroupid>"+CURRENT_WDS_OBJECT.gemsGroupId+"</gemsgroupid>"+
					"</switch>";
...
function updateWDS(){
	...
	displayFxCommissionMessage("Updating WDS...", COLOR_SUCCESS);
	$.ajax({
		...
		success: function(data){
			displayFxCommissionMessage("WDS updated successfully", COLOR_SUCCESS);
...
function updateWdsProfile(bCommission) {
	...
	$.ajax({
		...
		success: function(data){
			if (bCommission) {
				displayFxCommissionMessage("Commissioning WDS, please wait...", COLOR_SUCCESS);
				validateWds();
			}
		}
	});	
...
function commissionDevice(){
	...
	$.ajax({
		...
		success: function(data){
			startCommissioning();
			displayFxCommissionMessage("WDS commissioning in process...", COLOR_SUCCESS);
		},
		error: function() {
...
function getCommissionPlacementStatus(){
	...
	displayFxCommissionMessage(commission_placement_retry_counter+": WDS commissioning in process...", COLOR_SUCCESS);
}
...
function getCommissionPlacementStatus_Success(status){
	if(status.toUpperCase() == COMMISSIONED) {
		...
		try {
			displayFxCommissionMessage("WDS commissioned successfully.", COLOR_SUCCESS);
			getCommissionPlanObj("c_wds_floorplan").addSwitch('floor', '${floorId}', $("#wdsId").val());
		} catch(e){
			resetDevicePositionOnFloorplan();
			displayFxCommissionMessage("WDS commissioned successfully, but failed to do placement on floor plan.", COLOR_SUCCESS);
		}
...
//TODO 	this.fixtureChart.resetDevicePosition();
	commission_placement_retry_counter = 0;
	displayFxCommissionMessage(" WDS Commissioned Failed", COLOR_FAILURE);
	resetDevicePositionOnFloorplan();
	enableDisableUIControls(true);
	bRMAInProgress = false;
	alert("Could not communicate with WDS using the provided WDS ID. \n\nTry the following: \n- Verify that the WDS name is entered \n- Check that the WDS has power \n- Check that the EM server is connected to the 'building lighting network"); //TODO : message
}

function undoButtonHandler(){
	setWdsBasicInfo(CURRENT_WDS_OBJECT);
	//setfixtureDetailInfo(CURRENT_WDS_OBJECT);
}
...
function resizeWdsCommissionDialog(){
	...
	var otherGridEL = document.getElementById("other-fx-list-container");
	forceFitJQgridHeight(jQuery("#discwds-fx-list-grid"), otherGridEL.offsetHeight);
	jQuery("#discwds-fx-list-grid").setGridWidth(otherGridEL.offsetWidth);
}
...
<body> 
 <table id="fxcd-main-box" style="width:100%; height:100%;">
	 ...
	 <tr height="18px">
		 <td class="highlightGray"><div class="label">Available WDS</div></td>
		 <td class="highlightGray"><div class="label">WDS Configuration</div></td>
	 </tr>
	 <tr height="auto">
		  	...
		  	<table cellspacing=0 style="width:100%; height:100%;">
				...
				<tr style="height:100%;">
				 	<td id="other-fx-list-container" style="vertical-align: top; border-top:1px solid black;">
						<table id="discwds-fx-list-grid"></table>
					</td>
				</tr>
			</table>
		 </td>
							...
							<div id="fx-form-basic-tab" class="tab-container" style="display:none;">
								<form id="fx-basic-form" class="fxcd-form-body">
									...
									<!-- 
										...
										-->
										<div class="fieldWrapper">
											<div class="fieldlabel"><label for="wdstype">WDS Type:</label></div>
