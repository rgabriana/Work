...
public class FixtureCache {
  ...
  public synchronized long getLastFixtureCurrentDataId() {
    
    return ++lastCurrentDataId;
    
  } //end of method getLastFixtureCurrentDataId
  ...
  public void initializeDeviceMap() {
    ...
    lastCurrentDataId = fixtureMgr.getMaxFixtureCurrentDataId();
    System.out.println("last current data id -- " + lastCurrentDataId);
    List<Fixture> fixtureList = fixtureMgr.loadAllFixtures();
    if (fixtureList == null) {
      return;
    }
    //HashMap<Long, FixtureCurrentData> currDataMap = fixtureMgr.loadAllFixtureCurrentData();
    DeviceInfo device = null;
    int noOfFixtures = fixtureList.size();
    Fixture fixture = null;
    for (int i = 0; i < noOfFixtures; i++) {
      fixture = fixtureList.get(i);
      //device = new DeviceInfo(fixture, currDataMap.get(fixture.getId()));      
