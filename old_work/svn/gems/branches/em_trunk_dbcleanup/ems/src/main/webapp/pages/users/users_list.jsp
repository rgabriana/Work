...
<div class="outermostdiv">
	<script type="text/javascript">
    	...
    	var assignFacility = function(url){
    		clearLabelMessage();
        	$("#assignFacilityDialog").load(url+"?ts="+new Date().getTime()).dialog({
                title : "Assign Facility to User",
                width :  Math.floor($('body').width() * .35),
                height : 500,
                modal : true
            });
				...
				<td align="right" style="padding-right:5px;width:25%;">					
					<div>
						<form id="user-details" method="post" onsubmit="return beforeDeleteUser();" action="${actionURL}">
							...
							<input type="button" id="editTenantId" name="editTenantId" value="Edit" onclick="editUsersDetails('${editUserUrl}/${user.id}/edit.ems');"/>
							<input type="button" id="btnAssignFacility" onclick="return assignFacility('${assignFacilityUserUrl}/${user.id}/assignFacilityUser.ems');" value="Assign Facility"/>
