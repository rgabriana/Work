...
<div id="wds-list-topPanel" style="background: #fff">
	<c:if test="${page == 'floor' or page == 'area'}">
		<button id="newWds" onclick="javascript: parent.parent.showWdsCommissioningIdentifyWindow();">Add WDS</button>
...
var COMM_ERROR_INACTIVE_TIMED_OUT_GW_CH_CHANGE_CUSTOM = 16;
	...
	WDS_GRID = $("#wdsTable");
		WDS_GRID.jqGrid({
			...
			hoverrows: false,
			forceFit: true,
		   	colNames:['Name', 'Switch', 'Action'],
		   	colModel:[
		   		{name:'name', index:'name', sorttype:"string", width:"34%"},
		   		{name:'switch', index:'switch', sorttype:"string", width:"34%"},
		...
		<c:forEach items="${wdss}" var="wds">
			...
			localData.switch = "${wds.wdsSwitch.name}";
