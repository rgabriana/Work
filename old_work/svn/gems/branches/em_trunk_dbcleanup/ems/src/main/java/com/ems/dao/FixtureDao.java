...
public class FixtureDao extends BaseDaoHibernate {
	...
	public List<Fixture> loadFixtureLocationsByFloorId(Long id) {
		...
		try {
			Session s = getSession();
			String hsql = "SELECT f.id, f.name, f.xaxis, f.yaxis, f.groupId "
	...
	public List<Fixture> loadFixtureByFloorId(Long id) {
		...
		List<Fixture> fixtureList = session
				...
				.createAlias("currentData", "currentData", Criteria.LEFT_JOIN)
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
		if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
			return fixtureList;
		} else {
	...
	public List<Fixture> getAllCommissionedFixturesWithCurrentDataBySecGwId(Long secGwId) {
		...
		List<Fixture> fixtureList = session
				.createCriteria(Fixture.class)
				.createAlias("area", "area", Criteria.LEFT_JOIN)
				.createAlias("currentData", "currData", Criteria.LEFT_JOIN)
				.add(Restrictions.eq("state",
						ServerConstants.FIXTURE_STATE_COMMISSIONED_STR))
				.add(Restrictions.eq("secGwId", secGwId))
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)
	...
	public List<Fixture> loadAllFixtureBySecondaryGatewayId(Long secGwId) {
		...
		List<Fixture> fixtureList = session
				...
				.add(Restrictions.eq("secGwId", secGwId))
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)
				.addOrder(Order.asc("currentData.lastConnectivityAt")).list();
		if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
			return fixtureList;
		} else {
			return null;
	...
	/**
	 * Load fixture current data available in fixture current data table
	 *
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public HashMap<Long, FixtureCurrentData> loadAllFixtureCurrentData() {
	  ...
	  Session session = getSession();
	  List<FixtureCurrentData> fixtureList = session
	      .createCriteria(FixtureCurrentData.class)			
	      .setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	  if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
	    HashMap<Long, FixtureCurrentData> map = new HashMap<Long, FixtureCurrentData>();
	    Iterator<FixtureCurrentData> iter = fixtureList.iterator();
	    FixtureCurrentData data = null;
	    while(iter.hasNext()) {
	      data = iter.next();
	...
	} //end of method loadAllFixtureCurrentData
	...
	public void enablePushProfileAndGlobalPushProfile(Long fixtureId,boolean pushProfileStatus, boolean globalPushProfileStatus) {
	  ...
	  Session session = getSession();
	  Fixture dbFixture = (Fixture) session.load(Fixture.class, fixtureId);
	...
	public void updateStats(Fixture fixture) {
	  ...
	  Session session = getSession();
	  FixtureCurrentData dbFixture = fixture.getCurrentData();
	  boolean newCurrentData = false;
	  if(dbFixture != null) {
	    dbFixture = (FixtureCurrentData) session.load(FixtureCurrentData.class, dbFixture.getId());
	  } else {
	    dbFixture = new FixtureCurrentData();	    
	    dbFixture.setId(FixtureCache.getInstance().getLastFixtureCurrentDataId());
	    save(dbFixture);
	    newCurrentData = true;	    
	    System.out.println("current data did not exist in the database");
	  }
	  ...
	  if(newCurrentData) {
	    fixture.setCurrentData(dbFixture);
	    Fixture dbFix = (Fixture)session.load(Fixture.class, fixture.getId());
	    dbFix.setCurrentData(dbFixture);
	  }
	  /*	  
	...
	public void updateVersionSynced(long fixtureId, int versionSynced) {
	  ...
	  Session session = getSession();
	  Fixture fixture = (Fixture) session.get(Fixture.class, fixtureId);	
	  fixture.setVersionSynced(versionSynced);
	...
	public void updateVersion(String version, long id, long gwId) {
		...
		FixtureCurrentData currData = (FixtureCurrentData) session
		    .get(FixtureCurrentData.class, fixture.getCurrentData().getId());
		currData.setLastConnectivityAt(new Date());
		currData.setCurrApp((short)2);
	...
	public void updateBootInfo(Fixture fx, String upgrStatus) {
		...
		FixtureCurrentData currData = (FixtureCurrentData) session
		    .get(FixtureCurrentData.class, fx.getCurrentData().getId());
		currData.setLastConnectivityAt(fx.getLastConnectivityAt());
		currData.setCurrApp(fx.getCurrApp());
        ...
        try {
          /*
            ...
            */
            ...
            dbFixture.setFixtureName(fixtureName);
		...
		if (property != null && pid != null) {
			oRowCount = getSession()
					...
					.createAlias("gateway", "gw",
							CriteriaSpecification.LEFT_JOIN)
					.setFetchMode("gw", FetchMode.JOIN)
					.createAlias("currentData", "currentData",
					    CriteriaSpecification.LEFT_JOIN)
					.setFetchMode("currentData", FetchMode.JOIN)
					.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)
					.setProjection(Projections.rowCount());
			...
			oCriteria = getSession()
					...
					.add(Projections.property("currentData.lastConnectivityAt"), "lastConnectivityAt")
					.add(Projections.property("fx.name"), "fixtureName")
					.add(Projections.property("fx.isHopper"), "isHopper")
					.add(Projections.property("currentData.currApp"), "currApp")
			...
			if (order != null && !"".equals(order)) {
				if (order.equals("name")) {
					...
					order = "currentData.lastConnectivityAt";
				} else {
					order = "fx.id";
				}
				if ("desc".equals(orderWay)) {
	...
	public long getMaxFixtureCurrentDataId() {
	  ...
	  try {
	    Session s = getSession();
	    String hsql = "SELECT MAX(id) from FixtureCurrentData";
	    Query query = s.createQuery(hsql);
