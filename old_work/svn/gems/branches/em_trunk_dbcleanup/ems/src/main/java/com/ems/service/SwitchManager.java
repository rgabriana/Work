...
public class SwitchManager {
    ...
    public int manageGroupMembership(Long switchId) {
        ...
        logger.info(switchId + ": Join switch group " + groupNo);
    ...
    public int sendSwitchGroupParams(Long switchId, Long fixtureId) {
        ...
        // 32 bits
        int groupNo = Integer.parseInt(oSwitchGroup.getGroupNo().toString(), 16);
        List<SceneLevel> oList = sceneLevelDao.loadSceneLevelListBySwitchAndFixtureId(switchId, fixtureId);
        if (oList == null) {
            return 3;
        }
    ...
    public int sendSwitchGroupWdsParams(Long switchId) {
        ...
        if (oMarkedDelWdsList != null && oMarkedDelWdsList.size() > 0) {
            ...
            for (int count = 0; count < oMarkedDelWdsList.size(); count++) {
                ...
                iDelStatus = sendWdsDeletionMsg(oDelWds.getId());
                if (iDelStatus == 0) {
        ...
        // If all WDS are deleted, then no need to push the WDS configuration, but we ensure that the delete wds commands reach the SUs.
        if (oWdsList.size() == 0) {
            return 6;
        }
            ...
            for (int count = 0; count < wds_cnt; count++) {
                ...
                for (int pos = 0, idx = 2; idx < oList.size(); pos++, idx++) {
                    ...
                    if (pos < MAX_SCENES) {
                        ...
                        scene_toggle_order_list[pos] = (byte) (lOrder.byteValue());
                    }
                }
