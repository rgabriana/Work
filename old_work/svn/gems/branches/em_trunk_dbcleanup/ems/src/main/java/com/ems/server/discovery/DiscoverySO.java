...
//	}, 0, validateTimeout);      
   ...
   */
   public void discoveryWDS(String snapAddr, byte[] pkt, String gwIp) {
        ...
        System.out.println("WDS Discovered " + snapAddr + " via: " + gwIp);
        ...
        if (oWds == null) {
            oWds = new Wds();
            oWds.setWdsNo(Integer.valueOf("24" + wdsMgr.getNextWdsNo()));
        ...
        try {
            ...
            String sName = "WDS" + ServerUtil.generateName(snapAddr);
            oWds.setName(sName);
            Floor floor = floorMgr.getFloorById(gw.getFloor().getId());
            oWds.setFloorId(floor.getId());
  ...
  public void discoveryData(String snapAddr, byte[] pkt, String gwIp) {
    ...
    FixtureCurrentData currentData = null;
    if(fixture == null) {
      //fixture is not in the database
      fixture = new Fixture();
      currentData = new FixtureCurrentData(); 
      currentData.setId(FixtureCache.getInstance().getLastFixtureCurrentDataId());
      currentData.setLastConnectivityAt(new Date());
      currentData.setBaselinePower(new BigDecimal(0));
      currentData.setLastStatsRcvdTime(new Date()); 
      fixtureMgr.saveFixtureCurrentData(currentData);                 
      fixture.setCurrentData(currentData);
    } else {
      if(logger.isInfoEnabled()) {
    ...
    public int startSendingWdsDiscoveryPkts(final long floorId, final long gatewayId) {
        if (commStatus == COMM_STATUS_INPROGRESS) {
            if (wdsDiscovery)
                return 1;
            if (logger.isDebugEnabled()) {
                logger.debug(gatewayId + ": startSendingWdsDiscoveryPkts called");
            }
            readDiscoveryParams();
            ...
            new Thread("WDS_Discovery") {
                public void run() {
                    ...
                    while (wdsDiscovery) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(gatewayId + ": no. of WDS associated -- " + noOfWds);
                        }
                        if (noOfWds >= 1) {
                            wdsDiscovery = false;
                        }
  ...
  public int startNetworkDiscovery(final long floorId, final long gatewayId, final int iDeviceType) {
	...
	while(networkDiscovery) {
	  ...
	  if (iDeviceType == ServerConstants.DEVICE_FIXTURE) {
          ...
          noOfWds = (oWdsSwitchList != null) ? oWdsSwitchList.size() : 0;
          if(logger.isDebugEnabled()) {
            logger.debug(gatewayId + ": 3. no. of WDS associated -- " + noOfWds);
          }
          try {
            if (gw != null) {
          ...
          if(noOfWds >= noOfInstWds) {
            ...
            " Wds installed, " + noOfWds + " Wds discovered", 
            EventsAndFault.DISCOVER_EVENT_STR, EventsAndFault.INFO_SEV_STR);
          }
	  }
	}
...
//    System.out.println("distance -- " + firstPoint.distance(425, 197));
  ...
  private void changeHoppersWireless(Gateway gw, boolean defWireless, String discCommStr) {
    ...
    String eventDesc = "";
    if(defWireless) {
      DeviceServiceImpl.getInstance().setApplyWirelessDefaults(fixtArr, gw, (short)0, (byte)1);
...
//                deviceImpl.sendValidationCmd(oList, wdsArr, 0, type, gw);
    ...
    public int finishWDSCommissioning(long gwId, List<Wds> wdsList) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(gwId + ": finishWDSCommissioning");
        }
        final Gateway gw = gwMgr.loadGateway(gwId);
        if (gw == null) {
            installState = INSTALL_STATE_READY;
