...
public class WdsResponsePacket {
    ...
    public void processResponse() {
        ...
        if (oWds == null) {
            logger.error(snapAddr + ": There is no WDS, ignoring the node pkt - " + ServerUtil.getLogPacket(pkt));
            pkt = null;
            return;
        }
        ...
        case ServerConstants.ACK_TO_MSG:
            ...
            case ServerConstants.CMD_APPLY_SWITCH_PARAMS:
                logger.info("(WDS Commissioned): WDS packet Ack for apply switch param.");
                break;
            default:
                break;
            }
