...
public class CommandScheduler {
  ...
  public long addCommand(Fixture fixture, byte[] dataPacket, int msgType, boolean retryReq,
      int pktDelay) {
    
    int[] fixtureArr = new int[1];
    fixtureArr[0] = fixture.getId().intValue();
    long seqNo = addCommand(fixtureArr, dataPacket, msgType, retryReq, pktDelay);
    fixture = null;
    dataPacket = null;
    return seqNo;
  ...
  public class GwCmdJob implements Runnable {
	...
	if(pendingSUList.contains(fixture.getId())) {
	  ...
	  if(fixtureLogger.isInfoEnabled()) {
	    fixtureLogger.info(cmdSeq + ": pending " + fixture.getName());
	  }
	  continue;
	}
	byte[] snapByteArr = ServerUtil.getSnapAddr(fixture.getMacAddress());
	if(snapByteArr == null) {
	  fixtureLogger.error(cmdSeq + ":could not find " + fixture.getMacAddress());
	  suIter.remove();
	  continue;
	}
	...
	while(retryIter.hasNext()) {
      ...
      if(mPkt.getNoOfTargets() > 0) {
      	...
      	if(msgType == ServerConstants.SET_CURRENT_TIME) {
      		...
      		if(device == null && fixture instanceof Fixture) {
      			device = FixtureCache.getInstance().getDevice((Fixture)fixture);
      		}      		
      		int seqNo = 1;
          if(device != null) {         
