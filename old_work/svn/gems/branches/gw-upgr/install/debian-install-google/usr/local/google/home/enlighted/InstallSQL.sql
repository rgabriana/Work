...
DECLARE 
	...
	BEGIN		
	FOR rec_plugload IN(
	select id,coalesce(last_zb_update_time,date_trunc('hour',now()::timestamp) - interval '7 days')  from plugload where state='COMMISSIONED')
	LOOP		
	...
	--raise notice 'plugload id is %,%',rec_plugload.plugload_id,lastProcesssedCaptureAt;	
	...
	and capture_at >= lastProcesssedCaptureAt and plugload_id = rec_plugload.plugload_id group by capture_at,plugload_id limit 1) zb;
	...
	update plugload set last_zb_update_time = lastZbUpdateTime where id=rec_plugload.plugload_id;	
	END LOOP;	
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION aggregatedailyenergyconsumptionforplugload(timestamp without time zone)
  OWNER TO postgres;
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION aggregatehourlyenergyconsumptionforplugload(timestamp without time zone)
  OWNER TO postgres;
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'iptables.rules.static.ports', '22,8085,7,443,8443,80');
