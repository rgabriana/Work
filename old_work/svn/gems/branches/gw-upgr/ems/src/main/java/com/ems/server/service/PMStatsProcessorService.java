...
//					+ (System.currentTimeMillis() - startTime));
	...
	private void adjustTaskTunedSavings(Fixture fixture, EnergyConsumption ec, double savedPower, 
	    short avgVolts, int savingType, int lastVolts, long motionBits, int currState) {
	  ...
	  try {	
	    ...
	    if(currState == ServerConstants.CURR_STATE_MANUAL || fixture.getLastOccupancySeen() < 300) {
	    	// incase of manual lighting based occupancy is always 1 or if the motion is observed in the last 5 mins
