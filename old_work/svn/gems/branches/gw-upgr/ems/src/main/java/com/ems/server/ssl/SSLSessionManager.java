...
public class SSLSessionManager {
   ...
   */
	...
	public void initNwInterface() {
		sslLogger.info("calling init method in ssl session manager");
		
		systemConfigurationManager = (SystemConfigurationManager) SpringContext
				.getBean("systemConfigurationManager");
		
		networkSettingsManager = (NetworkSettingsManager) SpringContext.getBean("networkSettingsManager");

		String nimCorporate = networkSettingsManager.loadCurrentMappingByNetworkType("Corporate");
		String corporateMapping="eth0",buildingMapping="eth1";
		if(nimCorporate != null ){
			corporateMapping = nimCorporate;
		}
		String nimBuilding = networkSettingsManager.loadCurrentMappingByNetworkType("Building");
		if(nimBuilding != null){
			buildingMapping = nimBuilding;
		}else{
			if(nimCorporate!=null) buildingMapping = nimCorporate;
		}
		
		sslLogger.debug("corporate Mapping "+corporateMapping+" building mapping "+buildingMapping);
		...
		if (dhcpConfig != null) {
			if (dhcpConfig.getValue().contentEquals("true")) {
				bldgGemsIp = getIp4Address(buildingMapping);
				sslLogger.info("Listening on "+buildingMapping);
			} else {
				if(nimBuilding != null){
					bldgGemsIp = getIp4Address(buildingMapping);
					sslLogger.info("Listening on "+buildingMapping);
				}else{
					bldgGemsIp = getIp4Address(corporateMapping);
					sslLogger.info("Listening on "+corporateMapping);					
				}
	...
	// function to compute the intersection of specified protocols and available protocols
	public static String[] listIntersection(String[] l1, String[] l2) {
	...
	private SSLConnection establishConnection(Gateway gw) throws SSLException {
		...
		try {
			...
			// We must be cautious here, some protocols/ciphers may not be available if the OpenSSL version is outdated
			socket.setEnabledCipherSuites(listIntersection(SSLConstants.TLS_CIPHER_SUITES, socket.getSupportedCipherSuites()));
			socket.setEnabledProtocols(listIntersection(SSLConstants.TLS_PROTOCOL_VERSIONS, socket.getSupportedProtocols()));
