...
public class PlugloadImpl {
  ...
  private PlugloadProfileManager plugloadProfileMgr = null;
  ...
  private EmsThreadPool groupsSyncProcessThPool = null;
	...
	private PlugloadImpl() {
		...
		plugloadProfileMgr = (PlugloadProfileManager) SpringContext.getBean("plugloadProfileManager");
  ...
  public int getPlugloadConfigChecksum(Plugload plugload, int configChsumMap) {
  	...
  	try {
  		...
  		if((configChsumMap & 0x01) == 0x01) {
    		byte[] overrideByteArr = getOverride2ProfileByteArray(plugload);
  ...
  public byte[] getOverride2ProfileByteArray(Plugload plugload) {
	PlugloadProfileHandler prHand = null;
  	if (plugload.getGroupId() == 0) {
          // Custom
		  PlugloadGroups grp = groupMgr.getGroupById(plugload.getGroupId());
		  long prHandId = grp.getPlugloadProfileHandler().getId();
		  prHand = plugloadProfileMgr.getProfileHandlerById(prHandId);
		  //return prHand.getProfileByteArray(prHand.getOverride5(), prHand.getOverride6(), prHand.getOverride7(), prHand.getOverride8());
         
      } else {
          prHand = plugloadMgr.getProfileHandlerByGroupId(plugload.getGroupId());
          //return prHand.getProfileByteArray(prHand.getOverride5(), prHand.getOverride6(), prHand.getOverride7(), prHand.getOverride8());
      }
  	
  	
	byte[] morningByteArr = new ProfileData(prHand.getOverride5()).getByteArray();
	byte[] dayByteArr = new ProfileData(prHand.getOverride6()).getByteArray();
	byte[] eveningByteArr = new ProfileData(prHand.getOverride7()).getByteArray();
	byte[] nigthByteArr = new ProfileData(prHand.getOverride8()).getByteArray();
	
	byte[] profileData = new byte[morningByteArr.length + dayByteArr.length + eveningByteArr.length + nigthByteArr.length + 1];
		
	int i = 0;			
	//profileData[i++] = (byte)ServerConstants.WEEK_DAY_PROFILE;
	System.arraycopy(morningByteArr, 0, profileData, i, morningByteArr.length);
	i += morningByteArr.length;
	System.arraycopy(dayByteArr, 0, profileData, i, dayByteArr.length);
	i += dayByteArr.length;
  ...
  public void handleGroupsSynchronization(Plugload plugload, int groupsChecksum, int configChsumMap) {
  	
  	int emChecksum = getPlugloadConfigChecksum(plugload, configChsumMap);
    		...
    		for(int i = 0; i < noOfGroups; i++) {      			
    			...
    			if(groupNo > 12000000 && groupNo < 13000000) {
    				...
    				groupsMap.remove(motionGrp.getGemsGroup().getId());
      				GemsGroupPlugload ggf = gemsGrpMgr.getGemsGroupPlugload(motionGrp.getGemsGroup().getId(), plugload.getId());
      				if(ggf==null){
      					logger.error("GemsGroupPlugload is null for gemsGroupId: "+motionGrp.getGemsGroup().getId()+" and fixtureId: "+plugload.getId()+" .This should not happen in any case");
      				}else{
      					final MotionGroupPlugloadDetails mgfd = ggf.getMotionGrpPlDetails();
	      				...
	      				if(checksum != motionGrpChecksum) {
	      					...
	      					if(mgfd.getUseEmValue() == 1){
		      			        motionGrpMgr.sendMotionGroupDetailsToPlugload(motionGrp.getGemsGroup().getId(), ggf);
	      					}
	      				}
      				}
    			}
