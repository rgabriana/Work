...
#### CASE II:
######################################################################
#### enablePort.sh DISABLE/ENABLE eth1
...
then
	...
	then
		...
		cnt=`sudo cat /etc/iptables.rules | grep "$network -j DROP" | wc -l`	
		if [ $cnt -eq '0'  ]
	        then
	                sudo sed -i '/COMMIT/i -A INPUT -i '$network' -j DROP' /etc/iptables.rules
	        fi
