...
public class Fixture extends Device implements Serializable {
	...
	*/
    ...
    public void setManualAmbientValue(Integer manualAmbientValue) {
		...
		public void changeTriggerType(Boolean occ, Boolean light) {
			int currentType = getChangeTriggerType();
			int changeType = 0;
			if((light == null && (currentType & (1 << 3)) != 0) || (light != null && light)) {
				changeType += 8;
			}
			if((occ == null && (currentType & (1 << 2)) != 0) || (occ != null && occ)) {
				changeType += 6;
			}
			setChangeTriggerType(changeType);
		}
