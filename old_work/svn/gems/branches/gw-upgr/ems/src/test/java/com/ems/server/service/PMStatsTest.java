...
public class PMStatsTest {
	...
	public void buildEnergyData() {
		...
		ec1.setAvgTemperature(new Double(78));
		...
		ec2.setAvgTemperature(new Double(83));
	...
	public void testPMStats() {
		...
		try {
			BallastCache instance = Whitebox
					.invokeConstructor(BallastCache.class);
		} catch (Exception e1) {
		...
		for (int counter = 0; counter == 0; counter++) {
			...
			try {
				...
				pmService.setFixtureManager(fixtureManager);
				...
				Mockito.when(fixtureManager.getBallastById(1L)).thenReturn(
						bList.get(counter));
