...
public class EcService {
	...
	private int getMinuteTimeOffset(long levelId) {
		...
		EmInstance emInstance = emInstManager.loadEmInstanceById(emId);
		if(emInstance != null){
	...
	private List<SensorData> getRawSensorData(Long levelId, Long levelType, String startDateStr, String endDateStr, String attrib, String fixtureIds) {
		
		List<SensorData> sensorList = new ArrayList<SensorData>();
		if(levelType.intValue() == 4) {
			sensorList = fecManager.getFloorSensorData(levelId, startDateStr, endDateStr, attrib, fixtureIds);
		} else {
	...
	public String getSensorData(@PathParam("levelId") Long levelId, 
			...
			@PathParam("sDate") String startDateStr, @PathParam("tDate") String endDateStr,
			@DefaultValue("false") @QueryParam("details") Boolean details, @DefaultValue("") @QueryParam("attributes") String attributes,
			@DefaultValue("") @QueryParam("fixtures") String fixtureIds) {
		
		long apiTime = System.currentTimeMillis();
		ObjectMapper mapper = new ObjectMapper();
		String energyStr = new String();
		if(logger.isInfoEnabled()) {
			logger.info("agg data -- " + details + " level :wtype -- " + levelType + "time span -- " + timeSpan + " sdate -- " + startDateStr +
					" tDate --- " + endDateStr);
		}
		if(details == null) {
			details = false;
		...
		if(!details) {
			...
			if(!timeSpan.equals("5min")) {
				...
				try {
					List<SensorData> sensorList = getRawSensorData(levelId, levelType, startDateStr, endDateStr, attributes, fixtureIds);
	...
	@Path("getSensorDataCount/{levelId}/{levelType}/{sDate}/{tDate}")
	@GET
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public String getSensorDataCount(@PathParam("levelId") Long levelId, 
			@PathParam("levelType") Long levelType,
			@PathParam("sDate") String startDateStr, @PathParam("tDate") String endDateStr,
			@DefaultValue("") @QueryParam("fixtures") String fixtureIds) {
		
		long apiTime = System.currentTimeMillis();
		if(logger.isInfoEnabled()) {
			logger.info(" level :wtype -- " + levelType + " sdate -- " + startDateStr + " tDate --- " + endDateStr);
		}
		if(levelType.intValue() != 4) {		
			logger.error("not authorized to request at a non floor level");	
			return "-1";
			...
			try {
				return fecManager.getFloorSensorDataCount(levelId, startDateStr, endDateStr, fixtureIds);
			}
			catch(Exception e) {
				e.printStackTrace();
			}
		...
		if(logger.isDebugEnabled()) {
			logger.debug("get sensor data count api time - " + (System.currentTimeMillis() - apiTime));
		}
		return "-1";
