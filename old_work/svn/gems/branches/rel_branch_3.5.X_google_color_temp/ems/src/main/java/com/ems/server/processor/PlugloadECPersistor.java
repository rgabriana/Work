...
//										+ " for captureAt " + captureAt);
						...
						if(this.workQueue.get(captureAt) != null){
						...
						if(pec1 == null){
							...
							if(this.workQueue.get(captureAt) == null){
								this.workQueue.put(captureAt, map);
							}else{
								this.workQueue.get(captureAt).put(plugload.getSnapAddress(),plugloadEnergyConsumptionZB);
							}
						}
			...
			map = this.workQueue.get(captureAt);
			if (map != null) {
				try {
	...
	public void processRemainingCaptureATs(String captureAtTime) {
		...
		synchronized (this) {	
				...
				try {
					...
					logger.error(
					snapAddress
							+ ": Error in PL PMstats processing! "
							+ ", AvailableVMMemory: "
					...
					while(it.hasNext()){
						...
						while(it1.hasNext()){
							...
							if( o != null && o.size() == 2 ){
								...
								Long diffFactor = 5L;
								Double spreadManagedEnergyCumValues1 = ((BigInteger) o.get(0)[2]).doubleValue();
								Double spreadUnManagedEnergyCumValues1 = ((BigDecimal) o.get(0)[3]).doubleValue();	
								
								Double spreadManagedEnergyCumValues2 = ((BigInteger) o.get(1)[2]).doubleValue();
								Double spreadUnManagedEnergyCumValues2 = ((BigDecimal) o.get(1)[3]).doubleValue();
								
							//	logger.info("spreadManagedEnergyCumValues1="+spreadManagedEnergyCumValues1+" spreadManagedEnergyCumValues2="+spreadManagedEnergyCumValues2+" diffInMInutes="+diffInMinutes*100+" spreadDiffernce="+(spreadManagedEnergyCumValues1 - spreadManagedEnergyCumValues2));
								
								Double spreadManagedEnergyCumValues = (spreadManagedEnergyCumValues1 - spreadManagedEnergyCumValues2)/(100*diffInMinutes);
								Double spreadUnManagedEnergyCumValues = (spreadUnManagedEnergyCumValues1 - spreadUnManagedEnergyCumValues2)/(100*diffInMinutes);
								logger.info(" spread values are energy="+spreadManagedEnergyCumValues +" unmanaged_Energy="+spreadUnManagedEnergyCumValues);
								...
								if(spreadManagedEnergyCumValues >= 0 && spreadUnManagedEnergyCumValues >= 0) plugloadEnergyConsumptionManager.updateZeroBuckets(plugload.getId(),spreadManagedEnergyCumValues*diffFactor,spreadUnManagedEnergyCumValues*diffFactor,nzbCaptAT_higher,zbCaptAT);
							}
						}
					}									
				}
