...
<spring:url value="/services/org/motiongroup/updatemotiongroupfixtures/" var="updateGroupToFixtureUrl" scope="request" />
<spring:url value="/services/org/motiongroup/updatemotiongroupplugloads/" var="updateGroupToPlugloadUrl" scope="request" />
<style>
	.innerContainer{
		/* padding: 10px 20px; */
	}
...
<script type="text/javascript"> 
	...
	var getWidgetFloorPlanObj = function(objectName) {			
		if ($.browser.mozilla) {
	...
	var widget_fp = function(nodetype, nodeid) {		
		...
		if ($.browser.msie) {
			...
			FP_data +=  "<param name='flashVars' value='orgType=" + nodetype + "&orgId=" + nodeid + "&groupId=" + "${groupId}" + "&fixtureVersion=" + "${fixtureVersion}" + "&mode=GROUP_MODE&enablePlugloadFeature=${enablePlugloadProfileFeature}&modeid='/>";
			...
			FP_data +=  " flashvars='orgType=" + nodetype + "&orgId=" + nodeid + "&groupId=" + "${groupId}" + "&fixtureVersion=" + "${fixtureVersion}" + "&mode=GROUP_MODE&enablePlugloadFeature=${enablePlugloadProfileFeature}&modeid='/>";
			FP_data +=  "</object>";
		} else {
			FP_data = "<embed id='group_widget_floorplan' name='group_widget_floorplan' src='"+plotchartmoduleString+"' pluginspage='http://www.adobe.com/go/getflashplayer'";
			FP_data +=  " height='100%'";
	...
	var loadWidgetFP = function() {
		try{
			...
			getWidgetFloorPlanObj("group_widget_floorplan").changeLevelForWidget(treenodetype, treenodeid, 'GROUP_MODE', '', ${groupId});
		}
		catch (ex){
	...
	$(document).ready(function(){
		if(SELECTED_FIXTURES_TO_ASSIGN_MOTION_GROUPS != undefined && SELECTED_FIXTURES_TO_ASSIGN_MOTION_GROUPS.length > 0) {
			var dataXML = "";
			// Make service call to update the group with fixtures
			$.each(SELECTED_FIXTURES_TO_ASSIGN_MOTION_GROUPS, function(){
				// Create MotionGrpDetail Object
				var type="0";
				var value="0";
				var ambienttype="0";
				var loamval="0";
				...
				var tod="0";
				var lightLevel="0";
				dataXML += "<gemsGroupFixture><fixture><id>"+ this.id +"</id></fixture><motiongrpfxdetails><type>" + type + "</type><ambienttype>"+ambienttype+"</ambienttype><value>"+value+"</value><loambval>"+loamval
				+"</loambval><hiambval>"+hiambval+"</hiambval><tod>"+tod+"</tod><lightlevel>"+lightLevel+"</lightlevel></motiongrpfxdetails></gemsGroupFixture>";
			 });
			dataXML = "<gemsGroupFixtures>"+dataXML+"</gemsGroupFixtures>";
		 	$.ajax({
		 		type: 'POST',
		 		url: "${updateGroupToFixtureUrl}"+${groupId}+"?ts="+new Date().getTime(),
		 		data: dataXML,
		 		async: false,
		...
		if(SELECTED_PLUGLOADS_TO_ASSIGN_MOTION_GROUPS != undefined && SELECTED_PLUGLOADS_TO_ASSIGN_MOTION_GROUPS.length > 0) {
			var dataXML = "";
			// Make service call to update the group with plugloads
			$.each(SELECTED_PLUGLOADS_TO_ASSIGN_MOTION_GROUPS, function(){
				// Create MotionGrpDetail Object
				var type="0";
				var value="0";
				var ambienttype="0";
				...
				var tod="0";
				var lightLevel="0";
				dataXML += "<gemsGroupPlugload><plugload><id>"+ this.id +"</id></plugload><motiongrppgdetails><type>" + type + "</type><ambienttype>"+ambienttype+"</ambienttype><value>"+value+"</value><loambval>"+loamval
				+"</loambval><hiambval>"+hiambval+"</hiambval><tod>"+tod+"</tod><lightlevel>"+lightLevel+"</lightlevel></motiongrppgdetails></gemsGroupPlugload>";
			 });
			dataXML = "<gemsGroupPlugloads>"+dataXML+"</gemsGroupPlugloads>";
		 	$.ajax({
		 		type: 'POST',
		 		url: "${updateGroupToPlugloadUrl}"+${groupId}+"?ts="+new Date().getTime(),
		 		data: dataXML,
		 		async: false,
		 		success: function(data){
				},
