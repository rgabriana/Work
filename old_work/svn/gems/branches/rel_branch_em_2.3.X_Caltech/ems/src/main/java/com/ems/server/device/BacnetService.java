...
import com.occengine.service.OccBacnetService;
...
public class BacnetService implements EmsShutdownObserver {
    ...
    public static final int BACNET_MSGTYPE_ASSIGN_SU_PROFILE = 0xb;
    public static final int BACNET_MSGTYPE_PROFILE_DIM = 0xc;
    ...
    public static final int BACNET_MSGTYPE_SET_ZONE_DIM = 0x65;
    ...
    private String bacnetServerIp = "127.0.0.1";
   ...
   */
    private BacnetService() {
        ...
        //bacnet listner should start irrespective of bacnet is enabled or not as bacnet server could 
        //be started as part of BMS GW on a separate box
    ...
    private int fillSUData(byte[] responsePkt, int fillPos, Fixture fixture) {
        ...
        short groupId = fixture.getGroupId().shortValue();
        short ambientLevel = (short)(fixture.getLightLevel().shortValue() * 10.764);
        ...
        fillPos += 2;
        //group id 2 bytes
        ServerUtil.fillShortInByteArray(groupId, responsePkt, fillPos);
        fillPos += 2;
        //ambient light level 2 bytes
        ServerUtil.fillShortInByteArray(ambientLevel, responsePkt, fillPos);
        fillPos += 2;
        return fillPos;

    } // end of method fillSUData
    
    private byte[] getSUNames(Fixture fixture) {
	...
	try {
      ...
      //profile name
      String profileName = fixture.getCurrentProfile();
      byte[] profileArr = profileName.getBytes();
      int profileLen = profileArr.length;
      
      int namesLen = fixtNameLen + locLen + areaLen + profileLen + 8;
	...
	areaLen = areaLen - 256;
      ...
      //profile name
      if(profileLen > 127) {
      	profileLen = profileLen - 256;
      }
      namesArr[fillPos++] = (byte)profileLen;
      System.arraycopy(profileArr, 0, namesArr, fillPos, profileLen);
      fillPos += profileLen;
      namesArr[fillPos++] = 0;
    ...
    public void processBacnetPacket(byte[] bacnetPkt) {
        ...
        try {
            ...
            if(logger.isInfoEnabled()) {
                ...
                responsePkt = new byte[BACNET_HEADER_LEN + 2 + (noOfNodesReq * 37)];
            ...
            if(logger.isInfoEnabled()) {
                ...
                if (dimResult != BACNET_RES_DEVICE_NOT_FOUND) {
                    ...
                    //dim type is coming as 0. so, it is always absolute
            ...
            if(logger.isInfoEnabled()) {
            	logger.info("bacnet su details packet(id) - " + ServerUtil.getLogPacket(bacnetPkt));
            }
            	noOfNodesReq = ServerUtil.extractShortFromByteArray(bacnetPkt, BACNET_DATA_POS);
            	addrPos = BACNET_DATA_POS + 2;   	
            	...
            	responsePkt = new byte[BACNET_HEADER_LEN + 2 + 37 + namesLen];            	
            	fillPos = BACNET_DATA_POS;
            	// add no. of devices to the response packet
            	ServerUtil.fillShortInByteArray(1, responsePkt, fillPos);
            	fillPos += 2;   
            	...
            	sendBacnetPacket(responsePkt);
            	break;
            case BACNET_MSGTYPE_ASSIGN_SU_PROFILE:
            	if(logger.isInfoEnabled()) {
                logger.info("bacnet assign profile - " + ServerUtil.getLogPacket(bacnetPkt));
            	}
            	// assumption here is at a time bacnet can send only one device and that to blocking
            	pktPos = BACNET_DATA_POS + 2;
            	nodeId = ServerUtil.extractIntAsLongFromByteArray(bacnetPkt, pktPos);
            	pktPos += 4;
              int profileGrpId =  ServerUtil.extractShortFromByteArray(bacnetPkt, pktPos);                
              pktPos += 2;
              byte profileSched = bacnetPkt[pktPos++]; //1 auto 2 immediate
              fixture = fixtureMgr.getFixtureById(nodeId);
              result = BACNET_RES_SUCCESS;
              byte profileResult = BACNET_RES_SUCCESS;
              if (fixture == null) {
              	result = BACNET_RES_NO_DEVICES;
              	profileResult = BACNET_RES_DEVICE_NOT_FOUND;
              }
              responsePkt = new byte[BACNET_HEADER_LEN + 2 + 5];
              fillBacnetHeader(responsePkt, (byte) BACNET_MSGTYPE_ASSIGN_SU_PROFILE, result);
              fillPos = BACNET_DATA_POS;
              ServerUtil.fillShortInByteArray(1, responsePkt, fillPos);
              fillPos += 2;
              // result
              if (profileResult != BACNET_RES_DEVICE_NOT_FOUND) {
              	Groups groups = groupMgr.getGroupById((long)profileGrpId);
              	if(profileSched == 1) {
              		fixtureMgr.changeFixtureProfile(nodeId, (long)profileGrpId, groups.getName(), fixture.getCurrentProfile());
              	} else {
              		fixtureMgr.pushProfileToFixtureNow(nodeId, profileGrpId);
              	}
              }
              responsePkt[fillPos++] = profileResult;
              // id 4 bytes
              ServerUtil.fillLongAsIntInByteArray(nodeId, responsePkt, fillPos);
              fillPos += 4;
              sendBacnetPacket(responsePkt);
            	break;
            case BACNET_MSGTYPE_PROFILE_DIM:
              if(logger.isInfoEnabled()) {
                  logger.info("bacnet profile dim packet  - " + ServerUtil.getLogPacket(bacnetPkt));
              }
              pktPos = BACNET_DATA_POS;
              profileGrpId = ServerUtil.extractShortFromByteArray(bacnetPkt, pktPos);
              pktPos += 2;
              dimType = bacnetPkt[pktPos++];
              percentage = ServerUtil.extractShortFromByteArray(bacnetPkt, pktPos);
              //if it is a negative value. convert it to -ve percentage
              percentage = percentage & 0xff;    
              pktPos += 2;
              long duration = ServerUtil.extractIntAsLongFromByteArray(bacnetPkt, pktPos);
              pktPos += 4;
              result = BACNET_RES_SUCCESS;
              responsePkt = new byte[BACNET_HEADER_LEN + 2 + 5];
              fillBacnetHeader(responsePkt, (byte) BACNET_MSGTYPE_PROFILE_DIM, result);
              fillPos = BACNET_DATA_POS;
              ServerUtil.fillShortInByteArray(1, responsePkt, fillPos);
              fillPos += 2;
              // result             
              DeviceServiceImpl.getInstance().dimFixturesByGroup(profileGrpId, (byte)percentage, (int)duration, dimType);              
              sendBacnetPacket(responsePkt);
              break;
            case BACNET_MSGTYPE_ZONE_DISCOVER:
      				OccBacnetService.getInstance().processBacnetZoneDiscover(bacnetPkt);
              break;
      			case BACNET_MSGTYPE_ZONE_DATA:
      				OccBacnetService.getInstance().processBacnetZoneData(bacnetPkt);
              break;  
      			case BACNET_MSGTYPE_ZONE_STATUS:
      				OccBacnetService.getInstance().processBacnetZoneStatus(bacnetPkt);          
              break; 
      			case BACNET_MSGTYPE_BMS_CLIENT_INFO:
      				result = BACNET_RES_SUCCESS;
      				OccBacnetService.getInstance().processBmsClientInfo(bacnetPkt);
      				responsePkt = new byte[BACNET_HEADER_LEN + 2 + 5];
	              fillBacnetHeader(responsePkt, (byte) BACNET_MSGTYPE_BMS_CLIENT_INFO, result);
	              fillPos = BACNET_DATA_POS;
	              ServerUtil.fillShortInByteArray(1, responsePkt, fillPos);
	              fillPos += 2;                       
	              sendBacnetPacket(responsePkt);
      				break;
      			case BACNET_MSGTYPE_SET_ZONE_DIM:
      				if(logger.isInfoEnabled()) {
                        logger.info("bacnet profile dim packet  - " + ServerUtil.getLogPacket(bacnetPkt));
                    }
                    pktPos = BACNET_DATA_POS;
                    long zoneId = ServerUtil.extractIntAsLongFromByteArray(bacnetPkt, pktPos);
                    pktPos += 4;
                    dimType = bacnetPkt[pktPos++];
                    percentage = ServerUtil.extractShortFromByteArray(bacnetPkt, pktPos);
                    //if it is a negative value. convert it to -ve percentage
                    percentage = percentage & 0xff;    
                    pktPos += 2;
                    int time = ServerUtil.extractIntFromByteArray(bacnetPkt, pktPos);
                    pktPos += 4;
                    result = BACNET_RES_SUCCESS;
                    responsePkt = new byte[BACNET_HEADER_LEN + 2 + 5];
                    fillBacnetHeader(responsePkt, (byte) BACNET_MSGTYPE_SET_ZONE_DIM, result);
                    fillPos = BACNET_DATA_POS;
                    ServerUtil.fillShortInByteArray(1, responsePkt, fillPos);
                    fillPos += 2;
                    // result
                    OccBacnetService.getInstance().absoluteDimZone(percentage, zoneId, time);
                    sendBacnetPacket(responsePkt);
      				break;
            default:
	            if(logger.isInfoEnabled()) {
    ...
    private void sendGwCommissionedSensorsDetailsResponse(long gwId) {
      ...
      byte[] responsePkt = new byte[BACNET_HEADER_LEN + 2 + (noOfDevices * 37)];
      int fillPos = BACNET_DATA_POS;
      // add no. of devices to the response packet
    ...
    public void sendBacnetPacket(byte[] pkt) {
      ...
      if(logger.isInfoEnabled()) {
        logger.info("bacnet response packet(" + bacnetServerIp + ") - " + ServerUtil.getLogPacket(pkt));
      }
        ...
        try {
            ...
            gwSockConn.send(new DatagramPacket(pkt, 0, pkt.length, new InetSocketAddress(bacnetServerIp, port)));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (gwSockConn != null) {
    ...
    public class BacnetListenerDaemon extends Thread {
        ...
        public void run() {
          ...
          if(logger.isInfoEnabled()) {
            ...
            try {
                serverSocket = new DatagramSocket(BACNET_GEMS_LISTENER_PORT);
                ...
                while (ServerMain.getInstance().isRunning()) {
                    try {
                        ...
                        bacnetServerIp = dp.getAddress().getHostAddress();
                        if(logger.isInfoEnabled()) {
                          logger.info("bacnet packet(" + bacnetServerIp + ") - " + ServerUtil.getLogPacket(rcvByteArr));
                        }
