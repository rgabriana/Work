...
import com.ems.vo.RunningJobStatus;
...
//                  getScheduleDate(schedule.getCaptureEnd()) );
    ...
    */
    ...
    try {
    	...
    	byte[] dataPacket = new byte[1];
    	dataPacket[0] = enableDisable;
    	CommandScheduler.getInstance().addCommand(fixture, dataPacket, ServerConstants.SU_CMD_RGL_MSG_TYPE, true, 
    			MULTICAST_INTER_PKT_DELAY);
    } catch (Exception ex) {
        //ex.printStackTrace();
        fixtureLogger.error("error in designateRGL - " + ex.getMessage());
    }
    ...
    try {
    	...
    	CommandScheduler.getInstance().addCommand(fixture, dataPacket, ServerConstants.SU_CMD_CONFIG_HEART_BEAT_MSG_TYPE, true, 
    			MULTICAST_INTER_PKT_DELAY);
    } catch (Exception ex) {
        //ex.printStackTrace();
        fixtureLogger.error("error in configureHeartbeat - " + ex.getMessage());
    }
    ...
    try {
        ...
        //ex.printStackTrace();
        fixtureLogger.error("error in absoluteDimFixture - " + ex.getMessage());
    }
