...
<c:if test="${companySetup != 'true'}">
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			stickerMenuPresent = setInterval(function() {
				getScheduleEventList();
			},30*1000);
		...
		*/
		var stickerMenuPresent = null;
		function updateStickerMenu(data)
		{ 
			// DR NOTIFICATION DISPLAY LOGIC
			if(data!=null && data.drStatus!=null)
			{
				var active = data.drStatus.status;
	        	var totalduration = data.drStatus.timeremaning;

	        	removeStickerMenu();
				
				if(active == "true" && totalduration != 0)
				{
		        	var level = data.drStatus.level;
		        	level = toTitleCase(level);
		        	var type = data.drStatus.type;
					//Convert seconds into Minutes
					var duration = totalduration/60;
					duration = Math.round(duration.toFixed(1));
					...
					if(totalduration != -1)
					{
							...
							if(duration > 0)
							{
								spamNodetext6 = document.createTextNode(duration +" min");
							}
							else 
							{
								spamNodetext6 = document.createTextNode(totalduration +" sec");
							}
