...
*/
	...
	@SuppressWarnings("unchecked")
	public Long lastHourZoneEnergy(Long zone_id) {
		Long energy = 0L;
        String tableName = "energy_consumption";
        Calendar oCalendarEnd = Calendar.getInstance();
        Calendar oCalendarBegin = Calendar.getInstance();
        oCalendarEnd.set(Calendar.MINUTE, 0);
        oCalendarBegin.set(Calendar.MINUTE, 0);
        oCalendarEnd.add(Calendar.HOUR, -1) ;
        try {
            String hsql = "SELECT round(sum(ec.power_used)/12) as totalConsum from " + tableName + " ec where " +
            		" ec.fixture_id in (select d.id from device d , sensor s " +
			  		"where s.zone_id =" + zone_id + " and lower(s.mac) = lower(d.mac_address)) and ec.zero_bucket != 1 and " +
	  				" ec.capture_at <='" + DateUtil.formatDate(oCalendarBegin.getTime(), "yyyy-MM-dd HH:mm:ss") + "' "
					  + "and ec.capture_at >'" + DateUtil.formatDate(oCalendarEnd.getTime(), "yyyy-MM-dd HH:mm:ss") + "'";
            Query q = getSession().createSQLQuery(hsql.toString());
            List<Object> results = q.list();
            if (results != null && !results.isEmpty() && results.get(0) != null) {
	            energy = Long.parseLong(results.get(0).toString());
			}
        } catch (HibernateException hbe) {
            throw SessionFactoryUtils.convertHibernateAccessException(hbe);
        }
