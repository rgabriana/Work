--Kushal: Following removeAllTriggers() call should always be the first statement during upgrade
...
INSERT INTO event_type (id, "type", description) VALUES ((select coalesce(max(id),0)+1 from event_type), 'Fixture Group change', NULL);
...
BEGIN
	FOR rec IN (
	  ...
	  FROM fixture as fxt WHERE group_id =0 order by id)
	LOOP  	  
...
INSERT INTO event_type (id, "type", description, severity, active) VALUES ((select coalesce(max(id),0)+1 from event_type), 'EWS Upgrade', NULL, 5, 1);
...
INSERT INTO event_type (id, type, description, severity, active) VALUES ((select coalesce(max(id),0)+1 from event_type), 'EWS Discovery', NULL, 5, 1);

INSERT INTO event_type (id, type, description, severity, active) VALUES ((select coalesce(max(id),0)+1 from event_type), 'EWS Commissioning', NULL, 5, 1);
...
switch $TG_op {
    INSERT {
        ...
        if { ![string match "qrtz*" $TG_table_name] } {
            ...
            spi_exec "insert into wal_logs (id, creation_time, action, table_name, record_id, sql_statement) values (nextval('wal_logs_seq'), current_timestamp, 'INSERT', '$TG_table_name' , null, E'$statement' )"
        } 
    }
    ...
    UPDATE {
        ...
        if { ![string match "qrtz*" $TG_table_name] } {
            append statement "id = " "$NEW(id)"
        } else {
            set isFirst 0
            foreach field $TG_relatts {
                if { [string length $field] != 0 } {
                    set field_val [lindex [array get OLD $field] 1]
                    if { [string length $field_val] != 0 } {
                        if { $isFirst == 0 } {
                            set isFirst 1
                        } else {
        ...
        if { ![string match "qrtz*" $TG_table_name] } {
            ...
            spi_exec "insert into wal_logs (id, creation_time, action, table_name, record_id, sql_statement) values (nextval('wal_logs_seq'), current_timestamp, 'UPDATE', '$TG_table_name' , null, E'$statement' )"
        }
    ...
    DELETE {
        ...
        append statement "$TG_table_name" " where " 
        if { ![string match "qrtz*" $TG_table_name] } {
            append statement "id = " "$OLD(id)"
        } else {
            set isFirst 0
            foreach field $TG_relatts {
                if { [string length $field] != 0 } {
        ...
        if { ![string match "qrtz*" $TG_table_name] } {
            ...
            spi_exec "insert into wal_logs (id, creation_time, action, table_name, record_id, sql_statement) values (nextval('wal_logs_seq'), current_timestamp, 'DELETE', '$TG_table_name' , null, E'$statement' )"
        }
    }
}
return OK
...
BEGIN
  ...
  update ballasts set display_label =  trim(ballast_name) || '(' || trim(manufacturer) || ',' || trim(lamp_type) || ',' || wattage || 'W,' || lamp_num || ' bulb)' where lamp_num = 1;
  update ballasts set display_label =  trim(ballast_name) || '(' || trim(manufacturer) || ',' || trim(lamp_type) || ',' || wattage || 'W,' || lamp_num || ' bulbs)' where lamp_num > 1;
...
BEGIN	
    ...
    perform setval('system_energy_consumption_daily_seq', (select max(id)+1 from system_energy_consumption_daily));
    perform setval('system_energy_consumption_hourly_seq', (select max(id)+1 from system_energy_consumption_hourly));
    perform setval('system_energy_consumption_seq', (select max(id)+1 from system_energy_consumption));
...
INSERT INTO system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from system_configuration), 'enable.cloud.communication', '1');
...
BEGIN
    ...
    IF dr_type_column is null then
		...
		INSERT INTO dr_target (id, price_level, pricing, duration, target_reduction, enabled, dr_type, dr_status, jitter, start_after, start_time) VALUES (nextval('dr_target_seq'), 'Low', 0.105, 60, 10, 'No', 'MANUAL', 'Cancelled', 0, 0, current_timestamp);
		INSERT INTO dr_target (id, price_level, pricing, duration, target_reduction, enabled, dr_type, dr_status, jitter, start_after, start_time) VALUES (nextval('dr_target_seq'), 'Moderate', 0.24, 60, 25, 'No', 'MANUAL', 'Cancelled', 0, 0, current_timestamp);
		INSERT INTO dr_target (id, price_level, pricing, duration, target_reduction, enabled, dr_type, dr_status, jitter, start_after, start_time) VALUES (nextval('dr_target_seq'), 'High', 0.36, 60, 50, 'No', 'MANUAL', 'Cancelled', 0, 0, current_timestamp);
		INSERT INTO dr_target (id, price_level, pricing, duration, target_reduction, enabled, dr_type, dr_status, jitter, start_after, start_time) VALUES (nextval('dr_target_seq'), 'Special', 0.48, 60, 40, 'No', 'MANUAL', 'Cancelled', 0, 0, current_timestamp);
	END IF;
END
$$ LANGUAGE plpgsql;
...
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Breakroom_Normal', 'FALSE', '2');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Closed Corridor_AlwaysOn', 'FALSE', '5');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Closed Corridor_Normal', 'FALSE', '5');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Conference Room_Normal', 'FALSE', '3');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Highbay_AlwaysOn', 'FALSE', '16');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Highbay_Normal', 'FALSE', '16');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Open Office_AlwaysOn', 'FALSE', '9');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Open Office_Dim', 'FALSE', '9');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Open Office_Normal', 'FALSE', '9');
INSERT INTO canned_profiles_configuration (id, name, status, parentprofileid) VALUES (nextval('cannedprofile_configuration_seq'), 'Private Office_Normal', 'FALSE', '10');
...
(
	...
	occ_status integer DEFAULT 0,
	avg_temperature numeric,
	avg_ambient_light integer,     	
        CONSTRAINT sensor_key_pk PRIMARY KEY (id),
	CONSTRAINT unique_sensor UNIQUE(mac)
);

ALTER TABLE sensor OWNER TO postgres;

--
-- Name: sensor_seq; Type: SEQUENCE; Owner: postgres
--
...
(
	...
	last_status_update_time timestamp,
        ...
        heartbeat smallint,
        avg_ambient_light integer,
	setback_start_time timestamp,
	setback_end_time timestamp,
	bms_client_last_hb_time timestamp,
        last_bms_client_setback smallint,
	last_reliable_time timestamp,
	last_restored_time timestamp,
	vacant_since bigint,
        CONSTRAINT zone_key_pk PRIMARY KEY (id)
);

ALTER TABLE zone OWNER TO postgres;

--
-- Name: zone_seq; Type: SEQUENCE; Owner: postgres
--
...
ALTER TABLE zone_seq OWNER TO postgres;
...
(
	...
	setback_end_time timestamp,
	vacant_since bigint,
     	CONSTRAINT zone_status_key_pk PRIMARY KEY (id),
	CONSTRAINT unique_zone_status UNIQUE(zone_id, capture_time)
);

ALTER TABLE zone_status OWNER TO postgres;

--
-- Name: zone_status_seq; Type: SEQUENCE; Owner: postgres
--

CREATE SEQUENCE zone_status_seq
  INCREMENT 1
  MINVALUE 1
  NO MAXVALUE
  START 1
...
ALTER TABLE zone_status_seq OWNER TO postgres;

DROP TYPE zone_status_record;

CREATE TYPE zone_status_record AS (
  zone bigint,
  r_occ_status integer,
  min_temp numeric,
  max_temp numeric,
  avg_temp numeric,
  avg_amb numeric,
  min_status_time timestamp,
  no_of_sensors integer
);

CREATE OR REPLACE FUNCTION occ_status(hbTolerance integer, unOccPeriod integer, startTime timestamp, endTime timestamp, hbTime timestamp, hbFailedSensorPerc integer) RETURNS VOID
AS $$
DECLARE
  l_occ_status integer;
  l_fan_speed integer;
  l_setback integer;
  l_heartbeat integer;
  l_last_update_time timestamp;
  rec zone_status_record;
  l_failed_sensors integer;
  l_vacant_since integer;
BEGIN

  FOR rec IN (
    SELECT zone_id AS zone, MIN(occ_status) AS r_occ_status, MIN(avg_temperature) AS min_temp, MAX(avg_temperature) AS max_temp, AVG(avg_temperature) AS avg_temp, AVG(avg_ambient_light) AS avg_amb, min(last_status_time) AS min_status_time, COUNT(*) AS no_of_sensors 
    FROM sensor GROUP BY zone_id)
  LOOP

    l_heartbeat = 1;
    l_vacant_since = rec.r_occ_status;
    IF (rec.min_status_time < hbTime)
    THEN
	--there is at least one sensor which is not communicating. check for the percentages of sensors not communicating
	IF (hbFailedSensorPerc > 0)
	THEN
	  SELECT COUNT(*) INTO l_failed_sensors FROM sensor WHERE last_status_time < hbTime;
	  IF (l_failed_sensors * 100 / rec.no_of_sensors >= hbFailedSensorPerc)
	  THEN
	    l_heartbeat = 0;
      	    l_occ_status = -1;
	    l_vacant_since = 0;
    ...
    IF (l_heartbeat = 1)
    THEN
      IF (rec.r_occ_status > unOccPeriod)
      THEN
        l_occ_status = 0;
      ELSE
        l_occ_status = 1;
    ...
    l_last_update_time = now();

    UPDATE zone SET last_status_update_time = l_last_update_time, occ_status = l_occ_status, setback = l_setback, min_temperature = rec.min_temp, max_temperature = rec.max_temp, avg_temperature = rec.avg_temp, fan_speed = l_fan_speed, heartbeat = l_heartbeat, avg_ambient_light = rec.avg_amb, setback_start_time = startTime, setback_end_time = endTime, vacant_since = l_vacant_since WHERE id = rec.zone;

    INSERT INTO zone_status (id, zone_id, capture_time, occ_status, setback, min_temperature, max_temperature, avg_temperature, fan_speed, heartbeat, avg_ambient_light, setback_start_time, setback_end_time, vacant_since) VALUES (nextval('zone_status_seq'), rec.zone, l_last_update_time, l_occ_status, l_setback, rec.min_temp, rec.max_temp, rec.avg_temp, l_fan_speed, l_heartbeat, rec.avg_amb, startTime, endTime, l_vacant_since);
...
CREATE TABLE sensor_rgl
(
	id bigint NOT NULL,
	mac character varying,
	zone_id bigint,
	CONSTRAINT sensor_rgl_key_pk PRIMARY KEY (id),
	CONSTRAINT unique_sensor_rgl UNIQUE(mac)
);

ALTER TABLE sensor_rgl OWNER TO postgres;

--
-- Name: sensor_rgl_seq; Type: SEQUENCE; Owner: postgres
--

CREATE SEQUENCE sensor_rgl_seq
  INCREMENT 1
  MINVALUE 1
  NO MAXVALUE
  START 1
...
ALTER TABLE sensor_rgl_seq OWNER TO postgres;

CREATE TABLE occ_system_configuration (
  id bigint NOT NULL,
  name character varying,
  value character varying, 
  CONSTRAINT occ_system_configuration_pk PRIMARY KEY (id),
  CONSTRAINT unique_occ_system_configuration_name UNIQUE (name) 
);

ALTER TABLE occ_system_configuration OWNER TO postgres;

CREATE SEQUENCE occ_system_configuration_seq
  INCREMENT 1
  MINVALUE 1
  NO MAXVALUE
  START 1
...
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.hbEnlFailureTolerance', '10');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.hbBmsFailureTolerance', '10');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.unOccPeriod', '900');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.occEngineLoopTime', '1');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.endTimeTolerance', '900');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.rglRefreshTime', '60');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'prune.zoneStatus', '7');
INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.hbFailedSensors', '0');
...
(
  ...
  avg_temperature numeric,
  avg_ambient_light integer,
  CONSTRAINT sensor_history_key_pk PRIMARY KEY (mac, capture_time)
);

CREATE OR REPLACE FUNCTION add_sensor_history() RETURNS "trigger" AS $$
  BEGIN
    IF tg_op = 'UPDATE' THEN
      INSERT INTO sensor_history VALUES (new.mac, now(), new.occ_status, new.avg_temperature, new.avg_ambient_light); 
    END IF;
    RETURN new;
  END
$$ LANGUAGE plpgsql;

CREATE TRIGGER update_sensor_change AFTER UPDATE ON sensor
  FOR EACH ROW EXECUTE PROCEDURE add_sensor_history();

INSERT INTO occ_system_configuration (id, name, value) values ((select coalesce(max(id),0)+1 from occ_system_configuration), 'occ.enableEngine', 'true');