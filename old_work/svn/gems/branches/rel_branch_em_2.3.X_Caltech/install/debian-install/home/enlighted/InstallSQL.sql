...
INSERT INTO event_type (id, "type", description) VALUES ((select coalesce(max(id),0)+1 from event_type), 'Wireless Params', NULL);
...
-- not inserting to zone_status
--    INSERT INTO zone_status (id, zone_id, capture_time, occ_status, setback, min_temperature, max_temperature, avg_temperature, fan_speed, heartbeat, avg_ambient_light, setback_start_time, setback_end_time) VALUES (nextval('zone_status_seq'), rec.zone, l_last_update_time, l_occ_status, l_setback, rec.min_temp, rec.max_temp, rec.avg_temp, l_fan_speed, l_heartbeat, rec.avg_amb, startTime, endTime);
