...
/usr/bin/pg_dump -i -U $PGUSER -h $PGHOST -F c -a -t energy_consumption -t energy_consumption_hourly -t energy_consumption_daily -t plugload_energy_consumption -t plugload_energy_consumption_hourly -t plugload_energy_consumption_daily -f "temp.backup" $PGDATABASE
...
/usr/bin/pg_dump -i -U $PGUSER -h $PGHOST -F c -s -t energy_consumption -t energy_consumption_hourly -t energy_consumption_daily -t plugload_energy_consumption -t plugload_energy_consumption_hourly -t plugload_energy_consumption_daily -f  "$EC_DAILY_SCHEMA_BACKUP_FILE" $PGDATABASE
/usr/bin/pg_dump -i -U $PGUSER -h $PGHOST -b -F c -T energy_consumption -T energy_consumption_hourly -T energy_consumption_daily -T plugload_energy_consumption -T plugload_energy_consumption_hourly -T plugload_energy_consumption_daily -f  "${DAILY_BACKUP_FILE}" $PGDATABASE


if [ $? -eq 0 ]
then
	###First take the configuration back up
	echo "*****Starting Configuration Files backup**************" >>$LOG_DIR/dailybackup.log
	sh $ENLIGHTED_HOME/scripts/dailyconfigbackup.sh >> $LOG_DIR/dailybackup.log
	echo "*****ENDS Configuration Files backup**************" >>$LOG_DIR/dailybackup.log
    ...
    then
        cp $manifestfile ./
        tarFiles=`echo "$DAILY_BACKUP_FILE $EC_DAILY_BACKUP_FILE MANIFEST.MF $EC_DAILY_SCHEMA_BACKUP_FILE"`
    else
    	tarFiles=`echo "$DAILY_BACKUP_FILE $EC_DAILY_BACKUP_FILE $EC_DAILY_SCHEMA_BACKUP_FILE"`
	fi
...
BACKUP_OPTION_SELECTED=$(/usr/bin/psql -x -U $PGUSER $PGDATABASE -h $PGHOST -c "select value from system_configuration where name ='backup.option.selected'" | grep value | cut -d " " -f3)
BACKUP_USB_PATH=$(/usr/bin/psql -x -U $PGUSER $PGDATABASE -h $PGHOST -c "select value from system_configuration where name ='backup.usb.path'" | grep value | cut -d " " -f3)

#echo " ***** backupoption selected ${BACKUP_OPTION_SELECTED}" >>$LOG_DIR/dailybackup.log
#echo " ******** backub path ${BACKUP_USB_PATH} *************" >>$LOG_DIR/dailybackup.log

if [ "${BACKUP_OPTION_SELECTED}" == "${BACKUP_OPTION_USB}" ]
then
    echo "***** usb option selected **********" >>$LOG_DIR/dailybackup.log
    ...
    then
		...
		echo "*** USB not found at the path ${BACKUP_USB_PATH} ********" >>$LOG_DIR/dailybackup.log
		USBMEDIA=""
    fi
	...
	then
	    ...
	    $(/usr/bin/psql -x -U postgres ems -h localhost -c "insert into events_and_fault(id, event_time,severity, event_type, description,active) values (nextval('events_seq'), current_timestamp, 'Info', 'Attached Storage Backup', 'Daily backup to Attached Storage is successful.', true)")
	else
	    ...
	    echo "*****copied ${backupprefix}${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}_$today.tar.gz onto server directory ${BACKUP_DIR}" >>$LOG_DIR/dailybackup.log
	    $(/usr/bin/psql -x -U postgres ems -h localhost -c "insert into events_and_fault(id, event_time,severity, event_type, description,active) values (nextval('events_seq'), current_timestamp, 'Critical', 'Attached Storage Backup', 'Daily backup to Attached Storage failed.Please check Attached Storage Settings.Backup file is saved on EM server', true)")
	fi
else
	echo "***** sftp option selected **********" >>$LOG_DIR/dailybackup.log
	backups_no=$(ls -l *$DAILY_BACKUP_PREFIX*.tar.gz | wc -l )
    while [ "${backups_no}" -gt 4 ]
    do
        REMOVE_FILE=$(ls -ltr --time-style="+%Y-%m-%d %H:%M:%S" "$BACKUP_DIR" | grep .*$DAILY_BACKUP_PREFIX.*tar.gz | head -n 1 | tr -s ' ' ' ' | cut -d" " -f8)
	...
	then
		echo "******* SFTP remote backup start *******" >>$LOG_DIR/dailybackup.log
		#lftp sftp://${backupSftpUsername}:${backupSftpPassword}@${backupSftpIp}:${backupSftpDirectory}  -e "put ${SFTP_BACKUP_FILE_NAME}; bye"
		SFTP_BACKUP_FILE_NAME=$BACKUP_DIR/"${backupprefix}${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}_$today.tar.gz"
		echo "uploading backup file ${SFTP_BACKUP_FILE_NAME} onto sftp server" >>$LOG_DIR/dailybackup.log
		Sftpdailyuploadcommandoutput=$(lftp sftp://${backupSftpUsername}:${backupSftpPassword}@${backupSftpIp}  -e "set net:max-retries 2;set net:persist-retries 2;set net:reconnect-interval-base 2;set net:reconnect-interval-max 2;set net:reconnect-interval-multiplier 2;set net:timeout 2;set ftp:list-options -a; cd ${backupSftpDirectory}; put ${SFTP_BACKUP_FILE_NAME}; bye" 2>&1)
		#echo "**** ${Sftpdailyuploadcommandoutput} *******" >>$LOG_DIR/dailybackup.log
		if [[ ( "$Sftpdailyuploadcommandoutput" =~ "failed" ) || ( "$Sftpdailyuploadcommandoutput" =~ "error" ) ]]
	    then
	    	echo "******* Sftp remote backup failed.Please check Sftp server settings. Backup file is saved on EM server *******" >>$LOG_DIR/dailybackup.log
	    	$(/usr/bin/psql -x -U postgres ems -h localhost -c "insert into events_and_fault(id, event_time,severity, event_type, description,active) values (nextval('events_seq'), current_timestamp, 'Critical', 'Sftp Remote Backup', 'Daily backup to Sftp remote server failed.Please check Sftp server settings. Backup file is saved on EM server', true)")
		else
			echo "******* Sftp remote backup successful *******" >>$LOG_DIR/dailybackup.log
			$(/usr/bin/psql -x -U postgres ems -h localhost -c "insert into events_and_fault(id, event_time,severity, event_type, description,active) values (nextval('events_seq'), current_timestamp, 'Info', 'Sftp Remote Backup', 'Daily backup to Sftp remote server is successful', true)")
	    fi
	else
		echo "****** SFTP backup failed. Please check the sftp server settings. Backup file is saved on EM server******" >>$LOG_DIR/dailybackup.log
		$(/usr/bin/psql -x -U postgres ems -h localhost -c "insert into events_and_fault(id, event_time,severity, event_type, description,active) values (nextval('events_seq'), current_timestamp, 'Critical', 'Sftp Remote Backup', 'Daily backup to Sftp remote server failed.Please check Sftp server settings. Backup file is saved on EM server', true)")
	fi
	
	#sh $OPT_ENLIGHTED/DB/sftpbackup.sh $BACKUP_DIR/"${backupprefix}${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}_$today.tar.gz" >>$LOG_DIR/dailybackup.log
fi
