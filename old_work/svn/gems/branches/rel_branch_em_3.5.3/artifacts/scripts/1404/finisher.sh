...
sed -i s/5432/5433/g /etc/postgresql/9.4/main/postgresql.conf
echo "bytea_output = escape" >> /etc/postgresql/9.4/main/postgresql.conf
sed -i s/INTERFACES=\"\"/INTERFACES=\"eth1\"/g /etc/default/isc-dhcp-server
sed -i '4i\/opt\/enLighted\/tmp\/firstrun.sh' /etc/rc.local
echo "localhost:5433:*.postgres:postgres" > /home/enlighted/.pgpass
sudo chmod 600 /home/enlighted/.pgpass
