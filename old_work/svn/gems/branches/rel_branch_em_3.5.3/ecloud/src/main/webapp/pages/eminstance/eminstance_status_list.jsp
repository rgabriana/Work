...
<spring:url value="/services/org/emtaskmanagement/loadEmTasks/" var="getEmTaskList" scope="request" />
...
function start(pageNum, orderWay) {
		jQuery("#emTasksTable").jqGrid({
			...
			colNames: ["id", "EM Id", "Task Code", "Task Status","Progress Status","priority","Start Time","Offset Time","No.of attempts"],
		       ...
		       { name: "startTime", index: 'startTime', formatter:billinglistcustomdateformatter, sortable:false,width:'12%' },
		       { name: "offsetTime", index: 'offsetTime',formatter:billinglistcustomdateformatter, sortable:false,width:'12%'},
...
function billinglistcustomdateformatter(cellvalue, options, rowObject)
	{
		if ( cellvalue == undefined || cellvalue == null || $.trim(cellvalue) == ''){
			return '';
		}
		  var date = new Date(cellvalue);
		  options = $.extend({}, $.jgrid.formatter.date, options);
          return $.fmatter.util.DateFormat("", date, 'n/j/Y g:i A', options);
	}
