...
function delayResponseClientSide(){
	...
	$(document).ready(function() {
		...
		$(".errorMsg").css("height","0px");
		$(".commMsg").css("height","0px");
		var errCommHeight = 25;
		...
		<c:if test="${param.resetSuccess == 'true'}">
			$(".commMsg").css("height",errCommHeight+"px");
		</c:if>
		<c:if test="${param.resetFail == 'true'}">
			$(".commMsg").css("height",errCommHeight+"px");
		</c:if>
		
		$(".submitAction").css("marginTop",20);
		$(".submitAction").css("marginRight",10);
		if( ( $(".commMsg").height() > 0) || ($(".errorMsg").height() > 0 )){
			//$(".loginForm").css("width",500);
			//$(".submitAction").css("marginTop",30 + $(".errorMsg").height()+$(".commMsg").height());
			//$(".submitAction").css("marginLeft", -(500-350) - $(".submitAction").width() - 20 );
		}else{
			//$(".loginForm").css("width",350);
		}
...
<body>
	...
	<div class="login">
		...
		<div class="loginForm">
			...
			<c:if test="${param.error == 'true'}">
				<script>$('#error').html('<spring:message code="error.login.incorrect.general"/>');</script>
			</c:if>
			<c:if test="${param.sessionExpire == 'true'}">
