   ...
   */
	...
	public byte[] getInitialResponse() {
		...
		try {
			while (true) {
				...
				// sslLogger.info("received key - " +
				// ServerUtil.getLogPacket(rcvdByteArr));
				return rcvdByteArr;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void listenForPackets() {
		...
		new Thread("SSL Listener " + gwIp) {
			...
			public void run() {
				sslLogger.info(gwIp + " - " + getId() + " - " + getName() + " started...");
				...
				while (listenForPackets) {
					...
					try {
						...
						if (len == 0) {
							...
							// sleep for 10 milli seconds to avoid busy loop in
							// case socket is returning immediately
							ServerUtil.sleepMilli(10);
							continue;
						} else if (len == -1) {
						...
						sslLogger.error(gwIp + "(listenForPackets - ste): "
								+ ste.getMessage());
						listenForPackets = false;
					} catch (SocketException se) {
						sslLogger.error(gwIp + "(listenForPackets - se): "
								+ se.getMessage());
						listenForPackets = false;
					} catch (Exception e) {
						sslLogger.error(gwIp + "(listenForPackets - e): "
								+ e.getMessage());
						listenForPackets = false;
					} finally {
						if (!listenForPackets) {
							sslLogger.info(gwIp + " - " + getId() + " - " + getName() + " (" + listenForPackets + ") terminating...");
							terminateConnection();
							try {
			...
			if (rcvdHeaderLen == 4) {
				...
				if (currLen == expLen) {
					...
					if (pktLogger.isInfoEnabled()) {
						pktLogger.info(gwId + " " + gwIp
								+ ": received command - "
								+ ServerUtil.getLogPacket(rcvdPkt));
					}
					if (sslLogger.isInfoEnabled()) {
						sslLogger.info(gwId + " " + gwIp
								+ ": received command - "
								+ ServerUtil.getLogPacket(rcvdPkt));
					}
					if (sslLogger.isDebugEnabled()) {
						sslLogger.debug(gwIp + ": received " + expLen
								+ " bytes");
					}
					ZigbeeDeviceImpl.getInstance().getDeviceListener()
							.addGatewayResponse(rcvdPkt, gwIp, poolPkt);
					rcvdHeaderLen = 0;
					expLen = 0;
					currLen = 0;
				}
				...
				case 3:
					...
					expLen |= (c & 0xFF) - 4;
					if (sslLogger.isDebugEnabled()) {
						sslLogger.debug("expected length of the packet -- "
								+ expLen);
					}
					rcvdHeaderLen = 4;
					currLen = 0;
	...
	public void sendCommand(byte[] packet) throws Exception {
		if (pktLogger.isInfoEnabled()) {
			pktLogger.info(gwId + " " + gwIp + ": sending command - "
					+ ServerUtil.getLogPacket(packet));
		}
		if (sslLogger.isInfoEnabled()) {
			sslLogger.info(gwId + " " + gwIp + " " + sslSock.getLocalPort() + ": sending command - "
					+ ServerUtil.getLogPacket(packet));
		}
		try {
