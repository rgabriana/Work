...
plugloadMap = {}; //Map having gatewayid as key and no. of plugloads as value
...
$(document).ready(function() {
	...
	<c:forEach var="entry" items="${plugloadMap}" varStatus="status">
	plugloadMap["${entry.key}"] = "${entry.value}";
    </c:forEach>
...
function setGatewayData(){
	var gw_cnt = 0;
	$("#plgatewayCombo").empty();
	<c:forEach items="${gateways}" var="gateway">
		$('#plgatewayCombo').append($('<option></option>').val("${gateway.id}").html("${gateway.gatewayName} [${plugloadMap[gateway.id]}]"));
		gw_cnt++;
	</c:forEach>
...
function startValidation(){
	...
	selectedGateway = $("#plgatewayCombo").val();;
	...
	var count = GetNoOfPlugloadInGateway(selectedGateway);
	if(count == 0){						
		displayPlgInitialMessage("Gateway does not have any plugload assosciated with it.", COLOR_FAILURE);
		return;
	}
	
	startCommissionPlugload(selectType, selectedGateway);
}
function GetNoOfPlugloadInGateway(gatewayId){
	return plugloadMap[gatewayId];
}
...
<div id="pc-identify-wrapper" style="height:100%; width:100%;">
	<div class="fcsi-north">
		<textarea id="details-message-plugload">Select the Plugload you want to place from the list on the left side of the window, then right-click on the floorplan where you want to place the Plugload and select 'Commission and Place' in the menu that pops up.</textarea>
	</div>
<div class="fcsi-body">
	<table id="plugloadCommisionTable" style="width: 100%">
		<tr <c:if test="${isBulkCommission == false}"> style="visibility:hidden;"</c:if>>
			...
			<td><select class="text" id="plgatewayCombo" name="gatewayId" style="width: 200px;"> </select><br/>
