...
function onDeviceReady() {
	        	   ...
	        	   $(xml).find("revisionString").each(function()
	                                              {
	                                                //console.log($(this).text());
	        		   								completeVersionString = $(this).text();
                                                  });
	           },
	           error : function(){
	...
	if(window.localStorage.getItem("usernamekey") == null && window.localStorage.getItem("pwordkey") == null){
	...
	if ( window.localStorage.getItem("usernamekey") != "" &&  window.localStorage.getItem("pwordkey") != "" ){
		...
		if (un == "") {
			...
			if (pw == "") {
				...
				function(result) {
					...
					//console.log("loginResponceString "+result);
					if (loginResponceString == "101"
						...
						$(loginResponceXML).find("loginResponse").each(
							...
							// depending on version we start calling respective function
												//console.log("Response"+$(loginResponceXML));
				...
				function(error) {
					...
					$('#errorMessage').text("Please check server details");
					$("[type='submit']").button('enable');
