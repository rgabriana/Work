...
<spring:url value="/devices/wds/wds_form.ems" var="wdsDetailsUrl" scope="request" />
<spring:url value="/facilities/organization/installdevices.ems" var="installDevices"/>
<spring:url value="/events/list.ems" var="loadEvents"/>
<spring:url value="/facilities/profiletree.ems" var="profileTree" scope="request" />
...
var showSettings =function(){	
	...
	if(accordianSelect=='facility') {
		if (treenodetype == 'floor'){
			if(isFloorMapped == true){
				$("#ec").css('display','block');
				$("#ec").show();
				$("#lifp").css('display','block');
				$("#lifp").show();
				$("#liin").css('display','block');
				$("#liin").show();
		 		$("#install").show();
		 		$("#lief").css('display','block');
				$("#lief").show();
			}else{
				$("#liin").hide();
		 		$("#lief").hide();
			...
			$("#liin").css('display','none');
			$("#lief").css('display','none');
	...
	if (tabselected == 'install') {
		if (treenodetype == 'floor' && isFloorMapped == true){
			$("#lifp").css('display','block');
		}
		else{
	...
	else if (tabselected == 'settings' && isFloorMapped == true) {
		if (treenodetype == 'floor'){
			$("#lifp").css('display','block');
		}
		else{
	...
	else if (tabselected == 'energy consumption' && isFloorMapped == true) {
		if (treenodetype == 'floor'){
			$("#lifp").css('display','block');
		}
		else{
	...
	else if (tabselected == 'events' && isFloorMapped == true) {
		if (treenodetype == 'floor'){
			$("#lifp").css('display','block');
		}
		else{
...
function loadEvents() {
	removeWheelEvent();
	tabselected = 'events';
	$.cookie('uem_facilities_tab_selected', 'events',  {path: '/' });
    var ifr;
...
function refreshProfileTree() {
    ...
    var customerId = "${customerId}";
	url = "${profileTree}";
	url +="?customerId="+customerId+"&ts="+ new Date().getTime();
		$.ajax({
		        type: "GET",
		        cache: false,
		        async: false,
...
function showWdsDetailsForm(wdsId,pid) {
	var width;
	var height;
	var pageUrl;

	pageUrl = "${wdsDetailsUrl}";
	width= Math.floor($('body').width() * .45);
	height=310;
	
	
	$("#wdsFormDialog").load(pageUrl+"?wdsId="+wdsId+"&pid="+pid+"&ts="+new Date().getTime(), function() {
		$("#wdsFormDialog").dialog({
			modal:true,
			title: 'ERC Details',
			width :width,
			height : height,
