...
public class InventoryReportDao {
    ...
    public List<Object[]> getBallastCountByBallastName(String dbName, String replicaServerHost)
    {
        ...
        try {
            ...
            {
                logger.debug("getBallastCountByBallastName() returned null while fetching aggregated Count of data of EMInstance :" + replicaServerHost + "from DB :"+dbName);
            }
            return ballastList;  
        } catch (Exception e) {
            e.printStackTrace();
	...
	public List<Object[]> getBallastCountByBallastDisplayName(String dbName,
		...
		try {
			...
			if (ballastList == null && ballastList.size() == 0) {
				logger.debug("getBallastCountByBallastDisplayName() returned null while fetching aggregated Count of data of EMInstance :"
    ...
    public List<Object[]> getBulbsCountByBulbName(String dbName, String replicaServerHost)
    {
        ...
        try {
            ...
            {
                logger.debug("getBulbsCountByBulbName() returned null while fetching aggregated Count of data of EMInstance :" + replicaServerHost + "from DB :"+dbName);
            }
            return bulbList;  
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Object[]> getCommissionedFxTypeCount(String dbName, String replicaServerHost)
    {
        ...
        try {
            ...
            {
                logger.debug("getCommissionedFxTypeCount() returned null while fetching aggregated Count of data of EMInstance :" + replicaServerHost + "from DB :"+dbName);
            }
            return fxTypeList;  
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Object[]> getOtherDevicesCount(String dbName, String replicaServerHost)
    {
        Connection connection = null;
        Statement stmt = null;
        ResultSet rs = null;
        ...
        try {
            connection = getDbConnection(dbName, replicaServerHost);
            stmt = connection.createStatement();
            String query = "select ld.locator_device_type as deviceType, count(ld.id) as unmanagedEmergencyFixture FROM locator_device ld WHERE ld.locator_device_type ='Unmanaged_emergency_fixture' or ld.locator_device_type ='Unmanaged_fixture' group by ld.locator_device_type;";
            rs = stmt.executeQuery(query);
            List<Object[]> oList = new ArrayList<Object[]>();
            while(rs.next()) {
                Object[] oObj = new Object[2];
                oObj[0] = rs.getString("deviceType");
                oObj[1] = rs.getLong("unmanagedEmergencyFixture");
                oList.add(oObj);
            }
            if(oList==null && oList.size()==0)
            {
                logger.debug("getOtherDevicesCount() returned null while fetching aggregated Count of data of EMInstance :" + replicaServerHost + "from DB :"+dbName);
            }
            return oList;  
        } catch (Exception e) {
            e.printStackTrace();
	...
	public List<Object[]> getErcCountByVersionNo(String dbName,
        ...
        try {
            connection = getDbConnection(dbName, replicaServerHost);
            stmt = connection.createStatement();
            String query = "select distinct d.version as versionNo, count(d.id) as ercCount from device d join wds w on w.id=d.id where d.type ='WDS' and w.state='COMMISSIONED' group by d.version";
            rs = stmt.executeQuery(query);
            List<Object[]> ercList=new ArrayList<Object[]>();
            while(rs.next()) {
                Object[] ercObj = new Object[2];
                ercObj[0] = rs.getString("versionNo");
                ercObj[1] = rs.getLong("ercCount");
                logger.info("versionNo  : "+ ercObj[1] +"\n FxCount "+ercObj[0]+"\r\n");
                ercList.add(ercObj);
            }
            if(ercList==null && ercList.size()==0)
            {
                logger.debug("getErcCountByVersionNo() returned null while fetching aggregated Count of data of EMInstance :" + replicaServerHost + "from DB :"+dbName);
            }
            return ercList;  
        } catch (Exception e) {
            e.printStackTrace();
	...
	public Long loadAllCommissionedPlugloadsCount(String dbName, String replicaServerHost) {
		Connection connection = null;
        Statement stmt = null;
        ResultSet rs = null;
        if(dbName == null) {
        ...
        try {
            connection = getDbConnection(dbName, replicaServerHost);
            stmt = connection.createStatement();
            String query = "select count(d.id) as plCount from device d join plugload pl on pl.id=d.id where d.type ='Plugload' and pl.state='COMMISSIONED'";
            rs = stmt.executeQuery(query);
            List<Object[]> plList=new ArrayList<Object[]>();
            while(rs.next()) {
                Object[] plObj = new Object[1];
                plObj[0] = rs.getLong("plCount");
                logger.info("Plugload Count "+plObj[0]+"\r\n");
                plList.add(plObj);
            }
            if(plList==null && plList.size()==0)
            {
                logger.debug("loadAllCommissionedPlugloadsCount() returned null while fetching aggregated Count of data of Plugload :" + replicaServerHost + "from DB :"+dbName);
            }
            return (long) plList.size();  
        } catch (Exception e) {
            e.printStackTrace();
        }
        finally {
