...
then
    ...
    then
        ...
        then
            ...
            if [ -f "$BACKUPSCRIPT" ]
            then
            	if [ -f  "/var/www/em_mgmt/adminscripts/generate_backup_prefix.sh" ]
            	then
...
if [ -z "$GOOGLE_INSTALL" ];then
    /home/enlighted/scripts/avahiremoval.sh
else
    /usr/local/google/home/enlighted/scripts/avahiremoval.sh
fi
...
#  apache2 libapache2-mod-wsgi software-properties-common postgresql tomcat6 tomcat6-admin php5 python-django python-setuptools python-django-piston gettext ssh-askpass bind9 
