...
$(document).ready(function() {
	if(parent.accTabSelected == 'pf')
	{
		...
		<c:forEach var="plugloadgroup" items="${plugloadgroups}">
			$("#group").append('<option value="${plugloadgroup.id}">${plugloadgroup.name}</option>');
		</c:forEach>
		
		$("#group").val(plugloadProfilenodeid); 
		$("#group").attr("disabled", true);
	}
});
...
function submitfilter() {
	...
	if(parent.accTabSelected == 'pf')
	{
		profileDataString =  "profile_"+ $("#group").val();
	}else if(parent.accTabSelected == 'plpf'){
		profileDataString =  "plugloadprofile_"+ $("#group").val();
	}
	...
	start(userdata, 1, "eventTime", "desc");			
