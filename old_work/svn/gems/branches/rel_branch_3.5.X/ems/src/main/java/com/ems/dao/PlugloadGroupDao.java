...
public class PlugloadGroupDao  extends BaseDaoHibernate{
	...
	public void updatePlugloadGroupVisibility(Long groupId, boolean visibility) {
		Session session = getSession();
		session.createQuery("Update PlugloadGroups set displayProfile = :displayProfile where id = :id")
	...
	public PlugloadGroups editName(PlugloadGroups groups) {
	  ...
	  public TreeNode<FacilityType> loadPlugloadProfileHierarchy(boolean visibilityCheck) {
		...
		public List loadPlugloadList(Long pid,String selectedplugloads, int offset, int limit,AssignPlugloadList returnList) {
			List assignPlugloadList = new ArrayList();
			List<Long> tmp = new ArrayList<Long>();
			StringTokenizer st = new StringTokenizer(selectedplugloads, ",");
			...
			try {
	            String queryStr = "select a.id as plugloadId,a.name as plugloadName,a.groupId as currentGroupId,c.name as templateName , b.plugloadProfileTemplate.id as template_id from Plugload a,PlugloadGroups b,PlugloadProfileTemplate c where a.groupId=b.id AND b.plugloadProfileTemplate.id=c.id AND a.id in (:ids)";
				...
				if (limit >= 0) {
					...
					int totalpages = (int) (Math.ceil(query.list().size() / new Double(AssignPlugloadList.DEFAULT_ROWS)));
					returnList.setTotal(totalpages);
					query.setMaxResults(limit).setFirstResult(offset);
				}
				assignPlugloadList = query.list();
	        } catch (Exception e) {
	        }
			return assignPlugloadList;
		}
