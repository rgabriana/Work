...
try:
    ...
    cur.execute("select em.id as emId,c.name, em.name, em.database_name as datbaseName, rs.internal_ip as internalIp, " +
                " cast(em.last_connectivity_at as timestamp(0)), cast (es.set_time as timestamp(0)) , es.database_status, es.log , em.mac_id , cast(em.em_commissioned_date as timestamp(0)) " + 
    ...
    for row in rows:
        ...
        dataSynchLog = row[8];mac = row[9] ;commissionedDate = row[10];
