...
public class DRTargetDao extends BaseDaoHibernate {
    ...
    /**
     * Get all DRTarget objects which are enabled.
     * 
     * @return list of all DRTarget objects in the database
     */
    @SuppressWarnings("unchecked")
    public List<DRTarget> getEnabledDRTargets() {
        try {
        	Criteria criteria = getSession().createCriteria(DRTarget.class); 
         	Criterion rest= Restrictions.eq("enabled", DRTarget.ENABLED);
        	List<DRTarget> drTargets = criteria.add(rest).list();
            if (!ArgumentUtils.isNullOrEmpty(drTargets)) {
                return drTargets;
            }
        } catch (HibernateException hbe) {
...
public List getAllDRTargetsCanceledOrCompleted(Integer offset,Integer limit,DRTargetList mList) {
    ...
    public List<DRTarget> getAllManualDRTargets() {
        try {
            List<DRTarget> drTargets = getSession().createCriteria(DRTarget.class)
            		.add(Restrictions.eq("drType",DRType.MANUAL.getName().toUpperCase()))
    ...
    public void optOutOfDrByIdentifier(String drIdentifier) {
    	 try {
    		 String hql = "update DRTarget a set a.optIn = false, a.drStatus = '" + DRStatusType.Cancelled.getName() + "', a.enabled = '" + DRTarget.DISABLED +"' where a.drIdentifier = :id";
	...
	public DRTarget getEnabledDRTargetByDrEvent(Long id) {
        try {
            DRTarget drTarget = (DRTarget) getSession().get(DRTarget.class, id);
            if(drTarget != null) {
            	DRTarget drTarget1 = (DRTarget) getSession().createCriteria(DRTarget.class)
    					.add(Restrictions.eq("drIdentifier",drTarget.getDrIdentifier()))
    					.add(Restrictions.eq("enabled",DRTarget.ENABLED)).uniqueResult();
            	if (drTarget1 != null) {
            		drTarget = drTarget1;
            	}
            }
