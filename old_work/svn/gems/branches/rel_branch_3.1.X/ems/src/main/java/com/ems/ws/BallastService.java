		...
		try
		{
			for(Ballast ballast : ballastList)
			{
				...
				if(ballastExisting != null)
				{
					...
					eventManager.addEvent("Ballast upload failed: Ballast with the same display lable already exists: " + ballast.getDisplayLabel(), EventsAndFault.FIXTURE_CONFIGURATION_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
					status = 300;
					continue;
				}
