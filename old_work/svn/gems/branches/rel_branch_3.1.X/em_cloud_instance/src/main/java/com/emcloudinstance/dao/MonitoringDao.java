...
public class MonitoringDao extends AbstractJdbcDao{
	...
	public void updateFixtureDiagnostic(String mac){
		...
		for(FixtureDiagnosticsVO fixtureDiagnosticsVO:fixtureDiagnostisVOList){
			...
			fixtureDiagnosticsVO.setPowerUsedAverage(((BigDecimal)diagnosticsReferenceMap.get("powerUsedAvg")).doubleValue());
			fixtureDiagnosticsVO.setPowerUsedVariance(((BigDecimal)diagnosticsReferenceMap.get("powerUsedVariance")).doubleValue());
			...
			if(fixtureDiagnosticsVO.getFixtureDiagnosticReferenceId() != null && fixtureDiagnosticsVO.getFixtureDiagnosticReferenceId() > 0){
				String queryForUpdateOfFixtureDiagnosticReferenceRow = "update fixture_diagnostic_reference set power_used_average = " + diagnosticsReferenceMap.get("powerUsedAvg") +
					       " ,power_used_variance = " + diagnosticsReferenceMap.get("powerUsedVariance");
				...
				String queryForFixtureDiagnosticReferenceRow = "insert into fixture_diagnostic_reference(id, fixture_id,hour_of_day,power_used_average,power_used_variance) " +
		                 " values( nextval('fixture_diagnostic_reference_seq')," + fixtureDiagnosticsVO.getFixtureId() +"," + hour +","+ diagnosticsReferenceMap.get("powerUsedAvg") +"," + diagnosticsReferenceMap.get("powerUsedVariance") + ")";
			...
			if(ecHourlyVO != null){
				...
				Double stdDeviation = Math.pow(((BigDecimal)diagnosticsReferenceMap.get("powerUsedVariance")).doubleValue(), 0.5);
