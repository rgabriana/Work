	...
	public void runMigrationOrSync() throws SchedulerException
	{
		...
		if(replicaServerInfoManager.getCurrentEmStatus()== EmStatus.SPPA)
		{
			...
			case PLUGLOAD_ENERGY_CONSUMPTION_SUCCESS:
				logger.info("**Plugload energy data Migration successfull.") ;				
			...
			case PLUGLOAD_ENERGY_CONSUMPTION_DAILY_SUCCESS:
				logger.info("**Plugload energy data Daily Migration successfull.") ;				
				setReplicaMigrationFlagOnCloud(DatabaseState.MOTION_BIT_START.getName()) ;	
				break;
			case PLUGLOAD_ENERGY_CONSUMPTION_HOURLY_SUCCESS:
				logger.info("**Plugload energy data Hourly Migration successfull.") ;
			...
			case PLUGLOAD_ENERGY_CONSUMPTION_IN_PROGRESS:
				logger.info("**PLugload Energy data" +IN_PROGRESS_LOG) ;
			...
			case PLUGLOAD_ENERGY_CONSUMPTION_DAILY_IN_PROGRESS:
				logger.info("** Plugload Daily Energy data" +IN_PROGRESS_LOG) ;
				//check if running or not. if not start again
				jobName = CommonStateUtils.energyDailyTableName ;
				 nextState = DatabaseState.PLUGLOAD_ENERGY_CONSUMPTION_DAILY_IN_PROGRESS.getName() ;
				createAndFireBulkJob(jobName ,nextState) ;
			...
			case PLUGLOAD_ENERGY_CONSUMPTION_HOURLY_IN_PROGRESS:
				logger.info("** Plugload Hourly Energy data" +IN_PROGRESS_LOG) ;
				//check if running or not. if not start again
				jobName = CommonStateUtils.energyDailyTableName ;
				nextState = DatabaseState.PLUGLOAD_ENERGY_CONSUMPTION_HOURLY_IN_PROGRESS.getName() ;
				createAndFireBulkJob(jobName ,nextState) ;
