...
var COMM_ERROR_INACTIVE_TIMED_OUT_GW_CH_CHANGE_CUSTOM = 16;
	...
	WDS_GRID = $("#wdsTable");
		WDS_GRID.jqGrid({
			...
			hoverrows: false,
			forceFit: true,
		   	colNames:['Name', 'Switch','Gateway','Version','Battery Level','Capture At','Upgrade Status', 'Action'],
		   	colModel:[
		   		...
		   		{name:'batteryLevel', index:'batteryLevel', sortable:true, width:"15%" },
		   		{name:'batteryLevelCaptureAt', index:'batteryLevelCaptureAt', sortable:true, width:"15%" },
		   		{name:'upgradeStatus', index:'upgradeStatus', sortable:true, width:"15%"},
		   		{name:'action', index:'action', align:"right", hidden:(PAGE!="floor"), sortable:false, width:"15%"},
		   	],
		...
		<c:forEach items="${wdss}" var="wds">
			...
			<c:if test="${wds.batteryLevel == 'Normal'}">
			...
			<c:if test="${wds.batteryLevel == 'Low'}">
			...
			<c:if test="${wds.batteryLevel == 'Critical'}">
			...
			localData.batteryLevel =imageHolder+'${wds.batteryLevel}';
			localData.batteryLevelCaptureAt ="${wds.captureAtStr}";
			...
			localData.action = "";		
			<c:if test="${page == 'floor'}">
			localData.action += "&nbsp;<button onclick=\"javascript: parent.parent.showWdsEdit(${wds.id}, '${wds.name}');\">Edit</button>";
			</c:if>
