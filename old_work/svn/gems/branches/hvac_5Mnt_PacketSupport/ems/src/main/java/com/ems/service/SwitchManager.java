...
public class SwitchManager {
    ...
    public int managePlugloadGroupMembership(Long switchId) {
      ...
      while (itr.hasNext()) {
      	...
      	if ((groupPlugload.getNeedSync() & GemsGroupFixture.SYNC_STATUS_GROUP_SYNCD) == GemsGroupFixture.SYNC_STATUS_GROUP_SYNCD) {
    ...
    public int sendSwitchGroupParamsToPlugload(Long switchId, Long plugloadId) {
    	...
    	// 32 bits
    	int groupNo = Integer.parseInt(oSwitchGroup.getGroupNo().toString(), 16);
    	List<SceneLevel> oList = sceneLevelDao.loadSceneLevelListInCreationOrderBySwitchAndFixtureId(switchId, plugloadId);
    	if (oList == null) {
    		return 3;
    	}
    ...
    public byte[] getSwitchGroupParamsDataOfPlugload(Switch oSwitch, Long plugloadId) {
      ...
      int init_apply_time = oSwitch.getInitialSceneActiveTime() != null ? oSwitch.getInitialSceneActiveTime().byteValue() : 0;
      // 8 bits length of time in min to extend scene control (0-240)
      int extend_apply_time = oSwitch.getExtendSceneActiveTime() != null ? oSwitch.getExtendSceneActiveTime().byteValue() : 0;
      // 8 bits
      ...
      try {
      	...
      	for (int count = 0; count < plugloadArr.length; count++) {
      		...
      		if ((groupPlugload.getNeedSync() & GemsGroupFixture.SYNC_STATUS_SWITCHCONF_SYNCD) == GemsGroupFixture.SYNC_STATUS_SWITCHCONF_SYNCD) {
      			...
      			logger.info(plugloadId + " with gems group " + oSwitch.getGemsGroup().getId() + ": switch configuration has not sync'd yet");
      		}
      	}
      } catch (IOException ioe) {
      	logger.error(ioe.getMessage());
