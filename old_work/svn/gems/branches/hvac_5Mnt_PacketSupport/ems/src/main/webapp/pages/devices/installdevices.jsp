...
<spring:url value="/devices/plugloads/manage.ems" var="plugloadList" />
...
<spring:url value="/devices/scenetemplates/manage.ems" var="sceneTemplatesList" />
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		else if(innertabselected == 'plugloads') { $("#plugloads,#innerdevicecenter").click(); } 
		...
		else if(innertabselected == 'scenetemplates') { $("#scenetemplates,#innerdevicecenter").click(); }
		else manageGateways();		
	});
</script>
...
<script type="text/javascript">
	...
	function managePlugloads() {
		$.cookie('innertabselected', 'plugloads',  {path: '/' });
		var ifr;
		ifr = document.getElementById("plugloadsFrame");
		ifr.contentWindow.document.body.innerHTML = "&nbsp;<spring:message code='action.loading'/>";
		ifr.src = "${plugloadList}?ts="+new Date().getTime();
	...
	function manageSceneTemplates() {
		$.cookie('innertabselected', 'scenetemplates',  {path: '/' });
		var ifr;
		ifr = document.getElementById("scenetemplatesFrame");
		ifr.contentWindow.document.body.innerHTML = "&nbsp;<spring:message code='action.loading'/>";
		ifr.src = "${sceneTemplatesList}?ts="+new Date().getTime();
		return false;
	}
...
<div id="innerdevicecenter" class="ui-layout-center pnl_rht_inner">
	<ul
		...
		<c:if test="${enablePlugloadFeature == 'true'}"> 
			<li><a id="plugloads" href="#tab_plugloads"
			onclick="managePlugloads();"><span>Plugloads</span> </a></li>
		</c:if>
		...
		<li><a id="scenetemplates" href="#tab_scenetemplates"
			onclick="manageSceneTemplates();"><span>Scene Templates</span> </a>
		</li>
	</ul>
	<div class="ui-layout-content ui-widget-content ui-corner-bottom"
		...
		<c:if test="${enablePlugloadFeature == 'true'}"> 
		<div id="tab_plugloads" class="pnl_rht"
			style="width: 100%; height: 100%;">
			<iframe frameborder="0" id="plugloadsFrame"
				style="width: 100%; height: 100%;"></iframe>
				<script type="text/javascript">		
				$(function() {
					$(window).resize(function() {
				...
				$("#plugloadsFrame").css("height", $(window).height() - 40);
				</script>
		</div>
		 </c:if>
		...
		<div id="tab_scenetemplates" class="pnl_rht">
			<iframe frameborder="0" id="scenetemplatesFrame"
			style="width: 100%; height: 100%;"></iframe>
			<script type="text/javascript">		
			$(function() {
				$(window).resize(function() {
			...
			$("#scenetemplatesFrame").css("height", $(window).height() - 40);
			</script>
		</div>
