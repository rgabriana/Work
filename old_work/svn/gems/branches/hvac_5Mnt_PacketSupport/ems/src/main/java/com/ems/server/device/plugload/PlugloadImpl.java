...
import com.ems.server.device.DeviceServiceImpl.LockObj;
  ...
  private EmsThreadPool groupsSyncProcessThPool = null;
	...
	class ProfileData {
		...
		public byte[] getByteArray() {
			...
			int i = 0;			
			//min light level in sensor profile
			...
			reqPkt[i++] = 0;
			//ramp up time in sensor profile
			reqPkt[i++] = 0;
			//ambient sensitivity in sensor profile
			reqPkt[i++] = 0;
			//spare
			for(int j = 0; j < 9; j++) {
				reqPkt[i++] = 0;
