...
public class GemsPlugloadGroupManager {
  ...
  public String asssignPlugloadsToGroup(Long gid, List<GemsGroupPlugload> groupPlugloads, final int groupNo, final int groupType) {
  	...
  	while (itr.hasNext()) {
  		...
  		if (groupPlugload.getUserAction() == GemsGroupFixture.USER_ACTION_DEFAULT) {
  			if (groupPlugload.getNeedSync() == GemsGroupFixture.SYNC_STATUS_UNKNOWN) {
  ...
  public int removePlugloadFromGroup(Long gid, List<GemsGroupPlugload> groupPlugloads, final int groupNo,
