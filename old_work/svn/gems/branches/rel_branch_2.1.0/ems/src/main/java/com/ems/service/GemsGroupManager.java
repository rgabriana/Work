...
public class GemsGroupManager {
    ...
    public List<GemsGroup> loadGroupsByFixtureVersion(List<GemsGroup> groups,String fixtureVersion) {
        
    	List<GemsGroup> groupsByFloorAndFixtureVersion = new ArrayList<GemsGroup>();
    	if (groups != null){
    		for (GemsGroup gemsgroup : groups){
        		List<GemsGroupFixture> gemsGroupFixtureList = gemsGroupDao.getGemsGroupFixtureByGroup(gemsgroup.getId());
            	if(gemsGroupFixtureList != null && gemsGroupFixtureList.size() > 0){
            		GemsGroupFixture gemsGroupFixture = gemsGroupFixtureList.get(0);
            		if ( fixtureVersion == "1.5+"){
            			if (Double.parseDouble(gemsGroupFixture.getFixture().getVersion().substring(0,3)) >= 1.5 &&  Double.parseDouble(gemsGroupFixture.getFixture().getVersion().substring(0,3)) < 2.0 ){
        		    		groupsByFloorAndFixtureVersion.add(gemsgroup);
        		    	}
            		}else{
            			if (Double.parseDouble(gemsGroupFixture.getFixture().getVersion().substring(0,3)) >= 2.0 ){
        		    		groupsByFloorAndFixtureVersion.add(gemsgroup);
        		    	}
            		}
        ...
        return groupsByFloorAndFixtureVersion;
    }
    ...
    public void deleteEmptyGemsGroup(GemsGroup group) {
	    deleteGroup(group);
	    deleteGroupType(group.getType());
    }
