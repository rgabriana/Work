...
public class GemsGroupService {
    ...
    public Response createNewGroupType(GemsGroupType gemsGroupType) {
        ...
        userAuditLoggerUtil.log("Create group type: " + gemsGroupType.getGroupTypeName(), UserAuditActionType.Group_Update.getName());
        return new Response();
    }
    ...
    /**
     * Delete all empty groups in a floor
     * @return Response status
     * @throws UnsupportedEncodingException 
     */
    @Path("op/deleteemptygroups")
    @POST
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Response deleteEmptyGroupsInFloor(@CookieParam(FacilityCookieHandler.selectedFacilityCookie) String cookie) throws UnsupportedEncodingException {
    	FacilityCookieHandler cookieHandler = new FacilityCookieHandler(URLDecoder.decode(cookie, "UTF-8"));
        Long floorId = cookieHandler.getFacilityId();
    	List<GemsGroup> gemsGroups = gemsGroupManager.loadGroupsByFloor(floorId);
    	if (gemsGroups != null ){    
            for(int i=0;i<gemsGroups.size();i++){
            	Long gid = gemsGroups.get(i).getId();
            	List<GemsGroupFixture> gemsGroupFixtures = gemsGroupManager.getGemsGroupFixtureByGroup(gid);
            	if (gemsGroupFixtures == null || gemsGroupFixtures.size() == 0){
            		 GemsGroup deletingGroup = gemsGroups.get(i); 
            		 String deletingGroupName = deletingGroup.getGroupName();
            		 gemsGroupManager.deleteEmptyGemsGroup(deletingGroup);
            		 userAuditLoggerUtil.log("Deleted the Empty group: " + deletingGroupName, UserAuditActionType.Group_Update.getName());
            	}
            }
        }
    	return new Response();
