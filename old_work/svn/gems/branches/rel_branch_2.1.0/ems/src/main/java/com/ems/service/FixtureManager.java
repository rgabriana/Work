...
public class FixtureManager {
	...
	public void enablePushProfileAndGlobalPushProfile(Long fixtureId, boolean pushProfileStatus, boolean globalPushProfileStatus) {
		...
		ServerMain.getInstance().getDevice(fixtureId).getFixture().setPushProfile(pushProfileStatus);
		ServerMain.getInstance().getDevice(fixtureId).getFixture().setPushGlobalProfile(globalPushProfileStatus);
	}
    ...
    public Fixture updatePosition(Long fixtureId, Integer x, Integer y, String state) {
      ...
      ServerMain.getInstance().invalidateDeviceCache(fixtureId);
    ...
    public void startNetworkDiscovery(long floorId) {
        ...
        DiscoverySO.getInstance().startNetworkDiscovery(floorId, 0);
    ...
    public int startNetworkDiscovery(long floorId, long gatewayId) {
        ...
        int ret = DiscoverySO.getInstance().startNetworkDiscovery(floorId, gatewayId);
    ...
    public void updateLastConnectionTime(long fixtureId, long gwId) {
        fixtureDao.updateLastConnectionTime(fixtureId, gwId);
    }
    ...
    public void updateLastCommandSent(int[] fixtureArr, int msgType) {

        String cmd = ServerUtil.getCommandString(msgType);
        fixtureDao.updateLastCommandSent(fixtureArr, cmd);

    } // end of method updateLastCommandSent

    public void updateLastCommandStatus(Fixture fixture, int msg, String status, long gwId) {

        String cmd = ServerUtil.getCommandString(msg);
        fixtureDao.updateLastCommandStatus(fixture, cmd, status, gwId);
    ...
    public boolean rmaFixture(Long fromFixtureId, Long toFixtureId) {
        ...
        System.out.println("RMA: From: " + fromFixtureId + "(" + fixtureName + ") => " + " To: " + toFixtureId + "("
                + newFixtureName + ")");
        // update the new node snap address so that it is not unique. this is required
        // because delete is not happening before update
        fixtureDao.replaceFixture(toFixtureId, newFixtureName, newMacAddr, newSensorId, newSnapAddr + "_rma",
