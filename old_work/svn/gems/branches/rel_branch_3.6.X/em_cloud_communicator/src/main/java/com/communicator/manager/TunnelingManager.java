...
public class TunnelingManager {
	...
	public void StartTunnel(String port)
	{
		localPort = CommonUtils.getPropertyWithName("localTunnelingPort", CommunicatorConstant.configFilePath);
		keyPath = CommonUtils.getPropertyWithName("keyPath", CommunicatorConstant.configFilePath);
		if(!monitorTunnel(serverInfoManager.getHost(), port))
		{
		try{ 
			if(!port.equalsIgnoreCase("0"))
		    {
				StopTunnel();
				logger.info("---------------ESTABLISHING BROWSING TUNNEL TO MASTER SERVER AT "+ Calendar.getInstance().getTime().toString() +"----------------");
				ArrayList<String> cmdList = new ArrayList<String>();
				String removeSSHKeygen = "ssh-keygen -R " + serverInfoManager.getHost();
				cmdList.add(removeSSHKeygen);
				String cmd = "ssh -o \"StrictHostKeyChecking no\"" +
						...
						" enlighted@"+serverInfoManager.getHost();
	...
	public void StopTunnel() {
		...
		localPort = CommonUtils.getPropertyWithName("localTunnelingPort", CommunicatorConstant.configFilePath);
		keyPath = CommonUtils.getPropertyWithName("keyPath", CommunicatorConstant.configFilePath);
		if(monitorTunnel(serverInfoManager.getHost(), null))
		{
			logger.info("---------------CLOSING BROWSING TUNNEL TO MASTER SERVER AT "+ Calendar.getInstance().getTime().toString()+"----------------");
			try{
				ArrayList<String> cmdList = new ArrayList<String>();
				...
				cmd = "\":localhost:"+  "9090"+ " enlighted@"+serverInfoManager.getHost()+"\"";
				killCmd =  "pkill -f "+ cmd ;
				logger.info("Ending tunneling command for :- " + cmd);
				cmdList.add(killCmd);
				CommonUtils.executeListOfShellCmd(cmdList) ;
	...
	private boolean monitorTunnel(String cloudServerName, String port)
	{
		Boolean flag = false ;
		localPort = CommonUtils.getPropertyWithName("localTunnelingPort", CommunicatorConstant.configFilePath);
		 Runtime run = Runtime.getRuntime();
			try {
				Process pr = run.exec("sh "+System.getenv().get("OPT_ENLIGHTED")+"/communicator/tunnelMonitoringScript.sh " + cloudServerName + " " + (port == null ? localPort: port));
			    ...
			    if(port == null) {
				    pr = run.exec("sh "+System.getenv().get("OPT_ENLIGHTED")+"/communicator/tunnelMonitoringScript.sh " + cloudServerName + " " + "9090");
