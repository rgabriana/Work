...
public class FirmwareUpgradeDao extends BaseDaoHibernate {
	...
	public List<ImageUpgradeDeviceStatus> loadInprogressDeviceStatus(Date fromDate,Date toDate) {
		List<ImageUpgradeDeviceStatus> oList = new ArrayList<ImageUpgradeDeviceStatus>();
		String hsql = "from ImageUpgradeDeviceStatus where status = :inprogress or status=:schedule and startTime > '"+  DateUtil.formatDate(fromDate,"yyyy-MM-dd HH:mm:ss") + "' AND startTime < '" + DateUtil.formatDate(toDate,"yyyy-MM-dd HH:mm:ss") + "' ORDER BY start_time ASC";
        Query q = getSession().createQuery(hsql.toString());   
        q.setString("inprogress", ServerConstants.IMG_UP_STATUS_INPROGRESS);
        q.setString("schedule", ServerConstants.IMG_UP_STATUS_SCHEDULED);
        oList = q.list();
        return oList;
	}
