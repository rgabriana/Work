...
public class EmsStartup extends HttpServlet {
	...
	private int iAstroSyncInterval = 60; // in minutes
	...
	private void process(ServletContext context) {
		...
		try {
			...
			if (contactClosureConfigurationValue != null) {
				if ("".equalsIgnoreCase(contactClosureConfigurationValue.getValue())) {
					...
					ContactClosureManager contactClosureManager = (ContactClosureManager) SpringContext
					.getBean("contactClosureManager");
					if(contactClosureManager.getCCDataFromDB().getContactClosureVo().size() == 1){
						contactClosureManager.addPercentageContactClosureControl();
					}
				}
			} else {
			...
			// Astro clock timer interval fetch from System configuration
			SystemConfiguration astroSyncInterval = systemConfigurationManager
					.loadConfigByName("astroclock.sync");
			if (astroSyncInterval != null) {
				try {
					iAstroSyncInterval = Integer.valueOf(astroSyncInterval.getValue());
				}catch(NumberFormatException nfe) {
					iAstroSyncInterval = 60;
				}
			} else {
				logger.error("astro clock sync interval default to " + iAstroSyncInterval);
			}
		} catch (Exception e) {
			logger.error("Error in startup: "
					+ e.getMessage());
		}
		// ENL-2685 : End
		...
		BACnetConfigurationManager bacnetConfigurationManager = (BACnetConfigurationManager) SpringContext
				.getBean("bacnetConfigurationManager");
		List<BACnetConfiguration> bacConfDBList = bacnetConfigurationManager.loadAllBACnetConfig();
		List<BacnetObjectsCfg> bacObjCfgDBList = bacnetConfigurationManager.loadAllBacnetObjectCfgs();
		boolean setDetailedMode = false;
		try {
			if(bacConfDBList == null || bacConfDBList.isEmpty()){
				bacConfDBList = bacnetConfigurationManager.getIntialBACnetConfig();
				
				if(bacConfDBList!=null && !bacConfDBList.isEmpty()){
					for(BACnetConfiguration bacnetConfiguration : bacConfDBList){
						bacnetConfigurationManager.save(bacnetConfiguration);
					}
				} else {
					bacnet_logger.debug("bacnet.conf is not in configuration ");
				}
			}
			if(bacObjCfgDBList == null || bacObjCfgDBList.isEmpty()){
				bacObjCfgDBList = bacnetConfigurationManager.getIntialBacnetObjectCfg();
				if(bacObjCfgDBList!=null && !bacObjCfgDBList.isEmpty()){
					for(BacnetObjectsCfg bacnetObjCfg : bacObjCfgDBList){
						if(Constants.BACNET_FIXSUBHEADER.equals(bacnetObjCfg.getBacnetpointtype()) || Constants.BACNET_PLSUBHEADER.equals(bacnetObjCfg.getBacnetpointtype())){
							if("y".equals(bacnetObjCfg.getIsvalidobject())){
								setDetailedMode = true;
							}
						}
						bacnetConfigurationManager.saveBacnetObjectsCfg(bacnetObjCfg);
					}
				} else {
					bacnet_logger.debug("bacnet_objects.cfg is not in configuration ");
				}
			}
		}catch (Exception e) {
			logger.error("Error in intializing bacnet_configuration " + e.getMessage(), e);
		}
		...
		if (!bacnetUser.getEmail().equals(bacnetConfig.getRestApiKey())
			...
			final BACnetConfiguration dbConfSecretKey = bacnetConfigurationManager.loadBACnetConfigByName(Constants.REST_API_KEY);
			if(dbConfSecretKey!=null){
				dbConfSecretKey.setValue(bacnetUser.getEmail());
				bacnetConfigurationManager.save(dbConfSecretKey);
			} else {
				BACnetConfiguration bacConf = new BACnetConfiguration();
				bacConf.setName(Constants.REST_API_KEY);
				bacConf.setValue(bacnetUser.getEmail());
			}
			final BACnetConfiguration dbConfAPISecret = bacnetConfigurationManager.loadBACnetConfigByName(Constants.REST_API_SECRET);
			if(dbConfAPISecret!=null){
				dbConfAPISecret.setValue(bacnetUser.getSecretKey());
				bacnetConfigurationManager.save(dbConfAPISecret);
			} else {
				BACnetConfiguration bacConf = new BACnetConfiguration();
				bacConf.setName(Constants.REST_API_SECRET);
				bacConf.setValue(bacnetUser.getSecretKey());
			}
		}
		try {
			bacnetManager.saveConfig(bacnetConfig);
		}catch (Exception e) {
			logger.error("Unable to start bacnet service: " + e.getMessage());
		}
		...
		// Setup timer for Astro clock
		syslogger.info("Astro clock sync interval set (seconds): " + iAstroSyncInterval);

		Timer astroPoll = new Timer("Astro clock sync", true);
		AstroClockSync aclock = new AstroClockSync();
		astroPoll.schedule(aclock, new Date(), iAstroSyncInterval * 1000);
	...
	private class AstroClockSync extends TimerTask {
		boolean bRunning = false;
		@Override
		public void run() {
			if (bRunning == false) {
				bRunning = true;
				syslogger.info("Triggering astro-sync message...");
				DeviceServiceImpl.getInstance().sendAstroClockMsgToAllGateways();
				bRunning = false;
			}
		}
