...
//		            }
		    		 ...
		    		 //Nwe logic to get the new timezone related list
		    		 final SystemConfiguration sc = systemConfigurationManager.loadConfigByName("list.timezone");
		    		 final List<String> l = Arrays.asList(sc.getValue().split(","));
		    		 long count = 1;
		    		 for (final String tz : l){
		    			 final Timezone tzone = new Timezone();
	                 		tzone.setId(count++);
	                 		tzone.setName(tz.trim());
	                 		timezones.add(tzone);
		    		 }
