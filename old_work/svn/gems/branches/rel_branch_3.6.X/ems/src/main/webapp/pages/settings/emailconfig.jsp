...
$(document).ready(function() {
	...
	jQuery.validator.addMethod("notallowedports", function(value, element) {
		var ports= ["22","8085","7","443","8443","80","8080","5432","5433","9090"];
		var in_array = $.inArray(value, ports);
		if (in_array == -1) {
		    return true;
		}else{
			return false;
		}
	}, "Use Standard Ports Only");
	...
	$.validator.addMethod("hostregx", function(value, element, regexpr) {          
	    return regexpr.test(value);
	}, "Please enter a valid Smtp Host");
	$.validator.addMethod("protocolregx", function(value, element, regexpr) {          
	    return regexpr.test(value);
			...
			.validate(
					{
						rules : {
							host : {
								required : true,
								hostregx : /^((?:(?:(?:\w[\.\-\+]?)*)\w)+)((?:(?:(?:\w[\.\-\+]?){0,62})\w)+)\.(\w{2,6})$/
							},
							port : {
								required : true,positiveIntegerOnly : true, notallowedports: true,
							},
							user : {
								required : true,
								email : true
		...
		<form:form id="validationForm" action="${saveMail}" method="post" commandName="emailModel">
					...
					<div class="field">
						...
						<div class="formValue">
							...
							<input type="hidden" id="prevPort1" name="prevPort1" value="${emailModel.port}" />
							<form:input type="hidden" id="prevPort" name="prevPort" path="prevPort" />
						</div>
					</div>
					<div class="field">
						<div class="formPrompt">
