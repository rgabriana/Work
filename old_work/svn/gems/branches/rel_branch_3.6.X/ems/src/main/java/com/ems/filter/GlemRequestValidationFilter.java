...
public class GlemRequestValidationFilter implements Filter {
		...
		try {
			...
			if (!StringUtils.isEmpty(userId) && !StringUtils.isEmpty(ts) && !StringUtils.isEmpty(authToken)){
				...
				handleRequest(response, userId, ts,
						authToken, userManager, filterChain, request, true);
				return;
				...
				if (validated(request.getHeader(GLEM_AUTHORIZATION),
					...
					final String sysConfigApiKey = request.getHeader(GLEM_API_KEY);
					if (!StringUtils.isEmpty(sysConfigApiKey)){
						//New API logic access
						isUserIdExistsInReq = true;
						handleRequest(response, sysConfigApiKey, ts,
								request.getHeader(GLEM_AUTHORIZATION), userManager, filterChain, request, false);
					}else{
						status++;
	 ...
	 * @param authToken
	 * @param userManager
	 * @param flagForAuthToken if set to true it will work with new authToken mechanism header. Else will work the old way.
	 * @throws IOException
	 */
	public void handleRequest( HttpServletResponse response,
			final String userId, final String ts, final String authToken,
			final UserManager userManager, FilterChain filterChain, HttpServletRequest request, final boolean flagForAuthToken) throws IOException {
		String sResponse;
			...
			if(user != null){
				...
				if(!user.isActive() || user.isUserLocked()){
					...
					if(!StringUtils.isEmpty(secretKey)){
						String genAuthToken = "";
						if(flagForAuthToken){
							genAuthToken = getSaltBasedDigest(userId, ts, secretKey, DIGEST_ALGO);
						}else{
							genAuthToken = getSaltBasedDigest(userId, secretKey, ts, DIGEST_ALGO);
						}
						final String context = request.getContextPath();
				        path = path.replace(context, "");
				        if (!StringUtils.isEmpty(genAuthToken) && !genAuthToken.equals(authToken) && user.getEmail().equals(Constants.BACNET)){
				        	//Special case for bacnet configuration sys config to read without auth or using key in sys config given 
				        	if (path.startsWith(Constants.BACnet_CONFIG_PATH)){
				        		final SystemConfiguration shortkeyconf = systemConfigurationManager.loadConfigByName("enlightedBacnetConfiguration.authKey");
				        		if (shortkeyconf != null){
				        			if (StringUtils.isEmpty(shortkeyconf.getValue())){
				        				//Allow him// Short the genAuthToken with authToken
				        				genAuthToken = authToken;
				        			}else{
						        		if(flagForAuthToken){
											genAuthToken = getSaltBasedDigest(userId, ts, shortkeyconf.getValue(), DIGEST_ALGO);
										}else{
											genAuthToken = getSaltBasedDigest(userId, shortkeyconf.getValue(), ts, DIGEST_ALGO);
										}
				        			}
				        		}
				        	}
						...
						if(!StringUtils.isEmpty(genAuthToken) && genAuthToken.equals(authToken)){
							...
							if(lastCallTime == null){
								...
								try {
									...
									if(flagForAuthToken == true && path.equals(dto.path) && (currTime - lastCallTime < REPLAY_CHECK_TIME_LIMIT) ){
