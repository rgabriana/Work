...
public class UserDao extends BaseDaoHibernate {
    ...
    public User loadUserByUserName(String userId) {
        try {
            ...
            String hsql = "from User u where LOWER(u.email)=?";
            Query q = getSession().createQuery(hsql.toString());
            q.setParameter(0, userId.toLowerCase());
            results = q.list();
    ...
    /**
     * Load all users having secret key either null or empty
     * @return
     */
    @SuppressWarnings("unchecked")
    public List<User> loadAllUsersHavingNoSecretKey() {
        try {
            List<User> results = null;
            String hsql = "from User u where u.secretKey is null or trim(u.secretKey) = ''";
            Query q = getSession().createQuery(hsql.toString());
            results = q.list();
            if (results != null && !results.isEmpty()) {
                return results;
    ...
    public void updateUserDetails(String column, String value, String id) {
        ...
        if (!"role_id".equals(column) && !"approved_location_id".equals(column) && !"location_id".equals(column)  && !Constants.NO_LOGIN_ATTEMPT_COLUMN.equals(column)) {
    ...
    public Role getRoleByRoleType(RoleType roleType) {
    	try {
            List<Role> results = null;
            String hsql = "Select r from Role r where r.roleType = ?";
            Query q = getSession().createQuery(hsql.toString());
            q.setParameter(0, roleType);
            results = q.list();
            if (results != null && !results.isEmpty()) {
                return (Role) results.get(0);
            }
    ...
    public List<User> loadUsersByRole(RoleType roleType) {
    	try {
    	        ...
    	        final String hsql = "from User u where u.role is not null and u.role.roleType = :roleType"
    	                + " order by email asc";
    	        final Query q = getSession().createQuery(hsql);
