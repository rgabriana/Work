...
package com.enlightedinc.common
{
	...
	import com.enlightedinc.events.AllPlugloadChartUpdate;
	...
	[Event(name="updatecurrentcount", type="com.enlightedinc.events.CurrentSavingChartUpdate")]*/
	[Event(name="allplugloadchartupdate", type="com.enlightedinc.events.AllPlugloadChartUpdate")]
	...
	public class CountsCalculator extends EventDispatcher
	{
		...
		/**
		 * The method calculates Kwh counts for All Plugloads - All counts are derived from Raw Data coming from webservice.
		 * @return void
		 */
		
		public function calculateAllPlugloadKwhCounts(_dataProvider:ArrayCollection):void
		{
			flushCounts();
			...
			if(_dataProvider!=null && _dataProvider.length>0)
			{
				...
				for(var j:int=0; j<_dataProvider.length; j++)
				{
					...
					var basePower:Number =0;
					var unmanagedbasepowerused:Number =0;
					var object:Object = new Object();
					if((_dataProvider[j].basepowerused!=null && (!isNaN(_dataProvider[j].basepowerused)) && (_dataProvider[j].basepowerused>0))|| (_dataProvider[j].unmanagedbasepowerused!=null && (!isNaN(_dataProvider[j].unmanagedbasepowerused)) && (_dataProvider[j].unmanagedbasepowerused>0)))
					{
						// AS PER NEW DASHBOARD REVIEW CHNAGES
						// Using Raw Values of Ambient, Manual, Tasktuning, Occupancy Saving - To show series in the Stack chart
						ambientSaving = checkNaN(_dataProvider[j].ambientsaving);
						...
						unmanagedLoad = checkNaN(_dataProvider[j].unmanagedpowerused);
						basePower = checkNaN(_dataProvider[j].basepowerused);
						unmanagedbasepowerused = checkNaN(_dataProvider[j].unmanagedbasepowerused);
						totalAmbientSaving+=ambientSaving;
						totalOccSaving+=occSaving;
						totalTaskTuneupSaving+=taskTuneupSaving;
					}else
					...
					object.savedpower=_dataProvider[j].savedpower;
					object.unmanagedpowerused=unmanagedLoad;
					object.unmanagedbasepowerused=checkNaN(unmanagedbasepowerused);
					chartDataCollection.addItem(object);
				...
				createPeriodSavingData("power");
				calculatePeak();
				calculateYAxisMaxForAllPlugload(chartDataCollection);
			}else
			{
				YAxisMaxVaue = 1;
			}
			dispatchEvent(new AllPlugloadChartUpdate(AllPlugloadChartUpdate.ALL_PLUGLOAD_CHART_UPDATE)); 
		}
		...
		public function calculateCurrentCount(_dataProvider:ArrayCollection):void
		{
			...
			if(_dataProvider!=null && _dataProvider.length>0)
			{
				...
				dispatchEvent(new CurrentSavingChartUpdate(CurrentSavingChartUpdate.UPDATE_CURRENT_COUNT)); 
		...
		private function calculateYAxisMaxForAllPlugload(chartData:ArrayCollection):void{
			this.YAxisMaxVaue = 0;
			var count:Number =0;
			for each (var obj:Object in chartData)
			{
				try
				{
					if(currentUnitSelection == Constants.POWER_UNIT)
					{
						var tempObj:Number = obj.basepowerused +obj.unmanagedbasepowerused;
						if(tempObj > this.YAxisMaxVaue)
							this.YAxisMaxVaue = tempObj;
						//else //if(obj.unmanagedbasepowerused > this.YAxisMaxVaue)
							//this.YAxisMaxVaue = obj.unmanagedbasepowerused;
						
						if(tempObj ==0 && this.YAxisMaxVaue ==0 && count == chartData.length)
						{
							YAxisMaxVaue = 1;
						}
					}
