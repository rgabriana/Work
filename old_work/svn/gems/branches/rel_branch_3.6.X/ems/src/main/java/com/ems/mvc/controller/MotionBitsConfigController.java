...
public class MotionBitsConfigController {
	...
	public String getMotionBitsScheduleList(Model model, @RequestParam(value = "deleteStatus", required = false) Integer deleteStatus) throws EmsValidationException {
		
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "motionBitsConfigController.deleteStatus", deleteStatus);
	...
	@RequestMapping("/createdialog.ems")
	public String createMotionBit(Model model) {	
		MotionBitsScheduler mScheduler = new MotionBitsScheduler();
		model.addAttribute("mScheduler",mScheduler);
        return "motionbits/dialog";
	}
