...
<spring:url value="/services/org/switch/delete/" var="deleteSwitchUrl" scope="request" />
	...
	$(document).ready(function() {
		...
		SWITCH_GRID.jqGrid({
			...
			hoverrows: false,
			forceFit: true,
		   	colNames:['Name', 'Type', 'Initial Active Time', 'Action'],
		   	colModel:[
		   		{name:'name', index:'name', sorttype:"string", width:"30%"},
		   		{name:'type', index:'type', sorttype:"string", width:"30%"},
		   		{name:'initialsceneactivetime', index:'initialsceneactivetime', width:"10%"},
		   		{name:'action', index:'action', align:"right", sortable:false, hidden:(PAGE=="area"), width:"30%"}
		   	],
...
// 		   	pager: '#switchPagingDiv',
		...
		<c:forEach items="${switches}" var="switchem">
			var localData = new Object;
			localData.name = '<c:out value="${switchem.name}" escapeXml="true" />';
			if ("${switchem.modeType}" == "0") {
				localData.type = '<c:out value="Auto on/Auto off" escapeXml="true" />';
			}else {
				localData.type = '<c:out value="Manual on/Auto off" escapeXml="true" />';
			}
			localData.initialsceneactivetime = "${switchem.initialSceneActiveTime}";
			localData.id =  "${switchem.id}";
			localData.floor = "${switchem.floorId}";
			localData.area = "${switchem.areaId}";
			
			localData.action = "";
			<c:if test="${page == 'floor'}">
				localData.action += "<button onclick=\"$('#switch_message').text('');javascript: parent.parent.showWidgetDialog(${switchem.id});\">Edit</button>";
			</c:if>
			
			localData.action += "&nbsp;<button onclick=\"$('#switch_message').text('');javascript: deleteSwitch(${switchem.id});\">Delete</button>";
...
// 					alert("Switch deleted successfully.");
					if(data.status == 1) {
						$("#switch_message").text("Cannot delete the switch ("+switchName+").Please disassociate the fixtures,plugload and ERC associated with this switch");
