...
public class AreaManager {
    ...
    public HashMap<Long, BigInteger> getAllAreasByFloorIdWithNoOfFaultySensors(Long floorId) {
      return areaDao.getAllAreasByFloorIdWithNoOfFaultySensors(floorId);
    }
    
    public List<Object[]> getAllAreasByFloorIdWithNoOfSensors(Long floorId) {
      return areaDao.getAllAreasByFloorIdWithNoOfSensors(floorId);
    }
    
    public Object[] getOccupancyStatusOfArea(Long areaId) {
    	return areaDao.getOccupancyStatusOfArea(areaId);
    }

    public void updateArea(Area area) {
        try {
            Area area1 = getAreaById(area.getId());
            Boolean prev = area1.getZoneSensorEnable();
            ...
            Boolean now = area.getZoneSensorEnable();
            update(area1);
            if(prev != now) {
            	List<Fixture> l = fixtureDao.getFixturesByArea(area.getId());
            	if(l != null && l.size() > 0) {
        			for(Fixture f: l) {
        				if(now != null && now) {
        					f.enableZoneOccupancyTriggerType();
        				}
        				else {
        					f.disableZoneOccupancyTriggerType();
        				}
        				fixtureDao.update(f);
        			}
            	}
	...
	public List<Area> getAllAreasOfOrg(){
		return areaDao.getAllAreasOfOrg();
	}
