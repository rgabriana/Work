...
public class UemService {
	...
	public Response addUem(@PathParam("host") String host,
    	...
    	nameValMap.put("UemService.port", port);    	
			...
			if (gws.size() > 0) {
				...
				try {
					File drUserFile = new File(
							Constants.ENL_APP_HOME+"/Enlighted/cloudServerInfo.xml");
					if (drUserFile.exists()) {
						DocumentBuilderFactory docFactory = DocumentBuilderFactory
								.newInstance();
						DocumentBuilder docBuilder = docFactory
								.newDocumentBuilder();
						Document doc = docBuilder.parse(drUserFile
								.getAbsoluteFile());
						...
						if (server != null && server.getLength() > 0) {
							...
							macId = each.item(1).getFirstChild().getNodeValue();							
						}
	...
	public Response setPeriodicHB(List<Fixture> fixtures, @PathParam("enable") Short enable, @PathParam("triggerDelayTime") Integer triggerDelayTime, @PathParam("ack") Short ack) {
		...
		if(fixtures != null && fixtures.size() > 0) {
			for(Fixture f: fixtures) {
				...
				if(enable == 1) {
					fix.enableRealtimeOccupancyTriggerType();
				} else {
					fix.disableRealtimeOccupancyTriggerType();
				}
