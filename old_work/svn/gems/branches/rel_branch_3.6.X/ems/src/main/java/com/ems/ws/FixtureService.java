...
//      }
   ...
   /* *
	 ...
	 */
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin','Auditor','Employee','Bacnet')")
	@Path("op/assignblemode/{blemode}")
	@POST
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response setBleModeToFixtures(@PathParam("blemode") String blemode, List<Fixture> fixtures) {
		Response resp = new Response();
		...
		if (fixtures != null) {
			...
			DeviceServiceImpl.getInstance().sendBleMode(fixtureArr,(byte)bleMode.value());
