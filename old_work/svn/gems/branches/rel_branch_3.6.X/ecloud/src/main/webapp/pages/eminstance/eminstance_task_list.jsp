...
<spring:url value="/eminstance/scheduleEmCertificateUpgrade.ems" var="createEmCertificateTaskUrl" />
...
$(document).ready(function() {
    ...
    //display certificate sync btn if em version is greater than 3.6.0
    if(checkEmVersion()==true){
    	$("#newEmUpgradeCertificateButton").show();
    }else{
    	$("#newEmUpgradeCertificateButton").hide();
    }
	...
	$('#newEmUpgradeCertificateButton').click(function() {
		//clearLabelMessage();
	    $("#newEmTaskDialog").load("${createEmCertificateTaskUrl}?emInstanceId="+"${emInstanceId}"+"&ts="+new Date().getTime()).dialog({
	        title : "New EM Certificate Sync Task",
	        width :  Math.floor($('body').width() * .30),
	        minHeight : 250,
	        position : ['top',50],
	        modal : true
...
function checkEmVersion(){
	var selectedEmVersion = "${emInstanceVersion}";
	var splitEmVersionVal =  selectedEmVersion.split(".",3);	
	if((parseInt(splitEmVersionVal[0],10) >= 3) && (parseInt(splitEmVersionVal[1],10) >= 6) && (parseInt(splitEmVersionVal[2],10) >= 0)){
		return true;		
	} else{
		return false;
	}	
...
<div style="width: 100%;">
	<div id="outerDiv">
		...
		<security:authorize access="hasAnyRole('Admin','SystemAdmin','SupportAdmin')">
		<button id="newEmUpgradeCertificateButton">Add Certificate Sync Task</button>	
		</security:authorize>
