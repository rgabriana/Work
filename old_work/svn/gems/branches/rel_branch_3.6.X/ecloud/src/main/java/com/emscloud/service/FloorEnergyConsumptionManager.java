...
public class FloorEnergyConsumptionManager {
	...
	public List<SensorData> getFloorSensorData(long floorId, String fromDate, String toDate, String attributes, String fixtureIds) {
		...
		while(emIdIter.hasNext()) {
			...
			if(emInst != null){
				...
				sensorDataList.addAll(floorEcDao.getEmFloorSensorRawEnergyReadings(emInst.getDatabaseName(), replicaIp, 
						fromDate, toDate, emMapping.getEmFacilityId(), attributes, fixtureIds));
			}			
		}	
		return sensorDataList;
	...
	public String getFloorSensorDataCount(Long floorId, String fromDate, String toDate, String fixtureIds) {
		...
		while(emIdIter.hasNext()) {
			...
			if(emInst != null){
				String replicaIp = replicaServerManager.getReplicaServersbyId(emInst.getReplicaServer().getId()).getInternalIp();
				return floorEcDao.getEmFloorSensorRawEnergyReadingsCount(emInst.getDatabaseName(), replicaIp, fromDate, toDate, emMapping.getEmFacilityId(), fixtureIds);
			}			
		}	
		return "-1";
