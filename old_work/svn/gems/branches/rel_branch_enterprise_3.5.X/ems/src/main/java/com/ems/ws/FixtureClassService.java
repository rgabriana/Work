		...
		try
		{
			for(FixtureClass fxClass : fixtureClassList)
			{
				...
				if(fxClassExisting != null)
				{
					...
					// Override default severity for Fixture Configuration failure
					eventManager.addAlarm("Upload fixture type error: Fixture class with the same name already exists: " + fxClass.getName(), EventsAndFault.FIXTURE_CONFIGURATION_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
					status = 300;
					continue;
				}
				...
				if(bulb == null)
				{
					...
					// Override default severity for Fixture Configuration failure
					eventManager.addAlarm("Upload fixture type error: Bulb associated with the fixture type does not exist, name = " + fxClass.getBulb().getBulbName(), EventsAndFault.FIXTURE_CONFIGURATION_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
					status = 300;
					continue;
				}
				...
				if(ballast == null)
				{
					...
					// Override default severity for Fixture Configuration failure
					eventManager.addAlarm("Upload fixture type error: Ballast associated with the fixture type does not exist, name = " + fxClass.getBallast().getBallastName(), EventsAndFault.FIXTURE_CONFIGURATION_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
					status = 300;
					continue;
				}
