		...
		try
		{
			for(Bulb bulb : bulbList)
			{
				...
				if(bulbExisting != null)
				{
					...
					// Override default severity for Bulb Configuration Upload failure
					eventManager.addAlarm("Bulb upload failed: Bulb with the same name already exists: " + bulb.getBulbName(), EventsAndFault.FIXTURE_CONFIGURATION_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
					status = 300;
					continue;
				}
