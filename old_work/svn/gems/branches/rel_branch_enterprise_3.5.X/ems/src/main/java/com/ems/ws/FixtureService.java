		   		...
		   		while(iterator.hasNext())
		   		{
		   			...
		   			if(info.getType().equalsIgnoreCase(String.valueOf(SensorConfig.typePlugload))) {
		   				...
		   				if(plugload != null && plugload.getState().equals(ServerConstants.PLUGLOAD_STATE_COMMISSIONED_STR)) {
		   					...
		   					// Override default severity for Placed Plugload Commissioning failure
			   				eventsAndFaultManager.addAlarm("Placed Plugload with this macAddress is already commissioned. macAddress: " + info.getMac(), EventsAndFault.PLACED_PLUGLOAD_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
			   				continue;
		   				}
		   				if(plugload == null) {
		   			...
		   			if(fixture != null && fixture.getState().equals(ServerConstants.FIXTURE_STATE_COMMISSIONED_STR))
		   			{
	   					...
	   					// Override default severity for Placed Plugload Commissioning failure
		   				eventsAndFaultManager.addAlarm("Placed Fixture with this macAddress is already commissioned. macAddress: " + info.getMac(), EventsAndFault.PLACED_FIXTURE_UPLOAD, EventsAndFault.MAJOR_SEV_STR);
		   				continue;
		   			}
					if(fixture == null)
						fixture = new Fixture();
