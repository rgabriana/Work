...
public class FloorEnergyConsumptionDao {
	...
	public List<SensorData> getEmFloorSensorRawEnergyReadings(String dbName,
			...
			String attributes, String fixtureIds) {
		...
		try {
			...
			if (attributes != null && !"".equals(attributes)) {
				...
				if(fixtureIds != null && !"".equals(fixtureIds)) {
					rawQueryBuffer.append(" AND ec.fixture_id in ( " + fixtureIds + " )" );
				}
				...
				query = " SELECT fixture_id, capture_at + '-5 min', round(base_power_used, 2) AS baseEnergy, "
						...
						+ floorId
						+ ((fixtureIds != null && !"".equals(fixtureIds)) ? " AND ec.fixture_id in ( " + fixtureIds + " )" : "");
			}
			if(logger.isDebugEnabled()) {
	...
	public String getEmFloorSensorRawEnergyReadingsCount(String dbName, String replicaServer, String fromDate, String toDate, long floorId, String fixtureIds) {
		...
		try {
			connection = getDbConnection(dbName, replicaServer,
					DatabaseUtil.port);
			stmt = connection.createStatement();
			String query = " SELECT count(ec.id) from energy_consumption ec, device d WHERE capture_at > '"
						+ fromDate
						+ "' AND capture_at <= '"
						+ toDate
						+ "' AND d.id = ec.fixture_id AND zero_bucket != 1 AND floor_id = "
						...
						+ ((fixtureIds != null && !"".equals(fixtureIds)) ? " AND ec.fixture_id in ( " + fixtureIds + " )" : "");
			if(logger.isDebugEnabled()) {
				logger.debug("getEmFloorSensorRawEnergyReadingsCount query -- " + query);
			}
			rs = stmt.executeQuery(query);
			while (rs.next()) {
