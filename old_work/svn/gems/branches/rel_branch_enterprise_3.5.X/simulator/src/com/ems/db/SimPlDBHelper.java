...
public class SimPlDBHelper {
    ...
    public void updateProfile1(String suname, byte[] prf, int type) {
        ISqlJetCursor cursor = null;
        Map<String, Object> values = new HashMap<String, Object>();
        try {
            m_DB.beginTransaction(SqlJetTransactionMode.WRITE);
    ...
    public byte[][] fetchRecords(String mac)  {
        byte[][] prof = null;
        ISqlJetTable table = null;
        try {
            table = m_DB.getTable(SIM_PLUGLOAD_TABLE);
        } catch (SqlJetException e1) {
        ...
        try {
            m_DB.beginTransaction(SqlJetTransactionMode.READ_ONLY);
            cursor = table.lookup(SIM_PL_IDX.DEVICE_MAC_IDX.getName(), mac);
        } catch (SqlJetException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
            ...
            try {
                if (!cursor.eof()) {
                    do {
                         ...
                         prof = new byte[] []{cursor.getBlobAsArray(SIM_PL.SPROFILE_WEEKDAY.getName()),
                                                      cursor.getBlobAsArray(SIM_PL.SPROFILE_WEEKEND.getName()),
                                                      cursor.getBlobAsArray(SIM_PL.SPROFILE_HOLIDAY.getName()),
                                                      cursor.getBlobAsArray(SIM_PL.SPROFILE_OVERRIDE2.getName()),
                                                      cursor.getBlobAsArray(SIM_PL.GPRPFILE.getName())                                                      
                                                      
                                };
                        oLogger.info("fetching "+cursor.getRowId() + " : " + cursor.getString(SIM_PL.DEVICE_MAC.getName()) + " {"
                                + Utils.getPacket(cursor.getBlobAsArray(SIM_PL.SPROFILE_WEEKDAY.getName())) + "}, " + " {"
                                + Utils.getPacket(cursor.getBlobAsArray(SIM_PL.SPROFILE_WEEKEND.getName())) + "}, " + " {"
                                + Utils.getPacket(cursor.getBlobAsArray(SIM_PL.SPROFILE_HOLIDAY.getName())) + "}, " + " {"
                                + Utils.getPacket(cursor.getBlobAsArray(SIM_PL.SPROFILE_OVERRIDE2.getName())) + "}, " + " {"
