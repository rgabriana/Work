...
public class EmInstanceDao extends BaseDaoHibernate {
	...
	public List<EmInstance> getActiveEmInstanceWithDataSynch() {
		...
		oCriteria.add(Restrictions.eq("eminst.emState", 0L));
	...
	public List<EmInstance> loadEmInstancesByCustomerId(long id) {
		try {
			List<EmInstance> emInstanceList = sessionFactory
					.getCurrentSession().createCriteria(EmInstance.class)
					.add(Restrictions.eq("customer.id", id)).add(Restrictions.eq("emState", 0L))
					.addOrder(Order.asc("name")).list();
			if (emInstanceList != null && !emInstanceList.isEmpty()) {
				return emInstanceList;
			} else {
	...
	public EmInstanceList loadEmInstanceListByCustomerId(Long id,String orderby,
		...
		oRowCount.add(Restrictions.eq("eminst.emState",0L));
		oCriteria.add(Restrictions.eq("eminst.customer.id", id));
		oCriteria.add(Restrictions.eq("eminst.emState", 0L));
		oCriteria.createAlias("replicaServer","replicaServer", CriteriaSpecification.LEFT_JOIN);
		...
		if (orderby != null && !"".equals(orderby)) {
			...
			} else if (orderby.equals("replicaServerName")) {
	...
	public List<EmInstance> loadEmInstanceByReplicaServerId(Long id) {
		try {
			List<EmInstance> emInstanceList = sessionFactory
					.getCurrentSession().createCriteria(EmInstance.class)
					.add(Restrictions.eq("replicaServer.id", id)).add(Restrictions.eq("emState", 0L))
					.addOrder(Order.asc("name")).list();
	...
	public EmInstance loadEmInstanceById(long id) {
		try {
			...
			EmInstance emInstance = (EmInstance) sessionFactory
					.getCurrentSession().createCriteria(EmInstance.class)
					.add(Restrictions.eq("id", id)).add(Restrictions.eq("emState", 0L)).uniqueResult();
	...
	public EmInstance loadEmInstanceByMac(String mac) {
		try {
			...
			EmInstance emInstance = (EmInstance) sessionFactory
					.getCurrentSession().createCriteria(EmInstance.class)
					.add(Restrictions.eq("macId", mac).ignoreCase()).add(Restrictions.eq("emState", 0L)).uniqueResult();
	...
	{
            List<EmInstance> results = null;
            String hsql = "from EmInstance u where u.macId=? and u.emState=0";
	...
	public EmInstance getEmInstance(long id) {
		  String hsql = "from EmInstance u where u.id=? and u.emState=0";
		  Query q = sessionFactory.getCurrentSession().createQuery(hsql.toString());
          q.setParameter(0, id);          
...
//    	Object obj = getObject(EmInstance.class, id);
	 ...
	 {
		 String query = "select DISTINCT (e.replicaServer.id)  as replicaId ,count(e.id) as emCount  from EmInstance e, ReplicaServer r, Customer c where e.customer.id = c.id and e.replicaServer.id = r.id and e.sppaEnabled='true' and e.databaseName is NOT NULL  and e.emState = 0 group by "+
	    ...
	    public boolean checkPortInUseForSSHORWebappByAnyEM(long port) {
	    	boolean retVal = false;
			List<EmInstance> results = new ArrayList<EmInstance>();
			String hsql = "from EmInstance where  tunnelPort = ?";
	        Query q = sessionFactory.getCurrentSession().createQuery(hsql.toString());
	        q.setLong(0, port);
	        results = q.list();	
	        if(results != null && results.size() > 0) {
	        	...
	        	hsql = "from EmInstance where  sshTunnelPort = ?";
