...
public class ConnectionTemlpate<T> {
	...
	public Builder executePost(ReplicaServer rs, String url, String inputMediaType , String outpuMediaType) {
		WebResource webR = null;
		String actualUrl = null;
		Builder rb = null;
		try {
			
			actualUrl = "https://" + rs.getInternalIp() + encodeURL(url);
			Client client = ClientHelper.createKSClient() ;
			webR = client.resource(actualUrl);
			rb = webR.accept(outpuMediaType).type(inputMediaType);
		} catch (Exception ex) {
			logger.error(ex.getMessage(), ex);
