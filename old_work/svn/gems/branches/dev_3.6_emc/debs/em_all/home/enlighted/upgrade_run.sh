...
then
    ...
    then
        ...
        then
            ...
            if [ -f "$BACKUPSCRIPT" ]
            then
            	if [ -f  "$EM_MGMT_BASE/em_mgmt/adminscripts/generate_backup_prefix.sh" ]
            	then
                	backupprefix=$($EM_MGMT_BASE/em_mgmt/adminscripts/generate_backup_prefix.sh)
                fi
                /bin/bash $BACKUPSCRIPT "backup" "${backupprefix}beforeUpgradeDBBackup" "$OPT_ENLIGHTED/DB/DBBK" "$APP_ROOT" "$IP_ADDRESS" "F"
                ...
                /usr/bin/pg_dump -i -U $POSTGRESUSER -h $POSTGRESHOST -F c -b -f  "$upgradeBackupDir/${backupprefix}beforeUpgradeDBBackup.backup" $POSTGRESDATABASE
                if [ $? -ne 0 ]
                then
                    echo "UPGRADE ERROR: Database backup was not successful. Cannot continue with upgrade."
                    exit 2
	...
	then
		...
		echo "****debian not upgraded till date *********This is the first upgrade.*****" >> $LOGS_ERR
	fi
...
if [ -z "$GOOGLE_INSTALL" ];then
    $ENLIGHTED_HOME/scripts/avahiremoval.sh
else
    $ENLIGHTED_HOME/scripts/avahiremoval.sh
fi
...
#  apache2 libapache2-mod-wsgi software-properties-common postgresql $TOMCAT_PKGSEL_USER $TOMCAT_PKGSEL_ADMIN php5 python-django python-setuptools python-django-piston gettext ssh-askpass bind9 
...
if [ -z "$GOOGLE_INSTALL" ];then
    ...
    check=$(dpkg -l python-paramiko | grep "python-paramiko")
    if [[ "$check" =~ "python-paramiko" ]]
        then
            echo "*** python-paramiko is already installed ***" >> $LOGS_ERR
        else
            echo "*** Intalling python-paramiko ... ***" >> $LOGS_ERR
                sudo dpkg -i --force-overwrite ./archives/python-paramiko_1.7.6-2_all.deb
            if [ $? -ne 0 ]
            then
                echo "UPGRADE ERROR: EM Management upgrade failed. Could not install python-paramiko successfully."
                exit 3
            fi
    fi

    #curl_7.19.7-1ubuntu1.1_i386.deb
    check=$(dpkg -l curl | grep "ii" | grep "curl")
    if [[ "$check" =~ "curl" ]]
        then
            echo "*** curl is already installed ***" >> $LOGS_ERR
        else
            echo "*** Intalling curl... ***" >> $LOGS_ERR
                sudo dpkg -i --force-overwrite ./archives/curl_7.19.7-1ubuntu1.1_i386.deb
            if [ $? -ne 0 ]
            then
                echo "UPGRADE ERROR: EM Management upgrade failed. Could not install curl successfully."
                exit 3
            fi
    fi
fi
					...
					echo "step6;"
						...
						then
								...
								then
                                        ...
                                        cp ~enlighted/upgradeSQL.sql $ENL_APP_HOME/Enlighted/tempExtract/${DEBIAN_REV_NUMBER}_upgradeSQL.sql
