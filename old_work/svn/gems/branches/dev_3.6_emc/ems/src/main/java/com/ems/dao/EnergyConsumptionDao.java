...
public class EnergyConsumptionDao extends BaseDaoHibernate {
    ...
    public void aggregateFixtureHourlyData(Date toDate, Long gwId) {
      ...
      try {
          con = getSession().connection();
          con.setAutoCommit(false);
          calSt = con.prepareCall("{ call aggregateFixtureHourlyEnergyConsumption('" + new Timestamp(toDate.getTime())
                  + "', " + gwId + ")}");
          calSt.execute();
      } catch (Exception ex) {
          ex.printStackTrace();
...
*/
		...
		try {
			...
			Long startTime = startDate.getTimeInMillis();
				String query = "SELECT captureAt, "
						...
						+ "(SELECT COALESCE(round(avg(base_power_used) * 0.25,2),0)) as ec_base_power_used, "
						+ "(SELECT COALESCE(round(avg(power_used) * 0.25,2),0)) as ec_power_used "
						+ "from "+ tableName + " ec join device d on ec.fixture_id = d.id WHERE "
						+ " capture_at > '"
						+ DateUtil.formatDate(minTime, "yyyy-MM-dd HH:mm")
						+ "' "
						+ "AND "
				...
				Query q = getSession().createSQLQuery(query.toString());
				logger.debug(System.currentTimeMillis()+" :FIring query "+ q.getQueryString());
				List<Object[]> results = q.list();
				logger.debug(System.currentTimeMillis()+" Query retrieved and iteration started");
				if (results != null && !results.isEmpty()) {
					for (Iterator<Object[]> iterator = results.iterator(); iterator
							.hasNext();) {
						Object[] object = (Object[]) iterator.next();
				...
				logger.debug(System.currentTimeMillis()+" Iteration ENDS");
		} catch (HibernateException hbe) {
			throw SessionFactoryUtils.convertHibernateAccessException(hbe);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void deleteECDataByFloor(long floorId) {
        ...
        try {
            con = getSession().connection();
            con.setAutoCommit(false);
            calSt = con.prepareCall("{ call deleteecdatabyfloor("+new Long(floorId)+")}");
            calSt.execute();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
	...
	public void updateShortEnergyStats(EnergyConsumption ec) {
		
		try {
			String hql = "update EnergyConsumption set energyCum = :ecEnergyCum, energyTicks = :ecEnergyTicks, " +
					"price = :ecPrice, cost = :ecCost, powerUsed = :ecPowerUsed, basePowerUsed = :ecBasePowerUsed, " +
					"baseCost = :ecBaseCost, savedCost = :ecSavedCost, savedPowerUsed = :ecSavedPowerUsed, manualSaving " +
					"= :ecManualSaving, occSaving = :ecOccSaving, ambientSaving = :ecAmbientSaving, tuneupSaving = " +
					":ecTuneupSaving, savingType = :ecSavingType, zeroBucket = :ecZeroBucket, powerCalc = :ecPowerCalc " +
					"WHERE captureAt = :ecCaptureAt AND fixtureId = :ecFixtureId";
			Query query = getSession().createQuery(hql);
			query.setLong("ecEnergyCum", ec.getEnergyCum());
			query.setInteger("ecEnergyTicks", ec.getEnergyTicks());
			query.setFloat(":ecPrice", ec.getPrice());
			query.setFloat(":ecCost", ec.getCost());
			query.setBigDecimal("ecPowerUsed", ec.getPowerUsed());
			query.setBigDecimal("ecBasePowerUsed", ec.getBasePowerUsed());
			query.setFloat("ecBaseCost", ec.getBaseCost());
			query.setFloat("ecSavedCost", ec.getSavedCost());
			query.setBigDecimal("ecSavedPowerUsed", ec.getSavedPowerUsed());
			query.setBigDecimal("ecManualSaving", ec.getManualSaving());
			query.setBigDecimal("ecOccSaving", ec.getOccSaving());
			query.setBigDecimal("ecAmbientSaving", ec.getAmbientSaving());
			query.setBigDecimal("ecTuneupSaving", ec.getTuneupSaving());
			query.setShort("ecSavingType", ec.getSavingType());
			query.setShort("ecZeroBucket", ec.getZeroBucket());
			query.setShort("ecPowerCalc", ec.getPowerCalc());
			query.setDate("ecCaptureAt", ec.getCaptureAt());
			query.setLong("ecFixtureId", ec.getFixture().getId());
