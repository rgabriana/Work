...
<spring:url value="/services/org/switch/updateSwitchPlugloads/" var="updateSwitchToPlugloadUrl" scope="request" />
<style>
	.innerContainer{
		/* padding: 10px 20px; */
	}
...
<script type="text/javascript"> 
	...
	var widget_fp = function(nodetype, nodeid) {		
		...
		if ($.browser.msie) {
			...
			FP_data +=  "<param name='flashVars' value='orgType=" + nodetype + "&orgId=" + nodeid + "&switchId=" + "${switchId}" + "&sceneTemplateId="+"${sceneTemplateId}"+"&fixtureVersion=" + "${fixtureVersion}" + "&mode=SWITCH_MODE&enablePlugloadFeature=${enablePlugloadProfileFeature}&modeid='/>";			
			...
			FP_data +=  " flashvars='orgType=" + nodetype + "&orgId=" + nodeid + "&switchId=" + "${switchId}" + "&sceneTemplateId="+"${sceneTemplateId}"+ "&fixtureVersion=" + "${fixtureVersion}" + "&mode=SWITCH_MODE&enablePlugloadFeature=${enablePlugloadProfileFeature}&modeid='/>";
			FP_data +=  "</object>";
		} else {
			FP_data = "<embed id='widget_floorplan' name='widget_floorplan' src='"+plotchartmoduleString+"' pluginspage='http://www.adobe.com/go/getflashplayer'";
			FP_data +=  " height='100%'";
	...
	var loadWidgetFP = function() {
		try{
			...
			getWidgetFloorPlanObj("widget_floorplan").changeLevelForWidget(treenodetype, treenodeid, 'SWITCH_MODE', '', ${switchId});
		}
		catch (ex){
	...
	$(document).ready(function(){
		if(SELECTED_FIXTURES_TO_ASSIGN_SWITCH_GROUPS != undefined && SELECTED_FIXTURES_TO_ASSIGN_SWITCH_GROUPS.length > 0) {
			...
			dataXML = "<fixtures>"+dataXML+"</fixtures>";
		 	$.ajax({
		 		type: 'POST',
		 		url: "${updateSwitchToFixtureUrl}"+${switchId}+"/"+${sceneTemplateId}+"?ts="+new Date().getTime(),
		 		data: dataXML,
		 		async: false,
		...
		if(SELECTED_PLUGLOAD_TO_ASSIGN_SWITCH_GROUPS != undefined && SELECTED_PLUGLOAD_TO_ASSIGN_SWITCH_GROUPS.length > 0) {
			var dataXML = "";
			// Make service call to update the group with plugloads
			$.each(SELECTED_PLUGLOAD_TO_ASSIGN_SWITCH_GROUPS, function(){
				dataXML += "<plugload><id>"+ this.id +"</id></plugload>";
			 });
			dataXML = "<plugloads>"+dataXML+"</plugloads>";
		 	$.ajax({
		 		type: 'POST',
		 		url: "${updateSwitchToPlugloadUrl}"+${switchId}+"?ts="+new Date().getTime(),
		 		data: dataXML,
		 		async: false,
		 		success: function(data){
				},
