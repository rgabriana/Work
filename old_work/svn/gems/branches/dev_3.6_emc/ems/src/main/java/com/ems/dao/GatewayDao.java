...
public class GatewayDao extends BaseDaoHibernate {
    ...
    @SuppressWarnings("unchecked")
   	public List<Gateway> getActiveGatewayDevices(List<Gateway> list) {
        ...
        List<Object[]> results = (List<Object[]>)q.list();
        Map<Long,Integer> map = new HashMap<Long,Integer>();
        Map<Long,Integer> ercMap = new HashMap<Long,Integer>();
        if (results != null && !results.isEmpty()) {
        		for(Object[] a: results) {
        			Long dbGw = Long.parseLong(((BigInteger)a[0]).toString());
            		int totalDevices=Integer.parseInt(((BigInteger)a[1]).toString());
            		String deviceType = a[2].toString();
            		if(deviceType!=null && deviceType.compareToIgnoreCase("WDS")==0)
            		{
            			if(!ercMap.containsKey(dbGw))
            			{
            				ercMap.put(dbGw, totalDevices);
            			}
            		}
        ...
        for(Gateway gw: list) {
        	if(ercMap.size()>0)
        	{
        		gw.setNoOfActiveErc(ercMap.get(gw.getId()));
        	}
        	gw.setNoOfActiveSensors(map.get(gw.getId()));
        }
        return list;
    ...
    public void updateGatewayFloor(List<Gateway> gateways,Long floorId) {
        ...
        for(Gateway gateway : gateways){
        	Object gatewayObject = session.get(Gateway.class, gateway.getId());
        	if(gatewayObject != null){
        		Gateway gw = (Gateway) gatewayObject;
                Object floorObject = session.get(Floor.class, floorId);
        		if(floorObject != null){
	...
	public List<GatewayOutageVO> getGatewayOutageList(String property, Long pid) {
		List<GatewayOutageVO> gwOutageList= new ArrayList<GatewayOutageVO>();
		List<Object> tempGwList=null;
        ...
        Date from = oCalendar.getTime();
        
        String hql = "select g.id, g.name, g.location, g.xaxis, g.yaxis, g.lastConnectivityAt"
					+ " from Gateway g where g.gatewayType!=5 and g.commissioned = 'true' and g.lastConnectivityAt < '" + DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss")+"'";
        ...
        if (property != null && pid != null) {
               switch (orgType) {
               case CAMPUS: {
                     hql = hql + " and g.campusId = :pid ";
                     break;
               }
               case BUILDING: {
                     hql = hql + " and g.buildingId = :pid ";
                     break;
               }
               case FLOOR: {
                     hql = hql + " and g.floor.id = :pid ";
                     break;
               }
               case AREA: {
                     hql = hql + " and g.area.id = :pid ";
                     break;
               }
               default: {
        ...
        try {
			...
			tempGwList = q.list();
			if (tempGwList != null && !tempGwList.isEmpty())
		    {
				Iterator<Object> it = tempGwList.iterator();
	              while (it.hasNext()) {
	            	  Object[] rowResult = (Object[]) it.next();
	            	  GatewayOutageVO gatewayOutageVO = new GatewayOutageVO();
	            	  gatewayOutageVO.setGatewayId((Long)rowResult[0]);
	            	  gatewayOutageVO.setGatewayName((String)rowResult[1]);
	            	  gatewayOutageVO.setLocation((String)rowResult[2]);
	            	  gatewayOutageVO.setXposition((Integer) rowResult[3]);
	            	  gatewayOutageVO.setYposition((Integer) rowResult[4]);
	            	  gatewayOutageVO.setOutageSince((Date)rowResult[5]);
	            	  gatewayOutageVO.setDescription("Gateway is out");
	            	  gwOutageList.add(gatewayOutageVO);
	              }
	        }
		  }catch (Exception e) {
		    e.printStackTrace();
