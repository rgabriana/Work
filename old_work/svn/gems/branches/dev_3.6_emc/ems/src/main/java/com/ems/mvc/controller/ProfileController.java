...
import com.ems.util.ImportExportFileUtil;
...
public class ProfileController {
	...
	public String profileSetting(
			Model model,
			@CookieValue(FacilityCookieHandler.selectedProfileCookie) String cookie) throws EmsValidationException {
		...
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "profileController.jstree_profile_select", cookie);
    ...
    public String updateProfileData(@RequestParam("type") String type, @RequestParam("typeid") Long typeid, @RequestParam("profilename") String profilename,
            @RequestParam("weekdays") String weekdays, @ModelAttribute("profilehandler") ProfileHandler ph, Locale local) throws EmsValidationException {
	    ...
	    nameValMap.put("profileController.phType", type);
	    nameValMap.put("profileController.profileName", profilename);
	    nameValMap.put("profileController.weekdays", weekdays);
	    nameValMap.put("profileController.morningTime", ph.getProfileConfiguration().getMorningTime());
	    nameValMap.put("profileController.dayTime", ph.getProfileConfiguration().getDayTime());
	    nameValMap.put("profileController.eveningTime", ph.getProfileConfiguration().getEveningTime());
	    nameValMap.put("profileController.nightTime", ph.getProfileConfiguration().getNightTime());
		...
		ph1.setColorFixture(ph.getColorFixture());
		ph1.setCircadianOption(ph.getCircadianOption());
		ph1.setPwmBehaviour(ph.getPwmBehaviour());
		ph1.setSunriseTemperatureRatio(ph.getSunriseTemperatureRatio());
		ph1.setNoonTemperatureRatio(ph.getNoonTemperatureRatio());
		ph1.setSunsetTemperatureRatio(ph.getSunsetTemperatureRatio());
		ph1.setNightTemperatureRatio(ph.getNightTemperatureRatio());
...
//		}
    ...
    @ResponseBody
    public String realTimeProfilePush(@RequestParam("type") String type, @RequestParam("typeid") Long fixtureId, Locale local) throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "profileController.phType", type);
	...
	public String profileAddEditSetting(Model model,@RequestParam("groupId") Long groupId,@RequestParam("templateId") long templateId,@RequestParam("type") String type,
			@RequestParam(value="oldProfileName",required=false) String oldProfileName) throws EmsValidationException {
		Groups oGroup=null;
		
		Map<String,Object> nameValMap = new HashMap<String, Object>();
        nameValMap.put("profileController.profileType", type);
        nameValMap.put("profileController.profileName", oldProfileName);
        CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, nameValMap);
		...
		//Add title24 enabled or not on model
		final Title24 title24  = title24Manager.loadTitle24Details();
		if (title24 != null && title24.getCompliance() != null && !StringUtils.isEmpty(title24.getCompliance().getFlag()) && title24.getCompliance().getFlag().equalsIgnoreCase("true")){
			model.addAttribute("TITLE24FLAG", true);
		}else{
			model.addAttribute("TITLE24FLAG", false);
		}
	...
	@RequestMapping(value = "/updateProfileName.ems", method = {
			RequestMethod.POST, RequestMethod.GET })
	@ResponseBody
	public String renameProfile(@ModelAttribute("group") Groups group,@RequestParam("templateId") long templateId,Locale local) throws EmsValidationException {
    	//Need to Update existing profile name
		Groups groupObj = groupManager.getGroupById(group.getId());
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "profileController.groupName", group.getName());
    ...
    public String setVisibilityOfGroups(@RequestParam("selectedTemplateProfiles") String selectedTemplateProfiles) throws EmsValidationException{
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "profileController.selectedTemplateProfiles", selectedTemplateProfiles);
    ...
    public String createNewProfileData(@RequestParam("type") String type, @RequestParam("typeid") Long typeid, @RequestParam("profilename") String profilename,
            @RequestParam("weekdays") String weekdays, @ModelAttribute("profilehandler") ProfileHandler ph, @RequestParam("templateId") long templateId, Locale local) throws EmsValidationException {
	...
	@RequestMapping(value = "/filterProfile.ems", method = {
			RequestMethod.POST, RequestMethod.GET })
	public String filterProfile(Model model, @RequestParam(value = "fixtureAssociatedProfile", required = false) String fixtureAssociatedProfile) throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "profileController.fixtureAssociatedProfile", fixtureAssociatedProfile);
	...
	@ResponseBody
	    public String uploadProfile(HttpServletRequest request,@RequestParam("upload") MultipartFile file,Model model,Locale locale) throws JAXBException, XMLStreamException, IOException, EmsValidationException {
	        String content="";
	        String message="";
	        if(file!=null)
	        {
                ...
                //validate the contents first
                final boolean isValid = ImportExportFileUtil.validateXMLSchema(Constants.XSD_SENSOR, content);
                message ="{\"error\":1, \"message\" : \""
                        + messageSource.getMessage(Constants.ERROR_UPLOAD_PROFILE, null, locale) +
                        "\"}"; 
                if(!isValid){
                	//throw new IllegalArgumentException("Please check the profile format.");
                	return message;
                }
                ProfileHandler phObj = WebServiceUtils.convertPhStringToModel(content);
                //Code fix for ENL-4850 - Need to assign default override profile during import if not exists in the xml
                HashMap<String, String> scMap = systemConfigurationManager.loadAllConfigMap();
                final String sParamPrefix = "default.holiday.";
                Profile profile =  phObj.getOverride5();
                if(profile == null){
                	profile = new Profile();
                	profile.setMinLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.morning.min_level")));
                    profile.setOnLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.morning.on_level")));
                    profile.setMotionDetectDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.morning.motion_detect_duration")));
                    profile.setManualOverrideDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.morning.manual_override_duration")));
                    profile.setMotionSensitivity(Long.valueOf(scMap.get(sParamPrefix+ "profile.morning.motion_sensitivity")));
                    profile.setRampUpTime(Long.valueOf(scMap.get(sParamPrefix+ "profile.morning.ramp_up_time")));
                    profile.setAmbientSensitivity(Integer.valueOf(scMap.get(sParamPrefix+ "profile.morning.ambient_sensitivity")));
                    //phObj.setMorningProfileHoliday(profile);
                    phObj.setOverride5(profile);
                }
              
                profile =  phObj.getOverride6();
                if(profile == null){
                	profile = new Profile();
                	profile.setMinLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.day.min_level")));
                    profile.setOnLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.day.on_level")));
                    profile.setMotionDetectDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.day.motion_detect_duration")));
                    profile.setManualOverrideDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.day.manual_override_duration")));
                    profile.setMotionSensitivity(Long.valueOf(scMap.get(sParamPrefix+ "profile.day.motion_sensitivity")));
                    profile.setRampUpTime(Long.valueOf(scMap.get(sParamPrefix+ "profile.day.ramp_up_time")));
                    profile.setAmbientSensitivity(Integer.valueOf(scMap.get(sParamPrefix+ "profile.day.ambient_sensitivity")));
                    //phObj.setDayProfileHoliday(profile);
                    phObj.setOverride6(profile);
                }
                
                profile =  phObj.getOverride7();
                if(profile == null){
                	profile = new Profile();
                	profile.setMinLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.evening.min_level")));
                    profile.setOnLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.evening.on_level")));
                    profile.setMotionDetectDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.evening.motion_detect_duration")));
                    profile.setManualOverrideDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.evening.manual_override_duration")));
                    profile.setMotionSensitivity(Long.valueOf(scMap.get(sParamPrefix+ "profile.evening.motion_sensitivity")));
                    profile.setRampUpTime(Long.valueOf(scMap.get(sParamPrefix+ "profile.evening.ramp_up_time")));
                    profile.setAmbientSensitivity(Integer.valueOf(scMap.get(sParamPrefix+ "profile.evening.ambient_sensitivity")));
                    //phObj.setEveningProfileHoliday(profile);
                    phObj.setOverride7(profile);
                }
                
                profile =  phObj.getOverride8();
                if(profile == null){
                	profile = new Profile();
                	profile.setMinLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.night.min_level")));
                    profile.setOnLevel(Long.valueOf(scMap.get(sParamPrefix+ "profile.night.on_level")));
                    profile.setMotionDetectDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.night.motion_detect_duration")));
                    profile.setManualOverrideDuration(Long.valueOf(scMap.get(sParamPrefix+ "profile.night.manual_override_duration")));
                    profile.setMotionSensitivity(Long.valueOf(scMap.get(sParamPrefix+ "profile.night.motion_sensitivity")));
                    profile.setRampUpTime(Long.valueOf(scMap.get(sParamPrefix+ "profile.night.ramp_up_time")));
                    profile.setAmbientSensitivity(Integer.valueOf(scMap.get(sParamPrefix+ "profile.night.ambient_sensitivity")));
                    //phObj.setNightProfileHoliday(profile);
                    phObj.setOverride8(profile);
                }
                ...
                CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "profileController.fileName", fileName);
	        ...
	        message ="{\"success\":1, \"message\" : \""
	        		+ messageSource.getMessage(Constants.SUCCESS_UPLOAD_PROFILE, null, locale) +
	        		"\"}"; 
