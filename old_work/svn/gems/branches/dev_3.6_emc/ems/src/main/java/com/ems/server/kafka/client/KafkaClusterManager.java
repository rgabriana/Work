...
public class KafkaClusterManager {
	...
	public static void init() {
		...
		try {
			//TODO This will go away as we will need a way to get KafkaConfig per cluster mostly from the DB
			KafkaConfig config = new KafkaConfig(Constants.KAFKA_PROP_FILE);
			if(logger.isDebugEnabled()) {
	...
	public static KafkaCluster getCluster(String gatewayId) throws KafkaConfigException {
		...
		if(cluster == null) {
			if(clusters.size() == 1) {
				logger.trace("Only 1 Kafka Cluster Configured"); //TODO: We might want to change this
