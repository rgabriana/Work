...
public class GatewayImpl {
  ...
  private Map<String, Long> gwMap = Collections.synchronizedMap(new HashMap<String, Long>());
  private Map<String, Long> gwMacMap = Collections.synchronizedMap(new HashMap<String, Long>());
  ...
  public void addGatewayInfo(String gwIp) {
    ...
    if(ServerMain.getInstance().addGatewayInfo(gw)) {    
      ...
      gwMacMap.put(gw.getMacAddress(), gw.getId());
    }
  ...
  public GatewayInfo getGatewayInfoByMac(String gwMac) {
    
    if(gwMacMap.containsKey(gwMac)) {
      return ServerMain.getInstance().getGatewayInfo(gwMacMap.get(gwMac));
    }
    return null;
    
  } //end of method getGatewayInfo

  public void updateGatewayInfo(Gateway gwNew) {
	  Iterator<GatewayInfo> gwIter = ServerMain.getInstance().getGwMap().values().iterator();
	    GatewayInfo gwInfo = null;
	    Gateway gw = null;
	    while(gwIter.hasNext()) {
	      gwInfo = gwIter.next();
	      gw = gwInfo.getGw();
	      if (gw.getId() !=null && gwNew.getId()!= null && gw.getId().longValue() == gwNew.getId().longValue()) {
	    	  final String oldIp = gw.getIpAddress();
	    	  gwMap.remove(oldIp);
	    	  gwInfo.getGw().setIpAddress(gwNew.getIpAddress());
	    	  gwMap.put(gwNew.getIpAddress(), gwNew.getId());
	    	  
	    	  gwMacMap.remove(gw.getMacAddress());
	    	  gwInfo.getGw().setMacAddress(gwNew.getMacAddress());
	    	  gwMacMap.put(gwNew.getMacAddress(), gwNew.getId());
	    	  logger.info("Updated gwMap from IpAddress "+oldIp+" to " +gwNew.getIpAddress());
	      }
	    }   
  } //end of method getGatewayInfo
  ...
  //@TODO this need to be based on mac map instead of ip map so that both cloud connected and local connected gateways can be checked
...
//			+ gw.getWirelessNetworkId());
  ...
  public void rebootGatewaySystem(Gateway gw) {
      
	    byte[] dataPkt = new byte[1];
	    dataPkt[0] = 2; //rebooting to app2       
	    sendGwPkt(ServerConstants.GATEWAY_SYSTEM_REBOOT_CMD, gw, dataPkt, true);
  } 
  ...
  public void forwardPkt(Gateway gw, byte[] dataPkt) {
      sendGwPkt(ServerConstants.UEM_FORWARD_CMD, gw, dataPkt, false);
  } 
