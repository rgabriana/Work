...
public class MotionGroupManager {
    private static final Logger logger = Logger.getLogger("FixtureLogger");
    ...
    public void deleteMotionGroup(Long id) {
        ...
        //TODO : If any device is going to be added in future in the Motion group it is needed to be removed first before deleting the group.
        gemsGroupPlugloadDao.deleteGemsGroupPlugloadById((motionGroup.getGemsGroup().getId()));
        gemsGroupDao.deleteGemsGroup(motionGroup.getGemsGroup().getId());
        motionGroupDao.removeObject(MotionGroup.class, id);
    }
    
    public void deleteMotionGroupByGemsGroupId(Long gemsGroupId) {
        MotionGroup motionGroup = getMotionGroupByGemsGroupId(gemsGroupId);
        //TODO : If any device is going to be added in future in the Motion group it is needed to be removed first before deleting the group.
        gemsGroupPlugloadDao.deleteGemsGroupPlugloadById(gemsGroupId);
        gemsGroupDao.deleteGemsGroup(gemsGroupId);
        motionGroupDao.removeObject(MotionGroup.class, motionGroup.getId());
    }
    ...
    public boolean updateMotionGroupFixtureDetails(Long gemsGroupId, GemsGroupFixture ggfx, MotionGroupFixtureDetails mgfd) {
    	...
    	if (mgfd_src != null && mgfd != null) {
	    	if(ggfx.getMotionGrpFxDetails() != null){
	    		ggfx.getMotionGrpFxDetails().copy(mgfd);
	    	}
					...
					try {
						...
						DeviceServiceImpl
								...
								.sendMotionGroupConfiguration(
										fixtureArr,
										output.toByteArray(),
										ServerConstants.CMD_MOTION_GRP_APPLY_ACTION,DeviceType.Fixture);
	...
	public boolean sendMotionGroupDetailsToFixtureWithoutEmValues(Long groupId,
			GemsGroupFixture ggf) {
		if ((ggf.getNeedSync() & GemsGroupFixture.SYNC_STATUS_GROUP) == GemsGroupFixture.SYNC_STATUS_GROUP) {
			if (ggf.getUserAction() == GemsGroupFixture.USER_ACTION_DEFAULT
					|| ggf.getUserAction() == GemsGroupFixture.USER_ACTION_MOTION_PARAMS_PUSH) {
				...
				if (mgfd != null) {
					...
					try {
						...
						output.write(mgfd.getAmbientType().byteValue());
						output.write(mgfd.getUseEmValue().byteValue());
						//2 bytes for lo amb level
						output.write((byte)0);
						output.write((byte)0);
						//2 bytes for hi amb level
						output.write((byte)0);
						output.write((byte)0);
						output.write(ServerUtil.intToByteArray(mgfd.getTod()));
						output.write(mgfd.getLightLevel().byteValue());
				...
				if (mgpd != null) {
					...
					try {
						...
						DeviceServiceImpl.getInstance().sendMotionGroupConfiguration(plugloadArr, output.toByteArray(), 
								ServerConstants.CMD_MOTION_GRP_APPLY_ACTION,DeviceType.Plugload);
