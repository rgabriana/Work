...
$(document).ready(function(){
	...
	if(uploadStatus != null && uploadStatus == "false"){
		displayImageUpgradeMessage("${message}", COLOR_FAILURE);
	}
...
function loadWdsImageUploadCombo(){
	...
	UPLOAD_WDS_IMG_COMBO.append($('<option></option>').val("-1").html("Select a ERC image"));
...
//Load Plugload Images drop down
function loadPlugloadImageUploadCombo(){
	UPLOAD_Plugload_IMG_COMBO = $("#plugloadImageId");
	
	UPLOAD_Plugload_IMG_COMBO.empty();
	UPLOAD_Plugload_IMG_COMBO.append($('<option></option>').val("-1").html("Select a Plugload image"));
	<c:forEach items="${plugloadUpgradeimages}" var="plugloadImage">
	UPLOAD_Plugload_IMG_COMBO.append($('<option></option>').val("${plugloadImage}").html("${plugloadImage}"));
	</c:forEach>
}
...
function displayPlugloadUpgradeMessage(Message, Color) {
	$("#image_upload_message").html(Message);
	$("#image_upload_message").css("color", Color);
}
...
function clearPlugloadUpgradeMessage(Message, Color) {
	displayPlugloadUpgradeMessage("", COLOR_DEFAULT);
}
</script>
...
function validateImageUpgradeForm() {
	...
	var fileExtension1 = imageNameArray[imageNameArray.length - 2] + "." + imageNameArray[imageNameArray.length - 1];
	...
	if((fileExtension != "bin") && (fileExtension != "tar") && (fileExtension1 != "tar.xz") && (fileExtension1 != "tar.gz")){
		alert('enLighted application image with only \'.bin\' or \'.tar\' or \'.tar.xz\' or \'.tar.gz\' extension is allowed.\nPlease check the selected filename. \n\nFile name should be of one of the following format: \n[version]_su_app.bin \n[version]_gw_app.bin \n[version]_su_firm.bin \n[version]_gw_firm.bin \n[version]_su_pyc.bin \n[version]_su.bin \n[version]_gw.tar \n[version]_gw.tar.xz \n[version]_gw.tar.gz \n[version]_sw.tar \n[version]_sw.bin \n[version]_pl.bin');
		return false;
	}
	// 2.0 pattern
	if( (fileName.match(/su$/g) == null) && (fileName.match(/gw$/g) == null) && (fileName.match(/cu$/g) == null) && (fileName.match(/sw$/g) == null) && (fileName.match(/pl$/g) == null))  {
		// 1.0 pattern
		if ((fileName.indexOf("su_firm") < 0) 
				...
				&& (fileName.indexOf("_ble") < 0)
				&& (fileName.indexOf("pl") < 0))
		{
			alert('enLighted application image filename should contain \'su.bin\' or \'gw.tar\' or \'gw.tar.xz\' or \'gw.tar.gz\' or \'cu.bin\' or \'sw.bin\'  or \'su_firm\' or \'su_app\' or \'gw_app\' or \'gw_firm\' or \'su_pyc\' or \'pl.bin\' or \'_ble\' .\nPlease check the selected filename. \n\nFile name should be of one of the following format: \n[version]_su_app.bin \n[version]_gw_app.bin \n[version]_su_firm.bin \n[version]_gw_firm.bin \n[version]_su_pyc.bin \n[version]_pl.bin');
			return false;
		}
	}
...
function IsNumeric(sText)
{
	...
	function checksize(obj) { 
		...
		displayImageUpgradeMessage("", COLOR_DEFAULT);
		
		if(obj.files[0].size > IMAGE_FILESIZE_LIMIT) {
			$('#imgName').val('');
			var ImageFileSizeLimitInMB = Math.round(IMAGE_FILESIZE_LIMIT / (1024*1024) );
			displayImageUpgradeMessage("Upload file size should be less than "+ImageFileSizeLimitInMB+" MB.", COLOR_FAILURE);
		}
    }
	...
	<fieldset>
			...
			<form action="saveNewImages.ems" id="firmwareupgrade-register" name="firmwareUpgrade" method="post" enctype="multipart/form-data" onsubmit="javascript: return validateImageUpgradeForm();">
				<input type="file" name="upload" id="imgName" onChange="checksize(this);" />
			...
			<c:if test="${enablePlugloadFeature == 'true'}"> 
			<select id="plugloadImageId"></select>
			 </c:if>
			<button id='imgUpgradeSubmitBtn' onclick='javascript: parent.validateUpgradeForm(3,$("#gwImageId").val(),$("#fximageId").val(),$("#wdsimageId").val(),$("#plugloadImageId").val());'>
