...
import com.communicator.manager.TunnelingManager;
...
public class CommunicatorEntryPoint {
	...
	public static long sleepTime = 300 * 1000;
	...
	private static void connectToCloud() {
		...
		if ("1".equals(cloud_enable)) {
			...
			try {
				if(!sched.checkExists(new JobKey("DataPrepJob", sched.getSchedulerName()))) {
			...
			try {
				if(!sched.checkExists(new JobKey("CleanupJob", sched.getSchedulerName()))) {
			...
			cloudStart = true;
			TunnelingManager tunnelingManager = (TunnelingManager)springContext.getBean("tunnelingManager");
