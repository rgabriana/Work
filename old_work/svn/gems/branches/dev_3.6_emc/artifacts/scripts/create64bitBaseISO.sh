...
if [  -f $UBUISO ];
then 
	if verifyChecksum $HOME/download/ubuntu-14.04.3-server-amd64.iso "9e5fecc94b3925bededed0fdca1bd417";
	then
		echo "md5sums match not downloading download"
	else
		cd $HOME/download/
		rm ubuntu-14.04.3-server-amd64.iso
...
echo "$PASS" | sudo -S mount -o loop $HOME/download/ubuntu-14.04.3-server-amd64.iso iso
...
sudo mkdir -p newiso/pool/extras

##JAVA-SECUTIRY PATCHED VERSION
#java_debian_file=$(find $HOME/download/ -type f ! -path "*/.svn**" -path "*oracle-java*amd64.deb" | sort -nr | head -n 1)
java_debian_file=$(ls -t `find $HOME/download/ -type f ! -path "*/.svn**" -path "*oracle-java*amd64.deb"` | head -n 1)
#echo coping oracle-java8-jdk_8u60_amd64.deb to /var/cache/apt/archives/
#sudo cp $HOME/download/oracle-java8-jdk_8u60_amd64.deb /var/cache/apt/archives/
...
sudo cp $TOMCAT_DEBIAN/tomcat8_amd64.deb newiso/preseed/tomcat8.deb
...
sudo mkdir -p newiso/dists/stable/extras/binary-amd64
cd newiso
sudo apt-ftparchive packages ./pool/extras/ > ../Packages
cd ..
sudo gzip -c Packages | tee Packages.gz > /dev/null
sudo mv Packages* newiso/dists/stable/extras/binary-amd64/
cd newiso
...
sudo mkisofs -J -l -b isolinux/isolinux.bin -no-emul-boot -boot-load-size 4 -boot-info-table -z -iso-level 4 -c isolinux/isolinux.cat -o ./Enlighted_EM_Server1404_Base_64Bit.iso -joliet-long newiso/
GEMS_REV=$(svn info | grep 'Last Changed Rev' | cut -d ' ' -f4)
dt="$((`date '+%Y%m%d%H%M%S'`))"
sudo mv Enlighted_EM_Server1404_Base_64Bit.iso ISO_Build_Dir/ENL_release_1404_64Bit_${GEMS_REV}_${dt}.iso

# clean up
sudo umount iso
sudo rm -rf iso
sudo rm -rf newiso
sudo rm -rf /var/cache/apt/archives/oracle-java8-jdk_8u60_amd64.deb
