                                ...
                                for (Iterator<EMProfile> emProfileiterator = emProfiles.iterator(); emProfileiterator.hasNext();) {
                                    ...
                                    if(emProfile.getProfileNo() > 16){
	                                    ...
	                                    if(emProfileMapping==null)
	                                    {
	                                        ...
	                                        // This use case is possible when Profile is created from already derived profile. e.g. Open Office Max 25 derived from Open Office Max 50
	                                        if(derivedGrp==null)
	                                        {
	                                        ...
	                                        ProfileHandler ph1 = null;
	                                        //If derivedGrp is still null means this is add on default profile which will need to be inserted into profile_groups table 
	                                        // along with that will have to put entry in to em_profile_mapping table.
                                            if(derivedGrp==null && emProfile.getDerivedFromGroup().intValue()==-1)
                                            {
                                            	ph1 = profileManager.createProfile(groupkey,ServerConstants.DEFAULT_PROFILE_GID,true);
                                            }else
                                            {
                                            	ph1 = profileManager.createProfile(groupkey, derivedGrp.getId().intValue(),true);
                                            }
	                                        ...
	                                        if(emProfile.getDerivedFromGroup().intValue()==-1)
	                                ...
	                                if(totalProfileSavedCount==(emProfiles.size()-16))
	                                {
            ...
            for (Iterator<EmProfileMapping> emProfileMappingiterator = emProfileMapping.iterator(); emProfileMappingiterator.hasNext();) {
                ...
                if(facilityEmMapping!=null && facilityEmMapping.size()>0)
                {
                    try
                    {
                        ...
                        // Name after # will be Mac Id of the actual EM for which the profile is being sent
                        emProfile.setDerivedFromGroup(uemDBProfile.getDerivedFromGroup().getId());
                        emProfile.setName(uemDBProfile.getName()+"#"+emInstance.getName());
    ...
    public EMProfile PushNewProfileToEM(Long groupId, Long floorId)
    {
        ...
        if(floorId!=null && groupId!=null)
        {
            ...
            if(emProfileMapping==null)
            {
                ...
                if(newProfileNoForEM==null)
                {
                	 logger.error("Cloud not fetch max profile_no from EM. " + emInstance.getMacId());
                     return null;
                }
                if(newProfileNoForEM==0)
                {
                    logger.error("Max Profile creation limit of 255 reached on EM "+ emInstance.getMacId() + ". Maximum 255 Profiles can be created per EM Instance basis");
                    return null;
                }
