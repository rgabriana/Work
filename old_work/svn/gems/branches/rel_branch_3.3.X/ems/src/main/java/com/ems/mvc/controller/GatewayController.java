...
public class GatewayController {
	...
	public String loadGatwayObject(Model model,
		...
		if (dhcpConfig != null) {
			if("true".equals(dhcpConfig.getValue())){
...
//			model.addAttribute("gateways", (oArea.getFloor() != null ? (gatewayManager.loadFloorGateways(oArea.getFloor().getId())) : null));
	...
	public String commissionGateway(
		...
		case FLOOR: {
			...
			commissionedGatewayList = gatewayManager.loadCommissionedGateways();
			
			ArrayList<Integer> commissionedGatewayWirelessNetworkIdList = new ArrayList<Integer>();
			if(!ArgumentUtils.isNullOrEmpty(commissionedGatewayList)){
				for(Gateway gateway :commissionedGatewayList){
					commissionedGatewayWirelessNetworkIdList.add(gateway.getWirelessNetworkId());
				}
			}
			...
			if (gatewayId == null || gatewayId == 0) {
				...
				for(Gateway gateway:gatewayList){
					...
					gatewayWirelessNetworkIdVO.setWirelessNetworkId(i);
