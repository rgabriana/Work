...
//						"User not Found");
	...
	private boolean isToBypassRequest(Authentication authentication, User user) {
		...
		if(isForgotPasswordLoginRequest){
			...
			tempPassword = authentication.getCredentials().toString();
			try {
				tempPassword = CryptographyUtil.getDecryptedString(Constants.EM_PVT_FILE, tempPassword);
			} catch (Exception e) {
				e.printStackTrace();
				throw new LoginAttemptsException("Bad credentials. Please contact Administrator.");
			}
			String storedPassword = user.getForgotPasswordIdentifier()+  ServerMain.getInstance().getMacAddressByInterfaceName(Constants.NETWK_INTERFACE_ETH0);
			/*try {
				if("admin".equals(user.getEmail())) {
					storedPassword = PasswordUtils.extractPassword()[0];
					tempPassword = PasswordUtils.generateDigest(tempPassword);
				}else if(RoleType.Admin == user.getRole().getRoleType()){
					storedPassword = PasswordUtils.extractAdminTypeUserPassword(user.getEmail())[0];
					tempPassword = PasswordUtils.generateAdminTypeUserDigest(tempPassword,user.getEmail());
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				throw new LoginAttemptsException("Some Error has occured. Please contact Administrator.");
			} */
