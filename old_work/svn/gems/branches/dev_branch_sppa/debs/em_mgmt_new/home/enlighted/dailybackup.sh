...
DAILY_BACKUP_TAR="${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}.temptargz"
...
if [ -d "${outputdir}" ]
then
    cp $DAILY_BACKUP_TAR $outputdir/"${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}_$today.tar.gz"
    echo "*****copied ${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}_$today.tar.gz onto ${outputdir}" >>$LOG_DIR/dailybackup.log   
else
    ...
    cp $DAILY_BACKUP_TAR $BACKUP_DIR/"${DAILY_BACKUP_PREFIX}_${BACKUP_VERSION}_$today.tar.gz"
fi
