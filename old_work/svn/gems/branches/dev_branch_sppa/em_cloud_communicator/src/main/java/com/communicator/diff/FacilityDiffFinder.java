...
import com.communicator.model.vo.EnergyConsumptionVO;
...
public class FacilityDiffFinder implements IFinder {
	...
	public void getChangedData(List<CompanyVO> company, List<CampusVO> campus,
			...
			List<FixtureVO> fixture, List<GatewayVO> gateway , List<EnergyConsumptionVO> energy) {
			...
			try {
				...
				//Setting energy consumption Id for later use in getcloudEnergydata sql procedure. 
				pr = rt.exec("tail -1 " + dataPath + "final_cloud_energy_consumption");
				pr.waitFor();
				br = new BufferedReader(new InputStreamReader(
						pr.getInputStream()));
				String lastId  = null ;
				...
				//System.out.println(new Date().getTime());
				//System.out.println("Putting in property file");
				Properties prop = new Properties() ;
				prop.setProperty("lastEnergyConsumptionId", lastId);
				prop.store(new FileOutputStream("/var/lib/tomcat6/Enlighted/communicator.properties"),null) ;
