...
//				fixture.setPushProfile(true);	
		...
		if(packet.length > index + 1) {
			...
			logger.info(fixture.getId() + ": groups checksum -- " + groupsChecksum);
...
//				fixture.getId(), statsDate));
		...
		if(avgPower > ((compBasePowerUsed * 1.2))) {
		  logger.error(fixture.getId() + ": Fixture might be configured with wrong ballast(" + 
		      avgPower + ")");
		  //raise an event
...
//				+ (System.currentTimeMillis() - startTime));
		...
		if(savedPower > 0) {
		  ...
		  case ServerConstants.STATS_SAVING_TUNEUP:
				adjustTaskTunedSavings(fixture, ec, savedPower, avgVolts,
						savingType, lastVoltsReported, motionBitsLong);
			break;
	...
	private void adjustTaskTunedSavings(Fixture fixture, EnergyConsumption ec, double savedPower, 
	  ...
	  try {	
		...
		logger.debug(fixture.getId() + ": (" + avgVolts + " <= " + maxOnLevel + ")");
	      ...
	      if(savingType == ServerConstants.STATS_SAVING_OCC || (motionBits == 0 && 
		  ((lastVolts >= (maxOnLevel - 1)) || (lastVolts <= (minOnLevel + 1))))) {
