...
function startValidation(){
	...
	if(locatorDeviceName == null || locatorDeviceName == ""){
		displayLocatorDeviceLabelMessage("Please enter a locator Device Name", COLOR_FAILURE);
		return;
	}
...
function addLocatorDevice(){
	$.ajax({
		data: "",
		type: "POST",
		url: "${addLocatorDeviceUrl}"+$('#name').val()+"/locatorDeviceType/"+$('#locatorDeviceType').val().replace(/ /g,"_")+"/floorId/"+$('#floorId').val()+"/xaxis/"+Math.ceil(LOCATOR_DEVICE_XAXIS)+"/yaxis/"+Math.ceil(LOCATOR_DEVICE_YAXIS),
		...
		error: function(){
			displayLocatorDeviceLabelMessage("Locator Device is not created", COLOR_FAILURE);
		},
		dataType:"json"
	});
}
...
function updateLocatorDevice(){
	$.ajax({
		...
		error: function(){
			displayLocatorDeviceLabelMessage("Error.Locator Device is not created", COLOR_FAILURE);
		},
		dataType:"json"
	});
}
		...
		success: function(data){
			if(data == null){
				...
				if(LOCATOR_DEVICE_ID != data.id){
					displayLocatorDeviceLabelMessage("Locator Device with the name already exists", COLOR_FAILURE);
				}else{
					updateLocatorDevice();
				}
			}
