...
<div id="locatorDevices-list-topPanel" style="background: #fff">
	<c:if test="${page == 'floor'}">
		<div style="float: right;"><button id="createLocatorDevice" onclick="javascript:parent.parent.showLocatorDeviceForm();">Add</button></div>	
	</c:if>
...
<spring:url value="/services/org/locatordevice/delete/" var="deleteLocatorDeviceUrl" scope="request" />
	$(document).ready(function() {
		...
		jQuery("#locatorDevicesTable").jqGrid({
			...
			hoverrows: false,
			forceFit: true,
		   	colNames:['Name', 'Type','Action'],
		   	colModel:[
		   		{name:'name', index:'name', sorttype:"string", width:"40%"},
		   		{name:'locatorDeviceType', index:'locatorDeviceType',width:"40%"},
		...
		<c:forEach items="${locatordevices}" var="locatordevice">
			...
			localData.name = "${locatordevice.name}";
