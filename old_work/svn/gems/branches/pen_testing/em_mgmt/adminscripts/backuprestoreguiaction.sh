...
export SPPA_ENABLE_SQL="/home/enlighted/sppa.sql"
...
backupfunction() {
	...
	sh /home/enlighted/scripts/dailyconfigbackup.sh >> $LOG_DIR/backuprestore_error.log
	echo "***FINISHED with Configuration Files Backup***********" >> $LOG_DIR/backuprestore_error.log
	configBackupFile=`echo $(find /opt/enLighted/config_backup/ENL_all_config_backup*)`
	adrBackupFile=`echo $(find /opt/enLighted/adr/DB/ADR_db_backup)`
...
restore() {
    ...
    then
        ...
        then
        	psql -h $POSTGRESHOST -U $POSTGRESUSER -d $POSTGRESDATABASE < /var/www/em_mgmt/adminscripts/cloud_backup_before_restore.sql >> $LOG_DIR/backuprestore_error.log

            gunzip -c $backupfile | psql -h $POSTGRESHOST -U $POSTGRESUSER -d $POSTGRESDATABASE >> $LOG_DIR/backuprestore_error.log

            restoreStatus=$?

            psql -h $POSTGRESHOST -U $POSTGRESUSER -d $POSTGRESDATABASE < /var/www/em_mgmt/adminscripts/cloud_backup_after_restore.sql >> $LOG_DIR/backuprestore_error.log
        else
...
#	fi
	...
	then
				...
				sudo tar -xf $configBackupFileToRestore -C /
