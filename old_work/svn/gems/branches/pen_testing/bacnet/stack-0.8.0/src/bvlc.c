...
{       /* number of milliseconds to wait for a packet */
...
#ifdef ENLIGHTED_INC
    if (elf_get_config(CFG_IGNORE_OWN_BCAST_PACKETS) &&
        (sin.sin_addr.s_addr == bip_get_addr()))
    {
        /* Ignore own broadcast packets. */
        return 0;
    }
#endif /* ENLIGHTED_INC */
	...
	// todo 4 remove this breakpoint trap when done
	if (npdu_len <= 4 )
	{
		panic("why?"); // todo 4 diag bit 00016 still trips this up !!
		return 0 ;
	}
