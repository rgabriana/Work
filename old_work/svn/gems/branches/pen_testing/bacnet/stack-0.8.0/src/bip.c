...
#ifdef ENLIGHTED_INC_todo // - resolve bbmd with PGE, older versions... todonext9
    char bbmd_peer_ip[16];

    memset(&bbmd_peer_ip, 0, sizeof(bbmd_peer_ip));
    strcpy(bbmd_peer_ip, elf_get_db_bbmd_peer_ip_address_config());
    if (bbmd_peer_ip[0] != 0)
    {
        log_printf(LOG_INFO, "Sending to BBMD Peer IP address %s",
                   bbmd_peer_ip);
        bip_dest.sin_addr.s_addr = inet_addr(bbmd_peer_ip);
    }
    else
    {
        bip_dest.sin_addr.s_addr = address.s_addr;
        log_printf(LOG_INFO, "Sending to IP address %08x",
