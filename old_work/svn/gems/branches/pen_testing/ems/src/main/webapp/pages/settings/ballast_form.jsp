...
#addBallastTable .invalidField{border: 1px solid red; border-radius: 3px 3px 3px 3px; padding: 1.5px;}
</style>
...
function validateBallastForm(){
	...
	if ( $.trim($('#ballastFactor').val()) == ''){
		$("#ballastFactor").addClass("invalidField");
	...
	if ( $.trim($('#ballastWattage').val()) == ''){
		$("#ballastWattage").addClass("invalidField");
	...
	if ( $.trim($('#ballastManufacturer').val()) == ''){
		$("#ballastManufacturer").addClass("invalidField");
	...
	if ( $.trim($('#ballastBaselineLoad').val()) != ''){
		if(!IsDoubleValue($.trim($('#ballastBaselineLoad').val()))){
			$("#ballastBaselineLoad").addClass("invalidField");
	...
	var ballastName = $("#ballastName").val();
	var regExpStr = /^[A-Za-z0-9\s\-\_\/]+$/i;
	if(ballastName !=""){
		if(regExpStr.test(ballastName) == false) {
			displayBallastLabelMessage("Ballast Name is invalid", COLOR_FAILURE);
		   	$("#ballastName").addClass("invalidField");
		   	return false;   	
		}
	}
	
	var ballastLampType = $("#ballastLampType").val();
	if(ballastLampType !=""){
		if(regExpStr.test(ballastLampType) == false) {
			displayBallastLabelMessage("Ballast Lamp Type is invalid", COLOR_FAILURE);
		   	$("#ballastLampType").addClass("invalidField");
		   	return false;
		}
	}
	
	var ballastManufacturer = $("#ballastManufacturer").val();
	if(ballastManufacturer !=""){
		if(regExpStr.test(ballastManufacturer) == false) {
			displayBallastLabelMessage("Ballast Manufacturer is invalid", COLOR_FAILURE);
		   	$("#ballastManufacturer").addClass("invalidField");
		   	return false;
		}
	}
	
	var ballastDisplayLabel = $("#ballastDisplayLabel").val();
	var regExpString = /^[A-Za-z0-9\s\_\-\/\(\)\,\;]+$/i;
	if(ballastDisplayLabel !=""){
		if(regExpString.test(ballastDisplayLabel) == false) {
			displayBallastLabelMessage("Ballast Display Label is invalid", COLOR_FAILURE);
		   	$("#ballastDisplayLabel").addClass("invalidField");
		   	return false;
		}
	}
...
<table id="addBallastTable">
	<tr>
		<th >Item Number:</th>
		<td ><input id="itemNum" name="itemNum" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Ballast Name:</th>
		<td ><input id="ballastName" name="ballastName" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Input Voltage:</th>
		<td ><input id="ballastInputVoltage" name="ballastInputVoltage" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Lamp Type:</th>
		<td ><input id="ballastLampType" name="ballastLampType" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >No Of Lamps:</th>
	...
	<tr>
		...
		<td ><input id="ballastFactor" name="ballastFactor" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	
	<tr>
		<th >Wattage:</th>
		<td ><input id="ballastWattage" name="ballastWattage" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Ballast Manufacturer:</th>
		<td ><input id="ballastManufacturer" name="ballastManufacturer" onkeypress="clearMessage(this.id);" />
		</td>
	</tr>
	<tr>
		<th>Baseline Load:</th>
		<td ><input id="ballastBaselineLoad" name="ballastBaselineLoad" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th>Display Label:</th>
		<td ><input id="ballastDisplayLabel" name="ballastDisplayLabel" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th><span></span></th>
