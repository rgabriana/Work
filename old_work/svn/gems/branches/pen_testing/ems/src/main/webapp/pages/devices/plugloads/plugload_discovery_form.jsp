...
<spring:url value="/services/org/gateway/du/updatenoofplugloads/" var="updateNoOfPlugloadsUrl" scope="request" />
...
<title>Discover Plugloads</title>
...
function StartBtnHandler(){
	...
	if(1*noOfInstalledPl <= 1*noOfDiscoveredPl){
		displayPlDiscoveryMessage("No. of installed plugloads should not be less than or equal to no of discovered plugloads", COLOR_FAILURE);
		return;
	}
	
	if(noOfInstalledPl > 99999){
		displayPlDiscoveryMessage("No. of installed plugloads should be less than or equal to 99999", COLOR_FAILURE);
		return;		
	}
...
function updateGatewayNoOfSensors(){
	...
	$.ajax({
		url: "${updateNoOfPlugloadsUrl}gateway/"+gatewayId+"/noofplugloads/"+noOfSensors+"?ts="+new Date().getTime(),
		success: function(data){
...
function cancelDiscovery(){
	if (iDiscoveryInProgress == 0) {
		var choice = confirm("Are you sure you wish to cancel Plugload Discovery?");
		if(choice == true){
			stopPlDiscoveryTimer();
			reloadplugloadsFrame();
			$.ajax({
