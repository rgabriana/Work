...
<script type="text/javascript">
		...
		function setBgMessage(msg, color){
    		$("#messageDiv").css("color", color);
    		$("#messageDiv").html(msg);
    	}
		
		function isBgFormValid(){
			$("#messageDiv").html("");
			newBgSwGrpName = $("#bgsgname").val();
    		
    		var newMgrpNameregExpStr = /^[A-Za-z0-9\_\.\s\@\-\#]+$/i;
    		
    		if($.trim($("#bgsgname").val())==""){
    			setBgMessage("Please enter Switch name", "red");
    			return false;
    		}
    		
    		if(newBgSwGrpName!=""){
    			if(newMgrpNameregExpStr.test(newBgSwGrpName) == false) {
    				setBgMessage("Switch name is invalid", "red");
    			   	return false;   	
    			}
    		}
			
			newBgMgrpName = $("#bgmgname").val();
			var newMgrpNameregExpStr = /^[A-Za-z0-9\_\.\s\@\-\#]+$/i;
    		var bgmgroupReq = $('input[name="bgmgroup"]:checked').val();
			
			if(bgmgroupReq!=null && bgmgroupReq == 1){
				if($.trim($("#bgmgname").val())==""){
	    			setBgMessage("Please enter Motion Group name", "red");
	    			return false;
	    		}
			}
			
    		if(newBgMgrpName!=""){
    			if(newMgrpNameregExpStr.test(newBgMgrpName) == false) {
    				setBgMessage("Motion Group Name is invalid", "red");
    			   	return false;   	
    			}
    		}
    		return true;
	...
	<table style="width: 100%;">
		...
		<tr>
			...
			<td style="width: 20%;">
				<table>
					<tr>
						<td><label style="font-weight: bold">Apply fixture profile:</label></td>
					...
					<tr>
						<td><label style="font-weight: bold;width:30%!important;">Apply fixture profile:</label></td>
						<td>
							<!-- 
						...
						--> <select style="width:70%!important;" name="bgprofiles" id="bgprofiles">
			...
			<td style="width: 20%;">
				<table>
					<tr>
						<td><label style="font-weight: bold;">Apply plugload profile:</label></td>
						<td><input type="hidden" name="bgapplyplugloadprofile"
							id="bgapplyplugloadprofile" value="0" /><input type="checkbox"
							name="bgapplyplugloadprofile" id="bgapplyplugloadprofile" checked="checked"
							value="1" /></td>
					</tr>
					<tr>
						<td><label style="font-weight: bold;width:30%!important;">Apply plugload profile:</label></td>
						<td>
							<!-- 
						<select name="bgprofiles" id="bgprofiles">
								<c:forEach var="group" items="${groups}">
						...
						--> <select style="width:70%!important;" name="bgplugloadprofiles" id="bgplugloadprofiles">
								<c:forEach items="${plugloadProfileTreeHierarchy.treeNodeList}"
									var="template">
									<optgroup label="${template.name}">
										<c:forEach items="${template.treeNodeList}" var="profile">
											<option value="${profile.nodeId}">${profile.name}</option>
