...
<script type="text/javascript"> 
	...
	var widget_fp = function(nodetype, nodeid) {		
		...
		if ($.browser.msie) {
			...
			FP_data +=  "<param name='flashVars' value='orgType=" + nodetype + "&orgId=" + nodeid + "&switchId=" + "${switchId}" + "&fixtureVersion=" + "${fixtureVersion}" + "&mode=SWITCH_MODE&enablePlugloadFeature=${enablePlugloadProfileFeature}&modeid='/>";
			...
			FP_data +=  " flashvars='orgType=" + nodetype + "&orgId=" + nodeid + "&switchId=" + "${switchId}" + "&fixtureVersion=" + "${fixtureVersion}" + "&mode=SWITCH_MODE&enablePlugloadFeature=${enablePlugloadProfileFeature}&modeid='/>";
			FP_data +=  "</object>";
		} else {
			FP_data = "<embed id='widget_floorplan' name='widget_floorplan' src='"+plotchartmoduleString+"' pluginspage='http://www.adobe.com/go/getflashplayer'";
			FP_data +=  " height='100%'";
