...
#addBulbTable .invalidField{border: 1px solid red; border-radius: 3px 3px 3px 3px; padding: 1.5px;}
</style>
...
function buildXML()
{	
	var bulbname = $("#bulbname").val();
	var regExpStr = /^[A-Za-z0-9\s\-\_\/]+$/i;
	if ( bulbname.trim() == ''){
		$("#bulbname").addClass("invalidField");
		displayBulbLabelMessage("Bulb Name Field should not be empty", COLOR_FAILURE);
		return false;
	} else {
		if(regExpStr.test(bulbname) == false) {
			displayBulbLabelMessage("Bulb Name is invalid", COLOR_FAILURE);
		   	$("#bulbname").addClass("invalidField");
		   	return false;   	
		}
	}
	var bulbmanufacturer = $("#bulbmanufacturer").val();
	if ( bulbmanufacturer.trim() == ''){
		$("#bulbmanufacturer").addClass("invalidField");
		displayBulbLabelMessage("Manufacturer Field should not be empty", COLOR_FAILURE);
		return false;
	} else {
		if(regExpStr.test(bulbmanufacturer) == false) {
			displayBulbLabelMessage("Bulb Manufacturer is invalid", COLOR_FAILURE);
		   	$("#bulbmanufacturer").addClass("invalidField");
		   	return false;   	
		}
	}
	...
	if ( bulbtype.trim() == ''){
		$("#bulbtype").addClass("invalidField");
		displayBulbLabelMessage("Type Field should not be empty", COLOR_FAILURE);
		return false;
	} else {
		if(regExpStr.test(bulbtype) == false) {
			displayBulbLabelMessage("Bulb Type is invalid", COLOR_FAILURE);
		   	$("#bulbtype").addClass("invalidField");
		   	return false;   	
		}
	}
	...
	if ( !IsBulbNumericValue($('#bulbinitiallumens').val().trim())){
		$("#bulbinitiallumens").addClass("invalidField");
	...
	if ( !IsBulbNumericValue($('#bulblifeinsstart').val().trim())){
		$("#bulblifeinsstart").addClass("invalidField");
...
<table id="addBulbTable">
	<tr>
		<th >Bulb : </th>
		<td ><input id="bulbname" name="bulbname" onkeypress="clearMessage(this.id);" />
		</td>
	</tr>
	<tr>
		<th >Manufacturer :</th>
		<td ><input id="bulbmanufacturer" name="bulbmanufacturer" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Type :</th>
		<td ><input id="bulbtype" name="bulbtype" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th>Initial Lumens :</th>
		<td ><input id="bulbinitiallumens" name="bulbinitiallumens" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Design Lumens :</th>
		<td ><input id="bulbdesignlumens" name="bulbdesignlumens" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	
	<tr>
		<th >Energy : </th>
		<td ><input id="bulbenergy" name="bulbenergy" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	
	<tr>
		<th >Life Ins Start : </th>
		<td ><input id="bulblifeinsstart" name="bulblifeinsstart" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th >Life Prog Start :</th>
		<td ><input id="bulblifeprogstart" name="bulblifeprogstart" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th>Diameter : </th>
		<td ><input id="bulbdiameter" name="bulbdiameter" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th>Length : </th>
		<td ><input id="bulblength" name="bulblength" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th>Cri : </th>
		<td ><input id="bulbcri" name="bulbcri" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>	
	<tr>
		<th>Color Temperature : </th>
		<td ><input id="bulbcolortemp" name="bulbcolortemp" onkeypress="clearMessage(this.id);"/>
		</td>
	</tr>
	<tr>
		<th><span></span></th>
