...
public class CommonUtils {
    ...
    public  Boolean isRestrictedMigrationReady(String mac_id) {
		CloudHttpResponse response = cloudConnectionTemplate.executeGet(Constants.GET_LATEST_MIGRATION_STATUS+mac_id,Constants.ECLOUD_IP);
		String state= response.getResponse() ;
		if(state != null && DatabaseState.RESTRICTED_REMIGRATION_READY.getName().equals(state)) {
			return true;
		}
		else {
			return false;
