...
public class CommonUtils {
	...
	public static synchronized int getRandomPort() {
		return getNextFreePortInternal();
	}

	private static int getNextFreePortInternal() {
		...
		try {
			...
			logger.info("Error while getting ssh port retrying after increment" + e.getMessage() , e);
			if(NEXT_FREE_PORT < MAX_PORT_VALUE) {
				NEXT_FREE_PORT++;
				getNextFreePortInternal();
			}
			else port = -1;
		}
		return port;
