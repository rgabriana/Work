		...
		public function calculateCO2Counts(_dataProvider:ArrayCollection):void
		{
			...
			if(_dataProvider!=null && _dataProvider.length>0)
			{
				...
				for(var j:int=0; j<_dataProvider.length; j++)
				{
					...
					object.tasktuneupsaving = checkNaN(taskTuneupSaving * Constants.CARBON_FACTOR);
					object.ambientsaving =checkNaN(ambientSaving * Constants.CARBON_FACTOR);
					object.occsaving = checkNaN(occSaving *Constants.CARBON_FACTOR);
					object.powerused = checkNaN(powerUsed * Constants.CARBON_FACTOR);
					object.avgload=_dataProvider[j].avgload;
