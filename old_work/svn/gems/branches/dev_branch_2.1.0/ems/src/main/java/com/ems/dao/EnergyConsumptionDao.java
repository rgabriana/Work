...
public class EnergyConsumptionDao extends BaseDaoHibernate {
    ...
    public List loadMeterData(String columnName, String id) {
        try {
            ...
            String hsql = "select totalConsum,total,round(totalConsum*100/(total * 60)) as percentage,price,cost,baseTotalConsum "
                    ...
                    + "where ec.fixture_id in (select id from fixture where "
                    + columnName
                    + " ="
                    + id
                    + ")  "
    ...
    public List loadMeterDataWithDateRange(String columnName, String id, Date from, Date to) {
        ...
        try {
            ...
            hsql += "savedCost " + "from(select " + "sum(ec.power_used) as totalConsum,"
                    ...
                    + " ec where " + "ec.fixture_id in (select id from fixture where " + columnName + " =" + id + ")";
            if (tableName.equals("energy_consumption"))
    ...
    private List loadCurrentMeterData(String columnName, String id, Date from, Date to) {
        ...
        try {
            ...
            hsql += "select " + "sum(lastPowerUsed) as totalConsum," + "sum(lastSavedPowerUsed) as totalPowerSaved,"
                    ...
                    + "from energy_consumption where fixture_id in " + "(select id from fixture ";
    ...
    private List loadPeakAndMinLoadQuery(String tableName, String columnName, String id, Date from, Date to) {
      ...
      String sql = "select avg(ecl.load) as avgLoad, max(ecl.load) as peakLoad, min(ecl.load) as minLoad from "
              ...
              + " where fixture_id in (select id from fixture where " + columnName + "=" + id + ")";
      if (tableName.equals("energy_consumption"))
    ...
    private List loadHierarchyPeakAndMinLoadQuery(String tableName, String columnName, String id, Date from, 
	Date to) {
      
      StringBuffer sqlBuffer = new StringBuffer();
      sqlBuffer.append("SELECT ");
      if(tableName.equals("energy_consumption_daily") && (columnName.equals("id") || columnName.equals("fixture_id"))) {
	tableName = "energy_consumption_hourly"; //for fixture level, get it from hourly table.
	columnName = "fixture_id";	
      } 
      if(tableName.equals("energy_consumption_hourly")) {
	sqlBuffer.append("AVG(power_used), MAX(max_load), MIN(min_load) FROM ");
      } else if (tableName.contains("daily")) {		 
	sqlBuffer.append("AVG(avg_load), MAX(peak_load), MIN(min_load) FROM ");
      } else if(tableName.contains("hourly")) {	
	sqlBuffer.append("AVG(avg_load), MAX(avg_load), MIN(avg_load) FROM ");
      } 
	...
	sqlBuffer.append(" and ");
      ...
      sqlBuffer.append(" capture_At <= '");
      sqlBuffer.append(DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"));
      sqlBuffer.append("' and capture_at > '");
      sqlBuffer.append(DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"));
      sqlBuffer.append("'");     
      logger.debug("peakminquery - " + sqlBuffer.toString());
      Query query = getSession().createSQLQuery(sqlBuffer.toString());
      return query.list();
        
    } //end of method loadPeakAndMinLoadQuery
    ...
    private List loadCurrentLoad(String tableName, String columnName, String id, Date from, Date to) {
        String sql = "select sum(fixtureCurrentLoad) as currentLoad, sum(fixtureCurrentBaseLoad) as currentBaseLoad from (select "
                ...
                + "fixture_id in (select id from fixture where "
                + columnName
                + " ="
                + id
                + ") "
    ...
    public List<MeterRecord> loadMeterData(String columnName, String id, Date from, Date to) {
        ...
        try {
            ...
            if (columnName.equals("company_id")) {
                ...
                hsql += " ec.fixture_id in (select id from fixture where " + columnName + " =" + id + ") and";
            }
            if (tableName.equals("energy_consumption"))
    ...
    private boolean isHierarchyLevel(String columnName) {
      
      if(columnName.equals("company_id") || columnName.equals("campus_id") || columnName.equals("building_id")
	  || columnName.equals("bld_id") || columnName.equals("floor_id") || columnName.equals("id")) {
	return true;
      }
      return false;
    ...
    public List<DashboardRecord> loadDaySummary(Long id, String columnName, Date from, Date to) {
      ...
      if(isHierarchyLevel(columnName)) {
	return loadHierarchyDaySummary(id, columnName, from, to);
      }
    ...
    public List<DashboardRecord> loadHierarchyDaySummary(Long id, String columnName, Date from, Date to) {
      
      long startTime = System.currentTimeMillis();
      logger.debug("loadDaySummary From: " + from.toString() + ", To: " + to.toString());
      
      List<DashboardRecord> oRecords = new ArrayList<DashboardRecord>();
      String tableName = "energy_consumption_hourly";             
      if(columnName.equals("floor_id")) {
	tableName = "floor_energy_consumption_hourly";
      } else if(columnName.equals("campus_id")) {
	tableName = "campus_energy_consumption_hourly";
      } else if(columnName.equals("building_id")) {
	columnName = "bld_id";
	tableName = "bld_energy_consumption_hourly";	
      } else if(columnName.equals("company_id")){
	tableName = "company_energy_consumption_hourly";	         
      } else if(columnName.equals("id")){
	columnName = "fixture_id";
      }
               
      try {
	String hsql = "SELECT date_trunc('hour', ec.capture_at) AS Hour, "
	    + "ec.power_used as totalConsum, ec.base_power_used as baseTotalConsum, "
	    + "ec.saved_power_used as totalPowerSaved, ec.max_price as price, "
	    + "ec.cost as cost, ec.saved_cost as savedCost, "
	    + "ec.occ_saving as totalOccSaved, ec.tuneup_saving as totalTuneupSaving,"
	    + "ec.ambient_saving as totalAmbientSaved, ec.manual_saving as totalManualSaving, "
	    + "ec.base_cost as baseCost from " + tableName + " ec where ";
	if (!columnName.equals("company_id")) {
	  hsql += columnName + " = " + id + " and";
	}	
	hsql += " ec.capture_at <= '" + DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss") + "' "
	    + "and ec.capture_at >'" + DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss") + "' ORDER BY Hour";
	logger.debug("day bar graph query -- " + hsql.toString());
	Query q = getSession().createSQLQuery(hsql.toString());
	List<Object[]> results = q.list();
	Object[] avg_min_max = ((List<Object[]>)loadHierarchyPeakAndMinLoadQuery(tableName, columnName, id.toString(), to, from)).get(0);
	if (results != null && !results.isEmpty()) {
	  for (Iterator<Object[]> iterator = results.iterator(); iterator.hasNext();) {
	    try {
	      Object[] object = (Object[]) iterator.next();
	      ...
	      oRecord.setPeakLoad(((BigDecimal) avg_min_max[1]).floatValue());
	      oRecord.setMinLoad(((BigDecimal) avg_min_max[2]).floatValue());
	      oRecord.setBaseCost(((Double) object[11]).floatValue());
	      oRecords.add(oRecord);
	    } catch(Exception e) {
	      logger.warn("Error processing request: {" + id + ", " + columnName + ", " + from + ", " + to + "} =>"+ e.getMessage());
	    }
	...
	throw SessionFactoryUtils.convertHibernateAccessException(hbe);
      ...
      logger.debug("day summary time == " + (System.currentTimeMillis() - startTime));
      return oRecords;
    ...
    public List<DashboardRecord> loadWeekSummary(Long id, String columnName, Date from, Date to) {
      ...
      if(isHierarchyLevel(columnName)) {
	return loadHierarchyWeekSummary(id, columnName, from, to);
      }
      List<DashboardRecord> oRecords = new ArrayList<DashboardRecord>();
      String tableName = "energy_consumption";
      tableName = selectTable(from, to);
    ...
    public List<DashboardRecord> loadHierarchyWeekSummary(Long id, String columnName, Date from, Date to) {
        
      long startTime = System.currentTimeMillis();
      logger.debug("loadWeekSummary From: " + from.toString() + ", To: " + to.toString());
      
      List<DashboardRecord> oRecords = new ArrayList<DashboardRecord>();
      String tableName = "energy_consumption_daily";             
      if(columnName.equals("floor_id")) {
	tableName = "floor_energy_consumption_daily";
      } else if(columnName.equals("campus_id")) {
	tableName = "campus_energy_consumption_daily";
      } else if(columnName.equals("building_id")) {
	columnName = "bld_id";
	tableName = "bld_energy_consumption_daily";	
      } else if(columnName.equals("company_id")){
	tableName = "company_energy_consumption_daily";	         
      } else if(columnName.equals("id")){
	columnName = "fixture_id";
      }
	...
	hsql += " ec.capture_at <= '" + DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss") + "' "
	    + "and ec.capture_at >'" + DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss") + "' ORDER BY Day";
	logger.debug("week bar graph query -- " + hsql.toString());
	Query q = getSession().createSQLQuery(hsql.toString());
	List<Object[]> results = q.list();
	Object[] avg_min_max = ((List<Object[]>)loadHierarchyPeakAndMinLoadQuery(tableName, columnName, id.toString(), to, from)).get(0);
	if (results != null && !results.isEmpty()) {
	...
	throw SessionFactoryUtils.convertHibernateAccessException(hbe);
      ...
      logger.debug("week summary time == " + (System.currentTimeMillis() - startTime));
      return oRecords;
    ...
    public List<DashboardRecord> loadMonthSummary(Long id, String columnName, Date from, Date to) {
      ...
      if(isHierarchyLevel(columnName)) {
	return loadHierarchyMonthSummary(id, columnName, from, to);
      }
    ...
    public List<DashboardRecord> loadHierarchyMonthSummary(Long id, String columnName, Date from, Date to) {
      
      long startTime = System.currentTimeMillis();
      logger.debug("loadMonthSummary From: " + from.toString() + ", To: " + to.toString());
	...
	hsql += " ec.capture_at <= '" + DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss") + "' "
	    + "and ec.capture_at >'" + DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss") + "' ORDER BY Day";
	logger.debug("month bar graph query -- " + hsql.toString());
	Query q = getSession().createSQLQuery(hsql.toString());
	List<Object[]> results = q.list();
	Object[] avg_min_max = ((List<Object[]>)loadHierarchyPeakAndMinLoadQuery(tableName, columnName, 
	    id.toString(), to, from)).get(0);
	if (results != null && !results.isEmpty()) {
	  for (Iterator<Object[]> iterator = results.iterator(); iterator.hasNext();) {
	    try {
	      Object[] object = (Object[]) iterator.next();
	...
	throw SessionFactoryUtils.convertHibernateAccessException(hbe);
      ...
      logger.debug("month summary time == " + (System.currentTimeMillis() - startTime));
      return oRecords;
    ...
    public List<DashboardRecord> loadYearSummary(Long id, String columnName, Date from, Date to) {
      ...
      if(isHierarchyLevel(columnName)) {
	return loadHierarchyYearSummary(id, columnName, from, to);
      }
    ...
    public List<DashboardRecord> loadHierarchyYearSummary(Long id, String columnName, Date from, Date to) {
      
      long startTime = System.currentTimeMillis();
      logger.debug("loadYearSummary From: " + from.toString() + ", To: " + to.toString());
	...
	String hsql = "SELECT date_trunc('month', ec.capture_at  - interval '1 day') AS Month, "
            ...
            + "sum(ec.occ_saving) as totalOccSaved, sum(ec.tuneup_saving) as totalTuneupSaving,"
            + "sum(ec.ambient_saving) as totalAmbientSaved, sum(ec.manual_saving) as totalManualSaving, "
            + "sum(ec.base_cost) as baseCost from " + tableName + " ec where ";	
	if (!columnName.equals("company_id")) {
	  hsql += columnName + " = " + id + " and";
	}	
	hsql += " ec.capture_at <= '" + DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss") + "' "
	    + " and ec.capture_at >'" + DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss") + "'"
	    + " Group by Month Order by Month";
	logger.debug("year bar graph query -- " + hsql.toString());
	Query q = getSession().createSQLQuery(hsql.toString());
	List<Object[]> results = q.list();
	Object[] avg_min_max = ((List<Object[]>)loadHierarchyPeakAndMinLoadQuery(tableName, columnName, id.toString(), to, from)).get(0);
	if (results != null && !results.isEmpty()) {
	...
	throw SessionFactoryUtils.convertHibernateAccessException(hbe);
      ...
      logger.debug("year summary time == " + (System.currentTimeMillis() - startTime));
      return oRecords;
    ...
    public Integer getFixtureCount(Long id, String property, String columnName, Date from, Date to) {
        ...
        try {
            ...
            String hsql = "SELECT count(distinct ec.fixture_id) as fixtureCount " + "FROM " + property + " as ec "
                    + "where ec.fixture_id in (select id from fixture where " + columnName + " = " + id + ") "
...
/*    public List<DRRecord> loadAvgEnergyConsumptionBetweenPeriodsPerDay(Date fromTimeOfDay, Date toTimeOfDay,
	...
	if(!property.equals("company")) {  
	  hsql_custom += " and ec.fixture_id in (select id from fixture where " + property + 
	      " = " + pid + ")";
	...
	public Map<Integer, Double> getFixtureECOverPeriod(Date startTime, Date endTime) {
        ...
        try {
            ...
            String hsql = "SELECT ec.fixture_id, avg(ec.power_used) as powerUsed "
                    + " from energy_consumption ec " 
                    + " where ec.fixture_id in (select id from fixture where state = 'COMMISSIONED') "
                    + " and to_char(ec.capture_at, 'HH24:MI:SS') <=  '" + DateUtil.formatDate(endTime, "HH:mm:ss") + "'"
                    + " and to_char(ec.capture_at, 'HH24:MI:SS') >= '" + DateUtil.formatDate(startTime, "HH:mm:ss") + "'" 
                    + " and to_char(ec.capture_at, 'YYYY-MM-DD') in " +
                    	"(select to_char(ts.dates, 'YYYY-MM-DD') from " +
                    	"(select current_timestamp - s.t * interval '24 hour' as dates " +
                    	"from generate_series(1,30) as s(t)) ts " +
                    	"where trim(to_char(ts.dates, 'Day')) in " +
                    	"(select wd.day from fixture f, profile_handler ph, weekday wd " +
                    	"where f.id = ec.fixture_id and ph.id = f.profile_handler_id and " +
                    	"ph.profile_configuration_id = wd.profile_configuration_id and wd.type = 'weekday') " +
                    	"order by ts.dates desc limit 10)"
	...
	public Map<Integer, Double> getRecentFixtureEC() {
        ...
        try {
            ...
            String hsql = "SELECT ec.fixture_id, ec.power_used as powerUsed "
                    + " from energy_consumption ec " 
                    + " where ec.fixture_id in (select id from fixture where state = 'COMMISSIONED') "
                    + " and to_char(capture_at, 'YYYY-MM-DD HH24:MI:SS') <=  '" + DateUtil.formatDate(new Date(new Date().getTime() - 5*60*1000), "yyyy-MM-dd HH:mm:ss") + "'"
                    + " and to_char(capture_at, 'YYYY-MM-DD HH24:MI:SS') > '" + DateUtil.formatDate(new Date(new Date().getTime() - 10*60*1000), "yyyy-MM-dd HH:mm:ss") + "'" 
                    ;
	...
	public Date loadFirstRecordDate() {
		...
		try {
			...
			hsql = "SELECT ec.capture_at , id "
					...
					+ "ec.id = (select Min(id) from energy_consumption_hourly )";
