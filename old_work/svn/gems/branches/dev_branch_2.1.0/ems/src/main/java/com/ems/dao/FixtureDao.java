...
public class FixtureDao extends BaseDaoHibernate {
	...
	private static String updateStatsQueryStr = "Update Fixture set lastOccupancySeen = " +
	    ":lastOccupancySeen, lightLevel = :lightLevel, lastConnectivityAt = " +
	    ":lastConnectivityAt, lastStatsRcvdTime = :lastStatsRcvdTime, " + 
	    "currApp = :currApp, currentState = :currentState, wattage = :wattage, " + 
	    "secGwId = :secGwId, dimmerControl = :dimmerControl, avgTemperature = " +
	    ":avgTemperature, baselinePower = :baselinePower, versionSynced = " +
	    ":versionSynced, bulbLife = :bulbLife " + " where id = :id";
	...
	public List<Fixture> loadFixtureLocationsByFloorId(Long id) {
		...
		try {
			Session s = getSession();
			String hsql = "SELECT f.id, f.fixtureName, f.xaxis, f.yaxis, f.groupId "
	...
	public void updateProfileHandlerIdForFixture(Long fixtureId,
		...
		Session s = getSession();
		Fixture fixture = (Fixture) s.get(Fixture.class, fixtureId);
		fixture.setProfileHandler((ProfileHandler) s.load(ProfileHandler.class,
				profileHandlerId));
		fixture.setCurrentProfile(currentProfile);
		fixture.setOriginalProfileFrom(originalProfileFrom);
	...
	public ProfileHandler getProfileHandlerByFixtureId(Long fixtureId) {
		Session session = getSession();
		ProfileHandler profileHandler = ((Fixture) session.get(Fixture.class,
				fixtureId)).getProfileHandler();
		// try {
	...
	public Fixture getFixtureByMacAddr(String macAddr) {
		Session session = getSession();
		Fixture fixture = (Fixture) session.createCriteria(Fixture.class)
				.add(Restrictions.eq("mac_address", macAddr)).uniqueResult();
		return fixture;
	...
	public void updateRealtimeStats(Fixture fixture) {
		...
		// query.setLong("id", fixture.getId());
		// query.setInteger("lastOccupancySeen",
		// fixture.getLastOccupancySeen());
		// query.setInteger("lightLevel", fixture.getLightLevel());
		// query.setTimestamp("lastConnectivityAt", new java.util.Date());
		// query.setShort("globalProfileChecksum",
		// fixture.getGlobalProfileChecksum());
	...
	public void updateLastCommandStatus(Fixture fixture, String cmd,
			String status, long gwId) {
		Session session = getSession();
		session.update(fixture);
		...
		if (property != null && pid != null) {
			...
			if (bSearch) {
				if (searchField.equals("id")) {
					...
					oRowCount.add(Restrictions.like("fx.fixtureName", "%"
							+ searchString + "%"));
					oCriteria.add(Restrictions.like("fx.fixtureName", "%"
							+ searchString + "%"));
					...
					oRowCount.add(Restrictions.like("gw.gatewayName", "%"
							+ searchString + "%"));
					oCriteria.add(Restrictions.like("gw.gatewayName", "%"
							+ searchString + "%"));
				}
			}
			if (limit > 0) {
	...
	public FixtureList loadFixtureListWithSpecificAttrs(String property, Long pid, String order,
		...
		if (property != null && pid != null) {
			...
			oCriteria = getSession()
					...
					.add(Projections.property("fx.fixtureName"), "fixtureName")
					...
					.add(Projections.property("gw.gatewayName"), "gatewayNameForFilter")
	...
	public List getAllSensorData(long captureDate) {
		...
		try {
			Session s = getSession();
			String hsql = "SELECT f.id, f.fixtureName, ec.powerUsed, "
	...
	public List<FixtureOutageVO> getFixtureOutageList(String property, Long pid) {
	              ...
	              String hql = "select new com.ems.vo.model.FixtureOutageVO(f.id, f.fixtureName, f.location, f.xaxis, f.yaxis, ef.eventTime, ef.description) "
								+ " from Fixture f, EventsAndFault ef where f.id = ef.fixture.id and f.state = 'COMMISSIONED' and ef.eventType like 'Fixture Out'  and ef.resolvedOn is null ";
