...
--
-- Name: hier_hour_record; Type: TYPE; Schema: public; Owner: postgres
--
...
DECLARE 
	...
	rec_hier_hourly hier_hour_record;
	rec_hier_daily hier_daily_record;
BEGIN
	...
	LOOP
	  ...
	  LOOP  
		...
		THEN
			UPDATE energy_consumption_hourly set power_used = rec_hourly.agg_power, price = round(cast (rec_hourly.base_cost*12*1000/(rec_hourly.no_of_rec *rec_hourly.base_power) as numeric), 10), cost = rec_hourly.agg_cost, base_power_used = rec_hourly.base_power, base_cost = rec_hourly.base_cost, saved_power_used = rec_hourly.saved_power, saved_cost = rec_hourly.saved_cost, peak_load = rec_hourly.peak_load, min_load = rec_hourly.min_load, min_price = rec_hourly.min_price, max_price = rec_hourly.max_price, occ_saving = rec_hourly.occ_saving, ambient_saving = rec_hourly.amb_saving, manual_saving = rec_hourly.manual_saving, tuneup_saving = rec_hourly.tune_saving, avg_load = rec_hourly.avg_load WHERE fixture_id = fixtureId and capture_at = hour_time.capture_time;
		END IF;
	  END LOOP;

  	  --floor level aggregation for hourly
	FOR hier_hour_record IN ( 
	  SELECT floor_id as hier_id, SUM(power_used) AS agg_power, sum(cost) AS agg_cost, min(min_temperature) AS min_temp, max(max_temperature) AS max_temp, avg(ec.avg_temperature) AS avg_temp, SUM(base_power_used) AS base_power, sum(base_cost) AS base_cost, SUM(saved_power_used) AS saved_power, sum(saved_cost) AS  saved_cost, SUM(occ_saving) AS occ_saving, SUM(ambient_saving) AS amb_saving, SUM(tuneup_saving) AS tune_saving, SUM(manual_saving) AS manual_saving, AVG(price) AS price, min(price) AS min_price, max(price) AS max_price, sum(avg_load) AS avg_load, count(*) AS no_of_rec
	FROM energy_consumption_hourly as ec, fixture as f
	WHERE capture_at = toDate and fixture_id = fixtureId and f.id = ec.fixture_id and base_power_used != 0 GROUP BY floor_id)
	LOOP  
	  IF hier_hour_record.bas_power > 0 THEN
		UPDATE floor_energy_consumption_hourly set power_used = hier_hour_record.agg_power, price = hier_hour_record.price, cost = hier_hour_record.agg_cost, min_temperature = hier_hour_record.min_temp, max_temperature = hier_hour_record.max_temp, avg_temperature = round(hier_hour_record.avg_temp), base_power_used = hier_hour_record.base_power, base_cost = hier_hour_record.base_cost, saved_power_used = hier_hour_record.saved_power, hier_hour_record.saved_power, saved_cost = hier_hour_record.saved_cost, occ_saving = hier_hour_record.occ_saving, ambient_saving = hier_hour_record.amb_saving, tuneup_saving = hier_hour_record.tune_saving, manual_saving =  hier_hour_record.manual_saving, min_price = hier_hour_record.min_price, max_price = hier_hour_record.max_price, avg_load = hier_hour_record.avg_load WHERE floor_id = hier_hour_record.hier_id AND capture_at = hour_time.capture_time;
	  END IF;
	END LOOP;
