...
CREATE SEQUENCE switch_fixtures_seq
   INCREMENT 1
   START 1;
ALTER TABLE scene_seq OWNER TO postgres;

CREATE TABLE switch_fixtures (
    switch_id bigint,
    fixture_id bigint,
    id bigint DEFAULT nextval('switch_fixtures_seq'::regclass) NOT NULL
);

ALTER TABLE public.switch_fixtures OWNER TO postgres;

ALTER TABLE ONLY switch_fixtures
    ADD CONSTRAINT switch_fixtures_pkey PRIMARY KEY (id);
...
ALTER FUNCTION public.updateZeroBuckets(fixtureId integer, lastStatsRcvdDate timestamp with time zone, startDate timestamp with time zone, latestStatsDate timestamp with time zone) OWNER TO postgres;
...
ALTER TABLE gateway ADD COLUMN gateway_name character varying(255);
...
ALTER TABLE gateway ADD COLUMN app2_version character varying(50);
...
ALTER TABLE gateway ADD COLUMN x integer default 0;
ALTER TABLE gateway ADD COLUMN y integer default 0;
...
BEGIN
	...
	FOR i in 1..array_upper(default_groups, 1) LOOP
		...
		IF sGName <> sCurrGName THEN
			...
			ELSE
				UPDATE groups set name=sGName, profile_handler_id=1 where id = i;
			END IF;
		ELSE
			IF iCurrId IS NULL THEN
...
CREATE OR REPLACE FUNCTION update_location_change() RETURNS "trigger" AS $$
	BEGIN
	  IF tg_op = 'UPDATE' THEN
		...
		floor f,
		company co, 
		company_campus cc where  
		floor_id=f.id and 
		f.building_id=b.id and 
		b.campus_id=c.id and 
		cc.campus_id=c.id and 
		co.id=cc.company_id;
...
--Added by Sree 05/11
-- backup the energy consumption table and delete all the data upto previous hour and delete duplicates in the current hour
CREATE OR REPLACE FUNCTION backup_energy_consumption_once() RETURNS void
    AS $$
DECLARE 	
BEGIN
	...
	IF EXISTS (
	  ...
	  WHERE table_catalog = CURRENT_CATALOG AND 
		   ...
		   table_name = 'energy_consumption_backup') 
	THEN
	  RETURN;
	END IF;

	CREATE TABLE energy_consumption_backup AS SELECT * FROM energy_consumption;

	--delete all the data upto previous hour
	DELETE FROM energy_consumption WHERE capture_at < (SELECT date_trunc('hour', now()));

	--removing the records of zero buckets when original buckets are present in the current hour
	DELETE FROM energy_consumption as e2 where zero_bucket = 1 and exists (select e1.capture_at, e1.fixture_id from energy_consumption as e1 where e1.capture_at = e2.capture_at and e1.fixture_id = e2.fixture_id group by e1.capture_at, e1.fixture_id having count(*) > 1);

	--removing the record if there are two original buckets at the same time in the current hour
	DELETE FROM energy_consumption as e1 where e1.id in (select max(id) from energy_consumption as e2 where e2.capture_at = e1.capture_at and e2.fixture_id = e1.fixture_id group by e2.capture_at, e2.fixture_id having count(*) > 1);

END;
$$
LANGUAGE plpgsql;

select backup_energy_consumption_once();
...
ALTER TABLE gateway ADD COLUMN location character varying(500);
...
-- Added by Naveen 06/30
ALTER TABLE switch ADD COLUMN dimmer_control integer DEFAULT 0;
...
-- Adding columns active_control and scene_id to switch
ALTER TABLE switch ADD COLUMN active_control integer DEFAULT 1;
ALTER TABLE switch ADD COLUMN scene_id  bigint;
...
CREATE OR REPLACE FUNCTION updateZeroBuckets(fixtureId integer, lastStatsRcvdDate timestamp with time zone, startDate timestamp with time zone, latestStatsDate timestamp with time zone, newCU integer) RETURNS void
    AS $$
DECLARE 
	baseAvgPowerUsed numeric;
	latestCum numeric;
