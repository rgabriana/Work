				...
				<div id="profileTreeViewDiv" class="treeviewbg" oncontextmenu="return false;">
					<ul>
						<c:forEach items='${profileTreeHierarchy.treeNodeList}' var='template'>
							<li id='<c:out value="${template.nodeType.lowerCaseName}" escapeXml="true"/>_${template.nodeId}'><a href='#'>${template.name}</a>
								<ul>
									<c:forEach items='${template.treeNodeList}' var='profile'>
										<c:choose>
											...
											<c:when test="${template.nodeId == profile.nodeId}">
												<li rel="${profile.nodeType.lowerCaseName}" id='<c:out value="${profile.nodeType.lowerCaseName}"/>_${profile.nodeId}'><a href='#'><c:out value="${profile.name}_Default" escapeXml="true"/></a>
												</li>
											</c:when>
											<c:otherwise>
												<li rel="${profile.nodeType.lowerCaseName}" id='<c:out value="${profile.nodeType.lowerCaseName}"/>_${profile.nodeId}'><a href='#'><c:out value="${profile.name}" escapeXml="true"/></a>
