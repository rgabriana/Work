...
public class SSLSessionManager {
  ...
  private static ConcurrentHashMap<Long, Object> gwSslLockMap = 
  ...
  private SSLSessionManager() {
    ...
    /* moved initialization of ssl into a gw specific thread
...
//      	ServerUtil.printPacket("digest -- ", md.digest());
  ...
  private SSLConnection getSSLConnection(Gateway gw) throws SSLException {
    ...
    if(sslConn == null) {
      ...
      if(sslLogger.isInfoEnabled()) {
    	sslLogger.info(gw.getIpAddress() + ": could not retrieve the SSL connection from cache");
      }
  ...
  public void initNwInterface() {
	...
	systemConfigurationManager = (SystemConfigurationManager)SpringContext.getBean("systemConfigurationManager");
...
//      System.arraycopy(key, 0, gwPkt, 5, key.length);
  ...
  //function to create the SSL session object for a gateway if it is not created already
  ...
  private SSLConnection establishConnection(Gateway gw) throws SSLException {
    ...
    try {      
      ...
      if(bldgGemsIp != null && bldgGemsIp.length() != 0) {
    		sslLogger.debug(gwIp + ": local client bind");
    	  //YGC: Need to confirm if this creates routing issue
	...
	sslLogger.debug(gwIp + ": created the ssl connection");
      ...
      if (!authenticateGwSSL(sslConn, key, gw.getMacAddress())) {
      	...
      	eventMgr.addAlarm(gw, "GW SSL communication failure (Could not authenticate)", 
  ...
  // This method authenticates enLughted gateway using IP address before adding it is added to the db
  public synchronized boolean authEnlightedGateway(Gateway gw) throws SSLException {
	...
	key = ServerConstants.DEF_WLESS_SECURITY_KEY;
      ...
      try {      
        ...
        //bind to the local eth1 interface address
        if(bldgGemsIp != null && bldgGemsIp.length() != 0) {
    		sslLogger.debug(gwIp + ": local client bind on " + bldgGemsIp);
          InetSocketAddress localAddress = new InetSocketAddress(bldgGemsIp, 0);
          socket.bind(localAddress);
        }
        InetSocketAddress address = new InetSocketAddress(gwIp, ServerConstants.GW_UDP_PORT);
      ...
      sslLogger.debug(gwIp + ": after handshake");
        ...
        //socket connection is established, authenticate gateway
        if(sslLogger.isDebugEnabled()) {
      sslLogger.debug(gwIp + ": created the ssl connection " + gw.getMacAddress());
        }
