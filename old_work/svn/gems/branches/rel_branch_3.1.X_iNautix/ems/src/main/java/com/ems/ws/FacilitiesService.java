...
public class FacilitiesService {
     ...
     */
    @Path("floor/setimage/{campusname}/{bldgname}/{floorname}/{imageurl}")
    @POST
    @Consumes({"image/jpeg", "image/png"})
    public Response setFloorPlan(@PathParam("campusname") String campusName, @PathParam("bldgname") String bldgName, @PathParam("floorname") String floorName, @PathParam("imageurl") String imageURL, @RequestParam byte[] imageData) throws UnsupportedEncodingException {
    	...
    	try {
	        ...
	        if(campus == null)
	        {
	            ...
	            facilityTreeManager.onlyInValidateFacilitiesTreeCache();
	        }
...
//Service to get the parent node for any child in the tree
	...
	public String updateSeverDateTimeSettings(@PathParam("timezone") String timezone,
		...
		if (companies.size() > 0 && !companies.get(0).getTimeZone().equals(timezone)) {
			companyManager.setServerTimeZone(timezone);
