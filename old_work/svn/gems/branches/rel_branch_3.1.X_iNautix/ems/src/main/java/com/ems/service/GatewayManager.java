  ...
  private static final Logger logger = Logger.getLogger(GatewayManager.class.getName());
    ...
    public int addGateway(String strMACAddr, String strIPAddr, String strFloorId)
    {
    	...
    	try {
	        ...
	        try {
		        bAuthenticated = SSLSessionManager.getInstance().authEnlightedGateway(gateway);
	         }catch(Exception e) {
	        	System.out.println(gateway.getIpAddress() + " first attempt");
	        	 e.printStackTrace();
	        	 if (bAuthenticated == false) {
	        		System.out.println(gateway.getIpAddress() + " Reattempting authentication");
	        		bAuthenticated = SSLSessionManager.getInstance().authEnlightedGateway(gateway);
	        		if(bAuthenticated == false) {
	        			return 1;
	        		}
