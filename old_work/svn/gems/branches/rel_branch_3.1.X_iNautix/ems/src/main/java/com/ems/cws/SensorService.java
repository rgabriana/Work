...
public class SensorService {
    ...
    static final Logger apiLogger = Logger.getLogger("APILog");
    ...
    public Response dimFixture(@PathParam("mode") String mode, @PathParam("percentage") String percentage,
        ...
        apiLogger.info("Dim Percentage: " + percentage + " for Time: " + time + ", Fixtures Number: " + sensors.size());        
    ...
    /**
     * Get realtime dim level for list of sensors
     * 
     * @param sensors
     * 			list of sensors
     * @return sensor list
    ...
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public List<Sensor> getRealTimeDim(List<Sensor> sensors) {

    	apiLogger.info("Real Time for sensor list of size: " + sensors.size());
		Iterator<Sensor> itr = sensors.iterator();
		while (itr.hasNext()) {
			Fixture fixture = fixtureManager.getFixtureById(itr.next().getId());
			//Do I need to sleep between sending commands?
		...
		itr = sensors.iterator();
		List<Sensor> outSensors = new ArrayList<Sensor>();
		fixtureDao.getSession().clear();
		while(itr.hasNext()) {
			Fixture fixture = fixtureManager.getFixtureById(itr.next().getId());
			Sensor s = new Sensor();
			s.setId(fixture.getId());
			s.setCaptureTime(fixture.getLastConnectivityAt().toString());
			s.setLightLevel(fixture.getDimmerControl().shortValue());
			outSensors.add(s);
		}
