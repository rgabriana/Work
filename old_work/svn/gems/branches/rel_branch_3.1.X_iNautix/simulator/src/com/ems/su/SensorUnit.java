...
public class SensorUnit {
    ...
    Thread t;
    public volatile Thread blinker, mbitThread, fxHBThread;
    ...
    public void postToSU(int txnId, int msgType, byte[] args) {
        ...
        case CommandsConstants.GET_STATUS_MSG_TYPE:
            this.sendRealTimer((byte)msgType);
    ...
    protected void sendRealTimer(byte msgType) {
        GatewayPkt gatewayPkt = new GatewayPkt();
        int iPriority = Packets.PRIORITY_LEVEL_EQUAL;
        SURealTimeFrame suRealTimeFrame = new SURealTimeFrame(msgType, oProfile);
...
class GenerateFxStatsHB implements Runnable {
    ...
    public void run() {
        ...
        while (su.getFxHBRunning() == Thread.currentThread()) {
            su.sendRealTimer((byte)CommandsConstants.SU_CMD_HEART_BEAT_MSG_TYPE);
            try {
