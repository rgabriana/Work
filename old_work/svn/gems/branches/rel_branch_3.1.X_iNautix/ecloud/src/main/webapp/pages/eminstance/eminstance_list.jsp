...
function viewActionFormatter(cellvalue, options, rowObject) {
	...
	return "<security:authorize access="hasAnyRole('Admin')"><div id=\"divForm\"><div id=\"btn1\"><button onclick=\"javascript: onEdit('"+rowId+"');\">Edit</button>&nbsp;</div><div id=\"btn2\"><button onclick=\"javascript:alert('Delete feature coming shortly');\">Delete</button></div></div></security:authorize>";		
}
...
function start(pageNum, orderWay) {
		jQuery("#emInstanceTable").jqGrid({
			...
			colNames: ["id", "Name", "Version", "Mac Id","EM Address","Last connectivity","Call Home Health","Sync Connectivity","SPPA Price","Details","Action"],
		       colModel: [
...
{
if(supportadmin == true && systemadmin == true)
	{	
	jQuery("#emInstanceTable").jqGrid('hideCol', 'action');	
	}
}

function supportAndSystemAdmins()
{
//disables the column headers of the jqgrid for support admins and system admins.
supportadmin = true;
systemadmin = true;
admin = false;
}
...
<security:authorize access="hasAnyRole('SupportAdmin','SystemAdmin')">
<script type="text/javascript">
    supportAndSystemAdmins();
</script>
</security:authorize>
