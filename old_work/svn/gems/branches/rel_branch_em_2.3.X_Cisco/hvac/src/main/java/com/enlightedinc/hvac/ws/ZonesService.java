...
/*	@Path("/getOutageStatus/zone/{zoneId}")
	...
	@Path("/getAvgDimLevel/id/{zoneId}")
	@GET
	@Produces({MediaType.APPLICATION_XML})
	public Response getAvgDimLevel(@PathParam("zoneId") Long zoneId) {
		Response resp = new Response();
		Integer val = zonesManager.getAvgDimLevel(zoneId);
		if (val == -1) {
			resp.setStatus(1);
		}
		else {
...
/*	@Path("/getTotalPower/zone/{zoneId}")
	...
	@Path("/setDimLevel/id/{zoneId}/{percentage}/{time}")
	@GET
	@Produces({MediaType.APPLICATION_XML})
	...
	@Path("/getLastOccupancySeen/id/{zoneId}")
	@GET
	@Produces({MediaType.APPLICATION_XML})
	public Response getTimeSinceLastOccupancy(@PathParam("zoneId") Long zoneId) {
		Response resp = new Response();
		Integer val = zonesManager.getTimeSinceLastOccupancy(zoneId);
		if (val == -1) {
			resp.setStatus(1);
		}
		else {
	...
	@Path("/getOccupancyStatus/id/{zoneId}")
	@GET
	@Produces({MediaType.APPLICATION_XML})
	public Response getOccupancyStatus(@PathParam("zoneId") Long zoneId) {
		Response resp = new Response();
		Integer val = zonesManager.getOccupancyStatus(zoneId);
		if (val == -1) {
			resp.setStatus(1);
		}
		else {
