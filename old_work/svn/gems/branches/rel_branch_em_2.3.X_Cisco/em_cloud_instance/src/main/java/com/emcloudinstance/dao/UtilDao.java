...
public class UtilDao extends AbstractJdbcDao{
	...
	public boolean synchTableData(String mac, String tableName,
		...
		try{
			if(status)
			{
				commonUtils.setReplicaMigrationFlagLogOnCloud(mac,CommonStateUtils.getStateInProgressAccordingToTableName(tableName) , String.valueOf(getLastMinTableDataSynched(mac,tableName)) ) ;
			}else
			{
				commonUtils.setReplicaMigrationFlagLogOnCloud(mac,CommonStateUtils.getStateInProgressAccordingToTableName(tableName) , exceptionString);
			}
		}catch(Exception e)
		{
			e.printStackTrace() ;
