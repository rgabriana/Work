...
$(document).ready(function() {
    ...
    $('#newFixtureClassButton').click(function() {
    	$("#newFixtureClassDialog").load("${addFixtureClassUrl}"+"?ts="+new Date().getTime()).dialog({
            title : "Add Fixture Class",
...
function onEditFixtureClass(rowId){
	$("#newFixtureClassDialog").load("${editFixtureClassUrl}?fixtureClassId="+rowId+"&ts="+new Date().getTime()).dialog({
        title : "Edit Fixture Class",
        width :  Math.floor($('body').width() * .30),
        minHeight : 250,
        modal : true
    });
...
{
	if(confirm("Are you sure you want to delete the FixtureClass?") == true)
	{
		$.ajax({
	 		type: 'POST',
			...
			error: function(){
				alert("Error occured.Fixture class failed to delete");
			},
...
function startFixtureClassTable(pageNum, orderWay) {
		jQuery("#fixtureClassTable").jqGrid({
			...
			colNames: ["id", "Fixture Class Name","No of Ballasts","Voltage","Ballast","Bulb","Action"],
		       colModel: [
