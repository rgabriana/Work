		...
		<c:forEach items="${tenantsList}" var="tenant">
			<tr>
				...
				<td align="right" style="padding-right:5px;">
					<div>
						    <button id="${tenant.email}_editTenantBtn" onclick="editTenantsDetails('${editTenantUrl}/${tenant.id}/edit.ems');">Edit</button>
						    <c:if test="${tenant.status.name == 'ACTIVE'}">
								<button id="${tenant.email}_manageTenantuserBtn" onclick='loadTenantUsers("${tenant.id}");'>Manage Users</button>
							</c:if>
					</div>
				</td>
			</tr>
