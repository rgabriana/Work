...
INSERT INTO event_type (id, "type", description) VALUES ((select coalesce(max(id),0)+1 from event_type), 'Lamp Out', NULL);
INSERT INTO event_type (id, "type", description) VALUES ((select coalesce(max(id),0)+1 from event_type), 'Download Power Usage Characterization', NULL);
...
--by Default Bulb Configuration Feature will be enable - User has to set False to disable the feature
UPDATE system_configuration SET value = 'true' where name = 'bulbconfiguration.enable';
...
DECLARE
     ...
     inputVoltages int[] = ARRAY[110,277];
     ...
     ballastVoltPower_values text[] = ARRAY[
         ...
         [
             ...
             [110,82,81,77,73,69,65,70.5,66,61.5,58,54,49.5,45,40.5,36.5,31.5,25.5,17,12,12],
         ...
         [
	      	  --InputVoltage (110/277) followed by power readings for [10.0-0.5] volt
             [110,57,56.5,55,52.5,49.5,52,49.5,46.5,43,40,37,34,30.5,27,24,20.5,16.5,11,9,9],
             [277,56,56,53.5,51,49.5,47.5,44,41,38.5,35,32.5,29.5,26.5,24,21,18,13.5,8.5,8,8]
         ],
         --GE432MVPSN-V03
         [
	    	 --InputVoltage (110/277) followed by power readings for [10.0-0.5] volt
             [110,114,110.5,105.5,100.5,95.5,101.5,96,90.5,84.5,78.5,72.5,66,60,53.5,46,39.5,31,22,15,15],
...
SELECT setval('ballast_seq', (SELECT MAX(id) FROM ballasts));
