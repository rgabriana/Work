...
$em_login = "curl -d \"<root><request><messageType>1</messageType><body><loginRequest><appIp>EMS COMMUNICATOR</appIp></loginRequest></body></request></root>\" -c /tmp/gw_cookie.tmp -k https://localhost/ems/wsaction.action";
$em_update_gw = "curl --get -b /tmp/gw_cookie.tmp -k https://localhost/ems/services/org/gateway/updateIp/";
...
function updateGateway($ipAddress, $macAddress) {
  $result = exec($GLOBALS["em_login"]);
  $result = exec($GLOBALS["em_update_gw"]. $macAddress . "/".$ipAddress);
  exec("rm -f /tmp/gw_cookie.tmp");
}
