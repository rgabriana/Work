...
public class BallastVoltPowerService {
	...
	public Response getBallastCSVListByVoltageAndId(
		...
		output.append("#ballast type,"+ ballast.getBallastName().toString().trim() +"\r\n");
		output.append("#no. lamps,"+ ballast.getLampNum().toString().trim() +"\r\n");
		output.append("#lamp manufacturer,"+ ballast.getBallastManufacturer().toString().trim()+"\r\n");
		output.append("#lamp type,"+ ballast.getLampType().toString().trim() +"\r\n");
		output.append("#lamp wattage,"+ ballast.getWattage().toString().trim() +"\r\n");
		output.append("#line voltage,"+ voltage.toString().trim() +"\r\n");
	...
	public com.ems.ws.util.Response forgetBallastCurve(@PathParam("ballastId") Long ballastId,@PathParam("inputVoltage") Double inputVoltage) {
		...
		if(fXListWithBallastCurve!=null && fXListWithBallastCurve.length >0)
		{
			for(int fixtId: fXListWithBallastCurve) {
				Fixture fixture = fixtureManager.getFixtureById(new Long(fixtId));
