...
public class MotionBitsConfigManager {
    ...
    public MotionBitsScheduler saveMotionBitsSchedule(MotionBitsScheduler mbSchedule) {
	...
	public int loadMotionBitsScheduleByName(String name) {
		return motionBitsSchedulerDao.loadMotionBitsScheduleByName(name);
	}
	
	public boolean deleteMotionBitsScheduleById(Long id) {

		boolean ret = deleteMotionBitsSchedulerJob(id);
		MotionBitsScheduler scheduler = loadMotionBitsScheduleById(id);
		Long groupId = scheduler.getMotionBitGroup().getId();
	...
	public boolean deleteMotionBitsSchedulerJob(Long id)
	{
        ...
        try {
        	...
        	if(SchedulerManager.getInstance().getScheduler().checkExists(new JobKey(startJobName, schedule.getName())))
        	...
        	if(SchedulerManager.getInstance().getScheduler().checkExists(new JobKey(endJobName, schedule.getName())))
        	{
	...
	public String validateSchedule(String name, String startTime, String endTime, Long id) {
		String status = "";
		String fixtureNames = "";
		List<GemsGroupFixture> listFixtures = null;
		
		if(id.longValue() != 0L)
		{
			MotionBitsScheduler currentSchedule =  loadMotionBitsScheduleById(id);
			listFixtures = gemsGroupManager.getGemsGroupFixtureByGroup(currentSchedule.getMotionBitGroup().getId());
		}
		
		if(listFixtures == null || listFixtures.isEmpty())
			return status;
		...
		for(MotionBitsScheduler schedule:listSchedules) {
			...
			if(schedule.getCaptureStart() == "" || schedule.getCaptureEnd() == "")
				continue;
				
			for (GemsGroupFixture currentGroupFixture:listFixtures) {
				
				Fixture fixture = currentGroupFixture.getFixture();
				
				List<GemsGroupFixture> listGroupFixture = gemsGroupManager.getGemsGroupFixtureByGroup(schedule.getMotionBitGroup().getId());
	...
	public MotionBitsScheduler loadMotionBitsScheduleByGemsGroupId(Long id) {
		return (MotionBitsScheduler)motionBitsSchedulerDao.loadMotionBitsScheduleById(id);
	}
