...
public class FixtureImageUpgradeWorker extends Thread {
  ...
  public void startImageUpload(int[] fixtureIds, String fileName) {
    ...
    for(int i = 0; i < noOfFixtures; i++) {
      Fixture fixture = fixtureMgr.getFixtureById(new Long(fixtureIds[i]));   
      if(fixture == null) {
    ...
    public void run() {
      ...
      try {
      	...
      	for(int i = 0; i < noOfFixtures; i++) {
      		...
      		fixtureMgr.setImageUpgradeStatus(fixtureMgr.getFixtureById(fixture.getId()), ServerConstants.IMG_UP_STATUS_FAIL);
    ...
    public void ackImageUploadStart(final Fixture fixture) {
      ...
      synchronized(ackLock) {
        ...
        if(details != null && details.status.equals(ServerConstants.IMG_UP_STATUS_SCHEDULED)) {
          fixtureMgr.setImageUpgradeStatus(fixtureMgr.getFixtureById(fixture.getId()), ServerConstants.IMG_UP_STATUS_INPROGRESS);
...
//  	  EventsAndFault.FIXTURE_IMG_UP_STR, EventsAndFault.MAJOR_SEV_STR);
    ...
    public void currentNodeVersion(Fixture fixture, byte[] packet) {
      ...
      if(appId == 2) {
        ...
        fixtureMgr.updateVersion(swVersion, fixtureMgr.getFixtureById(fixtureId), gwId);
      } else {        
        logger.info(fixtureId + " Image upgrade failed");
        fixtureMgr.updateFirmwareVersion(swVersion, fixtureMgr.getFixtureById(fixtureId), gwId);
      }
      FixtImgUpgrDetails details = fixtureInProgressMap.get(fixtureId);
      if(details == null) {
      	return;
	...
	imgUpgrStatusStr = ServerConstants.IMG_UP_STATUS_INPROGRESS;
      ...
      if(imageUpgradeSuccess) {
        ...
        noOfSusSuccess++;
        fixtureMgr.setImageUpgradeStatus(fixtureMgr.getFixtureById(fixture.getId()), ServerConstants.IMG_UP_STATUS_SUCCESS);
