...
public class ProfileSyncUpImpl {
	...
	public void initiateProfileSyncActivity(Fixture oFixture,
	    ...
	    Fixture fixture =  fixtureMgr.getFixtureBySnapAddr(oFixture.getSnapAddress());
		...
		byte fixture_profile_no = (byte) fixtureMgr.getProfileNoForFixture(fixture);
		...
		if (fixture.isPushProfile() || fixture.isPushGlobalProfile()) {
			...
			if (profileGroupId != 0) {
				...
				if (bAssociationCheckRequired) {
					...
					Long groupId = fixtureMgr.assignSUGroupProfileToFixture(
							fixtureMgr.getFixtureById(fixtureId), profileGroupId);
			...
			if (bCheckProfileChecksum) {
				if ((sProfChecksum != calcSchedPrChecksum) || (gProfChecksum != calcGlobalPrChecksum)) {
					...
					ProfileHandler oFixturePFH = profileMgr
					...
					fixtureMgr.syncFixtureCustomProfile(oNewPFH, fixtureMgr.getFixtureById(fixtureId),
							group.getId());
					oNewPFH = null;
