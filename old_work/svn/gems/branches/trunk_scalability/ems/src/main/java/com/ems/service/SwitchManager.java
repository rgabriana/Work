...
public class SwitchManager {
    ...
    public void deleteSwitch(Long id) {
        ...
        gemsGroupManager.deleteGemsGroup(groupId);
	...
	public List getFloorData(Long id) {
        ...
        List<Fixture> fixturelist = fixtureManager.loadFixtureByFloorId(id);
        if (fixturelist != null) {
        ...
        List<Gateway> gatewaylist = gatewayManager.loadFloorGateways(id);
        if (gatewaylist != null) {
	...
	public List<SwitchDetail> loadSwitchDetailsByUserId(String uId) {
        ...
        User user = userManager.loadUserById(Long.valueOf(uId));
        List<SwitchDetail> switchDetails = new ArrayList<SwitchDetail>();
        List results;
    ...
    public List<Fixture> loadAllFixtureByGatewayId(Long gatewayId) {
        List<Fixture> fixtureList = fixtureManager.loadAllFixtureByGatewayId(gatewayId);
    ...
    public int manageGroupMembership(Long switchId) {
        ...
        SwitchGroup oSwitchGroup = switchGroupDao.loadSwitchGroupByGemsGroupId(oSwitch.getGemsGroup().getId());
        if (oSwitchGroup == null) {
            return 2;
        }
        int groupNo = Integer.parseInt(oSwitchGroup.getGroupNo().toString(), 16);
        List<GemsGroupFixture> oGGFxList = gemsGroupManager.getGemsGroupFixtureByGroup(oSwitch.getGemsGroup().getId());
        if (oGGFxList == null) {
    ...
    public int sendSwitchGroupParams(Long switchId, Long fixtureId) {
        ...
        try {
            ...
            if (DeviceServiceImpl.getInstance().getSuWirelessGrpConfigChangeAckStatus(fixtureId)) {
                GemsGroupFixture groupFixture = gemsGroupManager.getGemsGroupFixture(oSwitch.getGemsGroup().getId(),
                        fixtureId);
                groupFixture.setNeedSync(groupFixture.getNeedSync() | GemsGroupFixture.SYNC_STATUS_SWITCHCONF);
                gemsGroupManager.saveGemsGroupFixtures(groupFixture);
            } else {
    ...
    public byte[] getSwitchGroupParamsData(Switch oSwitch, Long fixtureId) {
      ...
      // 8 bits initial length of time in min to apply scene control (0-240) (zero is forever)
		int init_apply_time = oSwitch.getInitialSceneActiveTime() != null ? oSwitch
				.getInitialSceneActiveTime().byteValue() : 0;
      // 8 bits length of time in min to extend scene control (0-240)
		int extend_apply_time = oSwitch.getExtendSceneActiveTime() != null ? oSwitch
				.getExtendSceneActiveTime().byteValue() : 0;
      // 8 bits
    ...
    public int sendSwitchGroupWdsParams(Long switchId) {
        ...
        List<Wds> oWdsList = wdsManager.loadCommissionedWdsBySwitchId(switchId);
        if (oWdsList == null) {
            return 4;
        }
        
        // Clean up deleted WDS or dissassocited WDS;
        List<Wds> oMarkedDelWdsList = wdsManager.loadNotAssociatedWdsBySwitchId(switchId);
        ...
        try {
            ...
            for (int count = 0; count < fixtureArr.length; count++) {
                ...
                GemsGroupFixture groupFixture = gemsGroupManager.getGemsGroupFixture(oSwitch.getGemsGroup().getId(), fxId);
    ...
    public byte[] getSwitchGroupWdsParamsData(Switch oSwitch) {
      ...
      List<Wds> oWdsList = wdsManager.loadCommissionedWdsBySwitchId(oSwitch.getId());      
      ...
      try {          
          ...
          for (int count = 0; count < wds_cnt; count++) {
              ...
              ButtonManipulation oBM = wdsManager.loadWdsButtonManipulationById(oWds.getButtonMap().getId());
    ...
    public int sendSwitchGroupMsgToFixture(Long switchId, String action, int argument) {
        ...
        List<GemsGroupFixture> oGGFxList = gemsGroupManager.getGemsGroupFixtureByGroup(oSwitchGroup.getGemsGroup().getId());
        if (oGGFxList == null) {
            return 3;
        }
        if (oGGFxList.size() == 0) {
        ...
        try {
            ...
            DeviceServiceImpl.getInstance().sendGroupCommand(fixtureArr, ServerConstants.CMD_SWITCH_GRP_APPLY_ACTION,
                    output.toByteArray());
        } catch (IOException ioe) {
            logger.error(ioe.getMessage());
            return -1;
