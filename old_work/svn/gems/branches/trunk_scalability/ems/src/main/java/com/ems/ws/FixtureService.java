...
public class FixtureService {
	...
	*/
    ...
    public Response operationIdentify(@PathParam("fixtureId") Long fixtureId) {
        Response oResponse = new Response();
        fixtureManager.identifyFixture(fixtureManager.getFixtureById(fixtureId));
        return oResponse;
    }
	...
	public Response updateFixturePosition(List<Fixture> fixtures) {
		...
		while (itr.hasNext()) {
			...
			if (fixture.getXaxis() != null && fixture.getYaxis() != null) {
				fixtureManager.updatePosition(fixtureManager.getFixtureById(fixture.getId()),
	...
	public Response updateDuringCommission(Fixture fixture) {
		...
		savedFixture.setBallast(ballastManager.getBallastById(fixture.getBallast()
				.getId()));
		savedFixture
				.setBulb(bulbManager.getBulbById(fixture.getBulb().getId()));
	...
	public Response changeProfile(@PathParam("fixtureId") long fixtureId,
		...
		fixtureManager.changeFixtureProfile(fixtureManager.getFixtureById(fixtureId), groupId, currentProfile,
				originalProfile);
	...
	public Response deCommissionFixtures(List<Fixture> fixtures) {
		...
		while (itr.hasNext()) {
			...
			oResponse.setStatus(fixtureManager.deleteFixture(fixtureManager.getFixtureById(fixtureId)));
...
//			gemsGroupManager.deleteGemsGroupsFromFixture(fixtureId);
			oResponse.setStatus(fixtureManager
					.deleteFixtureWithoutAck(fixtureManager.getFixtureById(fixtureId)));
			oResponse.setMsg(String.valueOf(fixtureId)); // using message as
															// current fixture
															// id
			forgetFixtureCurve(fixtureId);
	...
	public Response deleteAllDiscoveredFixtures() {
		...
		while (itr.hasNext()) {
			...
			fixtureManager.deleteFixture(fixtureManager.getFixtureById(fixtureId));
			deleteFixtureCount++;
	...
	public Response deleteAllPlacedFixtures() {
		List<Fixture> fixtures=null ;
		Response oResponse = new Response();
		fixtures = fixtureManager.loadFixtureByState(ServerConstants.FIXTURE_STATE_PLACED_STR);
		if(fixtures!=null)
		{
		Iterator<Fixture> itr = fixtures.iterator();
		...
		Fixture fixture = new Fixture();
		String fixtureName = "";
		int deletePlacedFixtureCount =0 ;
		while (itr.hasNext()) {
			fixture = (Fixture) itr.next();
			fixtureId = fixture.getId();
			 //Let's find the name of fixture from cache. Let's put this in try/catch as failure
		...
		oResponse.setStatus(1);
		oResponse.setMsg(String.valueOf(deletePlacedFixtureCount));
    ...
    public Response rmaFixture(@PathParam("from")Long fromFixtureId, @PathParam("to")Long toFixtureId) {
      ...
      //Let's find the name of fixture from cache. Let's put this in try/catch as failure
        ...
        fixtureManager.enablePushProfileAndGlobalPushProfile(fixtureManager.getFixtureById(fromFixtureId), true, true);
	...
	public List<Ballast> getBallastList() {
		...
		List<Ballast> ballastList = ballastManager.getAllBallasts();		
		return ballastList;
    ...
    public List<BulbVO> getBulbsVOList() {
        ...
        List<BulbVO> bulbsList = bulbManager.getAllBulbVO();
        return bulbsList;
	...
	public Response setFixtureType(@PathParam("fxtype") Integer fxType, List<Fixture> fixtures) {
		...
		for(Fixture fx:fixtures )
		{
			...
			fixtureManager.updateFixtureType(fixtureManager.getFixtureById(fx.getId()), fxType);
		}
		if(fxType==1)
		{
   ...
   /* *
   		 ...
   		 try {
		        ...
		        location = campus.getName() + "->" + location;
		   		...
		   		while(iterator.hasNext())
		   		{
		   			...
		   			{
		   		       locatorDeviceManager.addLocatorDevice(info.getName(), "Unmanaged_fixture", floor.getId().toString(), info.getX().toString(), info.getY().toString()); 
		   		       continue;
		   			}
	...
	public void forgetFixtureCurve(Long fixtureId) {
		...
		fixtureManager.setUseFxCurveFlag(fixture, false);
	...
	public FixtureDetails getFixtureDetailsObject(@PathParam("fixtureId") Long fixtureId) {
        ...
        List<BallastVoltPower> voltPowerList = ballastVoltPowerManager.getBallastVoltPowerByBallastIdInputVoltage(fixture.getBallast().getId(),new Double(fixture.getVoltage()));
