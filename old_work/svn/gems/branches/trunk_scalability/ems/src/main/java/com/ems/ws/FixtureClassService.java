...
public class FixtureClassService {
    ...
    public Response deleteFixtureClassById (@PathParam("id") Long id) {
		...
		Integer fixtureCount = fixtureManager.getFixtureCountByFixtureClassId(id);
    ...
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Response bulkProfileAssign(@PathParam("currentfixturetypeid") Long currentFixturetypeId,List<Fixture> fixtureList) {
        ...
        if (fixtureIds != null && !fixtureIds.isEmpty()) {
            ...
            totalRecordUpdated = fixtureManager.bulkFixtureTypessignToFixture(fixtureIdsList, currentFixturetypeId);
        }
