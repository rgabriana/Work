...
public class GatewayInfo {
  ...
  private int setUtcTimeCmdFrequency = 5 * 60 * 1000; // 5 minutes
  ...
  private int count = 0;
  private Boolean gatewayCommError = true;
  ...
  public void setUtcCmdFrequency(int freq) {
  	...
  	if(freq > PerfSO.FIVE_MINUTE_INTERVAL) {
  		perfLogger.error(gwId + ": not taking the frequency as the frequency is more than 5 minutes");
  		return;
  	}
    ...
    public void run() {
      	...
      	try {	     		
      		if(ServerUtil.getCurrentMin() % 5 == 0) { //start at the 5th/10th/15th... minute
      			...
      			if(perfLogger.isDebugEnabled()) {
      				perfLogger.debug(gwId + ": starting the time sync task");      				
      			}
      			break;
      		}
      ...
      while(operationalMode != GW_DELETE_MODE) {
      	...
      	try {	  
      		...
      		DeviceServiceImpl.getInstance().sendUTCTime(getGw(), false);      		
      	}
      	catch(Exception ex) {
      		fixtureLogger.error(gwId + ": " + ex.getMessage());
      	}
  ...
  class GwHealthPollThread extends Thread {
	...
	try {	     
	  if(ServerUtil.getCurrentMin() % 5 == 0) { //start at the 5th/10th/15th... minute
	    if(perfLogger.isDebugEnabled()) {
	      perfLogger.debug(gwId + ": starting the Gw Stats buckets task");
	    }
	    break;
	  }
	  Thread.sleep(1000); //sleep for 1 sec
	...
	try {
		...
		if((timeSinceLastConnectivity) > 15 * 60 * 60 * 1000) {
			...
			eventMgr.addAlarm(getGw(), "gateway unreachable for (" + timeSinceLastConnectivity/1000/60 + " minutes)", EventsAndFault.GW_REACHABLILITY_FAILURE);
			GatewayInfo gwInfo = ServerMain.getInstance().getGatewayInfo(gwId);
			gwInfo.setGatewayCommError(true);
		}
	  GatewayImpl.getInstance().sendGatewayInfoReq(getGw());	  
	}
	catch(Exception ex) {
	  fixtureLogger.error(gwId + ": " + ex.getMessage());
	}
	ServerUtil.sleepMilli(gwHealthPollInterval);
      ...
      SSLSessionManager.getInstance().removeSSLConnection(gwId);
  ...
  public GatewayInfo(Gateway gw) {
    ...
    if(gatewayManager == null) {
    	gatewayManager = (GatewayManager)SpringContext.getBean("gatewayManager");
	}
  }
  ...
  public Gateway getGw() {
	  Gateway gw = null;
	  if(operationalMode != GW_DELETE_MODE) {
		   gw = gatewayManager.loadGateway(gwId);
		  if(gw == null) {
			  gatewayManager.cacheEvict(gwId);
		  }
	  }
	  return gw;
  }
...
//  }
  ...
  public void setOperationalMode(int operationalMode) {
    ...
    public void run() {
      ...
      fixtureLogger.debug((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + ": starting the retry thread");
      while(operationalMode != GW_DELETE_MODE) {
	try {
	  retryCommands();	  
	}
  ...
  public synchronized void retryCommands() {
	...
	if (cmdPkt.markRemoved || cmdPkt.retries == 0) {
          if(cmdPkt.markRemoved) {
            fixtureLogger.debug((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + ": command successful retries = " + 
	          ...
	          fixtureLogger.error((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + ": command possibly timedout");
          }
          iter.remove();
          continue;
        }	
	...
	int retryTime = DeviceInfo.COMMAND_RETRY_MILLIS;         
        if((System.currentTimeMillis() - cmdPkt.cmdTime) > retryTime) {
          try {
            fixtureLogger.info((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + "no. of pending devices -- " + noOfPendingDevices);
            if(cmdPkt.gwCommand) {
              fixtureLogger.info((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + " retry command " + ServerUtil.getLogPacket(cmdPkt.pkt));	 
              GatewayComm.getInstance().sendDataToGateway(gwId, cmdPkt.pkt);	    
            } else {
        	...
        	System.arraycopy(origPkt, orgPktPos, packet, pktPos, noOfBytesToCopy);
              ...
              fixtureLogger.info((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + " multicast retry command: " + ServerUtil.getLogPacket(packet));
              GatewayComm.getInstance().sendNodeDataToGateway(gwId, 
            		  getGw().getIpAddress(), packet);	    
            }	  
          }
          catch(Exception ex) {
            fixtureLogger.error(gwId + ": failed to send the command");
          }
...
//    } //end of constructor CommandPacket
  ...
  public synchronized void ackCmd(byte[] ackPkt) {
    ...
    fixtureLogger.debug((getGw() != null ? getGw().getIpAddress() : gwId.toString()) + ": got ack with seq no - " + key);
...
/**
 * @return the gatewayCommError
 */
public Boolean getGatewayCommError() {
	return gatewayCommError;
}

/**
 * @param gatewayCommError the gatewayCommError to set
 */
public void setGatewayCommError(Boolean gatewayCommError) {
	this.gatewayCommError = gatewayCommError;
}
