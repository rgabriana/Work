	...
	public CustomerDetailedBill viewCustomerBill(long custBillId) {
		...
		while(sppaBillIter.hasNext()) {
			...
			if(sppaBill.getBaselineEnergy() != null && sppaBill.getConsumedEnergy() != null) {
				...
				sppaCost = savedEnergy * site.getSppaPrice() / 1000;
				tax = sppaCost * site.getTaxRate() / 100;
			}
...
//		}
	...
	public void validateSiteBilling(Site site) {
		...
		System.out.println("validateing site (" + site.getGeoLocation() + ") from " + calFrom.getTime().toString() + " to " + calTo.getTime().toString());
	...
	public void validateSiteBilling(Site site, Date fromDate, Date toDate) {
		...
		while(emIdIter.hasNext()) {
			...
			while(anomalyIter.hasNext()) {
				...
				if(anomaly.equals("Baseline") || anomaly.equals("Profile") || anomaly.equals("Consumption")) {
				...
				} else if(anomaly.equals("Burn Hours")) {
		...
		if(noOfSensors > 0) {
			double burnHours = noOfEcRows * 365 / 12 / noOfSensors;
			if(site.getEstimatedBurnHours() < burnHours) {
				siteAnomalies.put("burnHours", "" + burnHours);
			}
		}
