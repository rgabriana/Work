...
public class EmInstanceManager {
	...
	public List<EmHealthDataVO> getEMHealthDataVOList() {
		...
		if (emsAuthContext.getCurrentUserRoleType().getName().equals(RoleType.Admin.getName())) {
			emInstanceList = emInstanceDao.GetHealthOfEmInstancesWithDataSynch();
		}else{
			...
			emInstanceList = emInstanceDao.GetHealthOfEmInstancesWithDataSynchByCustomerList(cList);
		}
	...
	public void generateSppaCert() throws IOException {
        ...
        try {
            ...
            logger.error("Command Failed to generate SPPACerts");
        }
    }
