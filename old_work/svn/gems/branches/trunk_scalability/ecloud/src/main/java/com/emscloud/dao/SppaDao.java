...
@SuppressWarnings("unchecked")
	...
	public Map<String, Object> validateSiteEm(String dbName, String replicaIp, Date fromDate, Date toDate) {
		...
		try {
			...
			// baseline anomaly
			String query = "SELECT ballast_name, baseline_load from ballasts where (baseline_load is null or baseline_load < 30) " +
					"AND id IN (SELECT DISTINCT ballast_id FROM fixture WHERE state = 'COMMISIONED')";
			rs = stmt.executeQuery(query);
			...
			if(sb != null) {
				siteAnomalyMap.put("Baseline", sb.toString());
			}
			
			// burn hours anomaly
			HashMap<String, Long> burnHourMap = new HashMap<String, Long>();
			query = "SELECT no_of_rows, no_of_sensors FROM (SELECT count(*) AS no_of_rows FROM energy_consumption WHERE capture_at > '" +
					DateUtil.formatDate(fromDate, "yyyy-MM-dd HH:mm:ss") + "' AND capture_at <= '" + 
					DateUtil.formatDate(toDate, "yyyy-MM-dd HH:mm:ss") + "' AND zero_bucket != 1) AS sq1, (SELECT count(*) AS no_of_sensors " + 
					"FROM fixture WHERE state = 'COMMISSIONED') AS sq2";
			rs = stmt.executeQuery(query);
			if(rs.next()) {
				burnHourMap.put("noOfEcRows", rs.getLong(1));
				burnHourMap.put("noOfSensors", rs.getLong(2));				
			}
			siteAnomalyMap.put("Burn Hours", burnHourMap);
			
			//profile anomaly
			HashMap<Integer, Long> profileMap = new HashMap<Integer, Long>();
			...
			if(sb != null) {
				siteAnomalyMap.put("Profile", sb.toString());
			}
			...
			if(rs.next()) {
				if(rs.getInt(1) > 0) {
					siteAnomalyMap.put("Consumption", rs.getInt(1));
				}				
			}
		}
		catch(Exception ex) {
