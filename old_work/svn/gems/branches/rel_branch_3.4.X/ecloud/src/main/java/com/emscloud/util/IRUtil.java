...
public class IRUtil {
    ...
    public static HashMap<String, Long> totalOtherDevices = new HashMap<String, Long>();
    ...
    public static List<Object[]> totalCommissionedErc = new ArrayList<Object[]>();
	...
	public static Long totalCommissionedErcCount = (long) 0;
    ...
    public static void resetData()
    {
        ...
        totalCommissionedErcCount = (long) 0;
        totalCommissionedSensors = new HashMap<String, Long>();
        totalOtherDevices = new HashMap<String, Long>();
        ...
        totalCommissionedErc =  new ArrayList<Object[]>();
    }
 // Aggregate Fixture Counts
    ...
    //Aggregate Other Device Count
    public static void aggregateSiteOtherDeviceData(HashMap<String, Long> emOtherDeviceData)
    {
        if(totalOtherDevices.isEmpty()){
        	totalOtherDevices.putAll(emOtherDeviceData);
        }else{
            if(emOtherDeviceData.size()>0){
                Set<String> keySet = emOtherDeviceData.keySet();
                Iterator<String> keySetIterator = keySet.iterator();
                while (keySetIterator.hasNext()) {
                   String key = keySetIterator.next();
                   Long value = emOtherDeviceData.get(key);
                   if(totalOtherDevices.containsKey(key)){
                       Long totalCount = totalOtherDevices.get(key);
                       totalCount+=value;
                       totalOtherDevices.put(key, totalCount);
                   }else{
                	   totalOtherDevices.put(key, (Long) value);
                   }
                }
            }
        }
    ...
    // Aggregate ERC Counts
    public static void aggregateErcData(List<Object[]> cuList)
    {
        if(totalCommissionedErc.isEmpty()){
        	totalCommissionedErc.addAll(cuList);
        }else{
            for(int i=0;i<cuList.size();i++)
            {
                Object[] totalCommErcItrObject = cuList.get(i);
                String ercVersion = (String) totalCommErcItrObject[0];
                Long totalCucount = ((Long) totalCommErcItrObject[1]).longValue();
                boolean isFound=false;
                for(int j=0;j<totalCommissionedErc.size();j++)
                {
                    Object[] gItrObject = totalCommissionedErc.get(j);
                    String gNewErcVersion = (String) gItrObject[0];
                    Long gCount = ((Long) gItrObject[1]).longValue();
                    if(ercVersion.equalsIgnoreCase(gNewErcVersion))
                    {
                        isFound=true;
                        gCount+=totalCucount;
                        gItrObject[1]=gCount;
                ...
                if(!isFound)
                {
                	totalCommissionedErc.add(totalCommErcItrObject);
                }
            }
        }
    }
      ...
      //Calculate Total Cus Associated
        ...
        if (totalCommissionedErc != null && !totalCommissionedErc.isEmpty()) {
            ...
            while (erciterator.hasNext()) {
                ...
                Long count = (Long) itrObject[1];
                totalCommissionedErcCount+= count;
            }
        }
