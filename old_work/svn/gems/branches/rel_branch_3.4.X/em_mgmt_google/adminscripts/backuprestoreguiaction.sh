...
#upgradeSQL path
export UPGRADE_SQL_PATH="$ehome/upgradeSQL.sql"
export SPPA_ENABLE_SQL="$ehome/sppa.sql"
...
backupfunction() {
    ...
    then
        ...
        then
            ...
            /usr/bin/pg_dump -i -U $POSTGRESUSER -v -F c -s -t energy_consumption -t energy_consumption_hourly -t energy_consumption_daily -f  "$EC_SCHEMA_BACKUP_FILE" $POSTGRESDATABASE
            /usr/bin/pg_dump -i -U $POSTGRESUSER -v -b -F c -T energy_consumption -T energy_consumption_hourly -T energy_consumption_daily -f  "${BACKUP_DIR}/${backupfile}_${BACKUP_VERSION}_$today.backup" $POSTGRESDATABASE

        else
            echo "*** Daily energy consumption backup is not there for today. Generating complete backup. ***" >> $LOG_DIR/backuprestore_error.log
            /usr/bin/pg_dump -i -U $POSTGRESUSER -v -b -F c -f  "${BACKUP_DIR}/${backupfile}_${BACKUP_VERSION}_$today.backup" $POSTGRESDATABASE
        fi
    else
        echo "*** Daily energy consumption backup is not there for today. Generating complete backup. ***" >> $LOG_DIR/backuprestore_error.log
        /usr/bin/pg_dump -i -U $POSTGRESUSER -v -b -F c -f  "${BACKUP_DIR}/${backupfile}_${BACKUP_VERSION}_$today.backup" $POSTGRESDATABASE
...
restore() {
	    ...
	    then
		    ...
		    then
		        ...
		        #echo "*** Backup File version int: ${versiononbackupfileint} ***" >> $LOG_DIR/backuprestore_error.log
  				 ...
  				 if [ "${versiononbackupfileint}" -lt "${versionsystemint}" ]    
  				 then
   					 echo "*** Versions are compatible ***" >> $LOG_DIR/backuprestore_error.log
   					 echo "*** System version: ${versiononsystem} ***"
   					 echo "*** Backup File version: ${versiononbackupfile} ***"
    ...
    then
        ...
        psql -U $POSTGRESUSER -c "SELECT pg_terminate_backend(pg_stat_activity.procpid) from pg_stat_activity where pg_stat_activity.datname = '${POSTGRESDATABASE}';"
        if [ $? -eq 0 ]
        then
    ...
    dropdb -U $POSTGRESUSER $POSTGRESDATABASE
    if [ $? -eq 0 ]
    then
    ...
    then
        ...
        then
            ...
            pg_restore -U $POSTGRESUSER -d $POSTGRESDATABASE -F c -v <  $backupfile
            if [ $? -eq 0 ]
            then
            ...
            pg_restore -U $POSTGRESUSER -d $POSTGRESDATABASE -F c -v -s <  $EC_SCHEMA_BACKUP_FILE
        
            if [ $? -eq 0 ]
            then
                echo "*** Dropping Energy Consumption constraints. ***" >> $LOG_DIR/backuprestore_error.log
                psql -U $POSTGRESUSER $POSTGRESDATABASE <  $DROPECINDEX
                if [ $? -eq 0 ]
                then
                    ...
                    pg_restore -U $POSTGRESUSER -d $POSTGRESDATABASE -F c -v -a <  $DAILY_EC_BACKUP_FILE
                    if [ $? -eq 0 ]
                    then
                        ...
                        psql -U $POSTGRESUSER $POSTGRESDATABASE <  $ADDECINDEX
                        if [ $? -eq 0 ]
                        then
        ...
        then
        	psql -U $POSTGRESUSER -d $POSTGRESDATABASE < /var/www/em_mgmt/adminscripts/cloud_backup_before_restore.sql >> $LOG_DIR/backuprestore_error.log

            gunzip -c $backupfile | psql -U $POSTGRESUSER -d $POSTGRESDATABASE >> $LOG_DIR/backuprestore_error.log

            restoreStatus=$?

            psql -U $POSTGRESUSER -d $POSTGRESDATABASE < /var/www/em_mgmt/adminscripts/cloud_backup_after_restore.sql >> $LOG_DIR/backuprestore_error.log
        else
            psql -U $POSTGRESUSER -d $POSTGRESDATABASE <  $backupfile >> $LOG_DIR/backuprestore_error.log
            restoreStatus=$?
        fi
...
#	else
#		psql -U $POSTGRESUSER $POSTGRESDATABASE -f $UPGRADE_SQL_PATH
...
#        if [ `psql -q -U $POSTGRESUSER $POSTGRESDATABASE -t -c "select 1 from system_configuration where name = 'cloud.communicate.type' and value = '2'"` -eq 1 ]
#        then
#            psql -U $POSTGRESUSER $POSTGRESDATABASE < $SPPA_ENABLE_SQL >> $LOG_DIR/backuprestore_error.log
#        fi
#	fi
