...
<spring:url value="/services/org/public/nonsecure/retrievepasskey" var="uploadPassword" scope="request" />
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		<c:if test="${param.resetSuccess == 'true'}">
			...
			$("#resetloginDiv").hide();
			$('#userNameTextBox').attr("disabled", false);
    		$(".forgotPasswordForm").height(320);
		<c:if test="${param.resetFail == 'true'}">
			$(".errorMsg").css("height","25px");
			$('#error').html('<spring:message code="error.login.incorrect.general"/>');
		</c:if>
		...
		<div class="forgotPasswordForm">
			...
			<form method="post" name="forgotPasswordForm" id="forgotPasswordFormId"
				...
				<div>
					...
					<input type="hidden" id="supportFileName" name="supportFileName" value="supp_key.txt"/>
				<div class="generatesuppkey">
						...
						<div id="loginButton">
							...
							<div>
								<span class="loginPrompt"><!--<spring:message code="forgot.file.label" /> -->
								</span>  
								<!-- <input class="forgotfileField" id="file" type="file" name="file" /> -->
							</div>
