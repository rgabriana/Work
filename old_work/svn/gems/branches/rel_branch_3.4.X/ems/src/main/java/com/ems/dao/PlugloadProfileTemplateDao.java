...
public class PlugloadProfileTemplateDao extends BaseDaoHibernate {
    ...
    public void updatePlugloadTemplateVisibility(Long templateId, boolean visibility) {
		Session session = getSession();		
		session.createQuery("Update PlugloadProfileTemplate set displayTemplate = :displayTemplate where id = :id")
				.setBoolean("displayTemplate", visibility)
				.setLong("id", templateId).executeUpdate();
	}
    
    @SuppressWarnings("unchecked")
	public List<Object> getPlugloadCountForPlugloadProfileTemplate(Long profileId) {
		try {
			Session s = getSession();
			String hsql = "SELECT plpt.id,count(pg.id) from Plugload pg, PlugloadProfileTemplate plpt where pg.state!='DELETED' and pg.groupId=:profileId group by plpt.id";
			Query query = s.createQuery(hsql);
