...
//									+ " for captureAt " + captureAt);
	...
	public void processRemainingCaptureATs() {
		if (plugloadEnergyConsumptionManager == null) {
			plugloadEnergyConsumptionManager = (PlugloadEnergyConsumptionManager) SpringContext.getBean("plugloadEnergyConsumptionManager");
		}
		...
		while(it.hasNext()){
			...
			Date captureAtTimeFromQueue = DateUtil.parseString(captAt, "yyyyMMddHHmmss");
			if(captureAtTimeFromQueue != null && captureAtTimeFromQueue.before(getTruncatedDate())){
