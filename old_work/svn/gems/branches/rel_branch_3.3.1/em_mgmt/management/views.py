...
def clouduemsettings(request):
    ...
    if request.GET.get('savestatus') != None:
    	return render_to_response('pages/clouduemsettings.html', {'savestatus':request.GET.get('savestatus')},context_instance=RequestContext(request, {'serverIpValue':serverIpValue,'uemServerIpValue':uemServerIpValue,'emMacValue':emMacValue,'cloudCommunicateTypeValue':cloudCommunicateTypeValue,'isCloudEnabled':isCloudEnabled,}))
    elif request.GET.get('savestatusuem') != None:
        return render_to_response('pages/clouduemsettings.html', {'savestatusuem':request.GET.get('savestatusuem')},context_instance=RequestContext(request, {'serverIpValue':serverIpValue,'uemServerIpValue':uemServerIpValue,'emMacValue':emMacValue,'uemCommunicateTypeValue':uemCommunicateTypeValue,'isUemEnabled':isUemEnabled, 'pktForwardingEnabled': pktForwardingEnabled}))
    elif request.GET.get('uploadStatus') != None:
        return render_to_response('pages/clouduemsettings.html', {'uploadStatus':request.GET.get('uploadStatus'),'fileuploadconfirmation': util.translate('file.upload.successful', {"filename": request.GET.get('filename')})},context_instance=RequestContext(request, {'serverIpValue':serverIpValue,'emMacValue':emMacValue,'cloudCommunicateTypeValue':cloudCommunicateTypeValue,'isCloudEnabled':isCloudEnabled,}))
    else:
    	return render_to_response('pages/clouduemsettings.html', context_instance=RequestContext(request,{'serverIpValue':serverIpValue,'uemServerIpValue':uemServerIpValue,'emMacValue':emMacValue,'cloudCommunicateTypeValue':cloudCommunicateTypeValue,'isCloudEnabled':isCloudEnabled}))    
