...
public class UtilDao extends AbstractJdbcDao{
	...
	public boolean synchTableData(String mac, String tableName,
		...
		{
			logger.debug("Em With Mac " + mac + " transfered its data to table " + tableName + " till " + String.valueOf(getLastMinTableDataSynched(mac,tableName))) ;
		}
		}}catch(Exception e){
		...
		try{
			if(status)
			{
				commonUtils.setReplicaMigrationFlagLogOnCloud(mac,CommonStateUtils.getStateInProgressAccordingToTableName(tableName) , String.valueOf(getLastMinTableDataSynched(mac,tableName)), new Date() ) ;
			}else
			{
				commonUtils.setReplicaMigrationFlagLogOnCloud(mac,CommonStateUtils.getStateInProgressAccordingToTableName(tableName) , exceptionString, null);
			}
		}catch(Exception e)
		{
			e.printStackTrace() ;
