...
import com.communicator.uem.NameValue;

@XmlRootElement
@XmlAccessorType(XmlAccessType.NONE)
public class UemResponse {
	...
	public void mapToList() {
		...
		if(namevalmap != null) {
			for(String key: namevalmap.keySet()) {
				try {
					if(UemParamType.valueOf(key).getName().equals(key)) {
						nameval.add(new NameValue(UemParamType.valueOf(key), namevalmap.get(key)));
					}
				}
