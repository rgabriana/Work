package com.enlightedinc.controllers
{
	...
	public class FixtureController
	{
		...
		private function fixtureHttpService_resultHandler(event:ResultEvent):void
		{
			...
			fpModel.profileData.disableAutoUpdate();
			fpModel.profileData.removeAll();
		...
		private function createProfileData():void
		{
			...
			for each (var fixObj : * in fpModel.commissionedFixtureData)
			{
				var obj:Object = new Object();
				obj["id"] =fixObj.groupid;
				obj["name"] =fixObj.currentprofile;
				hashMap[obj.name] = obj.id;
			}
			var result : Array = [];
			for (var key : * in hashMap)
			{
				var profileObj:Object = new Object();
				profileObj["id"] =hashMap[key];
				profileObj["name"] =key;
				result.push(profileObj);
			}
			fpModel.profileData=  new ArrayCollection(result);
		}
