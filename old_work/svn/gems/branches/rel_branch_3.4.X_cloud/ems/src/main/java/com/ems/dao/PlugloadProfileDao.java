...
public class PlugloadProfileDao extends BaseDaoHibernate {
	...
	public PlugloadProfileHandler savePlugloadProfileHandler(PlugloadProfileHandler plugloadProfileHandler) {
        try {
            //byte[] globalProfileByteArray = plugloadProfileHandler.getGlobalProfileByteArray();
           // short globalProfileChecksum = ServerUtil.computeChecksum(globalProfileByteArray);
            //plugloadProfileHandler.setGlobalProfileChecksum(globalProfileChecksum);
            short globalProfileChecksum = PlugloadImpl.getInstance().calculateGlobalProfileChecksum(plugloadProfileHandler);
            plugloadProfileHandler.setGlobalProfileChecksum(globalProfileChecksum);
            logger.debug("Global profile checksum: " + plugloadProfileHandler.getGlobalProfileChecksum());
        } catch (Exception e) {
        ...
        try {
            //byte[] profileByteArray = plugloadProfileHandler.getScheduledProfileByteArray();
            //short profileChecksum = ServerUtil.computeChecksum(profileByteArray);
            //plugloadProfileHandler.setProfileChecksum(profileChecksum);
        	
        	short profileChecksum = PlugloadImpl.getInstance().getPlugloadScheduledChecksum(plugloadProfileHandler);
        	plugloadProfileHandler.setProfileChecksum(profileChecksum);
        	logger.debug("Profile checksum: " + plugloadProfileHandler.getProfileChecksum());
