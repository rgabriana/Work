...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			{
				getProfileData();
				var userdata=  "1##"+ "profile_"+ profilenodeid +"#####END";
				start(userdata, 1, "", "asc");
			}else if(parent.accTabSelected == 'plpf'){
				getPlugloadProfileData();
				var userdata=  "1##"+ "plugloadprofile_"+ plugloadProfilenodeid +"#####END";
				start(userdata, 1, "", "asc");
			}
			else
		...
		var getPlugloadProfileData = function() 
		{
		    ...
		    if(initialSelectedNode){
		        ...
		        initialSelectedNode = "plugloadgroup_1"; 
		...
		function start(inputdata, pageNum, orderBy, orderWay) {
			jQuery("#eventsTable").jqGrid({
				...
				forceFit: true,
			   	...
			   	colModel:[
			   		{name:'eventTime',index:'eventTime', sorttype:"string", width:"15%"},
			   		{name:'fixturelocation',index:'fixturelocation', sorttype:"string", width:"25%"},
			...
			$("#t_eventsTable").append("<input type='checkbox' id='includeResolved' onclick='showOrHideResolved(this.checked)'/>");
			$("#t_eventsTable").append("<label>&nbsp;</label>");
		...
		function viewActionFormatter(cellvalue, options, rowObject) {
			return '<button onclick="showView('+ options.rowId + '); return false;" >' + cellvalue + '</>';
		}
		...
		function markResolved()	{
			if(selectedEvents.length > 0) {
				...
				jAlert('Please check at least one event.');
			}
		}
