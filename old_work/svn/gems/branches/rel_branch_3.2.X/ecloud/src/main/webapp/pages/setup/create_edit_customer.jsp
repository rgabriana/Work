...
function viewActionFormatter(cellvalue, options, rowObject) {
	...
	return "<button onclick=\"javascript:openEmInstanceList('"+rowId+"');\">Energy Managers</button>&nbsp;<security:authorize access="hasAnyRole('Admin','SystemAdmin','SupportAdmin','SPPA')"><button onclick=\"javascript: openFacilities('"+rowId+"');\">Facilities</button>&nbsp;</security:authorize>&nbsp;<security:authorize access="hasAnyRole('Admin','SystemAdmin','SPPA')"><button onclick=\"javascript: openPopup('"+rowId+"','edit');\">Edit</button>&nbsp;</security:authorize>&nbsp;<button onclick=\"javascript:openSitesList('"+rowId+"');\">Sites</button>"
}
	...
	jQuery("#customerTable").jqGrid({
		...
		colNames: ["id", "Name", "Address", "Email", "Contact","Report","Action","Billing","Inventory Report"],
	       colModel: [
	        ...
	        { name: "address", index: 'address', sortable:false, width:'15%',align: "left" },
	        ...
	        { name: "action", index: 'action',sortable:false,width:'30%', align: "left",formatter: viewActionFormatter},
