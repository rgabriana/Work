				...
				<div id="templateProfileTreeViewDiv" class = "treeviewbg" style="padding-bottom: 20px;">
					...
					<ul>
						<c:forEach items='${profileTreeHierarchy.treeNodeList}' var='template'>
							...
							class="selected"><a href='#' disabled="true" id='link_<c:out value="${template.nodeType.lowerCaseName}"/>_${template.nodeId}'>${template.name}</a>
								<ul>
									<c:forEach items='${template.treeNodeList}' var='profile'>
										<c:choose>
											<c:when test="${profile.name == 'Default'}">
													...
													><a href='#' disabled="true" id='link_<c:out value="${profile.nodeType.lowerCaseName}"/>_${profile.nodeId}'><c:out value="${profile.name}" escapeXml="true"/></a>
													</li>	
											</c:when>
											<c:otherwise>
												<li rel="${profile.nodeType.lowerCaseName}" id='<c:out value="${profile.nodeType.lowerCaseName}"/>_${profile.nodeId}'
												<c:if test="${profile.isSelected == true}">class='jstree-checked'</c:if>><a href='#' disabled="true" id='link_<c:out value="${profile.nodeType.lowerCaseName}"/>_${profile.nodeId}'><c:out value="${profile.name}" escapeXml="true"/></a>
												</li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
