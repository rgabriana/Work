...
public class UemCommunication {
	...
	static final Logger logger = Logger.getLogger(UemCommunication.class.getName());

	public UemCommunication(String url) {
		int timeout = 60 * 1000;
		try {
			urlToServer = new URL(url);
		} catch (MalformedURLException e) {
			state = CommunicatorConstant.state_comm_url_failed;
			return;
		}
		...
		if (urlToServer.getProtocol().toLowerCase().equals("https")) {
			...
			if (false == trustAllHosts()) {
				state = CommunicatorConstant.state_comm_init_failed;
				return;
			}
			...
			try {
				...
				state = CommunicatorConstant.state_comm_connect_failed;
				return;
			}
		...
		state = CommunicatorConstant.state_comm_success;
	}

	public UemCommunication(String url, String sessionId, String type, int timeout, String contentType, String acceptType) {
		...
		if(acceptType != null) {
			http.setRequestProperty("Accept", acceptType);
		}
	...
	public boolean sendData(String dataToSend) {
		...
		try {
			...
			state = CommunicatorConstant.state_comm_send_failed;
			return false;
		}
		return true;
	}
	...
	public boolean recvData() {
		try {
			...
			state = CommunicatorConstant.state_comm_read_failed;
			return false;
		}
	}
