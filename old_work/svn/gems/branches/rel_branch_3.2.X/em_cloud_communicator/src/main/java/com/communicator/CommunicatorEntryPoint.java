...
public class CommunicatorEntryPoint {
	...
	public static void main(String[] args) {
		...
		if("1".equals(uem_enable) && "1".equals(uem_pkt_forwarding_enable)) {
			...
			UEMHeartBeatManager uemHeartbeatManager = (UEMHeartBeatManager) springContext.getBean("uemHeartbeatManager");
			...
			while(true) {
				try {
					commReset = 0;
					if(CommunicatorConstant.uem_ip != null && !"".equals(CommunicatorConstant.uem_ip) && 
							CommunicatorConstant.uem_password != null && !"".equals(CommunicatorConstant.uem_password) && !" ".equals(CommunicatorConstant.uem_password) &&
									CommunicatorConstant.uem_username != null && !"".equals(CommunicatorConstant.uem_username)) {
						poll();
					}
					else {
						logger.info("UEM location = " + CommunicatorConstant.HTTPS + CommunicatorConstant.uem_ip + " and em is not registered.");
						Thread.sleep(60000);
						emManager.getUemInfo();
						uemHeartbeatManager.callUEM();
					}
			...
			while(!serverInfoManager.getCloudMode() && (CloudConnectionTemplate.key == null || CloudConnectionTemplate.key.equals("") || CloudConnectionTemplate.key.equals("key")) ) {
				serverInfoManager.init(true);
				try {
					Thread.sleep(30000);
				} catch (InterruptedException e) {
			...
			try {
				JobDetail syncDataJob = newJob(DataPrepJob.class)
						.withIdentity("DataPrepJob", 
			...
			try {
				cleanupJob = newJob(CleanupJob.class)
				...
				logger.info("Scheduled cleanup job to be run every 24 hours starting " + startAt );
			} catch (SchedulerException e) {
				logger.error(e.getMessage(), e);
			}
		}
