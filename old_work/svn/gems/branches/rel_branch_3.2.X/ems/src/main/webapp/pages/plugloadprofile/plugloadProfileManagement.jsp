...
<div id="profile-list-topPanel" style="background:#fff;">
		<div id="profile-dialog-form" style="padding-top: 10px;">
			...
			<!--
			<input type="button" id="importplugloadprofilebtn" onclick="javascript: parent.parent.onImportProfileHandler()"	value="Import plugload profile"/>
			<input type="button" id="exportAllPlugloadProfileBtn" onClick="exportAllProfiles(plugloadTemplateId)" value="<spring:message code='profile.label.exportallprofile'/>" style="float: left; margin-right:5px" />
			-->
			<br style="clear:both;"/>
...
var deletePlugloadProfileData = [];
		...
		<c:forEach items="${plugloadProfileList}" var="plugloadProfile">
			...
			if(localData.defaultProfile=="true")
			{
				...
				localData.action += "<button onclick=\"parent.parent.showPlugloadProfileDetailsForm(${plugloadProfile.id},'edit','true',plugloadTemplateId)\">"+ 
														...
														"</button>";
				/*localData.action += "<button onclick=\"parent.parent.showProfileDetailsForm(${plugloadProfile.id},'edit','true',plugloadTemplateId)\">"+ 
														"<spring:message code='profile.label.viewprofileBtn' />"+
													"</button>&nbsp;<button onclick=\"exportProfile(${plugloadProfile.id})\">"+ 
			...
			else
			{
				...
				/*localData.action += "&nbsp;<button onclick=\"exportProfile(${plugloadProfile.id})\">"+ 
				"<spring:message code='profile.label.exportprofile' />"+
				"</button>";*/
	...
	function exportAllProfiles(plugloadTemplateId)
	{
		var totalrecords = jQuery("#profileMgmtTable").jqGrid('getGridParam', 'records');
		if(totalrecords>0)
		{
			$("#exportProfileForm").attr("action", "${exportallprofilesUrl}/"+plugloadTemplateId+"?ts="+new Date().getTime());
			$('#exportProfileForm').submit();
		}else
		{
