...
public class EmsUserAuditDao extends BaseDaoHibernate {
	...
	public List<Object> getUserAudits(String order, String orderWay, List<Object> filter, int offset, int limit, Long tenantId) {
		...
		if(tenantId != null) {
			rowCount.add(Restrictions.sqlRestriction(" {alias}.user_id in (select u.id from users u where u.tenant_id = ? )", tenantId, Hibernate.LONG));
			data.add(Restrictions.sqlRestriction(" {alias}.user_id in (select u.id from users u where u.tenant_id = ? )", tenantId, Hibernate.LONG));
		}
