...
public class PlugloadDao extends BaseDaoHibernate {
	...
	public List<Plugload> loadPlugloadByProfileTemplateId(Long id) {
		...
		try {
		    ...
		    String hsql = "select pl.id, pl.name, pl.macAddress, pl.version, pl.gateway.id, pl.gateway.name, pl.location from Plugload as pl, PlugloadGroups as plg where pl.groupId=plg.id and plg.plugloadProfileTemplate.id=:plugloadProfileTemplateId";
	...
	public int getProfileNoForPlugload(long plugloadId) {
		...
		String sql = "from PlugloadGroups g where g.id = (SELECT group_id from Plugload p WHERE p.id = ?"; 
