...
public class FixtureClassDao extends BaseDaoHibernate{
	...
	public Long bulkFixtureTypessignToFixture(String fixtureIdsList,Long currentFixturetypeId) {
		...
		FixtureClass currentFixturetype = getFixtureClassById(currentFixturetypeId);
	...
	public Long bulkFixtureTypeAssignToLocatorDevices(String locatorDeviceIdsList,Long currentFixturetypeId) {
		int totalRecordUpdated = 0;
		
		try {
			if(locatorDeviceIdsList!=null && locatorDeviceIdsList.length()>0)
			{
				String hsql = "update locator_device set fixture_class_id=:currentfixturetypeid where locator_device_type in ('Unmanaged_fixture','Unmanaged_emergency_fixture') and id in ("+locatorDeviceIdsList+")";
				Query query = getSession().createSQLQuery(hsql);
				query.setLong("currentfixturetypeid", currentFixturetypeId);
				totalRecordUpdated=  query.executeUpdate();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
