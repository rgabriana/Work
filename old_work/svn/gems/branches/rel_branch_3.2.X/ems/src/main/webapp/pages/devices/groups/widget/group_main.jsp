...
<spring:url value="/services/org/motiongroup/updateGroupFixtures/" var="updateGroupToFixtureUrl" scope="request" />
<style>
	.innerContainer{
		/* padding: 10px 20px; */
	}
...
<script type="text/javascript"> 
	...
	var widget_fp = function(nodetype, nodeid) {		
		...
		if ($.browser.msie) {
			...
			FP_data +=  "<param name='flashVars' value='orgType=" + nodetype + "&orgId=" + nodeid + "&groupId=" + "${groupId}" + "&fixtureVersion=" + "${fixtureVersion}" + "&mode=GROUP_MODE&modeid='/>";
			...
			FP_data +=  " flashvars='orgType=" + nodetype + "&orgId=" + nodeid + "&groupId=" + "${groupId}" + "&fixtureVersion=" + "${fixtureVersion}" + "&mode=GROUP_MODE&modeid='/>";
			FP_data +=  "</object>";
		} else {
			FP_data = "<embed id='group_widget_floorplan' name='group_widget_floorplan' src='"+plotchartmoduleString+"' pluginspage='http://www.adobe.com/go/getflashplayer'";
			FP_data +=  " height='100%'";
