...
public class EnergyConsumptionDao extends BaseDaoHibernate {
    ...
    public void aggregateHourlyPlugloadData(Date toDate) {
        ...
        try {
            con = getSession().connection();
            con.setAutoCommit(false);
            calSt = con.prepareCall("{ call aggregatehourlyenergyconsumptionforplugload('" + new Timestamp(toDate.getTime())
                    + "')}");
            calSt.execute();
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public void aggregateDailyPlugloadData(Date toDate) {
        ...
        try {
            con = getSession().connection();
            con.setAutoCommit(false);
            calSt = con.prepareCall("{ call aggregatedailyenergyconsumptionforplugload('" + new Timestamp(toDate.getTime())
                    + "')}");
            calSt.execute();
        } catch (Exception ex) {
            ex.printStackTrace();
...
*/
	...
	public List<DashboardRecord> loadDaySummaryForManagedPlugload(Long pid,
		...
		tableName = selectTableForPlugload(to, from);
		...
		try {
			...
			if("plugload_energy_consumption".equals(tableName)){
				hsql = "SELECT date_trunc('hour', ec.capture_at) AS Hour, "
						+ "sum(ec.current_load) as totalConsum, "
						+ "sum(ec.base_load) as baseTotalConsum, "
						+ "sum(ec.saved_power) as totalPowerSaved, "
						+ "max(ec.price) as price, " + "sum(ec.cost) as cost, "
						+ "sum(ec.saved_cost) as savedCost, "
						+ "sum(ec.occ_saving) as totalOccSaved, "
						+ "sum(ec.tuneup_saving) as totalTuneupSaving,"
						+ "sum(ec.manual_saving) as totalManualSaving, "
						+ "avg(ec.current_load) as avgLoad, "
						+ "max(ec.current_load) as peakLoad, "
						+ "min(ec.current_load) as minLoad, "
						+ "sum(ec.base_cost) as baseCost " + "from " + tableName
						+ " ec where ";			
				if (!columnName.equals("company_id")) {
				...
				hsql = "SELECT date_trunc('hour', ec.capture_at) AS Hour, "
						+ "sum(ec.current_energy) as totalConsum, "
						...
						+ "sum(ec.tuneup_saving) as totalTuneupSaving,"
						+ "sum(ec.manual_saving) as totalManualSaving, "
						+ "avg(ec.current_energy) as avgLoad, "
						+ "max(ec.current_energy) as peakLoad, "
						+ "min(ec.current_energy) as minLoad, "
						+ "sum(ec.base_cost) as baseCost " + "from " + tableName
						+ " ec where ";
				if (!columnName.equals("company_id")) {
					filterTableName = "device";
				...
				avg_min_max = ((List<Object[]>) loadPeakAndMinLoadQueryForPlugload(
						"plugload_energy_consumption_hourly", columnName,
						pid.toString(), from, to)).get(0);
			}
	...
	public List<DashboardRecord> loadWeekSummaryForManagedPlugload(Long pid,
		...
		String tableName = "plugload_energy_consumption_hourly";
		try {
			String hsql = "SELECT date_trunc('day', ec.capture_at  - interval '1 hour') AS Day, "
					+ "sum(ec.current_energy) as totalConsum, "
					+ "sum(ec.base_energy) as baseTotalConsum, "
			...
			hsql += " ec.capture_at <= '"
					...
					+ " Group by Day Order by Day";
			System.out
					.println("======================== week query is " + hsql);
			Query q = getSession().createSQLQuery(hsql.toString());
			List<Object[]> results = q.list();
			Object[] avg_min_max = ((List<Object[]>) loadPeakAndMinLoadQueryForPlugload(
					"plugload_energy_consumption_hourly", columnName,
					pid.toString(), from, to)).get(0);
			if (results != null && !results.isEmpty()) {
				for (Iterator<Object[]> iterator = results.iterator(); iterator
	...
	public List<DashboardRecord> loadDaySummaryForAllPlugload(Long pid,
		...
		try {
			...
			if("plugload_energy_consumption".equals(tableName)){
				hsql = "SELECT date_trunc('hour', ec.capture_at) AS Hour, "
						+ "sum(ec.current_load) as powerUsed, "
						+ "sum(ec.current_unmanaged_load) as unmanagedPowerUsed, "
						+ "sum(ec.base_load) as basePowerUsed, "
						+ "max(ec.base_unmanaged_load) as unmanagedBasePowerUsed, " + "sum(ec.saved_power) as savedPower, "
						+ "sum(ec.saved_power_unmanaged) as unmanagedSavedPower, "
						+ "sum(ec.power_factor) as powerFactor, "
						+ "sum(ec.power_factor_unmanaged) as unmanagedPowerFactor "
						+ "from " + tableName
						+ " ec where ";	
				if (!columnName.equals("company_id")) {
					filterTableName = "device";
				...
				hsql = "SELECT date_trunc('hour', ec.capture_at) AS Hour, "
						+ "sum(ec.current_energy) as powerUsed, "
						+ "sum(ec.current_unmanaged_energy) as unmanagedPowerUsed, "
						+ "sum(ec.base_energy) as basePowerUsed, "
						+ "max(ec.base_unmanaged_energy) as unManagedBasePowerUsed, " + "sum(ec.saved_power) as savedPower, "
						+ "sum(ec.saved_power_unmanaged) as unmanagedSavedPower, "
						+ "sum(ec.power_factor) as powerFactor, "
						+ "sum(ec.power_factor_unmanaged) as unmanagedPowerFactor "
						+ "from " + tableName
			...
			if (results != null && !results.isEmpty()) {
				for (Iterator<Object[]> iterator = results.iterator(); iterator
					...
					try {
						...
						oRecord.setPowerFactor(((BigDecimal) object[7]).doubleValue());
						oRecord.setUnmanagedPowerFactor(((BigDecimal) object[8]).doubleValue());						
						oRecords.add(oRecord);
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public List<DashboardRecord> loadRecentSummaryForAllPlugload(Long pid,
		...
		try {
			String hsql = "SELECT ec.capture_at , "
					...
					+ "sum(ec.power_factor) as powerFactor, "
					+ "sum(ec.power_factor_unmanaged) as unmanagedPowerFactor, "
					+ "(avg(ec.current_load) + avg(current_unmanaged_load)) as avgLoad, "
					+ "(max(ec.current_load) + max(current_unmanaged_load)) as peakLoad, "
					+ "(min(ec.current_load) + min(current_unmanaged_load)) as minLoad "
					+ "from " + tableName
					+ " ec where ";
			if (!columnName.equals("company_id")) {
				String filterTableName = "device";
				...
				for (Iterator<Object[]> iterator = results.iterator(); iterator
					...
					if (object[1] != null) {
						try {
							...
							oRecord.setUnmanagedPowerUsed(((BigDecimal) object[2]).doubleValue());
							oRecord.setBasePowerUsed(((BigDecimal) object[3]).doubleValue() + ((BigDecimal) object[4]).doubleValue());
							oRecord.setUnmanagedBasePowerUsed(((BigDecimal) object[4]).doubleValue());
							oRecord.setSavedPower(((BigDecimal) object[5]).doubleValue() + ((BigDecimal) object[6]).doubleValue());
							oRecord.setUnmanagedSavedPower(((BigDecimal) object[6]).doubleValue());
							oRecord.setPowerFactor(((BigDecimal) object[7]).doubleValue() + ((BigDecimal) object[8]).doubleValue());
							oRecord.setUnmanagedPowerFactor(((BigDecimal) object[8]).doubleValue());
							oRecord.setAvgLoad(((BigDecimal) object[9]).floatValue());
							oRecord.setPeakLoad(((BigDecimal) object[10]).floatValue());
							oRecord.setMinLoad(((BigDecimal) object[11]).floatValue());
							oRecords.add(oRecord);
						} catch (Exception e) {
							logger.warn("Error processing request: {" + pid
									+ ", " + columnName + ", " + from + ", "
	...
	private List loadPeakAndMinLoadQueryForPlugload(String tableName,
		...
		// as power used in daily table is sum of energy for the entire day.
		if (tableName.equals("plugload_energy_consumption_daily")) {
		...
		String sql = "select avg(ecl.load) as avgLoad, max(ecl.load) as peakLoad, min(ecl.load) as minLoad from "
				+ "(select sum(current_energy) as load from "
				+ tableName
