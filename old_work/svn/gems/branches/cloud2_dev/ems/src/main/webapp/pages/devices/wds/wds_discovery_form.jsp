...
function loadGatewayData(){
	...
	if(gw_cnt==0){
		displayFxDiscoveryMessage("There is no commissioned Gateway in this floor. ERC cannot be discovered without a commissioned Gateway.", COLOR_FAILURE);
...
function StartBtnHandler(){
	...
	if(noOfInstalled == ""){
		displayFxDiscoveryMessage("No. of installed ERC should not be left blank", COLOR_FAILURE);
		return;
	}
	...
	if(isValidNumber == false){
		displayFxDiscoveryMessage("No. of installed ERC should be a Number", COLOR_FAILURE);
		return;
	}
	...
	if(1*noOfInstalled <= 1*noOfDiscovered){
		displayFxDiscoveryMessage("No. of installed ERC should not be less than or equal to no of discovered ERC", COLOR_FAILURE);
		return;
	}
	
	if(noOfInstalled > 99999){
		displayFxDiscoveryMessage("No. of installed ERC should be less than or equal to 99999", COLOR_FAILURE);
		return;		
	}
...
function startNetworkDiscovery_success(Status){
	if(Status == DISC_STATUS_STARTED) {				
		startDiscoveryTimer();
		displayFxDiscoveryMessage("Discovering ERC...", COLOR_SUCCESS);
		...
		success: function(data){
			...
			switch(bStatus) {
				case DISC_STATUS_SUCCESS:
					displayFxDiscoveryMessage("All ERC are discovered.", COLOR_SUCCESS);
...
function cancelDiscovery(){
	if (iDiscoveryInProgress == 0) {
		var choice = confirm("Are you sure you wish to cancel ERC discovery?");
		if(choice == true){
			stopDiscoveryTimer();
			reloadSwichtFrame();
			$.ajax({
...
<div id="fx-discovery-wrapper" style="height:100%; width:100%;">
		<form id="wds-discovery-form">
			...
			<div class="fieldWrapper">
				<div class="fieldlabel">Number of installed ERC:</div>
				<div class="fieldInput"><input class="text" id="noOfInstalled" name="noOfInstalled"/></div>
				<br style="clear:both;"/>
			</div>
			<div style="height:3px;"></div>
			<div class="fieldWrapper">
				<div class="fieldlabel">Number of discovered ERC:</div>
