...
public class SceneLevelDao extends BaseDaoHibernate {
    ...
    public List<SceneLevel> loadSceneLevelListInCreationOrderBySwitchAndFixtureId(Long switchId, Long fixtureId) {
        try {
            List<SceneLevel> results = null;
            String hsql = SCENELVL_CONTRACTOR + " from SceneLevel sl, Scene s where sl.sceneId=s.id and sl.switchId=? and sl.fixtureId=? order by sl.id";
            Query q = getSession().createQuery(hsql.toString());
            q.setParameter(0, switchId);
            q.setParameter(1, fixtureId);
            results = q.list();
