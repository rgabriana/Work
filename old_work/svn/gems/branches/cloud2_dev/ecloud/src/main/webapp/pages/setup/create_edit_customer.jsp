...
<spring:url value="/eminstance/billingListDetails.ems" var="emInstancesBillingListUrl" />
...
{
	$('#emInstanceCustomerId').val(rowId);
	$("#emInstanceListSubmitForm").attr("action", "${emInstancesBillingListUrl}");
	$("#emInstanceListSubmitForm").submit();
}
function viewActionFormatter(cellvalue, options, rowObject) {
	var rowId = rowObject.id;
	return "<button onclick=\"javascript:openEmInstanceList('"+rowId+"');\">Energy Managers</button>&nbsp;<security:authorize access="hasAnyRole('Admin')"><button onclick=\"javascript: openPopup('"+rowId+"','edit');\">Edit</button>&nbsp;</security:authorize>"
}
...
function openBirtInvoicePrompt(rowId){
	
	$("#invoiceBirtPromptDialog").load("${invoiceBirtPromptDialogUrl}?customerId="+rowId+"&ts="+new Date().getTime(), function() {
		  $("#invoiceBirtPromptDialog").dialog({
				modal:true,
				title: 'Enter Time Period of Report',
				width : 500,
				height : 200,
...
function viewGmbReportFormatter(cellvalue, options, rowObject) {
	 var rowId = rowObject.id;
	 return "<security:authorize access="hasAnyRole('Admin','SystemAdmin')"><button onclick=\"javascript:openGmbInvoicePrompt('"+rowId+"');\">Report</button></security:authorize>";
}

function viewBirtReportFormatter(cellvalue, options, rowObject) {
	 var rowId = rowObject.id;
	 return "<button onclick=\"javascript:openBirtInvoicePrompt('"+rowId+"');\">Invoice</button>";
}

function viewBillingFormatter(cellvalue, options, rowObject)
{
	var rowId = rowObject.id;
	return "<security:authorize access="hasAnyRole('Admin','SystemAdmin')"><button onclick=\"javascript:openEMInstanceBillingList('"+rowId+"');\">Billing</button></security:authorize>";
}
...
function startCustomerList(pageNum, orderWay) {
	jQuery("#customerTable").jqGrid({
		...
		colNames: ["id", "Name", "Address", "Email", "Contact","SPPA Price","Report","Action","Billing","Birt"],
	       colModel: [
	        ...
	        { name: "birt", index: 'birt',sortable:false,width:'10%', align: "center",formatter: viewBirtReportFormatter}],
...
function supportAdmins()
{
//disables the column headers of the jqgrid for support admins.
supportadmin = true;
admin = false;
systemadmin = false;
}
...
<security:authorize access="hasAnyRole('SupportAdmin')">
<script type="text/javascript">
    supportAdmins();
</script>
</security:authorize>
