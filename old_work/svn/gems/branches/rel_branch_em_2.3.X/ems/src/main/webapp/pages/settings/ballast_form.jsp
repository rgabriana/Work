...
function validateBallastForm(){
	...
	if ( $.trim($('#lampNum').val()) == ''){
		...
		if(!IsNumericValue($.trim($('#lampNum').val()))){
	...
	if ( $.trim($('#wattage').val()) == ''){
		...
		if(!IsNumericValue($.trim($('#wattage').val()))){
	...
	if ( $.trim($('#baselineLoad').val()) != ''){
		if(!IsDoubleValue($.trim($('#baselineLoad').val()))){
			displayLabelMessage("Baseline Load should be a Number", COLOR_FAILURE);
			return false;
		}
		baselineLoadValue = $.trim($('#baselineLoad').val());
	}else{
		baselineLoadValue = "";
	}
	...
	if($.trim($('#displayLabel').val()) != ''){
		displayLabelValue = $.trim($('#displayLabel').val());
	}else{
		displayLabelValue = "";
	}
...
function checkForDuplicateBallast(){
	...
	if("${mode}" == 'edit'){
		...
		"<inputVoltage>"+$.trim($('#inputVoltage').val())+"</inputVoltage>"+
		"<bulbType>"+$.trim($('#lampType').val())+"</bulbType>"+
		"<noOfBulbs>"+$.trim($('#lampNum').val())+"</noOfBulbs>"+
		"<ballastFactor>"+$.trim($('#ballastFactor').val())+"</ballastFactor>"+
		"<voltpowermapid>"+$.trim("${ballast.voltPowerMapId}")+"</voltpowermapid>"+
		"<bulbWattage>"+$.trim($('#wattage').val())+"</bulbWattage>"+
		"<ballastManufacturer>"+$.trim($('#ballastManufacturer'))+"</ballastManufacturer>"+
