...
//	}
	...
	public List<Fixture> getFixtures(String dbName, String replicaIp, Long floorId) {
		...
		try {
			...
			connection = DatabaseUtil.getDbConnection(dbName, replicaIp);
			stmt = connection.createStatement();
			String query = "select d.id, name, x, y, mac_address, current_profile, baseline_power, d.version, f.gateway_id from device d, fixture f, " +
					"ballasts b WHERE d.id = f.id AND f.ballast_id = b.id AND state = 'COMMISSIONED' AND floor_id = " + floorId;
			System.out.println("query - " + query);
			rs = stmt.executeQuery(query);
			while(rs.next()) {
	...
	public List<Device> getDevices(String dbName, String replicaIp, Long floorId) {
		...
		try {
			...
			connection = DatabaseUtil.getDbConnection(dbName, replicaIp);
			stmt = connection.createStatement();
			String query = "select id, name, x, y, mac_address, type, version from device d WHERE floor_id = " + floorId;
			System.out.println("query - " + query);
			rs = stmt.executeQuery(query);
			while(rs.next()) {
