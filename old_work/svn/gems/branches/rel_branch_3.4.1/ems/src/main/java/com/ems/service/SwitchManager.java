    ...
    public int sendSwitchGroupMsgToFixture(Long switchId, String action, int argument) {
        ...
        try {
            ...
            if (action.equalsIgnoreCase("scene")) {
                ...
                scene_control_action = (byte) ((ServerConstants.SU_SWITCH_GRP_BUTTON_MAP_SET_ABSOLUTE_DIM << 4) + argument);
            } else {
                scene_control_action = ServerConstants.SU_SWITCH_GRP_BUTTON_MAP_SET_TO_AUTO << 4;
            }
            output.write(scene_control_action);
            
            DeviceServiceImpl.getInstance().sendGroupBcastCommand(fixtureArr, ServerConstants.CMD_SWITCH_GRP_APPLY_ACTION,
                    output.toByteArray());
        } catch (IOException ioe) {
            logger.error(ioe.getMessage());
            return -1;
