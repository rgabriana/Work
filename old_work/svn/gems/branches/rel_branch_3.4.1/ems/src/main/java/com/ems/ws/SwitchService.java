...
//        oResponse.setStatus(switchManager.sendSwitchGroupWdsParams(switchId));
    ...
    protected Response sendSwitchGroupMsgToFixtures(Switch fetchedSwitch, int argument, String action) {
        ...
        {	        	
        		//For scene toggle/change , dimup , dimdown, auto, dim
        	if(action.equalsIgnoreCase("dim")) {
        		if (argument < 0 || argument > 100) {
        			oResponse.setMsg("Invalid dim percentage value " + argument);
        			oResponse.setStatus(5);
        			return oResponse;
        		}
     		   	m_Logger.debug("Switch2.x absolute dim: " + switchId + ", Percentage: " + argument);
     		   	userAuditLoggerUtil.log("Dim switch  " + mSwitchName
     		   			+ " by " + argument + "%",
     		   			UserAuditActionType.Switch_Fixture_Dimming.getName());
    ...
    public Response updateSwitchFixtures(@PathParam("switchId") Long switchId, List<Fixture> fixtures) {
    	Response response = updateSwitchGroupFixtures(switchId,fixtures);
        return response;
    }
    protected Response updateSwitchGroupFixtures(Long switchId,List<Fixture> fixtures)
    {
    	String resp = "";
        Switch sw = switchManager.getSwitchById(switchId);
