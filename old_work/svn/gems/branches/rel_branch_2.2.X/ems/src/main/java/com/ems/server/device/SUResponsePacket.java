    	...
    	case ServerConstants.REPT_PM_DATA_MSG_TYPE:	  	  
        	...
        	if (gwInfo.getOperationalMode() == GatewayInfo.GW_NORMAL_MODE) {
        		if (fixture.getState().equals(ServerConstants.FIXTURE_STATE_DELETED_STR)) {
        			logger.error(fixture.getId() + ": received pm stats from deleted fixture through gateway - " + gwId);        			
        			String sMsg = snapAddr + ": Fixture already deleted, resending default params to fixture ";
        			fixturelogger.info(sMsg);
        			eventMgr.addEvent(fixture, sMsg, EventsAndFault.WIRELESS_PARAMS_MISMATCH_STR, EventsAndFault.INFO_SEV_STR);
