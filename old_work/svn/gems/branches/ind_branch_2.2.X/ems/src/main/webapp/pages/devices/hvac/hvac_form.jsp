...
function validateForm()
{
	...
	if( $("#hvaccontrollerId").val().match(numericExpression)){
	}else
	{
		setHvacDeviceMessage("Please enter numeric values for Controller Id", COLOR_FAILURE);
		return false;
	}
...
function getHVACXML(){
	...
	var hvaccontrollerid = $("#hvaccontrollerId").val();
	var hvacType = $("#hvacdevicetypecombo").val();
	var xaxis = "${hvacDevice.xaxis}";
	var yaxis = "${hvacDevice.xaxis}";
	var floorid = "${floorid}";
	var hvacUpdate = "<hvacdevice><name>"+hvacDeviceName+"</name><controllerid>"+hvaccontrollerid+"</controllerid><deviceid>"+hvacid+"</deviceid><floorid>"+floorid+"</floorid><xaxis>"+xaxis+"</xaxis><yaxis>"+yaxis+"</yaxis><devicetype>"+hvacType+"</devicetype></hvacdevice>"; 
	alert(hvacUpdate);
	return hvacUpdate;
}
...
<table id="apf-wrapper-table" width=100% height=100%>
	...
	<tr>
		<td id="apf-form-container" valign="top" colspan="2">
			<div class="fieldLabel">Controller Id:</div> 
					<div><form:input id="hvaccontrollerId" path="controllerId" type="text" /></div>
