...
<spring:url value="/devices/switches/assignareatoswitchesandfixtures.ems" var="assignAreaToSwitchesAndFixturesDailogUrl" scope="request" />
...
<spring:url value="/devices/hvac/create/dialog.ems" var="havcCrateDialogUrl" scope="request" />
<spring:url value="/devices/hvac/hvacdetails.ems" var="havcDetailsDialogUrl" scope="request" />
...
function assignAreaToSwitchesAndFixtures(selSwitches,selFixtures){
	if(selSwitches != undefined) {
		SELECTED_SWITCHES_TO_ASSIGN_AREA = eval("("+selSwitches+")");	
	}
	...
	$("#assignAreaToSwitchesAndFixturesDailog").load("${assignAreaToSwitchesAndFixturesDailogUrl}"+"?ts="+new Date().getTime(), function() {
		$("#assignAreaToSwitchesAndFixturesDailog").dialog({
			modal:true,
			title: 'Assign area to Selected Switches and Fixtures',
			width : Math.floor($('body').width() * .40),
			height : 130,// Math.floor($('body').height() * .30),
			close: function(event, ui) {
				SELECTED_SWITCHES_TO_ASSIGN_AREA = []; //Reset global variable 'SELECTED_FIXTURES_TO_ASSIGN_AREA'
...
var SELECTED_HVAC=[];
function createHvacDeviceForm(selHvac)
{
	SELECTED_HVAC = eval("("+selHvac+")");	
	$("#createHvacDeviceDailog").load("${havcCrateDialogUrl}?xaxis="+SELECTED_HVAC[0].xaxis + "&yaxis="+SELECTED_HVAC[1].yaxis  + "&floorid="+SELECTED_HVAC[2].floorid +"&ts="+new Date().getTime(), function() {
		$("#createHvacDeviceDailog").dialog({
			modal:true,
			title: 'Create HVAC Device ',
			width : Math.floor($('body').width() * .40),
			height : 200,
			close: function(event, ui) {
				SELECTED_HVAC=[];
...
function showHvacDetailsForm(controllerId,hvacId,hvacName) {	
	$("#hvacDetailsDialog").load("${havcDetailsDialogUrl}?controllerid="+controllerId+"&hvacId="+hvacId+ "&ts="+ new Date().getTime(), function() {
		$("#hvacDetailsDialog").dialog({
			modal:true,
			title: hvacName,
			width : Math.floor($('body').width() * .22),
			height : 175,
			close: function(event, ui) {
				try {
					getFloorPlanObj("floorplan").plotChartRefresh();
...
<div id="createHvacDeviceDailog"></div>
<div id="hvacDetailsDialog"></div>
<div id="assignAreaToFixturesDailog"></div>
<div id="assignAreaToSwitchesDailog"></div>
<div id="assignAreaToSwitchesAndFixturesDailog"></div>
