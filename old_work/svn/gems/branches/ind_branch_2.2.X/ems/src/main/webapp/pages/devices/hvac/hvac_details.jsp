...
<spring:url value="/services/org/hvac/op/controller" var="hvacDeviceStatus" scope="request" />
<spring:url value="/services/org/hvac/op/controller" var="hvacUrl" scope="request" />
...
<script type="text/javascript">
	...
	function getModbusRunningStatus()
	{
		...
		$.ajax({
			url: "${hvacUrl}"+ "/"+ controllerId+ "/device/"+ hvacId + "/function/1003/-1",
			type: "POST",
			dataType:"json",
	...
	function loadTemperature()
	{
		...
		$.ajax({
			url: "${hvacUrl}" + "/"+ controllerId+ "/device/"+ hvacId + "/function/1000/-1",
			type: "POST",
			dataType:"json",
			contentType: "application/json; charset=utf-8",
			success: function(data){
	...
	function loadAmbientTemperature()
	{
		...
		$.ajax({
			url: "${hvacUrl}" + "/"+ controllerId+ "/device/"+ hvacId + "/function/1001/-1?ts="+new Date().getTime(),
			type: "POST",
			dataType:"json",
			contentType: "application/json; charset=utf-8",
			success: function(data){
	...
	function onHvac()
	{
		if(isOnInProcess==false)
		{
			...
			$.ajax({
				url: "${hvacUrl}" + "/"+ controllerId + "/device/" +hvacId+"/function/2002/9?ts="+new Date().getTime(),
				type: "POST",
				dataType:"json", 
				contentType: "application/json; charset=utf-8",
				success: function(data){
	...
	function offHvac()
	{
		if(isOffInProcess==false)
		{
			...
			$.ajax({
				url: "${hvacUrl}"+ "/"+ controllerId + "/device/" + hvacId+"/function/2002/1?ts="+new Date().getTime(),
				type: "POST",
				dataType:"json", 
				contentType: "application/json; charset=utf-8",
				success: function(data){
	...
	function incrementTemperature()
	{
		...
		if(temperatureVal!='NA')
		{
			...
			$.ajax({
				url: "${hvacUrl}"+ "/"+ controllerId + "/device/"+ hvacId + "/function/2000/"+temperatureVal+"?ts="+new Date().getTime(),
				type: "POST",
				dataType:"json", 
				contentType: "application/json; charset=utf-8",
				success: function(data){
	...
	function getFunctionStatus(deviceId, functionId, args)
	{
		setHvacDeviceMessage("Loading Temperature...", COLOR_SUCCESS);
		$.ajax({
			url: "${hvacDeviceStatus}"+"/" + controllerId +"/status/device/" +deviceId+"/function/"+functionId+"/args/"+args+"?ts="+new Date().getTime(),
			type: "GET",
			dataType:"json", 
			contentType: "application/json; charset=utf-8",
			success: function(data){
