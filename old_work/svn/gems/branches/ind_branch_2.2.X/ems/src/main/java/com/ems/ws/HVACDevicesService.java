...
public class HVACDevicesService {
	 ...
	 * @return response
	 */
	@Path("status/controller/{cid}/device/{deviceid}/function/{functionid}")
	@POST
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response postHVACDeviceResult(
			@PathParam("cid") String controllerId,
		...
		HvacDeviceStatusCache oDeviceStatus = HvacDeviceStatusCache
				.getInstance();
		oDeviceStatus.setFunctionResponseForDevice(controllerId, deviceId, functionId,
				oDeviceResponse);
		logger.info("controllerId :"+ controllerId + ", Device: " + deviceId + ", functionId: " + functionId
	 ...
	 */
	@Path("op/controller/{cid}/device/{deviceid}/function/{functionid}/{args:.*}")
	@POST
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response deviceOperation(@PathParam("cid") Integer controllerId,@PathParam("deviceid") Integer deviceId,
		...
		oConsole.append("COntroller: ").append(controllerId);
		...
		if (oFunction != null) {
			...
			case READ_ROOM_TEMP:
				procArgs = new String[] {sSudo, sProgram, "-o",
						String.valueOf(controllerId),String.valueOf(deviceId), String.valueOf(functionId) };
				break;
			case READ_AMB_TEMP:
				procArgs = new String[] {sSudo,  sProgram, "-o",
						String.valueOf(controllerId),String.valueOf(deviceId), String.valueOf(functionId) };
			...
			case SET_ROOM_TEMP:
				if (args != null) {
					...
					procArgs = new String[] {sSudo, sProgram, "-o",
							String.valueOf(controllerId),String.valueOf(deviceId), String.valueOf(functionId), String.valueOf(args) };
	 ...
	 */
	@Path("op/controller/{cid}/status/device/{deviceid}/function/{functionid}/args/{args}")
	@GET
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public DeviceResponse getDeviceFunctionStatus(@PathParam("cid") String controllerId,
			@PathParam("deviceid") String deviceId,
			@PathParam("functionid") String functionId,
			@PathParam("args") String args) {
		return HvacDeviceStatusCache.getInstance()
				.getFunctionResponseForDevice(controllerId,deviceId, functionId, args);
	}
