...
then
    ...
    EM_VER_NUMBER1=$(echo "$3" | cut -d '.' -f1)
    EM_VER_NUMBER2=$(echo "$3" | cut -d '.' -f2)
    EM_VER_NUMBER3=$(echo "$3" | cut -d '.' -f3)
...
upgradeAll() {
    ...
    DEBIAN_VERSION_NUMBER=$(dpkg-deb -f "$DEBIAN_FILE_NAME" Version)
    DEBIAN_REV_NUMBER=$(dpkg-deb -f "$DEBIAN_FILE_NAME" CurrentRevision)
	
    DEBIAN_VERSION_NUMBER1=$(echo $DEBIAN_VERSION_NUMBER | cut -d '.' -f1)
    DEBIAN_VERSION_NUMBER2=$(echo $DEBIAN_VERSION_NUMBER | cut -d '.' -f2)
    DEBIAN_VERSION_NUMBER3=$(echo $DEBIAN_VERSION_NUMBER | cut -d '.' -f3)
    ...
    then
        ...
        if [ $DEBIAN_VERSION_NUMBER1 -lt $EM_VER_NUMBER1 ]
	then		
            echo "UPGRADE ERROR: Cannot upgrade to lower version"
            status="2"
            return
        elif [ $DEBIAN_VERSION_NUMBER1 -eq $EM_VER_NUMBER1 -a $DEBIAN_VERSION_NUMBER2 -lt $EM_VER_NUMBER2 ]
	then		
            echo "UPGRADE ERROR: Cannot upgrade to lower version"
            status="2"
            return
	elif [ $DEBIAN_VERSION_NUMBER1 -eq $EM_VER_NUMBER1 -a $DEBIAN_VERSION_NUMBER2 -eq $EM_VER_NUMBER2 -a $DEBIAN_VERSION_NUMBER3 -lt $EM_VER_NUMBER3 ]
	then		
            echo "UPGRADE ERROR: Cannot upgrade to lower version"
            status="2"
            return
	elif [ $DEBIAN_VERSION_NUMBER1 -eq $EM_VER_NUMBER1 -a $DEBIAN_VERSION_NUMBER2 -eq $EM_VER_NUMBER2 -a $DEBIAN_VERSION_NUMBER3 -eq $EM_VER_NUMBER3 -a $DEBIAN_REV_NUMBER -lt $EM_REV_NUMBER ]
        then
            echo "UPGRADE ERROR: Cannot upgrade to lower version"
            status="2"
            return
    ...
    echo "*** Preparing all EM components for upgrade using env variables ***" >> $LOGS_ERR
    ...
    $UPGRADE_RUN_PATH/upgrade_run.sh "$DEBIAN_FILE_NAME" "$2" "$3" "$4" "$5" "$6"
    status="$?"
    echo "*** Status executing $UPGRADE_RUN_PATH/upgrade_run.sh is $status ***" >> $LOGS_ERR
    if [ "$status" -eq 3 ]
    then
        status="1"
        rollback="Y"
...
startTomcat() {
    ...
    if [[ "$startStatus" =~ "done" ]]
    then
        echo "*** Tomcat service is running. EM application should be up again ***" >> $LOGS_ERR
        echo "*** NOTE: If you are not able to access EM application in another 2-3 minutes, please raise an alarm ***" >> $LOGS_ERR
        return
    fi
    status="1"
    echo "*** UPGRADE ERROR: TOMCAT RESTART FAILED. Please do it manually ***" >> $LOGS_ERR
}
...
then
    ...
    then
        ...
        if [ "$status" == "0" ]
        then
		echo "*** NOTE: Done. ***" >> $LOGS_ERR
            #if [ -f "$BACKUPSCRIPT" ]
            #then
                #/bin/bash $BACKUPSCRIPT "backup" "afterUpgradeDBBackup" "$OPT_ENLIGHTED/DB/DBBK" "$APP_ROOT" "$IP_ADDRESS" &
            #fi
        fi
    fi
