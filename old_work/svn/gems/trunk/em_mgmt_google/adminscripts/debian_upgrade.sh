...
then
  ...
  if [ `psql -q -U postgres -t -c "select count(*) from pg_database where datname='ems'"` -eq 1 ]
  then
    # verify that switch_fixtures table exists (v2.1)
    if [ `psql -q -U postgres ems -t -c "select count(*) from pg_tables where tablename = 'switch_fixtures'"` -eq 1 ]
    then
      #check no. of groups for each sensor
      if [ `psql -q -U postgres ems -t -c "select count(*) from (select fixture_id, count(*) from (select id, fixture_id, group_id from gems_group_fixture union select id, fixture_id, switch_id from switch_fixtures) AS sq group by fixture_id having count(*) > 10) AS sq2"` -gt 0 ]
      then
...
upgradeAll() {
    ...
    $ehome/upgrade_run.sh "$DEBIAN_FILE_NAME" "$2" "$3" "$4" "$5" "$6"
    status="$?"
