...
restore() {
    ...
    then
        ...
        then
        	psql -U $POSTGRESUSER -d $POSTGRESDATABASE < $EM_MGMT_BASE/em_mgmt/adminscripts/cloud_backup_before_restore.sql >> $LOG_DIR/backuprestore_error.log

            gunzip -c $backupfile | psql -U $POSTGRESUSER -d $POSTGRESDATABASE >> $LOG_DIR/backuprestore_error.log

            restoreStatus=$?

            psql -U $POSTGRESUSER -d $POSTGRESDATABASE < $EM_MGMT_BASE/em_mgmt/adminscripts/cloud_backup_after_restore.sql >> $LOG_DIR/backuprestore_error.log
        else
