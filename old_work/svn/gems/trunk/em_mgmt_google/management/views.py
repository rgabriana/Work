...
def upgrade(request):
    path = os.environ["ENL_APP_HOME"]+'/Enlighted/UpgradeImages/'
    os.chdir(path)
    d = []
...
def saveUemsettings(request):
    ...
    pktFwdCB = request.POST.get('pktForward', False) 
    if pktFwdCB:
    ...
    savestatus = 'S'
    return HttpResponseRedirect('/em_mgmt/uemsettings?savestatus='+ savestatus)
...
def saveCloudsettings(request):
    ...
    if checkbox:
        ...
        util.updateSystemConfig('0', 'uem.enable')
        util.updateSystemConfig('true', 'enable.profilefeature')
        util.updateSystemConfig('', 'uem.apikey')
        util.updateSystemConfig('', 'uem.secretkey')
    util.updateSystemConfig(serverIp1, 'uem.ip')
