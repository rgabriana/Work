...
public class WDSImageUpgradeWorker extends Thread {
  ...
  private Long[] devices; //array of wdss which are upgraded by this worker
   ...
   *
   */
  public WDSImageUpgradeWorker(ImageUpgradeDBJob job) {
  ...
  public void startImageUpload(Long[] wdsIds, String fileName) {
  ...
  class GwWdsImgUpgThread extends Thread {
      ...
      try {
      	...
      	for(int i = 0; i < noOfWdss; i++) {
      		...
      		firmUpMgr.finishDeviceUpgrade(dbJob.getId(), wds.getId(),
      				ServerConstants.IMG_UP_STATUS_FAIL, details.noOfAttempts, description, wds.getVersion());
      		// Override default severity for ERC Image failure.
      		eventMgr.addAlarm(wds, description, EventsAndFault.EWS_IMG_UP_STR, EventsAndFault.MAJOR_SEV_STR);
    ...
    public void startMulticastImageUpload(ArrayList<Wds> wdsList, int imageType) {
      ...
      for(int i = 0; i < noOfWdss; i++) {
        ...
        //TODO
        eventMgr.addEvent(wds, description, EventsAndFault.EWS_IMG_UP_STR);
        wdsIds[i] = wds.getId();
        WdsImgUpgrDetails details = wdsInProgressMap.get(wds.getId());
        if(details != null) {
    ...
    private String updateUpgradeCompleteStatus(Wds wds, WdsImgUpgrDetails details,
    		int currVer,int imgUpgrStatus, String version) {
      ...
      if(imageUpgradeSuccess) {
        ...
        //TODO
        eventMgr.addEvent(wds, details.description, EventsAndFault.EWS_IMG_UP_STR);
        firmUpMgr.finishDeviceUpgrade(dbJob.getId(), wds.getId(),
    ...
    public String nodeRebooted(Wds wds, short currVer, int imgUpgrStatus, String version) {
      ...
      return updateUpgradeCompleteStatus(wds, details, currVer, imgUpgrStatus, version);
  ...
  private void broadcastDataPacket(int offset, int dataSize, long gwId) {
    ...
    WDSImpl.getInstance().broadcastImageUprade(gwId, dataArray,
  ...
  public String nodeRebooted(Wds wds, short currVer, long gwId, int imgUpgrStatus, String version) {

    if(gwWdsUpgThrMap.containsKey(gwId)) {
      return gwWdsUpgThrMap.get(gwId).nodeRebooted(wds, currVer, imgUpgrStatus, version);
    }
    return ServerConstants.IMG_UP_STATUS_NOT_PENDING;
