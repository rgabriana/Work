...
public class EnergyConsumptionManager {
    ...
    public void aggregateFixtureHourlyData(Date toDate, Long fixtureId) {
      energyConsumptionDao.aggregateFixtureHourlyData(toDate, fixtureId);
  }
    ...
    public void aggregateHourlyPlugloadData(Date toDate) {
        energyConsumptionDao.aggregateHourlyPlugloadData(toDate);
    }

    public void aggregateDailyPlugloadData(Date toDate) {
        energyConsumptionDao.aggregateDailyPlugloadData(toDate);
    }
    
    public void updatePlugloadZeroBucketData() {
        energyConsumptionDao.updatePlugloadZeroBucketData();
    }
    
    public void pruneData() {
      energyConsumptionDao.pruneData();
  }
...
/*    public List<DRRecord> loadAvgEnergyConsumptionBetweenPeriodsPerDay(Date fromTimeOfDay, Date toTimeOfDay,
    ...
    public Map<Integer, Double> getFixtureECOverPeriod(Date from, Date to) {
        return energyConsumptionDao.getFixtureECOverPeriod(from, to);
    }
    
    public Map<Integer, Double> getRecentFixtureEC() {
        return energyConsumptionDao.getRecentFixtureEC();
    }
    ...
    /**
     * Returns stats data for the fixtures specified time interval w.r.t org level
     * 
     * @param property
     *            (campus_id, building_id, floor_id, area_id, fixture_id)
     * @param pid
     *            unique property id
     * @param from
     *            latest date
    ...
    public List loadStatsInDateRange(String property, Long pid, Date from, Date to) {
        return energyConsumptionDao.loadStatsInDateRange(property, pid, from, to);
    }
    ...
    /**
     * Returns individual plugload groups summary in the specified time interval w.r.t org level
     * 
     * @param property
     *            (campus_id, building_id, floor_id, area_id)
     * @param pid
    ...
    public List<GroupECRecord> loadPlugloadGroupEnergyConsumption(String property, Long pid, Date from, Date to) {
        return energyConsumptionDao.loadPlugloadGroupEnergyConsumption(property, pid, from, to);
    }

	public Date loadFirstRecordDate() {
		
		return energyConsumptionDao.loadFirstRecordDate();
	}
	...
	public List<DashboardRecord> loadHourSummaryBetween(Long pid, String property,Date from , Date to) {
		  ...
		  return  energyConsumptionDao.loadHourSummaryBetween(pid, property, from,to);	
		     
	}
	
    /**
     * load aggregated daily summary for children nodes
     * 
     * @param columnName
     ...
     * @return
     */
    public List<DashboardRecord> loadChildrenDaySummary(Long id, String columnName, String childColumnName, Date dateOn) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(dateOn);
        Date from = cal.getTime();
        cal.setTime(dateOn);
        ...
        return energyConsumptionDao.loadChildrenNodeSummary(id, columnName, childColumnName, from, to);
    }
    
    /**
     * load aggregated weekly summary for children nodes
     * 
     * @param id
     * @param columnName
     * @param childColumnName
     ...
     * @return Dashboard records
     */
    public List<DashboardRecord> loadChildrenWeekSummary(Long id, String columnName, String childColumnName, Date dateOn) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(dateOn);
        Date from = cal.getTime();
        // reset
    ...
    /**
     * load aggregated monthly summary for children nodes
     * 
     * @param id
     * @param columnName
     * @param childColumnName
     ...
     * @return Dashboard records
     */
    public List<DashboardRecord> loadChildrenMonthSummary(Long id, String columnName, String childColumnName, Date dateOn) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(dateOn);
        Date from = cal.getTime();
        // reset
    ...
    /**
     * load aggregated yearly summary for children nodes
     * 
     * @param id
     * @param columnName
     * @param childColumnName
     ...
     * @return Dashboard records
     */
    public List<DashboardRecord> loadChildrenYearSummary(Long id, String columnName, String childColumnName, Date dateOn) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(dateOn);
        Date from = cal.getTime();
        // reset
    ...
    public List<DashboardRecord> loadChildrenSummaryInPeriod(Long id, String columnName, String childColumnName, Date from, Date to) {
        return energyConsumptionDao.loadChildrenNodeSummary(id, columnName, childColumnName, from, to);
    }
    
    public List<SensorHistory> getMotionBits(Long lastSyncId, Long limit) {
    	return energyConsumptionDao.getMotionBits(lastSyncId, limit);
    }
    
    public List<SensorHistory> getAvgTemperature(Long lastSyncId, Long limit) {
    	return energyConsumptionDao.getAvgTemperature(lastSyncId, limit);
    }
    
    public List<SensorHistory> getAmbientLight(Long lastSyncId, Long limit) {
    	return energyConsumptionDao.getAmbientLight(lastSyncId, limit);
    }
    
    public List<SensorHistory> getSensorPower(Long lastSyncId, Long limit) {
    	return energyConsumptionDao.getSensorPower(lastSyncId, limit);
    }
	
    public List<EcSyncVo> load15minFloorEnergyConsumptionForAllFloorWithZb(Date oFDate) {
		return energyConsumptionDao.load15minFloorEnergyConsumptionForAllFloorWithZb(oFDate);
	}
    public EnergyConsumption loadLatestEnergyConsumption() {
    	return energyConsumptionDao.loadLatestEnergyConsumption();
    }
    public EnergyConsumption loadFirstEnergyConsumption() {
    	return energyConsumptionDao.loadFirstEnergyConsumption();
    }

	public List<FacilityEnergyStats> getECDataByFloor(Date minTime,
			Date maxTime, Long floorId) {
		return energyConsumptionDao.getEnergyConsumptionByFloor(minTime, maxTime,floorId);
	
	}

	public List<SensorEnergyStats> getECDataByFixture(Date minTime, Date maxTime,Long floorId){
    	return energyConsumptionDao.getEnergyConsumptionByFloorAndFixture(minTime, maxTime,floorId);
    }

	public List<DashboardRecord> loadSummaryInPeriodForAllPlugload(Long pid,
			String propertyName, Date from, Date to) {
		long diff = from.getTime() - to.getTime();
        // Calculate difference in days
        long diffDays = diff / (24 * 60 * 60 * 1000);
    	Calendar cal = Calendar.getInstance();
        ...
        if (diffDays <= 1) {
            ...
            return energyConsumptionDao.loadDaySummaryForAllPlugload(pid, propertyName, from, to);
        } else if (diffDays <= 6) {
        	
        	to = cal.getTime();
            return energyConsumptionDao.loadWeekSummaryForAllPlugload(pid, propertyName, from, to);
        } else if (diffDays <= 31) {
        	
        	to = cal.getTime();
            return energyConsumptionDao.loadMonthSummaryForAllPlugload(pid, propertyName, from, to);
        }
        return energyConsumptionDao.loadYearSummaryForAllPlugload(pid, propertyName, from, to);
		
	}
	
	public List<DashboardRecord> loadSummaryInPeriodForManagedPlugload(Long pid,
			String propertyName, Date from, Date to) {
		long diff = from.getTime() - to.getTime();
        // Calculate difference in days
        long diffDays = diff / (24 * 60 * 60 * 1000);
        ...
        if (diffDays <= 1) {    
            return energyConsumptionDao.loadDaySummaryForManagedPlugload(pid, propertyName, from, to);
        } else if (diffDays <= 6) {    
        	to = cal.getTime();
            return energyConsumptionDao.loadWeekSummaryForManagedPlugload(pid, propertyName, from, to);
        } else if (diffDays <= 31) {    
        	to = cal.getTime();
            return energyConsumptionDao.loadMonthSummaryForManagedPlugload(pid, propertyName, from, to);
        }
        return energyConsumptionDao.loadYearSummaryForManagedPlugload(pid, propertyName, from, to);
		
	}

	public List<DashboardRecord> loadRecentSummaryForAllPlugload(Long pid,
			String property, Date oFDate) {
		 Calendar cal = Calendar.getInstance();
	        cal.setTime(oFDate);
	        ...
	        return energyConsumptionDao.loadRecentSummaryForAllPlugload(pid, property, from, to);
	...
	public List<DashboardRecord> loadRecentSummaryForManagedPlugload(Long pid,
		 ...
		 Calendar cal = Calendar.getInstance();
	        ...
	        return energyConsumptionDao.loadRecentSummaryForManagedPlugload(pid, property, from, to);
		
	}
	
	public List<DashboardRecord> load15minRecentSummary(Long pid, String property, Date oFDate) {
	 	Calendar cal = Calendar.getInstance();
        cal.setTime(oFDate);
        cal.add(Calendar.MINUTE, -5);
        Date from = cal.getTime();
        ...
        cal.add(Calendar.MINUTE, -20);
        Date to = cal.getTime();
		return energyConsumptionDao.load15minRecentSummary(to,from,pid,property);
	}
	
	public List<DashboardRecord> load15minRecentSummaryForAllPlugload(Long pid, String property, Date oFDate) {
	 	Calendar cal = Calendar.getInstance();
        cal.setTime(oFDate);
        cal.add(Calendar.MINUTE, -15);
        Date from = cal.getTime();
        ...
        Date to = cal.getTime();
		return energyConsumptionDao.load15minRecentSummaryForAllPlugload(to,from,pid,property);
	}
	
	public void deleteECDataByFloor(long floorId) {
        energyConsumptionDao.deleteECDataByFloor(floorId);
    }
	
	/**
   * update EnergyConsumption details without the id based on capture_at and fixture_id
   * 
   * @param energyConsumption
   *            com.ems.model.EnergyConsumption
   */
  public void updateShortEnergyStats(EnergyConsumption energyConsumption) {
      energyConsumptionDao.updateShortEnergyStats(energyConsumption);
  }
