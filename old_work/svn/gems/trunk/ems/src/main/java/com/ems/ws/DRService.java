...
public class DRService {
	...
	public Response getCurrentPricing() {
		...
		//TODO need to figure out to call this getCurrentPricing API at a particular facility
		response.setMsg(String.valueOf(pricingManager.getPrice(null, new Date())));
		return response;
	}
	...
	public Response cleanDrEvents(){
		...
		String filePath = Constants.OPT_ENLIGHTED+"/adr/adr_cleanup.sh";
	...
	public Response scheduleDR(DRTarget drTarget) {
		...
		try {
			...
			{
				res.setMsg("Start date is invalid. Please provide valid start time in 'yyyy-MM-ddTHH:mm:ss' format");
				res.setStatus(1);
				return res;
			...
			if(drTargetinDB==null)
			{
				...
				if(drTarget.getStartTime().getTime() < currentDate.getTime()){
					res.setMsg("Scheduling Start Time of DR Service prior to current time is not allowed." );
					res.setStatus(1);
					return res;
				}
    ...
    public List<DRTarget> getAllActiveDrList() {
		List<DRTarget> drTargets = new ArrayList<DRTarget>();
		drTargets = drTargetManager.getAllScheduledManualDRTargets();
		if(drTargets==null)
		{
			return new ArrayList<DRTarget>();
		}
		return drTargets;
	}
	...
	public Response updatedr(DRTarget drTarget) {
		...
		try {
			...
			{
				//EM-784 not allowed to update Completed DR Schedule from.
				if(drTargetToSave.getDrStatus()!=null && DRStatusType.Completed.getName().equals(drTargetToSave.getDrStatus())){
					res.setMsg("Modification of completed DR Service is not allowed" );
					res.setStatus(1);
					return res;
				} else if(drTargetToSave.getDrStatus()!=null && DRStatusType.Active.getName().equals(drTargetToSave.getDrStatus())){
					if(drTarget.getStartTime() != null && drTargetToSave.getStartTime() !=null && !(drTarget.getStartTime().getTime()==drTargetToSave.getStartTime().getTime())){
						res.setMsg("Changing Start Time of Active DR Service is not allowed" );
						res.setStatus(1);
						return res;
					}
				} else{
					if(drTarget.getStartTime() != null){
						Date currentDate = new Date();
						if(!(drTarget.getStartTime().getTime()==drTargetToSave.getStartTime().getTime()) && (drTarget.getStartTime().getTime() < currentDate.getTime())){
							res.setMsg("Changing Start Time of DR Service prior to current time is not allowed." );
							res.setStatus(1);
							return res;
						}
