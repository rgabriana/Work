...
public List getAllDRTargetsCanceledOrCompleted(Integer offset,Integer limit,DRTargetList mList) {
	...
	public List<OverrideSchedulesFacility> getFacilitiesOfDRTarget(Long drScheduleId) {
		...
		try {
      List<OverrideSchedulesFacility> facilityList = getSession().createCriteria(OverrideSchedulesFacility.class)
      		.add(Restrictions.eq("drTarget.id",drScheduleId))
      		.addOrder(Order.asc("id")).list();
	...
	public boolean isOverrideActiveOnFacility(Long id, Long facilityId, FacilityType facilityType) {
		
		try {
			OverrideSchedulesFacility osf = (OverrideSchedulesFacility) getSession().createCriteria(OverrideSchedulesFacility.class)
					.add(Restrictions.eq("facilityId", facilityId))
					.add(Restrictions.eq("facilityType", facilityType))
					.add(Restrictions.eq("drTarget.id", id))
					.addOrder(Order.asc("id")).uniqueResult();
			if(osf != null) {
				return true;
			}
	...
	} //end of method isOverrideActiveOnFacility
	
	public boolean isOverrideActiveOnFloor(Long scheduleId, Long floorId) {
		
		try {
			OverrideSchedulesFacility osf = (OverrideSchedulesFacility) getSession().createCriteria(OverrideSchedulesFacility.class)
					.add(Restrictions.eq("facilityId", floorId))
					.add(Restrictions.eq("facilityType", "FLOOR"))
					.add(Restrictions.eq("drTarget.id", scheduleId))
					.addOrder(Order.asc("id")).uniqueResult();
			if(osf != null) {
				return true;
			}
	...
	} //end of method isOVerrideActiveOnFloor

	public int deleteDRTarget(Long id) {
		int status;
		
		try {
			String hql = "delete from OverrideSchedulesFacility osf where osf.drTarget.id=?";
    ...
    /**
     * Get all Manual DRTarget objects.
     * 
     * @return list of all Manual DRTarget objects in the database
     */
    @SuppressWarnings("unchecked")
    public List<DRTarget> getAllScheduledManualDRTargets() {
        try {
        	//EM-542 : Adding OPEN ADR condition to get list Open ADR along with Manual DR.
        	// As per Original spects, DR service has been written for Manual DR
        	Criterion rest1= Restrictions.or(Restrictions.eq("drType",DRType.MANUAL.getName().toUpperCase()),Restrictions.eq("drType",DRType.OADR.getName().toUpperCase()));
            List<DRTarget> drTargets = getSession().createCriteria(DRTarget.class)
            		.add(rest1)
            		.add(Restrictions.ne("drStatus", DRStatusType.Cancelled.getName()))
            		.add(Restrictions.ne("drStatus", DRStatusType.Completed.getName()))
