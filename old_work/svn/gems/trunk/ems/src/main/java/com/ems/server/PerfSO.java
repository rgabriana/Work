...
import com.ems.server.processor.PlugloadPMStatsThread;
...
//    private EmsThreadPool statsProcessThPool = null;
    ...
    private PlugloadPMStatsThread plugloadStatsProcessThPool = null;
    private UpdatePMStatsThread updatePMStatsThread = null;
    
    private ShortPMStatsProcessor shortPMStatsProcessor = null;
...
//        updateZeroBucketUpdThread.start();
       ...
       // statsProcessThPool = new EmsThreadPool(noOfStatsProcessThreads, "StatsThread");
        ...
        plugloadStatsProcessThPool = new PlugloadPMStatsThread(noOfStatsProcessThreads, "StatsThread",
            pmStatsQueueThreshold, pmStatsProcessingBatchSize, pmBatchTimeMillis);
        plugloadStatsProcessThPool.start();        		
    ...
    public class PerfHourlyTask extends TimerTask {
        ...
        public void run() {
            ...
            try {
                ...
                //commenting the code below as this is for plugload. Will uncomment it once plugload is released.
                try {
                	//PlugloadECPersistor.getInstance().updateZeroBuckets();
                	PlugloadECPersistor.getInstance().processRemainingCaptureATs(DateUtil.formatDate(toDate, "yyyyMMddHHmmss"));
                	ecMgr.updatePlugloadZeroBucketData();
                	ecMgr.aggregateHourlyPlugloadData(toDate);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					logger.warn("Exception occurred while aggregating hourly plugload data"+e.getMessage());
				}
                
                try{
                	eventMgr.pruneEventsData();
                }catch(Exception e){
                	logger.warn("Exception occurred while pruning events data"+e.getMessage());
                }
                if (currHour == 0) { // midnight
                  ...
                  if(logger.isDebugEnabled()) {
                    ...
                    //commenting the code below as this is for plugload. Will uncomment it once plugload is released. 
                    try {
						ecMgr.aggregateDailyPlugloadData(toDate);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						logger.warn("Exception occurred while aggregating daily plugload data"+e.getMessage());
					}
                    ecMgr.pruneData();
                }
            } catch (Exception ex) {
...
//    } // end of method getBit
    ...
    public void updatePlugloadStats(Plugload plugload, byte[] packet, long gwId) {

      if(logger.isDebugEnabled()) {
        logger.debug(plugload.getId() + ": stats pkt - " + ServerUtil.getLogPacket(packet));
      }

      if (!plugload.getState().equals(ServerConstants.FIXTURE_STATE_COMMISSIONED_STR)) {
      	// plugload is not yet commissioned. So, ignore the stats
      	logger.error(plugload.getName() + ": Plugload is not yet commissioned");
      	return;
      }
      GatewayInfo gwInfo = ServerMain.getInstance().getGatewayInfo(gwId);
      if(gwInfo != null) {
      ...
      int seqNo = ServerUtil.extractIntFromByteArray(packet, 4);
      if(logger.isDebugEnabled()) {
      	logger.debug(plugload.getId() + ": seq no. for stats -- " + seqNo);
      }

      PlugloadImpl.getInstance().setCurrentTime(plugload, seqNo);      
      PlugloadPMStatsWork statsWork = new PlugloadPMStatsWork(plugload, packet, seqNo);
      plugloadStatsProcessThPool.addWork(statsWork);
...
//
    public void addZeroBucket(DeviceInfo device, Fixture fixture, long lastStatsTime) {
        ...
        try {
            ...
            Double price = priceMgr.getPrice(fixture, statsDate);
            ec.setPrice(price.floatValue());
            ec.setCaptureAt(statsDate);
