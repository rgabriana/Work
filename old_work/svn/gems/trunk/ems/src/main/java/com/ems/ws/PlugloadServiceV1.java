...
public class PlugloadServiceV1 {
	...
	public List<Plugload> loadAllPlugLoad(@PathParam("property") String property, @PathParam("floor_id") Long floor_id) {
		...
		if (property.equalsIgnoreCase("floor")) {
			plugloadList = plugloadManager.loadPlugloadByFloorId(floor_id);
		} else if (property.equalsIgnoreCase("area")) {
			plugloadList =  plugloadManager.loadPlugloadByAreaId(floor_id);
		}
		if(plugloadList==null)
		{
			return new ArrayList<Plugload>(); 
		}
		Iterator<Plugload> itr = plugloadList.iterator();
		while (itr.hasNext()) {
			Plugload plugload = (Plugload) itr.next();
			if(plugload.getAvgVolts()==null )
			{
				plugload.setAvgVolts((float) -1);
			}
		}
		return plugloadList;
	}
