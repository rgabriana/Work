...
public class SwitchController {
    ...
    String createSwitch(Model model, @RequestParam("xaxis") String xaxis, @RequestParam("yaxis") String yaxis, @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie) throws EmsValidationException {
	...
	public String loadFixtureObject(Model model, @RequestParam("switchId") long switchId, @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie) throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource,	systemConfigurationManager,	"faclities.em_facilites_jstree_select",	cookie);
	...
	@ResponseBody
	public Switch saveSwitch(Switch switchObj) throws EmsValidationException{
		CommonUtils.isParamValueAllowedAndThrowException(messageSource, systemConfigurationManager, "switchcontroller.name", switchObj.getName());
    ...
    public String manageSwitches(Model model, @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie) throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource,	systemConfigurationManager,	"faclities.em_facilites_jstree_select",	cookie);
...
//            model.addAttribute("switches", (oArea.getFloor() != null ? (switchManager.loadSwitchByFloorId(oArea.getFloor().getId())) : null));
    ...
    public String switchSettingDialog(Model model, @RequestParam("switchId") long switchId) {
        ...
        final List<Plugload> plugloadsDB = switchManager.loadPlugloadsBySwitchId(switchId);
        if(plugloadsDB != null){
        	model.addAttribute("plugloads", plugloadsDB);
        	model.addAttribute("plugloadlightlevels", plugloadManager.loadLevelsBySwitchId(switchId));
        }
        return "devices/switches/settings/dialog";
    } 

    @RequestMapping("/assignareatoswitches.ems")
    public String assignAreaToSwitchDialog(Model model, @CookieValue(value = FacilityCookieHandler.selectedFacilityCookie, required = false) String cookie)throws EmsValidationException {
		CommonUtils.isParamValueAllowedAndThrowException(messageSource,	systemConfigurationManager,	"faclities.em_facilites_jstree_select",	cookie);
	    FacilityCookieHandler cookieHandler = new FacilityCookieHandler(cookie);
        ...
        return "devices/switches/assignarea/dialog";
    }
}
