    	...
    	if("TenantAdmin".equals(roleType)) {
        	...
        	if(filter != null) {
        		...
        		if(filter.get(2) != null) {
        			String[] parts = filter.get(2).toString().split("_");
        			if("profile".equals(parts[0])){
        				...
        				Long plugloadGroupId = Long.parseLong(parts[1]);
            			rowCount.add(Restrictions.sqlRestriction(" {alias}.device_id in (select inplug.id from plugload inplug where inplug.group_id = ? )", plugloadGroupId, Hibernate.LONG));
            			data.add(Restrictions.sqlRestriction(" {alias}.device_id in (select inplug.id from plugload inplug where inplug.group_id = ? )", plugloadGroupId, Hibernate.LONG));
        			}
...
//				.add(Projections.property("gw.name"))
    ...
    /**
     * Will return list of all events from eventType. Typically this should return a single record. If there are multiple records then
     * there may be some issue in the execution or implementation regarding this eventType.
     * This is created for email notification alarm in case of failure.
     * @param eventType
     * @return
     */
    @SuppressWarnings("unchecked")
	public List<EventsAndFault> getEventsAndFaultsByEventType(String eventType) {
        List<EventsAndFault> eventsAndFaults = getSession().createCriteria(EventsAndFault.class)
                .add(Restrictions.eq("eventType", eventType)).addOrder(Order.desc("eventTime")).list();
        return eventsAndFaults;
    }
    ...
    /**
     * Return all events that are
     * @param eventType
     * @return
     */
    @SuppressWarnings("unchecked")
	public List<EventsAndFault> getEventsAndFaultsByEventTypeNoDevice(String eventType) {
        List<EventsAndFault> eventsAndFaults = getSession().createCriteria(EventsAndFault.class)
                .add(Restrictions.eq("eventType", eventType))
                .add(Restrictions.isNull("device.id"))
                .addOrder(Order.desc("eventTime")).list();
        return eventsAndFaults;
    }
    ...
    @SuppressWarnings("unchecked")
    public List<EventsAndFault> getEventsAndFaultsByDeviceId(Long plId) {

    	List<EventsAndFault> eventsAndFaults = getSession().createCriteria(EventsAndFault.class)
    			.add(Restrictions.eq("device.id", plId)).add(Restrictions.eq("active", true))
    			.addOrder(Order.desc("eventTime")).list();
    	return eventsAndFaults;

    } //end of method getEventsAndFaultsByDeviceId

    public EventsAndFault getUnresolvedEventsAndFaultByERCId(Long erc_id) {
        EventsAndFault  eventsAndFault = (EventsAndFault)getSession().createCriteria(EventsAndFault.class)
                .add(Restrictions.eq("device.id", erc_id)).add(Restrictions.eq("active", true))
                .add(Restrictions.eq("eventType", EventsAndFault.ERC_BATTERY_LEVEL_EVENT)).uniqueResult();
        return eventsAndFault;
    }
    ...
    public void pruneEventsData() {
        ...
        try {
            ...
            calSt = con.prepareCall("{ call cleanupeventsfault()}");
            calSt.execute();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
