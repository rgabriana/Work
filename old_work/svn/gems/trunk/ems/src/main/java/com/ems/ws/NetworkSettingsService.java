...
public class NetworkSettingsService {
	...
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Path("apply/settings")
	public Response applyNetworkSettings(NetworkDetails networkDetails) {
		Response response = new Response();
		...
		try{
			...
			if (networkDetails != null) {
				...
				for(NetworkSettings netsettings : networkDetails.getInterfaces() ){
					Map<String,Object> nameValMap = new HashMap<String, Object>();
					if(!"DHCP".equalsIgnoreCase(netsettings.getConfigureIPV4())){
						nameValMap.put("networkSettingsServices.ipaddress", netsettings.getIpaddress());
						nameValMap.put("networkSettingsServices.configureIPV4", netsettings.getConfigureIPV4());
						nameValMap.put("networkSettingsServices.subnet_mask", netsettings.getSubnet_mask());
						if(!netsettings.isIs_dhcp_server()){
							nameValMap.put("networkSettingsServices.default_gateway", netsettings.getDefault_gateway());						
						}					
						//nameValMap.put("networkSettingsServices.dns", netsettings.getDns());
						//nameValMap.put("networkSettingsServices.search_domain_fields", netsettings.getSearch_domain_fields());						
			...
			if (networkSettingsManager.applyNetworkSettings(
				...
				response.setMsg("Network settings succeessfully appplied");
				return response;
			}else{
				status = ApplyNetworkStatus.FAILURE.getName();
				...
				response.setMsg("Network Settings are not applied. Please contact admin.");
				return response;
			}
		} catch (Exception e) {
