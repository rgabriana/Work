...
public class ApplicationEntryPointController {
	...
	public String createCompany(@ModelAttribute("company") Company company) throws EmsValidationException {
		...
		if(serverReboot){
			return "redirect:companySetup.ems?serverReboot="+serverReboot;
		}else{
	...
	@RequestMapping(value = "companySetup.ems", method = {RequestMethod.POST, RequestMethod.GET})
	public ModelAndView companySetup(@RequestParam(value = "serverReboot", required = false) Boolean serverReboot) {
		...
		if(companyManager.getAllCompanies().size() < 1) {
			...
			if(!isTZFound){
				syslog.error("The user.timezone "+timezone+" set in java not found on OS. Still setting it in the company setup.");
	        ...
	        if(serverReboot != null){
	        	modelAndView.addObject("serverReboot", serverReboot);
	        	if(serverReboot){
	        		try {
						Thread.sleep(5000);
					} catch (InterruptedException e) {
	...
	public ModelAndView createBuilding() {
		...
		mvc.addObject("companyLatitude", comp.getLatitude());
		mvc.addObject("companyLongitude", comp.getLongitude());
		return mvc;
	}
