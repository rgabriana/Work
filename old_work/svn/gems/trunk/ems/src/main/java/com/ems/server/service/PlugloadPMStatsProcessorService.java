...
public class PlugloadPMStatsProcessorService {
    ...
    //uint16_t numResetsByCu;     // value of suvars.numResetsByCu at end of period
		...
		*/    
		...
		Double price = pricingManager.getPrice(plugload, statsDate);
		if(timingLogger.isDebugEnabled()) {
		  timingLogger.debug(plugload.getId() + ": after price " + (System.currentTimeMillis() - startTime));
		}		
		float energy = 0;
		float predefinedBaseEnergy = plugload.getPredefinedBaselineLoad()/12;
		...
		if(zeroBucket == PMStatsProcessorService.ZERO_BUCKET) {
			...
			if(logger.isDebugEnabled()) {
				...
				logger.debug("predefined baseline load "+plugload.getPredefinedBaselineLoad());
			}
			...
			if(plugload.getPredefinedBaselineLoad() != null && plugload.getPredefinedBaselineLoad() > 0 && plugload.getPredefinedBaselineLoad() >= baseLoad){
				predefinedBaseEnergy = plugload.getPredefinedBaselineLoad()/12;
				savedEnergy = predefinedBaseEnergy - energy;
				ec.setBaseCost((float) predefinedBaseEnergy * price.floatValue() / 1000);
				ec.setSavedCost(ec.getBaseCost() - ec.getCost());
			}
			if(plugload.getPredefinedBaselineLoad() != null && plugload.getPredefinedBaselineLoad() > 0 && baseLoad > plugload.getPredefinedBaselineLoad()){
				eventsAndFaultManager.addAlarm(plugload, "Plugload " + plugload.getName() + " is configured with incorrect baseline. Predefined load:" + plugload.getPredefinedBaselineLoad()+""
						+ "Actual baseline:"+baseLoad, 
						EventsAndFault.PLUGLOAD_BASELINE_MISMATCH_STR);
			}
