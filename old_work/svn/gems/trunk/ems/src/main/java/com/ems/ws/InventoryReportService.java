...
public class InventoryReportService {
	...
	public Response exportInventoryReport() {
      ...
      //Calculate Total bulbs Associated
	    ...
	    Iterator<Object[]> iterErc = totalErcAssociated.iterator();
	    output.append("ERC"
                + ","
                + "Count ("+ totalErcCount+")");
	    while(iterErc.hasNext()) {
	    	
	    	Object[] itrObject = (Object[]) iterErc.next();
            String ercVer = ((String) itrObject[0]);
            Long erccount = ((BigInteger) itrObject[1]).longValue();
            
	        output.append("\r\n");
            output.append("ERC (Version - "+(String) ercVer +")"
                    + ","
                    + (erccount)
                    );
	    }
