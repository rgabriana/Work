...
public class FixtureService {
	...
	public List<Fixture> getFixtureList(@PathParam("property") String property,
		...
		// nameValMap.put("fixtureService.limit", limit);
	...
	*/
	...
	/**
	 * Set the selected fixtures in auto mode with color request.
	 *
	 * @param fixtures
	 *            List of fixtures
	 *            "<fixtures><fixture><id>1</id></fixture></fixtures>"
	 ...
	 */
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin','Employee')")
	@Path("op/colortemperatureauto")
	@POST
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response sendColorTemperatureAuto(List<Fixture> fixtures) {
		m_Logger.debug("Fixtures: " + fixtures.size());
		StringBuilder fixtString = new StringBuilder("");
		int[] fixtureList = new int[fixtures.size()];
		int count = 0;
		...
		fixtureManager.sendColorTemperatureAuto(fixtureList);
		userAuditLoggerUtil.log("Setting Mode of  fixtures " + fixtString
				+ " to Auto with color request" + "",
				UserAuditActionType.Fixture_Mode_Change.getName());
		return oStatus;
	}
...
//			gemsGroupManager.deleteGemsGroupsFromFixture(fixtureId);
    ...
    public Response rmaFixture(@PathParam("from")Long fromFixtureId, @PathParam("to")Long toFixtureId) {
      ...
      //Let's find the name of fixture from cache. Let's put this in try/catch as failure
        ...
        List<GemsGroupFixture> gemsGroupFixtureList = null;
        gemsGroupFixtureList = gemsGroupManager.getGemsGroupFixtureByFixtureId(fromFixtureId);

        if(!ArgumentUtils.isNullOrEmpty(gemsGroupFixtureList)){

        	for(GemsGroupFixture gemsGroupFixture : gemsGroupFixtureList){
        		if (gemsGroupFixture.getMotionGrpFxDetails() != null) {
        			gemsGroupFixture.getMotionGrpFxDetails().setUseEmValue((short) 1);
        			gemsGroupFixture.setUserAction(GemsGroupFixture.USER_ACTION_DEFAULT);
            		gemsGroupFixture.setNeedSync(GemsGroupFixture.SYNC_STATUS_GROUP);
...
//      }
	...
	public Response enabledisableHopper(@PathParam("enabled") Boolean enable,
	   ...
	   /* oStatus = CommonUtils.isParamValueAllowed(messageSource, systemConfigurationManager, "isAuto", enable);
		...
		DeviceServiceImpl.getInstance().enabledisableHoppers(suMap,
				enable);
    	...
    	if (gw != null) {
			...
			while (itr.hasNext()) {
				...
				if(fixture != null && fixture.getModelNo() != null && fixture.getModelNo().contains("SU")){
		    	  ...
		    	  suMap.put("SU", su2HopperArrList);
		    	  suMap.put("RS", rs2HopperArrList);
		    	  suMap.put("Default", hopperArrList);
				fixtureList[count++] = fixture.getId().intValue();
   ...
   /* *
	...
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin')")
   	...
   	public Response setPlacementInfo(PlacementInfoVO placementInfo) {
   		return fixtureManager.setPlacementInfo(placementInfo);
	...
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin')")
    @Path("op/initreaddress")
    @POST
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Response initReAddressDaliFixture(List<Fixture> fixtures) {
        Response oResponse = new Response();
        if (fixtures != null) {
        	int count = 0;
	        int[] fixtureArr = new int[fixtures.size()];
			Iterator<Fixture> itr = fixtures.iterator();
			while (itr.hasNext()) {
				Fixture fixture = (Fixture) itr.next();
				fixtureArr[count++] = fixture.getId().intValue();
			}
			DeviceServiceImpl.getInstance().sendReSearchWiringTestCmd(fixtureArr);
	        oResponse.setStatus(0);
        }else {
        	oResponse.setStatus(1);
        }
	...
	/**
	 * Allows selected set of fixture to assign BLE Mode
	 * floor plan
	 *
	 * @param blemode
	 *            { OFF, SCAN, ADVERTISE, BEACON, SCAN_RAW, SCAN_NOCEAN}
	 * @param mac String containing MAC address if mode is SCAN_RAW or SCAN_NOCEAN
	 * @param fixtures
	 *            list of fixtures
	 *            "<fixtures><fixture><id>1</id></fixture></fixtures>"
	 * @return response Success (total updated fixture count) / Failure (-1)
	 */
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin','Auditor','Employee','Bacnet')")
	@Path("op/assignblemode/{blemode}/{mac}")
	@POST
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response setBleModeToFixtures(@PathParam("blemode") String blemode, @PathParam("mac") String mac, List<Fixture> fixtures) {
		Response resp = new Response();
		m_Logger.info("BLE Mode: " + blemode + ", MAC Address: " + mac);
		byte[] macBytes = new byte[] {0, 0, 0, 0, 0, 0};
		if (mac != null && !mac.isEmpty()) {
			String [] fields = mac.split(":");
			if (fields.length != 6) {
				resp.setStatus(-1);
				String msg = "Wrong number of fields: " + fields.length;
				resp.setMsg(msg);
				m_Logger.debug(msg);
				return resp;
			}
			...
			for (String field : fields) {
				Integer fValue;
				try {
					fValue = Integer.valueOf(field, 16);
				} catch (NumberFormatException e) {
					resp.setStatus(-1);
					String msg = "Not a Hex value: " + field;
					resp.setMsg(msg);
					m_Logger.debug(msg);
					return resp;
				}
				if (fValue != (fValue & 0xff)) {
					resp.setStatus(-1);
					String msg = "Value too large: " + Integer.toHexString(fValue);
					resp.setMsg(msg);
					m_Logger.debug(msg);
					return resp;
				}
				macBytes[index++] = fValue.byteValue();
			}
		}
		...
		if (fixtures != null) {
			...
			while (itr.hasNext()) {
				Fixture fixture = (Fixture) itr.next();
				fixtureArr[count++] = fixture.getId().intValue();
				fixtureIdsList+=fixture.getId().intValue();
				 if (itr.hasNext()) {
					  fixtureIdsList += ",";
				  }
			}
			BLEModeType bleMode = BLEModeType.valueOf(blemode);
			totalRecordUpdated = fixtureManager.updateFixtureBleMode(fixtureIdsList, blemode);
			DeviceServiceImpl.getInstance().sendBleMode(fixtureArr, bleMode.value(), macBytes);
			resp.setStatus(totalRecordUpdated.intValue());
		} else {
			resp.setStatus(-1);
		}
		return resp;
	 ...
	 */
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin','Auditor','Employee')")
	@Path("op/setdualchannelled/{percentage}/{time}")
	@POST
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response updateDualChannelLEDValue(@PathParam("percentage") String percentage,
			@PathParam("time") String time, List<Fixture> fixtures) {
		m_Logger.debug("Dual Channel LED Configurations : Percentage: " + percentage + ", Time: " + time
				+ ", Fixtures: " + fixtures.size());
		Response oResponse = new Response();
		int count = 0;
		String fixtureIdsList="";
		...
		fixtureManager.updateDualChannelLEDValue(fixtureArr,Integer.parseInt(percentage), Integer.parseInt(time),fixtureIdsList);
		userAuditLoggerUtil.log("Dual Channel LED Configurations for fixtures " + fixtureIdsList + " to "
			+ percentage + "% in " + " for " + time
			+ " minutes", UserAuditActionType.Fixture_Dimming.getName());
		return oResponse;
	}
	/**
	 * Returns ble enabled fixture location list
	 * @param property
	 *            (floor)
	 * @param pid
	 *            floor unique identifier
	 ...
	 */
	@PreAuthorize("hasAnyRole('Admin','FacilitiesAdmin','Auditor','Employee','Bacnet')")
	@Path("blefixture/list/{property}/{pid}")
	@GET
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public List<Fixture> getBLEFixtureLocationList(
			@PathParam("property") String property, @PathParam("pid") Long pid,
			@PathParam("limit") String limit) {
		ArrayList<Fixture> fixtLocationList = new ArrayList<Fixture>();
		Response resp = new Response();
		...
		List results = null;
		if (property.equalsIgnoreCase("floor")) {
			results = fixtureManager.loadBleFixtureListByFloorId(pid);
		}
		if (results != null && !results.isEmpty()) {
			Iterator fixtLocationIt = results.iterator();
			while (fixtLocationIt.hasNext()) {
				...
				fxLocation.setModelNo(dataArr[7].toString());
				fixtLocationList.add(fxLocation);
			}
		}
		return fixtLocationList;
