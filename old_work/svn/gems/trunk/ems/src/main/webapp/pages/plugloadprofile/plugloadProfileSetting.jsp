						...
						<div class="settingAccordionFormDiv">
							<fieldset>
								...
								<div class="settingAccordionFormField" >
									...
									<div class="settingAccordionFormInputField"><form:input   class='validate[required,custom[integer],min[1],max[10],funcCall[validateCombinedLevel]] text-input' path="noOfMissedHeartbeats" size="3"/></div>									
									<span>unsuccessful attempts at obtaining occupancy status from motion group members,</span>
								</div>
								<div class="settingAccordionFormField" >
									<span>waiting up to</span>
									<div class="settingAccordionFormInputField"><form:input   class='validate[required,custom[integer],min[1],max[600],funcCall[validateCombinedLevel]] text-input' path="heartbeatInterval" size="3"/></div>									
...
function validateCombinedLevel(field) {
	...
	var noOfMissedHeartbeats = parseInt($('#noOfMissedHeartbeats').val());
	var heartbeatInterval = parseInt($('#heartbeatInterval').val());
	var totNoOfHeartBeats = noOfMissedHeartbeats * heartbeatInterval;
	if(totNoOfHeartBeats > 600) {
		return '<spring:message code="error.valid.max.waiting.time.plugload"/>';
	}
}
