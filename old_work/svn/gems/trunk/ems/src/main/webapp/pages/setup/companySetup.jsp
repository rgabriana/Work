...
/* 	if(dhcpPresent != "")
	...
	}, "Please enter a valid Organization name. Allowed Special Characters are @ # - _ : . ,");
	
	$.validator.addMethod("addressregx", function(value, element, regexpr) {          
	    return regexpr.test(value);
	}, "Please enter a valid Organization address. Allowed Special Characters are @ # - _ : \\ / . , * &");
	
	$.validator.addMethod("contactregx", function(value, element, regexpr) {          
	    return regexpr.test(value);
	}, "Please enter a valid Contact phone. Allowed Special Characters are - . : @ ( ) ? # + * \\ / ; , _");
	//- . : @ ( ) ? # + _ * ; /
	$.validator.addMethod("validdomainvalidation", CheckIsValidDomain, "Please enter domain only in correct format.");
	...
	$("#company-register").validate({
		...
		messages: {
			...
			newPassword: {
				...
				passwordValidate: '<spring:message code="error.passwords.policy"/>'
			},
			confirmPassword: {
				required: requirederr,
			...
			//},
			validDomain:{ validdomainvalidation: '<spring:message code="error.invalid.domainname"/>'},
			latitude:{
				required: requirederr,
...
function enableDisableEmcMode(){
	
	if ($('#emcMode').is(':checked')) {
		$('#emcKafkaServer').removeAttr('disabled');
	}else{
		$('#emcKafkaServer').attr('disabled', 'disabled');
	}
}
				...
				<div class="formContainer">
					<form id="network-details" method="post">
						...
						<div class="field">
							...
							<div class="formValue"><span>Internet IP Address : ${system[0]} , Subnet Mask : ${system[2]} , Default Gateway (Router) : ${system[3]} </span></div>
						</div>
						<br/>
						<br/>						
						<div class="field">
					...
					<form:form id="company-register" commandName="company" method="post" action="${actionURL}">
						...
						<div class="field">
							...
							<div class="formValue">
								...
								<span class="formValueSpan" id="gemstime1" style="margin: 0px 0px 0px 5px; display: inline; font-size: 1em; color: red"><spring:message code="companySetup.datetime.warning"/></span>
							</div>
						</div>
						
						<div class="field">
							<div class="formPrompt"><span><spring:message code="companySetup.label.lattitude"/></span></div>
							<div class="formValue"><form:input id="latitude" path="latitude" name="latitude" ></form:input></div>
						</div>
						
						<div class="field">
							<div class="formPrompt"><span><spring:message code="companySetup.label.longitude"/></span></div>
							<div class="formValue"><form:input id="longitude" path="longitude" name="longitude" ></form:input></div>
						</div>
						...
						<div class="field">
							<div class="formPrompt"><span><spring:message code="companySetup.title24.flag.title"/></span></div>
							 <div class="formValue"><form:select name="title24.compliance.flag" path="title24.compliance.flag" onchange="" id="title24ComplianceFlag" style="width: 60px;">
									<form:option value="false"><spring:message code="lov.no"/></form:option>
									<form:option value="true"><spring:message code="lov.yes"/></form:option>
									</form:select>
							</div> 
						...
						<div class="field">
							<div class="formPrompt"><span><spring:message code="companySetup.label.emcmode"/></span></div>
							<div class="formValue"><form:checkbox id="emcMode" name="emcMode" path="emcMode" onclick="enableDisableEmcMode();"/>
							<span class="formValueSpan" style="margin: 0px 0px 0px 5px; display: inline; font-size: 1em; color: red"><spring:message code="companySetup.emcmode.warning"/></span>
							</div>
						</div>
						
						<div class="field">
							<div class="formPrompt"><span><spring:message code="companySetup.label.emckafkaserver"/></span></div>
							<div class="formValue"><form:input id="emcKafkaServer" path="emcKafkaServer" name="emcKafkaServer" ></form:input>
							</div>
						</div>
						...
						<div class="field">
							<div class="formPrompt"><span><spring:message code="companySetup.label.enablePricing"/></span></div>
							<div class="formValue">							
									<form:select id="pricingType" path="pricingType" name="pricingType" style="width: 200px;">
									<form:option value="1"><spring:message code="companySetup.pricing.fixed"/></form:option>
									<form:option value="2"><spring:message code="companySetup.pricing.timeofday"/></form:option>									
									</form:select>	
							</div>
						</div>		
						-->
						<div class="field">
							<div class="formPrompt"><span></span></div>
							<div class="formValue"><input class="saveAction" id="submit" type="submit" value="<spring:message code='label.next'/>"></input>
							<span class="formValueSpan" id="serverRebootMessage" style="margin: 0px 0px 0px 5px; display: inline; font-size: 1em; color: red"></span>
							</div>
						</div>
...
<script type="text/javascript">
	...
	if('${serverReboot}' == "true" ){
		$('#serverRebootMessage').text('Update successful. Rebooting server. Please wait for 2-3 minutes.');
	}else{
		$('#serverRebootMessage').text('');
	}
	...
	<c:if test="${mode == 'admin'}">
		...
		$(".topLevelContainer").css("height", $(window).height() - 100);
