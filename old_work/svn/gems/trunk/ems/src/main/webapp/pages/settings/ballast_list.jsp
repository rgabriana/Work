...
$(document).ready(function() {
    ...
    $('#newBallastButton').click(function() {
    	$("#newBallastDialog").load("${newBallastUrl}"+"?ts="+new Date().getTime()+"&from=ballastlist").dialog({
            title : "New Ballast",
...
function onDelete(rowId)
{
	if(confirm("Are you sure you want to delete the Ballast?") == true)
	{
		$.ajax({
			...
			if(data.status == 2) {
				alert("Ballast cannot be deleted as it is associated with a Fixture Type.");
			}
			else{
		    ...
		    viewrecords: true,
		   	...
		   	loadComplete: function(data) {
		   		...
		   		{	
		   			...
		   			{
		   			displayBallastListLabelMessage("Failed to import CSV file. Light level values should be in the range of 0 to 100 and in the multiple of 5.","red");
		   			}
		   			else
		   			{
...
<div style="width: 100%; height: 100%; background: #fff; padding: 0px 5px 0px 0px" >
	<div id="ballastDiv" style="padding: 5px 5px 5px 5px">
			<button id="newBallastButton">Add</button><div style="display: inline;padding-left:100px;"><input type="checkbox" name="mcheckedballast" onclick="applyBallastToggleCall()">&nbsp;<b>Show Used Ballasts</b></input></div>
			<div style="display: inline;padding-left:150px;"><span id="voltpowermapmessage" style="font-weight:bold"></span></div>			
