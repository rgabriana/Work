...
#mForm .fieldWrapper .fieldValue1{float: left;padding-right:10px;}
#mForm .invalidField{border: 1px solid red; border-radius: 3px 3px 3px 3px; padding: 1.5px;}
</style>
...
function validateFixtureClassForm(){	
	...
	if ( $.trim($('#fixtureClassName').val()) == ''){
		$("#fixtureClassName").addClass("invalidField");
		displayLabelMessage("Fixture Type Name Field should not be empty", COLOR_FAILURE);		
		return false;
	}	

	//Check for valid fixture name - special characters not allowed. Only Letter, digit, space and underscore allowed
	var isValidFixtureName = validateFixtureClassName();	
     if(isValidFixtureName == false){
    	 $("#fixtureClassName").addClass("invalidField");
    	 displayLabelMessage("Fixture Type Name must contain only letters, numbers, or underscore", COLOR_FAILURE);
     	return false;
 	}
	
	if ( $.trim($('#fixtureClassNoOfBallasts').val()) == ''){
		$("#fixtureClassNoOfBallasts").addClass("invalidField");
	...
	if ( $.trim($('#fixtureClassVoltage').val()) == ''){
		$("#fixtureClassVoltage").addClass("invalidField");
	...
	if ( $.trim($('#fixtureClassBallasts').val()) == ''){
		$("#fixtureClassNoOfBallasts").addClass("invalidField");
		displayLabelMessage("Ballast Field should not be empty", COLOR_FAILURE);
		return false;
	}else{
		if(!IsNumericValue($.trim($('#fixtureClassNoOfBallasts').val()))){
			$("#fixtureClassNoOfBallasts").addClass("invalidField");
			displayLabelMessage("Ballast Field is invalid", COLOR_FAILURE);
			return false;
		}
	}
	
	if ( $.trim($('#fixtureClassBulbs').val()) == ''){
		$("#fixtureClassBulbs").addClass("invalidField");
		displayLabelMessage("Ballast Field should not be empty", COLOR_FAILURE);
		return false;
	}else{
		if(!IsNumericValue($.trim($('#fixtureClassBulbs').val()))){
			$("#fixtureClassBulbs").addClass("invalidField");
			displayLabelMessage("Bulb Field is invalid", COLOR_FAILURE);
			return false;
		}
	}
...
function validateFixtureClassName()
{
	var isValidFixtureName = true;
	var name ="";	
	name = $('#fixtureClassName').val();	
    var regExpStr = /^[a-z0-9\_\s]+$/i;
    if(regExpStr.test(name) == false) {    	
    	isValidFixtureName = false;
    }	
	return isValidFixtureName;
}
...
<div style="clear: both;padding-left:10px;padding-top:10px;"><span id="error"></span></div>
<div id="mForm" style="margin-left:50px;margin-top:20px;height:300px; overflow: auto;">		
			...
			<div class="fieldInput" style="display:inline-block;padding-left: 10px;"><input id="fixtureClassName" name="fixtureClassName" style="min-width:210px;" onkeypress="clearMessage(this.id);" /></div>
			<br style="clear:both;"/>
			</div>
						
			<div class="fieldWrapper" >
			<div class="fieldlabel" style="display:inline-block;"><b>Ballast:</b></div>		
			<div class="fieldValue1" style="display:inline-block;padding-left: 10px;"><select id="fixtureClassBallasts" style="width:100%; height:100%;min-width:210px;" onkeypress="clearMessage(this.id);" ></select></div>
			<div id="ballastBtnDiv" class="fieldValue1" style="display:inline-block;"><c:if test="${mode == 'add'}"><button type="button" id="btnShowBallastForm" onclick="addBallastFormShow();" style="min-width:75px;">Add Ballast</button></c:if></div>
			<br style="clear:both;"/>
			</div>
			...
			<div class="fieldValue1" style="display:inline-block;padding-left: 10px;"><select id="fixtureClassNoOfBallasts" name="fixtureClassNoOfBallasts" style="min-width:215px;" onkeypress="clearMessage(this.id);" ></select></div>
			<br style="clear:both;"/>			
			</div>	
			...
			<div class="fieldValue1" style="display:inline-block;padding-left: 10px;"><select id="fixtureClassBulbs" style="width:100%; height:100%;min-width:215px;" onkeypress="clearMessage(this.id);" > </select></div>
			<div id="bulbBtnDiv" class="fieldValue1" style="display:inline-block;"><c:if test="${mode == 'add'}"><button type="button" id="btnShowBulbForm" onclick="addBulbFormShow();" style="min-width:75px;">Add Bulb</button></c:if></div>
			<br style="clear:both;"/>
			</div>
			...
			<div class="fieldValue1" style="display:inline-block;padding-left: 10px;"><select id="fixtureClassVoltage" name="fixtureClassVoltage" style="min-width:215px;" onkeypress="clearMessage(this.id);"></select></div>			
			<br style="clear:both;"/>					
			</div>			
			...
			<div class="fieldValue1" style="display:inline-block;padding-left: 10px;">
			<c:if test="${mode == 'add'}">
			...
			<div class="fieldValue1" style="display:inline-block;"><input type="button" style="min-width:75px;" id="btnClose" value="<spring:message code="action.cancel" />" onclick="closeFixtureClassDialog()"></div>
			<br style="clear:both;"/><br>					
			</div>		
</div>
