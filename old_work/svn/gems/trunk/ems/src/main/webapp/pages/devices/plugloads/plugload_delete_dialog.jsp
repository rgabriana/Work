...
<style>
	 td.pdd-button-row{background-color: #EEEEEE;}
</style>
...
$(document).ready(function() {
	...
	disablePlugloadDeleteDialogButtons(true);
...
function deleteplugloadsOneByOne(){	
	if(deleteplugloadData.length == 0){
		...
		disablePlugloadDeleteDialogButtons(false);
		return false;
	}
...
function disablePlugloadDeleteDialogButtons(isDisable){
	if(isDisable){
		$("#pdd-close-btn").attr("disabled", true);
		$("#pdd-retry-btn").attr("disabled", true);
		$("#pdd-retry-btn").hide();
		$("#pdd-forcedelete-btn").attr("disabled", true);
		$("#pdd-forcedelete-btn").hide();
	}else{
		$("#pdd-close-btn").removeAttr("disabled");
		var fixNum = jQuery('#plugload-deleting-table').jqGrid('getGridParam', 'records');
		if(fixNum > 0) {
			$("#pdd-retry-btn").show();
			$("#pdd-forcedelete-btn").show();
		}
		$("#pdd-retry-btn").removeAttr("disabled");
		$("#pdd-forcedelete-btn").removeAttr("disabled");
	}
}

function deletePlugloadDeleteCloseHandler(){
	    
	$("#deletePlugloadDialog").dialog("close");
}

function deletePlugloadRetryHandler(){	
...
function deletePlugloadForcefullyHandler(){
    var bProceed = false;
...
<table style="margin:5px;width:450px;height:300px;">
	...
	<tr>
		<td align="center" class="pdd-button-row">
			<button id="pdd-close-btn" onclick="javascript: deletePlugloadDeleteCloseHandler();">Close</button>
			<button id="pdd-retry-btn" onclick="javascript: deletePlugloadRetryHandler();">Retry</button>
			<button id="pdd-forcedelete-btn" onclick="javascript: deletePlugloadForcefullyHandler();">Force Delete</button>
		</td>
	</tr>
</table>
