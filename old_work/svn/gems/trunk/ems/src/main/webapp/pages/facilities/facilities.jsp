...
<spring:url value="/dr/list.ems" var="loadActiveDr"/>
...
<spring:url	value="/services/org/deletedevices/floor/" var="deleteDevicesUrl" scope="request" />
<spring:url	value="/services/org/deletedevicesStatus/floor/" var="deleteDevicesStatusUrl" scope="request" />	
<style type="text/css">
...
var flash_plugload_profile_ec = function(nodetype, nodeid) {
	...
	var EC_data = "";
	if ($.browser.msie) {
		EC_data = "<object id='plugloadprofileenergysummary' classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='https://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab' width='100%' height='100%'>";
		EC_data +=  "<param name='src' value='"+energysummarymoduleString+"'/>";
		EC_data +=  "<param name='padding' value='0px'/>";
		EC_data +=  "<param name='wmode' value='opaque'/>";
		EC_data +=  "<param name='flashvars' value='orgType=" + nodetype + "&orgId=" + nodeid +  "&contextRoot=" + '${contextPath}' +"'/>";
		EC_data +=  "<embed id='plugloadprofileenergysummary' name='plugloadprofileenergysummary' src='"+energysummarymoduleString+"' pluginspage='http://www.adobe.com/go/getflashplayer'";
		EC_data +=  " height='100%'";
		EC_data +=  " width='100%'";
		EC_data +=  " padding='0px'";
		EC_data +=  " wmode='opaque'";
...
var loadPlugloadProfileEC = function() {
	
	try{
		getFloorPlanObj("plugloadprofileenergysummary").updateEnergyConsumption(plugloadProfilenodetype, plugloadProfilenodeid, "day");	
	}
	catch (ex){
		flash_plugload_profile_ec(plugloadProfilenodetype, plugloadProfilenodeid);
	}
}
...
var showflash=function(){	
	...
	if (tabselected == 'install') {
		if (treenodetype == 'floor' || treenodetype == 'area'){
			...
			$("#liacdr").css('display','block');
		}else{
			$("#lifp").css('display','none');
			$("#liacdr").css('display','none');
		}
		$('#in').click();
	}
	else if (tabselected == 'settings') {
...
function loadActiveDr() {
	removeWheelEvent();
	tabselected = 'activedr';
	$.cookie('em_facilities_tab_selected', 'activedr',  {path: '/' });
    var ifr;
    ifr = document.getElementById("activeDrFrame");
    ifr.contentWindow.document.body.innerHTML = "&nbsp;<spring:message code='action.loading'/>";
   	ifr.src="${loadActiveDr}?ts="+new Date().getTime();
   	forcefitFacilitiesTabs();
    return false;
}
...
function forcefitPlugloadProfileTabs(){
	var $header = $('#header');
    var $footer = $('#footer');
    var plugloadTemplateSubDivHeight = Math.floor(($(this).height() - $header.height() - $footer.height()) * 0.95);
    $('#plugloadTemplateSubDiv').height(plugloadTemplateSubDivHeight);
}

function forcefitFacilitiesTabs(){
	var $header = $('#header');
    var $footer = $('#footer');
    var facilitiesSubDivHeight = Math.floor(($(this).height() - $header.height() - $footer.height()) * 0.95);
    $('#innercenterSubDiv').height(facilitiesSubDivHeight);
}
...
function deleteAllDevicesByFloor(floorId){
	
	$.ajax({
        type: "POST",					            
        contentType : "application/xml",
        url: "${deleteDevicesUrl}"+floorId+"?ts="+new Date().getTime(),
    	dataType : "json",
        beforeSend: function(){
        	$.blockUI({ 
        		message: 'Please wait.....',
	...
	var ip = location.host;
						pollServer = setInterval( function() {
							 $.ajax({
			                    ...
			                    dataType: "html",
			                    crossDomain: true,
			                    url: "${deleteDevicesStatusUrl}"+floorId+"?ts="+new Date().getTime(),
			                    beforeSend: function(){
			                    },
			                    success: function(msg){
			                    	if(msg == 'SUCCESS'){
										...
										$('.blockMsg').html('<a href="' + location.protocol + '//' + ip  + '/ems' + '" id="login" >Devices purged.Please re-login to the application.</a>');
										...
										$('.blockMsg').html('Some error occured. Please contact Admin');
				                    	$(window).unbind('beforeunload');
				                    	$("div").css('cursor', 'default');
			                    	}			                    	
			                    },
							 ...
							 if(count > 5) {
								clearInterval(pollServer);
								$('.blockMsg').html('Devices purged. Please contact admin in case of any issues.');
								$(window).unbind('beforeunload');
								$("div").css('cursor', 'default');
							 } 
...
<spring:url value="/devices/plugloads/placed_plugload_commission_form.ems" var="placedCommissionPlugloadUrl" scope="request" />
...
<spring:url value="/devices/plugloads/placed_plugload_commission_start_identify.ems" var="startCommissionPlacedPlugloadUrl" scope="request" />
...
<spring:url value="/devices/fixtures/assignblemodetofixtures.ems" var="assignBleModeToFixturesDailogUrl" scope="request" />
...
<spring:url value="/devices/fixtures/ambientThreshold.ems" var="ambThresholdPromptUrl" scope="request" />
<script type="text/javascript">
...
var fixtureRmaTimerInProgress = 0;
...
var plugloadRmaTimerInProgress = 0;

function showPlacedPlugloadCommissioningForm(gatewayId){
	var data = "gatewayId="+gatewayId+"&ts="+new Date().getTime();
	
	
	$("#placedPlugloadCommissioningDialog").load("${placedCommissionPlugloadUrl}", data, function() {
		  $("#placedPlugloadCommissioningDialog").dialog({
				modal: true,
				title: "Commission Placed Plugloads",
				width : Math.floor($('body').width() * .98),
				height : Math.floor($('body').height() * .96),
				closeOnEscape: false,
				open: function(event, ui){
					$(this).parent().children().children('.ui-dialog-titlebar-close').hide();
					resizePlacedPlugloadCommissionDialog();
				},
				resizeStop: function(event, ui) {
					resizePlacedPlugloadCommissionDialog();
				},
				close: function(event, ui) {
					...
					$("#placedPlugloadCommissioningDialog").html("");
				}
			});
		});
	return false;
}
function showCommissionPlacedPlugloadWindow(){
	
	$("#placedPlugloadCommissioningStartIdentifyDialog").load("${startCommissionPlacedPlugloadUrl}", function() {
		  $("#placedPlugloadCommissioningStartIdentifyDialog").dialog({
				title: "Start Identify-Config-Commission-Placed-Plugloads",
				width : 400,
				height : 180,
				closeOnEscape: false,
				modal: true
...
var SELECTED_FIXTURES_TO_ASSIGN_BLEMODE = [];
function assignBleModeToFixtures(selFixtures){
	if(selFixtures != undefined) {
		SELECTED_FIXTURES_TO_ASSIGN_BLEMODE = eval("("+selFixtures+")");	
	}
	$("#assignBleModeToFixturesDailog").load("${assignBleModeToFixturesDailogUrl}"+"?ts="+new Date().getTime(), function() {
		$("#assignBleModeToFixturesDailog").dialog({
			modal:true,
			title: 'Assign BLE Mode to fixture',
			width : Math.floor($('body').width() * .40),
			height : 130,// Math.floor($('body').height() * .30),
			close: function(event, ui) {
				SELECTED_FIXTURES_TO_ASSIGN_BLEMODE = []; //Reset global variable 'SELECTED_FIXTURES_TO_ASSIGN_BLEMODE'
				getFloorPlanObj("floorplan").plotChartRefresh();
			}
		});
	});
...
var SELECTED_PLUGLOAD_TO_ASSIGN_AREA = [];
function assignAreaToSelectedDevices(selSwitches,selFixtures,selWds,selplugloads){
	SELECTED_SWITCHES_TO_ASSIGN_AREA = [];
	SELECTED_FIXTURES_TO_ASSIGN_AREA = [];
	SELECTED_WDS_TO_ASSIGN_AREA = [];
	SELECTED_PLUGLOAD_TO_ASSIGN_AREA=[];
	...
	if(selplugloads != undefined && selplugloads != "") {
		SELECTED_PLUGLOAD_TO_ASSIGN_AREA = eval("("+selplugloads+")");	
	}
...
function showSceneTemplatePrompt(scenetemplateId){	
	...
	$("#sceneTemplatePromptDialog").load(url, function() {
		$("#sceneTemplatePromptDialog").dialog({
			...
			height:450,
			close: function(event, ui) {
				$("#sceneTemplatePromptDialog").html("");
				$("#sceneTemplatePromptDialog").dialog("destroy");
...
function showSceneLightLevelPrompt(sceneTemplateID,mode){
	...
	$("#sceneLightLevelPromptDialog").load("${sceneLightLevelPromptUrl}?sceneTemplateID="+sceneTemplateID+"&mode="+mode, function() {
		$("#sceneLightLevelPromptDialog").dialog({
			...
			height:180,
			close: function(event, ui) {
				$("#sceneLightLevelPromptDialog").html("");
				$("#sceneLightLevelPromptDialog").dialog("destroy");
			}
		});
	}); 
}
...
function showWidgetDialogWithTemplate(switchId,switchName,fixtureVersion,sceneTemplateId) {
	dialogLayout_settings = {
			zIndex:				0		// HANDLE BUG IN CHROME - required if using 'modal' (background mask)
		,	resizeWithWindow:	false	// resizes with the dialog, not the window
		,	spacing_open:		6
	...
	if(switchId == null || switchId == ""){
		...
		if(switchName != null && switchName != "") {
			...
			// First call a service to create a virtual switch; get the id back and then pass it on to the widget dialog
			$("#widgetDialog").load("${createSwitchUrl}?switchName="+encodeURIComponent(switchName)+"&fixtureVersion="+fixtureVersion+"&sceneTemplateId="+sceneTemplateId, function() {
				$("#widgetDialog").dialog({
					modal:true,
					title: 'Edit Switch Group',
					width:  Math.floor($('body').width() * .98),
		...
		$("#widgetDialog").load("${switchWidgetDlgUrl}?switchId="+ switchId+"&ts="+new Date().getTime(), function() {
			$("#widgetDialog").dialog({
				...
				close: function(event, ui) {
					...
					$.ajax({
		    			type: "POST",
		    			url: "${applySwitchChangesURL}?switchId="+switchId+"&sceneTemplateId="+sceneTemplateId,
		    			datatype: "html",
		    			success: function(msg) {
		    			}
		    		});	
...
function showLocatorDeviceForm(locatorDeviceId, xaxis, yaxis,page) {
	...
	$("#locatorDeviceFormDialog").load(url, function() {
		...
		$("#locatorDeviceFormDialog").dialog({
			...
			close: function(event, ui) {
				//getFloorPlanObj("floorplan").plotChartRefresh();
				$("#locatorDeviceFormDialog").html("");
				$("#locatorDeviceFormDialog").dialog("destroy");
...
function refreshFacilitiesTree() {
	$.ajax({
		type: "GET",
		cache: false,
		async: false,
...
function showAmbientThresholdForm(selFixtures,selFixtureNames) {
	if(selFixtures){
		...
		var options = "?selFixtureNames="+ selFixtureNames;
		$("#ambientThreshloldDialog").load("${ambThresholdPromptUrl}" +options, function() {
			$("#ambientThreshloldDialog").dialog({
				autowidth: true,
				scrollOffset: 0,
				forceFit: true,
				modal:true,
				title: 'Set Sensor(s) Ambient Threshold Value',
				width: 500,
				height: 300,
				close: function(event, ui) {
				}
...
<div id="innercenter" class="ui-layout-center">
	<ul>		
		...
		<security:authorize access="hasAnyRole('Admin','FacilitiesAdmin', 'TenantAdmin')">
			<li id="liacdr" ><a id="acdr" href="#active_dr" onclick="loadActiveDr();"><span>Active DR</span></a></li>
		</security:authorize>
		
	</ul>
	<div class="ui-layout-content ui-widget-content ui-corner-bottom" style="border-left: 0; border-top: 0; padding: 0px;" id="innercenterSubDiv">
		...
		<security:authorize access="hasAnyRole('Admin','FacilitiesAdmin', 'TenantAdmin')">
			<div id="active_dr" class="pnl_rht"><iframe frameborder="0" id="activeDrFrame" style="width: 100%; height: 100%;"></iframe></div>
		</security:authorize>
	</div>		
</div>
...
<div id="placedPlugloadCommissioningDialog"></div>
...
<div id="placedPlugloadCommissioningStartIdentifyDialog"></div>
...
<div id="assignBleModeToFixturesDailog"></div>
...
<div id="ambientThreshloldDialog"></div>
