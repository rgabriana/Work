...
<spring:url value="/services/org/replicaservermanage/duplicate/add/" var="checkDuplicateAddReplicaServer" scope="request" />
<spring:url value="/services/org/replicaservermanage/duplicate/edit/" var="checkDuplicateEditReplicaServer" scope="request" />
...
$(document).ready(function(){
	...
	$.validator.addMethod("nameregx", function(value, element, regexpr) {          
		...
		return true;
	}, "Please enter valid Replica Server Name. Allowed Special Characters are @ # - _ : . ,");
	
	$.validator.addMethod("ipregx", function(value, element, regexpr) {          
		value = $.trim(value);
		if (value != "") {
			return regexpr.test(value);
		}
		return true;
	}, "Please enter valid IP. Allowed Special Characters are - .");	
	
	$.validator.addMethod("macidregx", function(value, element, regexpr) {          
		value = $.trim(value);
		if (value != "") {
			return regexpr.test(value);
		}
	...
	$("#create_replicaServer").validate({
		rules: {
			...
			macId: {
				required: true,
				macidregx: /^([0-9a-f]{2}[\:]){5}([0-9a-f]{2})$/i,
			},
			uid: {
				required: true,
			}
