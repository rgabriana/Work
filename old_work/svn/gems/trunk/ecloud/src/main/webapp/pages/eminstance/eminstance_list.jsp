...
<spring:url value="/services/org/eminstance/checkrestrictedmigration/" var="checkrestrictedmigration" scope="request" />
...
function onRestrictedMigration(rowId, pauseResume){	
	...
	$.ajax({
		url : "${checkrestrictedmigration}" + rowId ,
		async : false,
...
function start(inputdata, pageNum, orderBy, orderWay) {
		jQuery("#emInstanceTable").jqGrid({
			...
			colNames: ["id", "Name", "Version", "Mac Id","EM Address","Last connectivity","Call Home Health","Sync Connectivity","SSH Port","Replica Server","Details","Action"],
		       colModel: [
		       { name:'id', index:'id', hidden: true},
		       { name: 'name', index: 'name',sortable:true,sorttype:'string',width:'7.5%'},
		       ...
		       { name: "replicaServerName", index: 'replicaServerName',sortable:true,width:'10%'},
		       { name: 'details',index:'details', align:"center", sortable:false, width:"4%",formatter: viewDetailsFormatter},
		       { name: "action", index: 'action',sortable:false,width:'21.5%', align: "right",formatter: viewActionFormatter}],
