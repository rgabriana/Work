...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$.validator.addMethod("nameregx", function(value, element, regexpr) {          
			...
			return true;
		}, "Please enter valid Customer Name. Allowed Special Characters are @ # - _ : . ,");
		...
		$.validator.addMethod("addressregx", function(value, element, regexpr) {          
			...
			return true;
		}, "Please enter valid Address. Allowed Special Characters are @ # - _ \\ / & * : . ,");
		...
		$.validator.addMethod("contactregx", function(value, element, regexpr) {          
			...
			return true;
		}, "Please enter valid Contact Details. Allowed Special Characters are - . ( ) +");
		...
		$("#edit_customer").validate({
			...
			messages: {
				...
				contact: {
					required: '<spring:message code="error.above.field.required"/>',
					minlength: '<spring:message code="error.invalid.customer.contact.minlength"/>',
					maxlength: '<spring:message code="error.invalid.customer.contact.maxlength"/>',
				}
			}
		});
		...
		$('#saveCustomerBtn').click(function(){
			if(!$('#name').val().trim())
             {
				 alert('Customer Name can not be left blank.');
                 return;
             } else if(custNameRegExp.test($('#name').val().trim())) {
