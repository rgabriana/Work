		...
		override protected function commitProperties():void {
			...
			if (bChanged) {
				if(_chartItem != null)
				{
					...
					{
						if (_image != null) {
							//trace("commit " + rendererType  + " _chartItem.item.snapaddress " + _chartItem.item.snapaddress);
							_label.text = "";
							...
							else if(rendererType == Constants.FIXTURE_PROFILE)
							{
								...
								updateLabelRenderer(_label.text);
							}else if(rendererType == Constants.MOTION_GROUP)
							{
...
//			glow.quality = BitmapFilterQuality.MEDIUM;
		...
		private function showFixtureDetails(event:MouseEvent) : void
		{
			...
			{
				parentDocument.fpModel.mouseOverFixture = null;
				...
				ExternalInterface.call("showFixtureForm", _chartItem.item.id , parentDocument.m_propertyId);
			}
		}
	}
}
