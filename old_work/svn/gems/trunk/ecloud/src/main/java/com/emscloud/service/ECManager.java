...
import com.emscloud.job.EMPropertiesSyncJob;
...
public class ECManager {
		...
		if (bProcess == true) {
			...
			if (emLastTime != null) {
				...
				if (emLastTime.getLastSyncAt() != null) {	
					 ...
					 calendar.setTime(emLastTime.getLastSyncAt());					
					 TimeZone toTimeZone = TimeZone.getTimeZone(em.getTimeZone());
					 calendar.add(Calendar.MILLISECOND, toTimeZone.getRawOffset()); //add offset based on EM's timezone
					 if(toTimeZone.inDaylightTime(calendar.getTime())){
						 calendar.add(Calendar.MILLISECOND, toTimeZone.getDSTSavings()); // EDAC-374 add DST savings offset only if DST is ON in the target timezone
					 }
					 SimpleDateFormat simpleFormatter = new SimpleDateFormat("yyyyMMddHHmmss");
					 lastSyncTime= simpleFormatter.format(calendar.getTime());
	...
	public void start15MinEnergySyncCronJob() {
		...
		try {
			...
			if (SchedulerManager
				...
				// create job
				JobDetail sync15minJob = newJob(Get15minEnergyAggregationFromEmJob.class)
						.withIdentity(
								syncJobName,
	...
	public void startTimeZoneSyncJob() {

		String syncJobName = "timezonesyncjob";
		String syncTriggerName = "timezonesynctrigger";
		try {
			// check if job exist, if not create.
			// Delete the older Quartz job and create a new one
			if (SchedulerManager.getInstance().getScheduler().checkExists(
							new JobKey(syncJobName, SchedulerManager.getInstance().getScheduler().getSchedulerName()))) {
				if (SchedulerManager.getInstance().getScheduler().deleteJob(
								new JobKey(syncJobName, SchedulerManager.getInstance().getScheduler().getSchedulerName())) == false)
					logger.debug("Failed to delete Quartz job" + syncJobName);
			}
				
			// create job
			JobDetail sync15minJob = newJob(EMPropertiesSyncJob.class)
					.withIdentity(syncJobName,
							SchedulerManager.getInstance().getScheduler().getSchedulerName()).build();
			// create trigger
			SimpleTrigger sync15minJobTrigger = (SimpleTrigger) newTrigger()
					.withIdentity(syncTriggerName,	SchedulerManager.getInstance().getScheduler().getSchedulerName())
	...
	public List<DashboardRecord> loadEnergyDataForPeriod(Long id, String propertyName, Date from, Date to) {
    	...
    	if (diffDays <= 1) {
        	if(propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0)
        	{
                ...
                Object[] avg_min_max = ((List<Object[]>)organizationEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0)
        	{
                ...
                Object[] avg_min_max = ((List<Object[]>)campusEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(),  DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.BUILDING.getName()) == 0)
        	{
                ...
                Object[] avg_min_max = ((List<Object[]>)bldEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.FLOOR.getName()) == 0)
        	{
                ...
                Object[] avg_min_max = ((List<Object[]>)floorEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0)
        	{
    			...
    			while(orgIter.hasNext()) {
    				...
    				DashboardRecord dashboardRecord = EcUtil.convertToDashboardRecordForWeekly(data, avg_min_max[0] != null ? ((BigDecimal) avg_min_max[0]).floatValue() : null, avg_min_max[1] != null ? ((BigDecimal) avg_min_max[1]).floatValue() : null, avg_min_max[2] != null ? ((BigDecimal) avg_min_max[2]).floatValue() : null);
    				dashboardRecordList.add(dashboardRecord);
    			}
        	}
        	if(propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0)
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0)
        	{
        		List<OrganizationEnergyConsumptionDaily> consumptionHourlies = organizationEnergyConsumptionDao.getOrganizationMonthlyEnergyData(id,  DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"));
                Object[] avg_min_max = ((List<Object[]>)organizationEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(),  DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0)
        	{
        		List<CampusEnergyConsumptionDaily> consumptionHourlies = campusEnergyConsumptionDao.getCampusMonthlyEnergyData(id, DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"));
                Object[] avg_min_max = ((List<Object[]>)campusEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(),  DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.BUILDING.getName()) == 0)
        	{
        		List<BldEnergyConsumptionDaily> consumptionHourlies = bldEnergyConsumptionDao.getBldMonthlyEnergyData(id, DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"));
                Object[] avg_min_max = ((List<Object[]>)bldEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.FLOOR.getName()) == 0)
        	{
        		List<FloorEnergyConsumptionDaily> consumptionHourlies = floorEnergyConsumptionDao.getFloorMonthlyEnergyData(id, DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"));
                Object[] avg_min_max = ((List<Object[]>)floorEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
	...
	public List<DashboardRecord> loadCurrentEnergyData(Long id, String propertyName, Date dateOn) {
        ...
        // reset
        cal.setTime(dateOn);
        cal.add(Calendar.MINUTE, -35); // EDAC-356 time window increased to 30 minutes for current baseline data. Out of this data the lastest 15 min data will be returned.
		...
		if (propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0) {
			...
			Object[] avg_min_max = ((List<Object[]>) organizationEnergyConsumptionDao
					.loadPeakAndMinLoadQuery(id.toString(),  DateUtil.formatDate(from, "yyyy-MM-dd HH:mm:ss"), DateUtil.formatDate(to, "yyyy-MM-dd HH:mm:ss"))).get(0);
			...
			while (orgIter.hasNext()) {
				...
				DashboardRecord dashboardRecord = EcUtil.convertToDashboardRecordForCurrentData(data, avg_min_max[0] != null ? ((BigDecimal) avg_min_max[0]).floatValue() : null, avg_min_max[1] != null ? ((BigDecimal) avg_min_max[1]).floatValue() : null, avg_min_max[2] != null ? ((BigDecimal) avg_min_max[2]).floatValue() : null);
				dashboardRecordList.add(dashboardRecord);
			}
		}
		if (propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0) {
	...
	@SuppressWarnings("unchecked")
	public List<DashboardRecordV2> loadEnergyDataForPeriod(Long id, String propertyName, Date from, Date to, String timezone, String pattern) {
        long diff = to.getTime() - from.getTime();
        // Calculate difference in days
        long diffDays = diff / (24 * 60 * 60 * 1000);
    	Calendar cal = Calendar.getInstance();
    	...
    	ArrayList<DashboardRecordV2> dashboardRecordList = new ArrayList<DashboardRecordV2>();
        
    	// For daily 
    	if (diffDays <= 1) {
        	if(propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0)
        	{
        		List<OrganizationEnergyConsumptionHourly> consumptionHourlies = organizationEnergyConsumptionDao.getOrganizationHourlyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)organizationEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
                
    			Iterator<OrganizationEnergyConsumptionHourly> orgIter = consumptionHourlies.iterator();
    			while(orgIter.hasNext()) {
    				OrganizationEnergyConsumptionHourly data = orgIter.next();
    				
    				DashboardRecordV2 dashboardRecord = EcUtil.convertToDashboardRecordV2(data, avg_min_max[0] != null ? ((BigDecimal) avg_min_max[0]).floatValue() : null, 
    																							avg_min_max[1] != null ? ((BigDecimal) avg_min_max[1]).floatValue() : null,
    																							avg_min_max[2] != null ? ((BigDecimal) avg_min_max[2]).floatValue() : null, 
    																									timezone, pattern);
    				dashboardRecordList.add(dashboardRecord);
    			}
        	}
        	if(propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0)
        	{
        		List<CampusEnergyConsumptionHourly> consumptionHourlies = campusEnergyConsumptionDao.getCampusHourlyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)campusEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
                
    			Iterator<CampusEnergyConsumptionHourly> orgIter = consumptionHourlies.iterator();
    			while(orgIter.hasNext()) {
    				CampusEnergyConsumptionHourly data = orgIter.next();
    				
    				DashboardRecordV2 dashboardRecord = EcUtil.convertToDashboardRecordV2(data, avg_min_max[0] != null ? ((BigDecimal) avg_min_max[0]).floatValue() : null, avg_min_max[1] != null ? ((BigDecimal) avg_min_max[1]).floatValue() : null, avg_min_max[2] != null ? ((BigDecimal) avg_min_max[2]).floatValue() : null, timezone, pattern);
    				dashboardRecordList.add(dashboardRecord);
    			}
        	}
        	if(propertyName.compareToIgnoreCase(FacilityType.BUILDING.getName()) == 0)
        	{
        		List<BldEnergyConsumptionHourly> consumptionHourlies = bldEnergyConsumptionDao.getBldHourlyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)bldEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.FLOOR.getName()) == 0)
        	{
        		List<FloorEnergyConsumptionHourly> consumptionHourlies = floorEnergyConsumptionDao.getFloorHourlyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)floorEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0)
        	{
        		List<OrganizationEnergyConsumptionDaily> consumptionHourlies = organizationEnergyConsumptionDao.getOrganizationDailyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)organizationEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0)
        	{
        		List<CampusEnergyConsumptionDaily> consumptionHourlies = campusEnergyConsumptionDao.getCampusDailyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)campusEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.BUILDING.getName()) == 0)
        	{
        		List<BldEnergyConsumptionDaily> consumptionHourlies = bldEnergyConsumptionDao.getBldDailyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)bldEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.FLOOR.getName()) == 0)
        	{
        		List<FloorEnergyConsumptionDaily> consumptionHourlies = floorEnergyConsumptionDao.getFloorDailyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)floorEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0)
        	{
        		List<OrganizationEnergyConsumptionDaily> consumptionHourlies = organizationEnergyConsumptionDao.getOrganizationMonthlyEnergyData(id,  DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)organizationEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0)
        	{
        		List<CampusEnergyConsumptionDaily> consumptionHourlies = campusEnergyConsumptionDao.getCampusMonthlyEnergyData(id,  DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)campusEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.BUILDING.getName()) == 0)
        	{
        		List<BldEnergyConsumptionDaily> consumptionHourlies = bldEnergyConsumptionDao.getBldMonthlyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)bldEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
        	...
        	if(propertyName.compareToIgnoreCase(FacilityType.FLOOR.getName()) == 0)
        	{
        		List<FloorEnergyConsumptionDaily> consumptionHourlies = floorEnergyConsumptionDao.getFloorMonthlyEnergyData(id, DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"));
                Object[] avg_min_max = ((List<Object[]>)floorEnergyConsumptionDao.loadPeakAndMinLoadQuery(id.toString(), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", from, "UTC"), DateUtil.createDateStrWithTimeZone("yyyy-MM-dd HH:mm:ss", to, "UTC"))).get(0);
	...
	@SuppressWarnings("unchecked")
	public List<DashboardRecordV2> loadCurrentEnergyData(Long id, String propertyName, Date dateOn,  String timezone, String pattern) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(dateOn);
        cal.add(Calendar.MINUTE, -5);
        Date to = cal.getTime();
		...
		if (propertyName.compareToIgnoreCase(FacilityType.ORGANIZATION.getName()) == 0) {
			...
			while (orgIter.hasNext()) {
				...
				DashboardRecordV2 dashboardRecord = EcUtil.convertToDashboardRecordForCurrentData(data, avg_min_max[0] != null ? ((BigDecimal) avg_min_max[0]).floatValue() : null, avg_min_max[1] != null ? ((BigDecimal) avg_min_max[1]).floatValue() : null, avg_min_max[2] != null ? ((BigDecimal) avg_min_max[2]).floatValue() : null, timezone, pattern);
				dashboardRecordList.add(dashboardRecord);
			}
		}
		if (propertyName.compareToIgnoreCase(FacilityType.CAMPUS.getName()) == 0) {
