...
public class EcUtil {
	...
	public static DashboardRecord convertToDashboardRecord(AggregatedStatsData data, Float avgLoad, Float peakLoad, Float minLoad) {
		
		DashboardRecord dashboardRecord = new DashboardRecord();
		
		dashboardRecord.setCaptureOn(data.getCaptureAt());
		dashboardRecord.setPowerused(data.getEnergy().doubleValue());
		dashboardRecord.setBasePowerUsed(data.getBaseEnergy());
		dashboardRecord.setSavedPower(data.getSavedEnergy().doubleValue());
		
		dashboardRecord.setPrice(data.getPrice());
		dashboardRecord.setCost(data.getCost().floatValue());
		dashboardRecord.setSavedCost(data.getSavedCost().floatValue());
		dashboardRecord.setBaseCost(data.getBaseCost().floatValue());
		dashboardRecord.setOccsaving(data.getOccSavings().doubleValue());
		dashboardRecord.setTasktuneupsaving(data.getTuneupSavings().doubleValue());
		dashboardRecord.setAmbientsaving(data.getAmbientSavings().doubleValue());
		dashboardRecord.setManualsaving(data.getManualSavings().doubleValue());
		
		dashboardRecord.setAvgLoad(avgLoad);
		dashboardRecord.setPeakLoad(peakLoad);
		dashboardRecord.setMinLoad(minLoad);
		
		return dashboardRecord;
		
	}
	
public static DashboardRecord convertToDashboardRecordForWeekly(AggregatedStatsData data, Float avgLoad, Float peakLoad, Float minLoad) {
		
		DashboardRecord dashboardRecord = new DashboardRecord();
		
		dashboardRecord.setCaptureOn(DateUtil.addDays(data.getCaptureAt(), -1)); // as the EC data is pulled from floorenergyconsumptiondaily table, we need to subtract one day because the existing logic stores the captureat date as +1
		dashboardRecord.setPowerused(data.getEnergy().doubleValue());
		dashboardRecord.setBasePowerUsed(data.getBaseEnergy());
		dashboardRecord.setSavedPower(data.getSavedEnergy().doubleValue());
...
public static DashboardRecordV2 convertToDashboardRecordV2(AggregatedStatsData data, Float avgLoad, Float peakLoad, Float minLoad, String timezone, String datePattern) {
		
		DashboardRecordV2 dashboardRecord = new DashboardRecordV2();
		
		dashboardRecord.setCaptureOn(DateUtil.createDateStrWithTimeZone(datePattern, data.getCaptureAt(),timezone));
		dashboardRecord.setPowerused(data.getEnergy().doubleValue());
		dashboardRecord.setBasePowerUsed(data.getBaseEnergy());
		dashboardRecord.setSavedPower(data.getSavedEnergy().doubleValue());
	...
	//this method is used to get the current data. A factor is 4 i multiplied because on the EM side the data was multiplied by a factor of 0.25 because it is saved in 15 min context
	public static DashboardRecord convertToDashboardRecordForCurrentData(AggregatedStatsData data, Float avgLoad, Float peakLoad, Float minLoad) {
		
		DashboardRecord dashboardRecord = new DashboardRecord();
		
		dashboardRecord.setCaptureOn(data.getCaptureAt());
		dashboardRecord.setPowerused(data.getEnergy().doubleValue()*4); // To get the current load, this data was originally multiplied by 0.25 from EM because it is saved in 15 min tables
		dashboardRecord.setBasePowerUsed(data.getBaseEnergy()*4); // To get the current baseline, this data was originally multiplied by 0.25 from EM because it is saved in 15 min tables
		dashboardRecord.setSavedPower(data.getSavedEnergy().doubleValue()*4);
		
		dashboardRecord.setPrice(data.getPrice());
		dashboardRecord.setCost(data.getCost().floatValue()*4); 
		dashboardRecord.setSavedCost(data.getSavedCost().floatValue()*4);
		dashboardRecord.setBaseCost(data.getBaseCost().floatValue()*4);
		dashboardRecord.setOccsaving(data.getOccSavings().doubleValue()*4);
		dashboardRecord.setTasktuneupsaving(data.getTuneupSavings().doubleValue()*4);
		dashboardRecord.setAmbientsaving(data.getAmbientSavings().doubleValue()*4);
		dashboardRecord.setManualsaving(data.getManualSavings().doubleValue()*4);
	...
	//this method is used to get the current data. A factor is 4 i multiplied because on the EM side the data was multiplied by a factor of 0.25 because it is saved in 15 min context
		public static DashboardRecordV2 convertToDashboardRecordForCurrentData(AggregatedStatsData data, Float avgLoad, Float peakLoad, Float minLoad, String timezone, String datePattern) {
