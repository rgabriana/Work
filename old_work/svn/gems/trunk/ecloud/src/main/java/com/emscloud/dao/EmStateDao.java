...
static final Logger logger = Logger.getLogger(EmStateDao.class.getName());
	...
	public EmState resetPreviousFlag(Long id) {
		try{
			List<EmState> emStateList = sessionFactory.getCurrentSession().createCriteria(EmState.class)
					.add(Restrictions.eq("emInstanceId",id))
					//.add(Restrictions.ne("databaseState", DatabaseState.REPLICA_UNREACHABLE))
					.addOrder(Order.desc("id")).list();
		
		if(emStateList != null && !emStateList.isEmpty() && emStateList.size() > 1){
			 String currentState = emStateList.get(0).getDatabaseState().getName();
			 int index = 1; 
			 EmState ems = emStateList.get(index++);
			 while(ems != null && (ems.getDatabaseState().getName().equalsIgnoreCase(currentState) || ems.getDatabaseState().getName().equalsIgnoreCase(DatabaseState.REPLICA_UNREACHABLE.getName()))) {
			 ...
			 if(ems.getDatabaseState().getName().equalsIgnoreCase(DatabaseState.RESTRICTED_REMIGRATION_IN_PROGRESS.getName()))
			 {
				 state = DatabaseState.RESTRICTED_REMIGRATION_READY ;
			 }
