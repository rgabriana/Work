...
sub dumpData($$$$) {
    ...
    my @floors = ();
    for my $em (@$ems) {
        my $r = DB::EmReplica->new("replica10.enlightedcloud.net", "em_900_$em");
        ...
        for my $floor (@$floors) {
            ...
            for my $entry (@$hData) {
                ...
                my $commCount = $r->getCommFixtureCount($floor->{id}, $prev_hour, $capture);
    ...
    for my $floor (sort @floors) {
        print PHILE ",$floor occupied, $floor comm, $floor fixture count, $floor occupied percent, $floor baseline, $floor consumed, $floor occupancy savings, $floor task tuning, $floor ambient savings, $floor manual savings";
    }
    print PHILE "\n";
    ...
    for my $capture (sort {
        ...
        for my $floor (sort keys %{$data->{$capture}}) {
            print PHILE ",$data->{$capture}->{$floor}->{occyFixtureCount}, $data->{$capture}->{$floor}->{commFixtureCount}, $data->{$capture}->{$floor}->{fixtureCount}, $data->{$capture}->{$floor}->{occyPct}, $data->{$capture}->{$floor}->{baseline}, $data->{$capture}->{$floor}->{consumed}, $data->{$capture}->{$floor}->{occy}, $data->{$capture}->{$floor}->{tuning}, $data->{$capture}->{$floor}->{ambient}, $data->{$capture}->{$floor}->{manual}";
        }
        print PHILE "\n";
    }
}

dumpData(\@Ems, $from, $to, "${prefix}_dell.csv");
