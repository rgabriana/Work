...
sub getCommFixtureCount($$$$) {
    my $self = shift;
    my $floor = shift;
    my $from = shift;
    my $to = shift;
    my $dbh = $self->connection();
    my $sth = $dbh->prepare("SELECT count(e.*)/12 FROM energy_consumption e, device d WHERE d.id=e.fixture_id AND d.floor_id=$floor AND e.capture_at>'$from' AND e.capture_at<='$to' AND e.zero_bucket!=1");
    $sth->execute();
    return $sth->fetchall_arrayref()->[0]->[0];
}
