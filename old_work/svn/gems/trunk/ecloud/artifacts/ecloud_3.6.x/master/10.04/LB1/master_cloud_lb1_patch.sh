...
export XFER="wget --timestamping --verbose --debug --append-output=$LOG_DIR/$SRC_LOG --inet4-only --user=$username --password=$password"
export MASTER_OS_SRC=https://pl3.projectlocker.com/enlightedinc/Gems/svn/gems/trunk/ecloud/artifacts/ecloud_3.6.x/master/10.04/LB1
export SECURITY_SRC=https://pl3.projectlocker.com/enlightedinc/Gems/svn/gems/trunk/ecloud/artifacts/tools/security
export SETUP_CONFIG=master_cloud_lb1_patch.config
export SUMFILE=LB1_3.6.x_10.04.sum
...
$XFER --output-document=$src_dir/$SUMFILE $MASTER_OS_SRC/LB1_3.6.x_10.04.sum
...
echo "Master LB1 patch complete.  Log files can be found at $LOG_DIR"
