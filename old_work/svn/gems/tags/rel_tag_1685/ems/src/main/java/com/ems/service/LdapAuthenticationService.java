...
public class LdapAuthenticationService {
	...
	static final Logger logger = Logger
			.getLogger(LdapAuthenticationService.class.getName());
	...
	public boolean anonymousAuthentication(LDAPConnection conn, String host,
		...
		try {
			...
			if (conn.isBound()) {
				logger.debug("Anonymous authentication successfull");
				return true;
			} else {
				logger.debug("Anonymous authentication not successfull");
				return false;
			}
	...
	public boolean simpleBindAuthentication(int version, LDAPConnection conn,
		...
		try {
			logger.debug("Simple bind Authntication started");
			// connect to the server
			conn.connect(host, port);
			...
			if (conn.isBound()) {
				logger.debug("Simple bind Authntication sucessfull");
				return true;
			} else {
				logger.debug("Simple bind Authntication not sucessfull");
				return false;
			}
		}
	...
	public boolean SSLBindAuthentication(int version, LDAPConnection conn ,String host, int SSLPort,
		...
		try {
			logger.debug("SSL bind authentication");
			...
			if (conn.isBound()) {
				logger.debug("SSL bind Authntication sucessfull");
				return true;
			} else {
				logger.debug("SSL bind Authntication not sucessfull");
				return false;
			}
		} catch (LDAPException e) {
	...
	public String searchCn( int version , LDAPConnection conn, String host, int port , String userName, String baseDn, String searchattr,String dn, String passwd , boolean isanonymousacess ) {
	        ...
	        try {
	            ...
	            {
	            	logger.debug("Search not successfull , Anonymous acess failed");
	            	return cnValue ;
	            }
	        }
	        catch( LDAPException e ) {
