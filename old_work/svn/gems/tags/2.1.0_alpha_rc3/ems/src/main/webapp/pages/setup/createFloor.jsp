			...
			$(msg).find('floor').each(function() {
				...
				if(isSetup == "N") {
					$("<tr></tr>").html( 
							...
							$(this).find('id').text() + " type='button' onclick='edit(this);' value=<spring:message code='action.edit'/> /> </td>"
					).appendTo("#floorList");
				}
				else {
					$("<tr></tr>").html( 
		...
		success: function(msg) {
			...
			if(count > 0) {
				$(msg).find('building').each(function() {
					...
					if(loadBuilding != null) {
						if(loadBuilding == $(this).find('id').text()) {
							$("#building").html($(this).find('name').text());
	...
	jConfirm('Are you sure you want to delete '+ name+"?",'<spring:message code="deletion.confirmation.title"/>',function(result){
        if(result) {
        	$.ajax({
        		...
        		success: function(msg) {
        			if(msg == "F") {
        				$("#deleteStatus").html("Floor " + name  + " cannot be deleted. There exists atleast one area under this floor." );
        			}
        			else if(msg == "S") {
        				<c:if test="${mode == 'admin'}">
	...
	<div>
		...
		<form:form id="create-floor" commandName="floor" method="post"
			...
			<div id="ifBuilding">
				...
				<div class="field">
					<div class="formPrompt">
						<span><spring:message code="floorSetup.label.upload.plan" /></span>
					</div>
					<div class="formValue">
