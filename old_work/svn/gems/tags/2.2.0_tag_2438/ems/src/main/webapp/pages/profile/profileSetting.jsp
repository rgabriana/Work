					...
					<div class="settingProfileNameFieldLabel">
							...
							<select class="text" id=derivedProfile name="derivedProfile" onchange="javascript: changeProfile()">
													<c:forEach items="${groups}" var="group">
														<c:choose>
															<c:when test="${typeid==group.id}">
																<c:choose>
																	<c:when test="${group.profileNo==1}">
																		<option  selected="selected" value="${group.id}">${group.name}</option>
																	</c:when>
																	<c:when test="${group.defaultProfile=='true'}">
																		<option  selected="selected" value="${group.id}">${group.name}_Default</option>
																	</c:when>
																	<c:otherwise>
																		<option selected="selected" value="${group.id}">${group.name}</option>
																	</c:otherwise>
															...
															<c:otherwise>
																<c:choose>
																	...
																	<c:when test="${group.defaultProfile=='true'}">
																		<option value="${group.id}">${group.name}_Default</option>
																	</c:when>
																	<c:otherwise>
							...
							<%-- <c:if test="${typeid !='1' || type != 'group' }"> --%>
								 <c:if test="${management=='new'}">
									 <input type="button" class="updatebtn" onclick="submitHeader(1);"
											value="Save"/>
								 </c:if>
								 <c:if test="${management=='edit'}">
								  <c:if test="${group.defaultProfile=='false'}">
										 <div style="padding-bottom: 10px">						
												<input type="button" class="updatebtn" onclick="submitHeader(0);"
