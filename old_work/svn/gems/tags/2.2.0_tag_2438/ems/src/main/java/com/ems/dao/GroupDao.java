...
//        return null;
    	  try {
	        ...
	        String hsql = "from Groups g order by g.id";
	        Query q = getSession().createQuery(hsql.toString());
	        groupsList = q.list();
     ...
     */
    @SuppressWarnings("unchecked")
    public List<Groups> loadAllProfiles() {
//        try {
//            List<Groups> results = null;
//            String hsql = "Select new Groups(g.id,g.name,g.profileHandler.id, g.profileHandler.profileChecksum, g.profileHandler.globalProfileChecksum, g.profileHandler.profileGroupId, g.profileNo,g.profileTemplate.id,g.defaultProfile) from Groups where g.profileTemplate.id!=null g order by g.id";
//            Query q = getSession().createQuery(hsql.toString());
//            results = q.list();
//            if (results != null && !results.isEmpty()) {
//                return results;
...
//        return null;
    ...
    public List<Groups> loadAllGroupsExceptDeafult() {
        try {
            List<Groups> results = null;
            String hsql = "Select g from Groups g where g.name!='Default' and g.profileNo>0 order by g.id";
            Query q = getSession().createQuery(hsql.toString());
            results = q.list();
            if (results != null && !results.isEmpty()) {
                return results;
...
//        return null;
        ...
        List<Groups> groupsList = null;
        String hsql = "from Groups g where g.name = ?";
        Query q = getSession().createQuery(hsql.toString());
	...
	public List<Groups> loadAllProfileTemplateById(long templateId) {
        List<Groups> groupsList = null;
        try {
	        String hsql = "from Groups g where g.displayProfile='true' and g.profileTemplate.id = ? order by name";
	        Query q = getSession().createQuery(hsql.toString());
	...
	public Byte getMaxProfileNo() {
		byte profileNo = 0;
		  try {
		    ...
		    //Todo: Need to add tenant conditions g.tenant_id==null
		    String hsql = "SELECT MAX(profileNo) from Groups g where g.profileNo <=255";
		    Query query = s.createQuery(hsql);
		    profileNo =  (Byte) query.uniqueResult();
		  } catch (Exception e) {
		    e.printStackTrace();
		  }
		  return (byte) (profileNo+1);
	}
	...
	public List<Object> getAllFixtureCountForProfile() {
		try {
			Session s = getSession();
			String hsql = "SELECT g.id,count(f.id) from Fixture f, Groups g where f.groupId=g.id group by g.id";
			Query query = s.createQuery(hsql);
			return query.list();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> getFixtureCountForProfile(Long profileId) {
		try {
			Session s = getSession();
			String hsql = "SELECT g.id,count(f.id) from Fixture f, Groups g where f.groupId=:profileId group by g.id";
			Query query = s.createQuery(hsql);
	...
	public List<Object> getAllFixtureCountForTenantProfile(Long tenantId) {
		try {
			Session s = getSession();
			String hsql = "SELECT g.id,count(f.id) from Fixture f, Groups g where f.groupId=g.id and g.tenant.id=:tenantId group by g.id";
			Query query = s.createQuery(hsql);
