	...
	isColumnExists = column_exists('profile_no','groups');
	 --Raise Notice '=> %', isColumnExists;
	  IF isColumnExists = 'f' THEN
		   ...
		   FOR grpRec IN select id, profile_handler_id, name from groups WHERE id <=16 LOOP
		   --Raise Notice '=> %', grpRec.name;
			IF (grpRec.id <= 16) THEN
				...
				INSERT INTO profile_template (id,name) values (nextval('profile_template_seq'),grpRec.name);
				UPDATE groups g set template_id = grpRec.id where g.id = grpRec.id ;
			END IF;
		   END LOOP;
