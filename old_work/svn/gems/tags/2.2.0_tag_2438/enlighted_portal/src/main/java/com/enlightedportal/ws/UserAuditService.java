...
public class UserAuditService {
	...
	public String getAuditsList(@RequestParam("data") String userdata) throws UnsupportedEncodingException {
		...
		List<Object> userAudits = userAuditManager.getUserAudits(orderBy, orderWay, query,
				(page - 1) * DEFAULT_ROWS, DEFAULT_ROWS);
		records = (Long) userAudits.get(0);
		...
		for (int i = 1; i < userAudits.size(); i++) {
			Object[] each = (Object[]) userAudits.get(i);
			if (i > 1) {
				output.append(", ");
			}
			output.append("{ \"id\": \""
