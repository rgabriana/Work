...
public class FixtureController {
    ...
    public String assignProfileToFixtureDialog(Model model){
		...
		if (emsAuthContext.getCurrentUserRoleType() != RoleType.Admin) {
			if(emsAuthContext.getCurrentUserRoleType()==RoleType.TenantAdmin)
			{
				...
				if(tenant != null)
				{
					...
					profileList = groupManager.loadAllGroupsForTenantUser(tenantId);
				}
			}
		} else {
		...
		facilityTreeHierarchy = groupManager.loadProfileHierarchyForUser(emsAuthContext.getUserId(),false);
	...
	@RequestMapping("/assignprofiletofixturesemployeerole.ems")
	 public String assignProfileToFixtureDialog(Model model, @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie){
       FacilityCookieHandler cookieHandler = new FacilityCookieHandler(cookie);
       Long id = cookieHandler.getFacilityId();
       model.addAttribute("pid", id);
       model.addAttribute("currenttime", new Date().getTime());
       model.addAttribute("fixtures",fixtureManager.loadAllFixtures());
       model.addAttribute("profiles", groupManager.loadAllGroupsExceptDeafult());       
