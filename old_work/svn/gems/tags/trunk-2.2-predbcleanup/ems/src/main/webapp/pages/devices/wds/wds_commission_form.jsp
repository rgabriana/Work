...
$(document).ready(function() {
	...
	jQuery("#discwds-wds-list-grid").jqGrid({
		...
		forceFit: true,
		scrollOffset: 0,
	   	colNames:["id", "Others (<span id='othr-wds-cnt'>"+othr_fx_cnt+"</span> WDS)", "",""],
	   	colModel:[
	...
	if(status.toUpperCase() == COMMISSIONED) {
		...
		try {
			...
			if(SWITCH_ID != undefined && SWITCH_ID != "")
				getCommissionPlanObj("c_wds_floorplan").addWds('floor', '${floorId}', $("#wdsId").val(), SWITCH_ID);
			else
	...
	$.ajax({
		...
		success: function(data){
			if (data.status == 0) {
				...
				displayFxCommissionMessage("Please restart WDS commissioning.");
			}
		}
	});
}
