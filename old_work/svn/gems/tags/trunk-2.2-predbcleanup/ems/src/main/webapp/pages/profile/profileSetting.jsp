...
var submit=false;
	$(document).ready(function() {
		...
		{
			$("#profilename").val("");
			//$('#edit-Profilename').removeAttr("readonly");
			//$('#edit-advanced input').removeAttr("readonly");
		...
		}else if(manageType=='edit')
		{
			...
			if($("#isdefaultProfile").val()=='true')
			{
				 $("#profilename").val(profileName+"_Default");
				// FOr Default profiles all fields should be non editable
				//$('#edit-Profilename input').attr('readonly', 'readonly'); 
				//$('#edit-advanced input').attr('readonly', 'readonly');
		...
		$('.help').click(function() {
			...
			'<p class="dialogbigpad">&bull;&nbsp;&nbsp;&nbsp;Max light level when on (0-100)(%) -- the maximum level between 0 and 100 percent to set the light. This is the value to be reduced when there is too much illumination by the light. Typically the value would be 100, which would be reduced to no more than 25 or 30.</p>	<br />' + 
		...
		$('.helpAdvanced').click(function() {
			$("#helpDialog").html('<p>This screen allows the following low level configuration attributes to be changed:</p> <br />' +
					...
					'<p class="dialogbigpad">&bull;&nbsp;&nbsp;&nbsp;Min level before off (0 - 25) (%) -- the light level for the fixture before turning off. When turning a light off due to lack of occupancy, the light fixture will first be set at this level for the length of time specified by field "Linger before off" and then turned off.</p><br />' +
					'<p class="dialogbigpad">&bull;&nbsp;&nbsp;&nbsp;Linger before off (0 - 600) (sec) -- the time in seconds to hold on the fixture before turning off. This field is used with field "Min level before off" to specify the behavior before turning off a light due to lack of occupancy.</p><br />' +
					...
					'<p class="dialogbigpad">&bull;&nbsp;&nbsp;&nbsp;Highbay (0 = disabled, 1 = enabled) </p><br />' +
					'<p class="dialogbigpad">&bull;&nbsp;&nbsp;&nbsp;Motion threshold (1 = Minimum threshold, 10 = Maximum threshold) </p><br />' +
	...
	function changeProfile()
	{
		...
		reload_dialog(groupId,templateId);
	}
</script>
...
<div id="basicConfigurationTop1" style="border:1px;border-style:solid; vertical-align:middle; border-color:#F2F2F2;padding-left: 10px;padding-right: 10px;padding-top: 5px;padding-bottom: 10px;">
	<div  class="settingDivOuterForTopBox">
		<div class="upperdiv">
			...
			<!--<form:form commandName="group" id="edit-Profilename" method="post" action="${updateProfileNameUrl}" onsubmit="return false;">-->
						<!--<form:hidden path="id"/>-->
						...
						<div class="settingProfileNameFieldLabel">
									...
									<div class="settingProfileNameFieldValue">
										<input id="profilename"  name="profilename"  value="${group.name}" class='validate[required] text-input' value="" size="20"/>
									</div>
					</div>
<%-- 					<c:out value="${group.derivedFromGroup.id}"></c:out> --%>
					<div class="settingProfileNameFieldLabel">
							...
							<select class="text" id=derivedProfile name="derivedProfile" onchange="javascript: changeProfile()">
									...
									<c:forEach items="${groups}" var="selgroup">
										<c:choose>
											<c:when test="${typeid==selgroup.id}">
												<c:choose>
														<c:when test="${selgroup.defaultProfile=='true'}">
															<option selected="selected" value="${selgroup.id}">${selgroup.name}_Default</option>
														</c:when>
														<c:otherwise>
											...
											<c:when test="${selgroup.defaultProfile=='true'}">
												<option value="${selgroup.id}">${selgroup.name}_Default</option>
											</c:when>
											<c:otherwise>
								...
								<c:forEach items="${groups}" var="selgroup">
											<c:choose>
												<c:when test="${group.derivedFromGroup.id==null || group.derivedFromGroup.id==1}">
													<option  selected="selected" value="1">Default</option>
												</c:when>
												<c:when test="${group.derivedFromGroup.id==selgroup.id}">
													<c:choose>
														<c:when test="${selgroup.defaultProfile=='true'}">
															<option value="${selgroup.id}">${selgroup.name}_Default</option>
														</c:when>
												...
												<c:when test="${selgroup.profileNo==1}">
													<option  selected="selected" value="${selgroup.id}">${selgroup.name}</option>
												</c:when>
											</c:choose>
									</c:forEach>
							...
							<%-- <c:if test="${typeid !='1' || type != 'group' }"> --%>
								 <c:if test="${management=='new'}">
									 <input type="button" id="saveProfileBtn" class="updatebtn" onclick="submitHeader(1);"
											value="Save"/>
								 </c:if>
								 <c:if test="${management=='edit'}">
								  <c:if test="${group.defaultProfile=='false'}">
										 <div style="padding-bottom: 10px">						
												<input type="button" id="updateProfileBtn" class="updatebtn" onclick="submitHeader(0);"
				...
				<div class="profbtn">
					<c:if test="${typeid !='1' || type != 'group' }">	
						...
						<c:if test="${management=='edit'}">
							 <c:if test="${group.defaultProfile=='false'}">
								 <div style="padding-bottom: 10px">						
										<input type="button" class="updatebtn" onclick="submitHeader();"
											value="<spring:message code='action.update'/>" />
								</div>
							 </c:if>
						</c:if>
						
						<%-- <input type="button" class="cancelbtn" onclick="resetHeader();"
...
// 		$("#edit-advanced").validationEngine('attach' , {

// 			isOverflown: true,

// 			overflownDIV: "#profileaccordion"
			...
			<div class="settingDivOuter">
				<div style="padding: 10px 10px 0px 10px;">
						...
						<div style="float: right;">
							...
							<c:if test="${management=='edit'}">
								 <c:if test="${group.defaultProfile=='false'}">
								 	<input type="button" class="updatebtn" onclick="submitAdvanced();"  value="<spring:message code='action.update'/>"/>&nbsp;&nbsp;&nbsp;
								 </c:if>
							 </c:if>
							 -->
							<%-- <input type="button" class="cancelbtn" onclick="resetAdvanced();" value="<spring:message code='action.cancel'/>"/> --%>
							<spring:url value="/themes/default/images/helpquestion.png" var="imgfacilities" />
							<div style="float:right; cursor:pointer"><img class="helpAdvanced" src="${imgfacilities}" alt="Help" title="Help"/></div>
						</div>
					<!--<spring:url value="/profile/updateAdvancedConfiguration.ems" var="updateAdvancedConfig"/>
					<form:form commandName="profilehandler" id="edit-advanced" method="post" onsubmit="return false;" action="${updateAdvancedConfig}">-->
						<input type="hidden" name="type" value="${type}"/>
						<input type="hidden" name="typeid" value="${typeid}"/>
...
/* 	function resetHeader() {
		$('#edit-basic').validationEngine('hide');
		$("#invalidTimeRange").hide();
		$('#edit-basic')[0].reset();
	}
	
	function resetAdvanced() {
		$('#edit-advanced').validationEngine('hide');
		$('#edit-advanced')[0].reset();
	} */
	
	
	function updateProfileName() {
		displayAdvancedMessage("", "green");
		var isValid = $("#edit-Profilename").validationEngine('validate');
		if(isValid) {
			$.post(
					"${updateProfileNameUrl}",
					$("#edit-Profilename").serialize(),
					function(data){
						var response = eval("("+data+")");
						if(response.success==1){ //Success
							displayProfileRenameMessage(response.message, "green");
						} else { //Failure
							displayProfileRenameMessage(response.message, "red");
						}
					}
			);
		}
	...
	function submitAdvanced() {
		displayAdvancedMessage("", "green");
		var isValid = $("#edit-advanced").validationEngine('validate');
		if(isValid) {
			$.post(
					"${updateAdvancedConfig}",
					$("#edit-advanced").serialize(),
					function(data){
						var response = eval("("+data+")");
						if(response.success==1){ //Success
							displayAdvancedMessage(response.message, "green");
						} else { //Failure
							displayAdvancedMessage(response.message, "red");
						}
					}
			);
		}
	...
	function submitHeader(requestType) {
		var isValid = $("#edit-basic").validationEngine('validate');
		
		if(isValid==true && submit==false)
		{
			submit=true;
			var url;
			if(requestType==0)
		...
		{
			displayBasicMessage("Profile already saved", "red");
		}
