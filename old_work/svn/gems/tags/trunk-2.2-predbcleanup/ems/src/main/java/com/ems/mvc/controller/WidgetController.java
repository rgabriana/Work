...
public class WidgetController {
    ...
    public String createNewGroup(@RequestParam("groupName") String groupName, @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie) {
    	...
    	try {
			...
			gemsGroup.setFloor(floor);
			gemsGroup = gemsGroupManager.createNewGroup(gemsGroup);
			motionGroupManager.saveOrUpdateMotionGroup(new MotionGroup(null, new Integer("1" + GGroupType.MotionGroup.getId() + motionGroupManager.getNextGroupNo()), gemsGroup )).getId();
		} catch (SQLException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
    ...
    public String showMotionBitsGroupWidgetDialog(Model model, @RequestParam("groupId") Long groupId) {
        ...
        model.addAttribute("scheduler", motionBitsGroup);
        model.addAttribute("groupId", groupId);
        return "TODO";
    }
    ...
    public String createNewMotionBitsGroup(@RequestParam("groupName") String groupName, @CookieValue(FacilityCookieHandler.selectedFacilityCookie) String cookie) {
    	...
    	return "redirect:/devices/widget/motionbitsgroup/show.ems?groupId=" + groupId;
    }
	...
	public String editAndApplyMotionBitsGroupChanges(@ModelAttribute("scheduler") MotionBitsScheduler scheduler, Locale local) {
		...
		scheduler1.setDisplayName(scheduler.getDisplayName());
