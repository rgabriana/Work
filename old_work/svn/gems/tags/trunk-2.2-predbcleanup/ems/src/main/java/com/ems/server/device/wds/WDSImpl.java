...
public class WDSImpl {
    ...
    public int setWdsWirelessParams(long wdsId, long gatewayId) {
        ...
        if (oWds == null) {
            ...
            logger.error(wdsId + ": There is no WDS");
            return 1;
        }
        Long gemsGroupId = 1L;//oWds.getGemsGroupId();
        if (gemsGroupId == null) {
            logger.error(wdsId + ": There is no WDS group");
            return 2;
        }
        
        GemsGroup oGemsGroup  = gemsGroupMgr.loadGemsGroup(gemsGroupId);
        // Send details to WDS
        Gateway gw = gwMgr.loadGateway(gatewayId);
        ...
        logger.info("Configuring WDS: " + oWds.getName() + " with wdsID: " + wdsno);
