...
//        oResponse.setMsg(oGemsGroup.getId().toString());
     ...
     * @return Response status
     */
    @Path("op/deletegroup/gtype/{type}/groupid/{gid}")
    @POST
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Response deleteGroup(@PathParam("type") int gType, @PathParam("gid") Long gid) {
        ...
        if (gemsGroupFixtures != null) {
            iStatus = gemsGroupManager.removeFixturesFromGroup(gid, gemsGroupFixtures, groupNo, gType);
        }
        if (iStatus == 0) {
            if (gType == GGroupType.MotionGroup.getId()) {
