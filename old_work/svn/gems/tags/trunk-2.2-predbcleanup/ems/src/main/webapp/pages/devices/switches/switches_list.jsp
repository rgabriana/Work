...
// 		   	pager: '#switchPagingDiv',
		...
		<c:forEach items="${switches}" var="switch">
			...
			localData.action += "&nbsp;<button onclick=\"$('#switch_message').text('');javascript: deleteSwitch(${switch.id}, '${switch.name}');\">Delete</button>";
...
// 					alert("Switch deleted successfully.");
					if(data.status == 1) {
						$("#switch_message").text("Cannot delete the switch ("+switchName+").Please disassociate the fixtures and WDS associated with this switch");
