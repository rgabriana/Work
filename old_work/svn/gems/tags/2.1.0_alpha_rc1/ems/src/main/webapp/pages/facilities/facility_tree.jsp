...
<div id="accordionfacility" class="left-menu">
	<security:authorize access="hasAnyRole('Admin','FacilitiesAdmin')">
		<c:if test="${!viewTreeOnly}">
			...
			<div class="accinner">		
				<div id="profileTreeViewDiv" class="treeviewbg">
					<ul>
	...
	<!-- Tree view starts here -->		
		<div id="facilityTreeViewDiv" class="treeviewbg">
			<ul>
				<c:forEach items='${facilityTreeHierarchy.treeNodeList}' var='node1'>
					<li rel="company" id='<c:out value="${node1.nodeType.lowerCaseName}"/>_${node1.nodeId}'><a href='#'>${node1.name}</a>
						<ul>
							<c:forEach items='${node1.treeNodeList}' var='node2'>
								<li rel="campus" id='<c:out value="${node2.nodeType.lowerCaseName}"/>_${node2.nodeId}'><a href='#'>${node2.name}</a>
									<ul>
										<c:forEach items='${node2.treeNodeList}' var='node3'>
											<li rel="building" id='<c:out value="${node3.nodeType.lowerCaseName}"/>_${node3.nodeId}'><a href="">${node3.name}</a>
												<ul>
													<c:forEach items='${node3.treeNodeList}' var='node4'>
														<li rel="floor" id='<c:out value="${node4.nodeType.lowerCaseName}"/>_${node4.nodeId}'><a href="">${node4.name}</a>
															<ul>
																<c:forEach items='${node4.treeNodeList}' var='node5'>
																	<li rel="area" id='<c:out value="${node5.nodeType.lowerCaseName}"/>_${node5.nodeId}'><a href="">${node5.name}</a></li>
																</c:forEach>
															</ul></li>
													</c:forEach>
												</ul></li>
