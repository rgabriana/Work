...
//        updateZeroBucketUpdThread.start();
        ...
        if (sysMgr != null) {
            try {
                ...
                if (tempConfig != null) {
                    logger.debug("From database command teperature offset for 1.x SUs -- " + tempConfig.getValue());
                    temperatureOffsetSU1 = Integer.parseInt(tempConfig.getValue());
                }
            } catch (Exception e) {
                logger.info("Could not read the parameters of PerfSo)");
            }
        }
       // statsProcessThPool = new EmsThreadPool(noOfStatsProcessThreads, "StatsThread");
        statsProcessThPool = new PMStatsThread(noOfStatsProcessThreads, "StatsThread",pmStatsQueueThreshold,pmStatsProcessingBatchSize);
    ...
    private void startHourlyTask() {
        ...
        new Thread() {
            public void run() {
                ...
                while (true) {
                    try {
                        if (ServerUtil.getCurrentMin() == 2) { // giving 2 minutes to collect all the
    ...
    public static int getFIXTURE_OUTAGE_DETECT_WATTS() {
     ...
     * interval in sec
     */
    public static double getAvgPower(int calibVal, int pulses, int interval, Fixture fixture) {

        if (pulses < 2) {
            return 0;
        }
        double avgPower = ((double) calibVal * 3600 * (pulses - 1)) / (interval * (double) 10000000); // 10e-7 as calib
                                                                                                      // is 0.1 micro
        ...
        if (ServerMain.getInstance().isApplyECScalingFactor() && avgPower != 0) {
            ...
            if (fVolts == 277) {
                errPercentage = ServerMain.getInstance().getScalingFactorFor277V() * Math.log(avgPower)
...
//    } // end of method adjustAmbTaskTunedSavings
    ...
    private void parseEvents(String snapId, String eventMask) {
        ...
        if ((eventBytes[1] & 128) == 1) { // 1000 0000
            ...
            event.setFixture(fixtureMgr.getFixtureBySnapAddr(snapId));
            eventMgr.save(event);
        }
...
//                while (true) {
//                    try {
//                        if (ServerUtil.getCurrentMin() % 5 == 2) { // start at the 7th/12th/17th... minute
...
//
//    private void addZeroBucket(DeviceInfo device, Fixture fixture, long lastStatsTime) {
//
...
//
//            try {
//                HashMap<Long, DeviceInfo> deviceMap = ServerMain.getInstance().getDeviceMap();
...
//                        // zero bucket
//                        if (device.getLastStateRcvdTime() != null
//                                && (lastStatsTime - device.getLastStateRcvdTime().getTime()) < TEN_MINUTE_INTERVAL) {
//                            continue;
//                        }
//                        Fixture fixture = fixtureMgr.getFixtureById(device.getDbId());
//                        addZeroBucket(device, fixture, lastStatsTime);
//                    } catch (Exception ex) {
//                        logger.error("Adding zero bucket -- " + ex.getMessage());
//                    }
//                    Thread.sleep(1000);
//                } // end of while
//            } catch (Exception ex) {
