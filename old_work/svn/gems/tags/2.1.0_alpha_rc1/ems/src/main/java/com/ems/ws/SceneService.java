...
public class SceneService {
	...
	public Scene saveScene(Scene scene) {
		userAuditLoggerUtil.log("Create Scene  : " + scene.getName() + "(" + scene.getId() +")");
    ...
    public List<Scene> saveSceneList(List<Scene> scenes) {
       ...
       userAuditLoggerUtil.log("Save Scene List");
       return newscenes;
    }
	...
	public Response deleteScene(@PathParam("id") long id){
		...
		userAuditLoggerUtil.log("Delete Scene with Id : " +id);
		return new Response();
	}
    ...
    @Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    public Response saveSceneLevel(List<SceneLevel> sceneLevels) {
	     if(sceneLevels != null){
	         for(SceneLevel level : sceneLevels){
	             switchManager.saveSceneLevel(level);
	         }
	     }
         return new Response();
    }
	...
	public Response deleteSceneLevel(@PathParam("id") long id){
		...
		userAuditLoggerUtil.log("Delete Scene Level with id  : " + id);
		return new Response();
	}
}
