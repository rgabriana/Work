...
var PAGE = "${page}";
	...
	function start(inputdata, pageNum, orderBy, orderWay) {
		jQuery("#fixtureTable").jqGrid({
			url: '<spring:url value="/services/org/fixture/list/filter/${page}/${pid}"/>',
			userData: "userdata",
			mtype: "POST",
			...
			formatter: {
		   	...
		   	colModel:[
				...
				{name:'status', index:'status', align:"center", sortable:true, width:"5%", searchoptions:{sopt:['eq']}, formatter: statusImageRenderer},
		   		{name:'snapaddress', index:'snapaddress', sortable:true, width:"12%", searchoptions:{sopt:['cn']}},
		   		{name:'name', index:'name', sortable:true, width:"20%", searchoptions:{sopt:['cn']}},
		   		{name:'currapp', index:'currapp', sortable:true, width:"6%", search:false, formatter: currAppRenderer},
		   		{name:'version', index:'version', sortable:true, width:"8%", search:false},
		   		{name:'gateway', index:'gateway', sortable:true, width:"10%", search:false, formatter: gatewayNameRenderer},
		   		...
		   		{name:'state', index:'state', align:"right", hidden:(PAGE!="floor"), sortable:true, width:"14%", search:false, formatter: actionImageRenderer}
		   	],
		   	jsonReader: { 
		        root:"fixture", 
		        page:"page", 
