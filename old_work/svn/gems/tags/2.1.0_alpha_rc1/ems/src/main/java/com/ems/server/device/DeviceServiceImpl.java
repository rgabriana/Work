   ...
   */
    ...
    public void dimFixtures(int[] fixtureArr, int percentage, int time) {
        ...
        if (time == 1773) {
            ...
            DeviceInfo device = ServerMain.getInstance().getDevice((long) fixtureArr[0]);
            if (device != null) {
    ...
    public void waitForProfilePkt(long fixtureId) {
        ...
        while (--retries > 0) {
            if (ServerMain.getInstance().isPFPktMarked(new Long(fixtureId))) {
                // no pending tasks.
    ...
    public void setWirelessParams(long fixtureId, long gatewayId) {
        ...
        System.arraycopy(keyArr, 0, dataPkt, i, keyArr.length);
        i += 17;
        profileLogger.debug("encryption type == " + gw.getWirelessEncryptType().byteValue());
    ...
    public void setCurrentTime(Fixture fixture, int seqNo) {
        ...
        try {
            // byte[] timeByteArr = ServerUtil.intToByteArray(currentTime);
            // byte[] gmtByteArr = ServerUtil.intToByteArray(gmtOffset);
            // byte[] dstByteArr = ServerUtil.intToByteArray(dst);
...
//            pktIndex += 4;
            ...
            // byte[] dataPacket = new byte[timeByteArr.length + gmtByteArr.length + dstByteArr.length +
            // seqNoArr.length];
            // System.arraycopy(timeByteArr, 0, dataPacket, 0, timeByteArr.length);
            // System.arraycopy(gmtByteArr, 0, dataPacket, timeByteArr.length, gmtByteArr.length);
            // System.arraycopy(dstByteArr, 0, dataPacket, timeByteArr.length + gmtByteArr.length,
            // dstByteArr.length);
            // System.arraycopy(seqNoArr, 0, dataPacket, timeByteArr.length + gmtByteArr.length +
            // dstByteArr.length, seqNoArr.length);
    ...
    public void nodeBootInfo(Fixture fixture, byte[] packet, long gwId) {
        ...
        fixtureLogger.info(fixture.getId() + ": image(CurrVer= " + currVersion + ", OtherVer= " + otherVersion
                + ") booted to app" + appId);
        ImageUpgradeSO.getInstance().nodeRebooted(fixture, currRevision, otherRevision, appId, gwId, imgUpgrStatus);
        ...
        ServerMain.getInstance().getDevice(fixture).setLastStatsSeqNo(1);
        try {
        ...
        fixtureMgr.updateBootInfo(fixture);
    ...
    public void initiateProfileSyncActivity(Fixture fixture, byte gProfChecksum, byte sProfChecksum,
        ...
        profileLogger
                .debug(fixtureId + ": (" + sMsgType + ") global profile checksum from fixture -- " + gProfChecksum);
    ...
    private void sendProfileToFixture(ProfileHandler prHand, long fixtureId, int msgType) {
        ...
        try {
            ...
            CommandScheduler.getInstance().addCommand(fixture, dataPacket, msgType, true, UNICAST_PKTS_DELAY);
            ServerUtil.sleepMilli(UNICAST_PKTS_DELAY);
    ...
    public void dimFixturesBySwitch(long switchId, int mode, int duration) {
        ...
        List<SceneLevel> oSceneLevelList = switchMgr.loadSceneLevelsBySwitchId(switchId);
