...
//        }
    ...
    public synchronized void missingPacketRequest(Fixture fixture, byte[] packet) {
      ...
      logger.debug(fixture.getId() + ": no. of missing - " + noOfMissingPkts + 
...
//  	  EventsAndFault.FIXTURE_IMG_UP_STR, EventsAndFault.MAJOR_SEV_STR);
    ...
    public void currentNodeVersion(Fixture fixture, byte[] packet) {
      ...
      if(details.appUpgraded == ServerConstants.ISP_APP2_INIT_OPCODE) {
      	updateUpgradeCompleteStatus(fixture, details, appId, revision, 0, 0);
      }
      
    } //end of method currentNodeVersion
    
    private void updateUpgradeCompleteStatus(Fixture fixture, FixtImgUpgrDetails details, 
    		byte appId, int currVer, int otherVer, int imgUpgrStatus) {
      ...
      if(imageUpgradeSuccess) {
      	...
      	fixtureInProgressMap.remove(fixture.getId());
        logger.info(fixture.getId() + ":image upgrade successful and node rebooted.");
        noOfSusSuccess++;
        fixtureMgr.setImageUpgradeStatus(fixture.getId(), ServerConstants.IMG_UP_STATUS_SUCCESS);
        eventMgr.addEvent(fixture, details.description, 
        		EventsAndFault.FIXTURE_IMG_UP_STR, EventsAndFault.INFO_SEV_STR);	
        firmUpMgr.finishDeviceUpgrade(dbJob.getId(), fixture.getId(), 
    ...
    public void nodeRebooted(Fixture fixture, short currVer, short otherVer, byte appId, int imgUpgrStatus) {
      ...
      updateUpgradeCompleteStatus(fixture, details, appId, currVer, otherVer, imgUpgrStatus);
  ...
  public void nodeRebooted(Fixture fixture, short currVer, short otherVer, byte appId,
      long gwId, int imgUpgrStatus) {
    
    if(gwFixUpgThrMap.containsKey(gwId)) {
      gwFixUpgThrMap.get(gwId).nodeRebooted(fixture, currVer, otherVer, appId, imgUpgrStatus);
    }
