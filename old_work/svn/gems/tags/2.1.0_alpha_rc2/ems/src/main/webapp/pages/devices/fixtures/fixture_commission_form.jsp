...
function initExitCommissionProcess(){
	...
	displayFxCommissionMessage("Exiting Commissioning...", COLOR_SUCCESS);
	$.ajax({
		url: "${exitCommissionUrl}"+urlOption+"?ts="+new Date().getTime(),
		type: "GET",
		contentType: "application/json; charset=utf-8",
		success: function(data){
			displayFxCommissionMessage("Done", COLOR_SUCCESS);
		}
	});	
}
...
function exitFixtureCommissionWindow(){
	...
	// TODO: Need a elegant way to deal with exit.
	initExitCommissionProcess();
}

function exitWindow(){
  	$("#fixtureCommissioningDialog").dialog("close"); //COMMENT ME (for testing)
}
				...
				<tr style="height:30px;">
				 	<td  class="highlightGray" style="border:0px none;">
				 		...
				 		<span  style="float:right;">
					 		...
					 		<button id="fxcd-undo-btn">Undo</button>
