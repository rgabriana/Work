...
var flash_fp = function(nodetype, nodeid) {		
	...
	*/
	...
	if ($.browser.msie) {
		...
		FP_data +=  "<param name='src' value='${plotchartmodule}'/>";
		...
		FP_data +=  "<embed id='floorplan' name='floorplan' src='${plotchartmodule}' pluginspage='http://www.adobe.com/go/getflashplayer'";
		FP_data +=  " height='100%'";
		FP_data +=  " width='100%'";
		FP_data +=  " padding='0px'";
...
<spring:url value="/devices/fixtures/assigngroupstoswitches.ems" var="assignGroupsToFixturesDailogUrl" scope="request" />
		...
		$("#switchFormDialog").dialog({
			...
			close: function(event, ui) {
				...
				try{
					...
					//alert("Failed to refresh floor plan");
				}
			}
		});
	});
...
function assignUserToSwitches(selSwitches){
	...
	$("#assignUserToSwitchesDailog").load("${assignUserToSwitchesDailogUrl}"+"?ts="+new Date().getTime(), function() {
...
function assignGemsGroupToFixtures(selFixtures){
	if(selFixtures != undefined) {
		SELECTED_FIXTURES_TO_ASSIGN_GROUPS = eval("("+selFixtures+")");	
	}
	
	$("#assignGroupsToFixturesDailog").load("${assignGroupsToFixturesDailogUrl}"+"?ts="+new Date().getTime(), function() {
...
<div id="innercenter" class="ui-layout-center">
	<ul>		
		...
		<security:authorize access="hasAnyRole('Admin','FacilitiesAdmin')">
			<li><a id="os" href="#settings" onclick="loadSettings();"><span>Settings</span></a></li>
