  ...
  //TODO no use case found. Can be removed.
    ...
    public List<Timezone> getTimezoneList() {
        ...
        for (i = 0; i < ids.length; i++) {
            /*
             * if (ids[i].matches(TIMEZONE_ID_PREFIXES)) { System.out.println("id -- " + ids[i] + " name - " +
             * TimeZone.getTimeZone(ids[i]).getDisplayName()); }
             */
            tzone = new Timezone();
    ...
    public void registerCompany(Company company, String newPassword) {
        if (getAllCompanies().size() < 1) {
            ...
            company.setTimeZone(getTimezoneList().get(company.getTimezone().intValue()).getName());
            save(company);
            setServerTimeZone(company.getTimeZone());
    ...
    public void updateCompany(Company company) {
        ...
        String newTimeZone = getTimezoneList().get(company1.getTimezone().intValue()).getName();
        boolean changeTimeZone = !company1.getTimeZone().equals(newTimeZone);
        company1.setTimeZone(getTimezoneList().get(company1.getTimezone().intValue()).getName());
        ...
        update(company1);
        if (changeTimeZone) {
            setServerTimeZone(company1.getTimeZone());
        }
