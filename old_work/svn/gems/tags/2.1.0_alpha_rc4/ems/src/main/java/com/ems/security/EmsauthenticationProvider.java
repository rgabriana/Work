...
public class EmsauthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (authTypeConfig != null) {
            authenticationType = AuthenticationType.valueOf(authTypeConfig.getValue());
        }
        ...
        // Let's check if the user belongs to a tenant and tenant is not active or
        ...
        if (user != null) {
            if (user.getTenant() != null) {
                ...
                if (tenant.getStatus() == TenantStatus.INACTIVE) {
                    throw new DisabledException("Tenant is inactive. Please contact Administrator");
                }

                if (facilityTreeManager.loadTenantFacilitiesHierarchy(tenant.getId()).getTreeNodeList().isEmpty()) {
                    throw new FacilityNotAssignedException(
                            "Tenant of the user has no assigned facility. Please contact Administrator");
                }
            }
        }
        ...
        if (user != null) {
            ...
            if (authenticationType == AuthenticationType.LDAP && (!"admin".equals(authentication.getName()))) {
                authenticateAgainstLdap(authentication, ldapUrl, ldapAuthType, ldapSecurityPrincipal);
            } else {
        ...
        userAuditLoggerUtil.log("User " + user.getEmail() + " logged in", UserAuditActionType.Login.getName());
    ...
    private void authenticateAgainstLdap(Authentication authentication, String ldapUrl, String ldapAuthType,
