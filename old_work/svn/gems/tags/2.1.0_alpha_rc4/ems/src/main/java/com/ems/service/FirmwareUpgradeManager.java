...
//			logger.debug("CU 20 App pattern: " + strCU20AppPattern);
	...
	public int saveImageVersion(int deviceType, int[] deviceIds, String imageFileName, User user) {
		int result = 0;

		final ArrayList<ImageUpgradeJob> jobList = new ArrayList<ImageUpgradeJob>();
		if (deviceIds != null && deviceIds.length > 0) {
			ImageUpgradeJob imgJob = new ImageUpgradeJob();
			imgJob.setDeviceType(deviceType);
			imgJob.setDeviceIds(deviceIds);
			imgJob.setFileName(imageFileName);
			jobList.add(imgJob);
...
// FIXME : Date calculation from hidden field from image upgrade form
//		Date date = null;
//		if(!ArgumentUtils.isNullOrEmpty(dateOn)){
//			date = DateUtil.parseString(dateOn,"MM/dd/yyyy");	
//		}
//		if(!ArgumentUtils.isNullOrEmpty(timeOn)){
//			date = DateUtil.setTimeInDate(timeOn, date);
//		}
		Date date = new Date();
	...
	public void finishDeviceUpgrade(Long jobId, Long deviceId, String status,
	    int noOfAtts, String desc) {
	  firmwareUpgradeDao.finishDeviceUpgrade(jobId, deviceId, status, noOfAtts, desc);
