...
<style>
	...
	div#commissionDialogBox form{background-color:white;}
...
// 			setGatewayLocation();
			...
			var flash_floor = function(nodetype, nodeid) {
				...
				*/
				...
				if ($.browser.msie) {
					FP_data = "<object id='c_floorplan' classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab' width='100%' height='100%'>";
			...
			$('#showClearText')
					.click(
							function() {
								document
										.getElementById('wirelessEncryptKey').type = this.checked ? "text"
										: "password";
							});
	});
		...
		$.ajax({
			...
			success : function(msg) {
				$(msg).find('gateway')
						.each(
								function() {
									$("#gid").val($(this).find('id').text());
									$("#gatewayName").val($(this).find('name').text());
									...
									$("#wirelessEncryptKey").val($(this).find('wirelessencryptkey').text());
	...
	function commissionDevice() {
		if ($("#channel").val() == 4 && $("#wirelessNetworkId").val() == 6854 && $("#wirelessEncryptKey").val() == "enLightedWorkNow") {
			...
			<div class="form-content_wrapper">
				...
				<div class="form-column-left-wrapper">
					...
					<div class="fieldWrapper">
						...
						<div class="fieldInput"><input class="text" id="gatewayName" name="gatewayName" /></div>
						<br style="clear:both;"/>
					</div>
				...
				<table width="100%" cellspacing=0>
					<tr>
						<td width="210px" valign="top">
							...
							<input id="gwc-commission-btn" type="button" class="button" onclick="commissionDevice();" value="<spring:message code='gatewayForm.label.commissionAndPlaceBtn'/>" /> 
						</td>
