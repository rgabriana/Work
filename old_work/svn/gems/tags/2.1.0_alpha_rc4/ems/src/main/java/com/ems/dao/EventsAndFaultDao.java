...
public class EventsAndFaultDao extends BaseDaoHibernate {
    ...
    public List<Object> getEventsAndFaults(String order, String orderWay, List<Object> filter, String roleType, int offset, int limit) {
    	...
    	if("TenantAdmin".equals(roleType)) {
    		...
    		rowCount = getSession().createCriteria(EventsAndFault.class, "ef")
        			.createAlias("fixture", "fixt", CriteriaSpecification.INNER_JOIN)
        	...
        	if(filter != null) {
        		...
        		if(searchString != null) {
        			rowCount.add(Restrictions.or(Restrictions.sqlRestriction("lower(to_char({alias}.event_time, 'YYYY:MM:DD HH12:MI:SS AM')) like ? ", "%" + searchString.toLowerCase() + "%", Hibernate.STRING), 
        					Restrictions.or(Restrictions.ilike("fixt.location", searchString, MatchMode.ANYWHERE),
            						Restrictions.or(Restrictions.ilike("fixt.fixtureName", searchString, MatchMode.ANYWHERE),
        		...
        		if(groupId != null) {
        			rowCount.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.group_id = ? )", groupId, Hibernate.LONG));
        			data.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.group_id = ? )", groupId, Hibernate.LONG));
        		}
        		...
        		if(orgType != null && orgId != null) {
        			switch(orgType) {
        				case CAMPUS: {
        					rowCount.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.campus_id = ? )", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.campus_id = ? )", orgId, Hibernate.LONG));
        					break;
        				}
        				case BUILDING: {
        					rowCount.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.building_id = ? )", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.building_id = ? )", orgId, Hibernate.LONG));
        					break;
        				}
        				case FLOOR: {
        					rowCount.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.floor_id = ? )", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.floor_id = ? )", orgId, Hibernate.LONG));
        					break;
        				}
        				case AREA: {
        					rowCount.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.area_id = ? )", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.area_id = ? )", orgId, Hibernate.LONG));
        					break;
        				}
        				case COMPANY: {
        					break;
        				...
        				default: {
        					rowCount.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.campus_id = ? )", new Long("0"), Hibernate.LONG));
                			data.add(Restrictions.sqlRestriction(" {alias}.fixture_id in (select infixt.id from fixture infixt where infixt.campus_id = ? )", new Long("0"), Hibernate.LONG));
        				}
        			}
        		}
    	...
    	//All events
    		...
    		rowCount = getSession().createCriteria(EventsAndFault.class, "ef")
        			.createAlias("fixture", "fixt", CriteriaSpecification.LEFT_JOIN)
        			.createAlias("gateway", "gw", CriteriaSpecification.LEFT_JOIN)
        			.setFetchMode("fixt", FetchMode.JOIN)
        			.setFetchMode("gw", FetchMode.JOIN)
        			.setProjection(Projections.rowCount());
        	...
        	if(filter != null) {
        		...
        		if(orgType != null && orgId != null) {
        			switch(orgType) {
        				case CAMPUS: {
        					rowCount.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.campus_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.campus_id = ? ) or {alias}.fixture_id is null )", orgId, Hibernate.LONG));
        				...
        				case BUILDING: {
        					rowCount.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.building_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.building_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
        					
        					rowCount.add(Restrictions.sqlRestriction(" ({alias}.gateway_id in (select ingate.id from gateway ingate where ingate.building_id = ? ) or {alias}.gateway_id is null) ", orgId, Hibernate.LONG));
                            data.add(Restrictions.sqlRestriction(" ({alias}.gateway_id in (select ingate.id from gateway ingate where ingate.building_id = ? ) or {alias}.gateway_id is null) ", orgId, Hibernate.LONG));
        					
        					break;
        				}
        				case FLOOR: {
        					rowCount.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.floor_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.floor_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
        				...
        				case AREA: {
        					rowCount.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.area_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
        					data.add(Restrictions.sqlRestriction(" ({alias}.fixture_id in (select infixt.id from fixture infixt where infixt.area_id = ? ) or {alias}.fixture_id is null) ", orgId, Hibernate.LONG));
    	...
    	data.setProjection(Projections.projectionList().add(Projections.property("id"), "id")
				...
				.add(Projections.property("fixt.fixtureName"), "fixtureName")
				...
				.add(Projections.property("gw.gatewayName"))
    ...
    public List<EventsAndFault> getEventsAndFaultsByFixtureId(Long fixture_id) {
        List<EventsAndFault> eventsAndFaults = getSession().createCriteria(EventsAndFault.class)
                .add(Restrictions.eq("fixture.id", fixture_id)).add(Restrictions.eq("active", true))
    ...
    public List<EventsAndFault> getEventsAndFaultsByGatewayId(Long gateway_id) {
        List<EventsAndFault> eventsAndFaults = getSession().createCriteria(EventsAndFault.class)
                .add(Restrictions.eq("gateway.id", gateway_id)).add(Restrictions.eq("active", true))
                .addOrder(Order.desc("eventTime")).list();
        return eventsAndFaults;
    }
	...
	public List<EventsAndFault> getEventsOnFaultyFixtures(Integer floorId, java.util.Date toDate,
        ...
        SQLQuery sqlQuery = getSession()
                .createSQLQuery(
                        "select ef.event_time,ef.severity,ef.event_type,ef.description,ef.active,ef.fixture_id,fl.name as floorName,b.name as buildingname,c.name as campusname from events_and_fault ef inner join fixture f on ef.fixture_id=f.id inner join floor fl on  fl.id=f.floor_id inner join building b on b.id=f.building_id inner join campus c on c.id=f.campus_id where f.floor_id="
                                + floorId
    ...
    public List getFaultyFloorsByCampusAndBuilding(Integer locationId, String locationType, java.util.Date toDate,
        ...
        if (locationType.equals("campus")) {
            queryString = "select distinct( floor_id) from fixture where id in ( select fixture_id from events_and_fault where  event_time>='"
                    + fromDate
                    + "' and event_time<='"
                    + toDate
	...
	public List<Fixture> getFaultyFixtures(Integer floorId, java.util.Date toDate, java.util.Date fromDate) {
        SQLQuery sqlQuery = getSession().createSQLQuery(
                "select id,x,y from fixture where id in (select fixture_id from events_and_fault where  event_time>='"
                        + fromDate + "' and event_time<='" + toDate
        ...
        if (!ArgumentUtils.isNullOrEmpty(outageVO.getNodeType())) {
            ...
            if (session != null) {
                try {
                    ...
                    q.append(nodeQuery);
                    q.append(" and id in (select fixture_id from events_and_fault where active = 't')");
        ...
        if (!ArgumentUtils.isNullOrEmpty(outageVO.getNodeType())) {
            ...
            if (session != null) {
                try {
                    ...
                    q.append(nodeQuery);
                    q.append(" and id in (select gateway_id from events_and_fault where active = 't')");
                    SQLQuery query = session.createSQLQuery(q.toString());
                    List idList = query.list();
                    List<Long> ids = new ArrayList<Long>();
                    for (Object object : idList) {
