...
public class FixtureDao extends BaseDaoHibernate {
    ...
    // public static final String FIXTURE_CONTRACTOR = "Select new Fixture(f.id,"
    ...
    public List<Fixture> loadFixtureByGroupId(Long id) {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .add(Restrictions.eq("groupId", id)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        ...
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where f.groupId=?" + " and (state != '"
    ...
    public List<Fixture> loadFixtureByFloorId(Long id) {
        ...
        // Session s = getSession();
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where f.floor.id=?" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query q = s.createQuery(hsql.toString());
        // q.setParameter(0, id);
        // results = q.list();
    ...
    public List<Fixture> loadFixtureByBuildingId(Long id) {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .add(Restrictions.eq("buildingId", id)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
        ...
        // try {
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where buildingId=?" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
    ...
    public List<Fixture> loadFixtureByCampusId(Long id) {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .add(Restrictions.eq("campusId", id)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
        ...
        // try {
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where campusId=?" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query q = getSession().createQuery(hsql.toString());
        // q.setParameter(0, id);
        // results = q.list();
    ...
    public List<Fixture> loadFixtureByAreaId(Long id) {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .add(Restrictions.eq("area.id", id)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
        ...
        // String hsql = "from Fixture where area.id = :areaId" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
    ...
    public List<Fixture> loadFixtureBySubareaId(Long id) {
        ...
        // try {
        // List<Fixture> fixtures = null;
        // String hsql = "from Fixture where subArea.id = :subareaId" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query query = getSession().createQuery(hsql.toString());
    ...
    public void updateProfileHandler(ProfileHandler profileHandler) {
        ...
        if (profileHandler.getProfileConfiguration() != null) {
            logger.debug("Config id is: " + profileHandler.getProfileConfiguration().getId());
        }
    ...
    public void changeFixtureProfile(Long fixtureId, Long groupId, Long globalPFId, String currentProfile,
...
//            enablePushGlobalProfileForFixture(fixtureId);
    ...
    public void updateProfileHandlerIdForFixture(Long fixtureId, Long profileHandlerId, String currentProfile,
            String originalProfileFrom) {
        Session s = getSession();
        Fixture fixture = (Fixture) s.get(Fixture.class, fixtureId);
    ...
    public ProfileHandler getProfileHandlerByFixtureId(Long fixtureId) {
        ...
        if (profileHandler != null) {
            profileHandler.setDayProfile((Profile) getObject(Profile.class, profileHandler.getDayProfile().getId()));
            profileHandler.setDayProfileHoliday((Profile) getObject(Profile.class, profileHandler
                    .getDayProfileHoliday().getId()));
            profileHandler.setDayProfileWeekEnd((Profile) getObject(Profile.class, profileHandler
                    .getDayProfileWeekEnd().getId()));

            profileHandler.setEveningProfile((Profile) getObject(Profile.class, profileHandler.getEveningProfile()
                    .getId()));
            profileHandler.setEveningProfileHoliday((Profile) getObject(Profile.class, profileHandler
                    .getEveningProfileHoliday().getId()));
            profileHandler.setEveningProfileWeekEnd((Profile) getObject(Profile.class, profileHandler
                    .getEveningProfileWeekEnd().getId()));

            profileHandler.setMorningProfile((Profile) getObject(Profile.class, profileHandler.getMorningProfile()
                    .getId()));
            profileHandler.setMorningProfileHoliday((Profile) getObject(Profile.class, profileHandler
                    .getMorningProfileHoliday().getId()));
            profileHandler.setMorningProfileWeekEnd((Profile) getObject(Profile.class, profileHandler
                    .getMorningProfileWeekEnd().getId()));

            profileHandler
                    .setNightProfile((Profile) getObject(Profile.class, profileHandler.getNightProfile().getId()));
            profileHandler.setNightProfileHoliday((Profile) getObject(Profile.class, profileHandler
                    .getNightProfileHoliday().getId()));
            profileHandler.setNightProfileWeekEnd((Profile) getObject(Profile.class, profileHandler
                    .getNightProfileWeekEnd().getId()));

            profileHandler.setProfileConfiguration((ProfileConfiguration) getObject(ProfileConfiguration.class,
    ...
    public ProfileHandler getProfileHandlerByGroupId(Long groupId) {
        Session session = getSession();
        ProfileHandler profileHandler = ((Groups) session.get(Groups.class, groupId)).getProfileHandler();
        // try {
        // profileHandler = (ProfileHandler) session
    ...
    public Fixture update(Fixture fixture) {
        ...
        // "Update Fixture set " + "fixtureName = :fixtureName," + "sensorId = :sensorId," + "type = :type,"
        // + "noOfBulbs = :noOfBulbs," + "bulbWattage = :bulbWattage," + "wattage = :wattage,"
        // + "ballastManufacturer = :ballastManufacturer," + "bulbManufacturer = :bulbManufacturer,"
        // + "ballast = :ballast," + "bulb = :bulb," + "noOfFixtures = :noOfFixtures,"
        // + "description = :description," + "notes = :notes," + "voltage = :voltage,"
        ...
        // query.setString("ballastManufacturer", fixture.getBallastManufacturer());
    ...
    public Fixture updateFixture(Fixture fixture) {
        Session session = getSession();
        session.update(fixture);
        // Fixture fixture = (Fixture) session.load(Fixture.class, fixtureObj.getId());
        //
        ...
        //
        // Ballast ballast = (Ballast) session.load(Ballast.class, fixtureObj.getBallast().getId());
        // fixture.setBallast(ballast);
        //
        // Bulb bulb = (Bulb) session.load(Bulb.class, fixtureObj.getBulb().getId());
    ...
    public List getAllFixtures() {
        Session session = getSession();
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
    ...
    public List<Fixture> loadAllFixtureByGatewayId(Long gatewayId) {
        ...
        // try {
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where f.gateway.id=?" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query q = getSession().createQuery(hsql.toString());
    ...
    /**
     * return list of commissioned fixture objects associated with secondary gateway
     * 
     * @param secGwId
    ...
    public List<Fixture> getAllCommissionedFixturesBySecGwId(Long secGwId) {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                ...
                .add(Restrictions.eq("secGwId", secGwId)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).addOrder( Order.asc("id")).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
    ...
    public List<Fixture> loadAllFixtureBySecondaryGatewayId(Long secGwId) {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .add(Restrictions.eq("secGwId", secGwId)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).addOrder( Order.asc("lastConnectivityAt")).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
        ...
        // try {
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where f.secGwId=?" + " and (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query q = getSession().createQuery(hsql.toString());
    ...
    public List<Fixture> loadAllOrgFixtures() {
        ...
        // try {
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f" + " where (state != '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query q = getSession().createQuery(hsql.toString());
        // results = q.list();
        // if (results != null && !results.isEmpty()) {
    ...
    /**
     * Load fixtures who custom profile_handler points to default (i.e global) profile_handler.
     * 
     * @return
     */
    public List<Fixture> loadAllFixturesWithDefaultPFID() {
        ...
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .add(Restrictions.eq("profileHandler.id", 1L)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
        ...
        // try {
        // List<Fixture> results = null;
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f" + " where f.profileHandler.id = 1";
        // Query q = getSession().createQuery(hsql.toString());
        // results = q.list();
        // if (results != null && !results.isEmpty()) {
        // return results;
    ...
    public Fixture getFixtureBySnapAddr(String snapAddr) {
        Session session = getSession();
        Fixture fixture = (Fixture) session.createCriteria(Fixture.class).add(Restrictions.eq("snapAddress", snapAddr))
                .createAlias("area", "area", Criteria.LEFT_JOIN).createAlias("ballast", "ballast").createAlias("bulb", "bulb")
                .uniqueResult();
        return fixture;

        // Fixture fixture = null;
        // try {
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where snapAddress = :snap";
        // Query q = getSession().createQuery(hsql.toString());
    ...
    public Fixture getDeletedFixtureBySnapAddr(String snapAddr) {
        Session session = getSession();
        Fixture fixture = (Fixture) session.createCriteria(Fixture.class).createAlias("area", "area", Criteria.LEFT_JOIN)
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
        ...
        // Fixture fixture = null;
        // try {
        // String hsql = "from Fixture f where f.snapAddress = :snap and " + " (f.state = '"
        // + ServerConstants.FIXTURE_STATE_DELETED_STR + "')";
        // Query q = getSession().createQuery(hsql.toString());
        // q.setString("snap", snapAddr);
        // fixture = (Fixture) q.uniqueResult();
    ...
    public Fixture getFixtureByIp(String ip) {
        Session session = getSession();
        Fixture fixture = (Fixture) session.createCriteria(Fixture.class).add(Restrictions.eq("ip_address", ip))
                .uniqueResult();
        return fixture;

        // Fixture fixture = null;
        // try {
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where ip_address = ?";
        // Query q = getSession().createQuery(hsql.toString());
        // q.setParameter(0, ip);
        // fixture = (Fixture) q.uniqueResult();
        // } catch (HibernateException hbe) {
    ...
    public Fixture getFixtureByMacAddr(String macAddr) {
        Session session = getSession();
        Fixture fixture = (Fixture) session.createCriteria(Fixture.class).add(Restrictions.eq("mac_address", macAddr))
                .uniqueResult();
        return fixture;

        // Fixture fixture = null;
        // try {
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where mac_address = ?";
        // Query q = getSession().createQuery(hsql.toString());
    ...
    public List getUnValidatedFixtureList(int floorId) {
        Session session = getSession();
        List<Fixture> fixtureList = session.createCriteria(Fixture.class)
                .add(Restrictions.or(Restrictions.eq("state", ServerConstants.FIXTURE_STATE_COMMISSIONED_STR), Restrictions.eq("state", ServerConstants.FIXTURE_STATE_PLACED_STR)))
                .add(Restrictions.eq("floor.id", floorId)).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
        ...
        // try {
        // Session s = getSession();
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where f.floor.id=? "
    ...
    public String getCommissionStatus(long fixtureId) {
        ...
        // String sStatus = (String) getSession().createQuery("Select state from Fixture where id = :id")
        // .setLong("id", fixtureId).uniqueResult();
        // if (sStatus != null)
        // return sStatus;
        // return "";
    ...
    public List<Fixture> getSortedFixtures() {
        Session session = getSession();
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).addOrder(Order.asc("x"))
                .addOrder(Order.asc("y")).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
    ...
    public Fixture getFixtureName(String name) {
        Session session = getSession();
        List<Fixture> fixtureList = session.createCriteria(Fixture.class).add(Restrictions.eq("name", name)).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
        ...
        // Fixture fixture = null;
        // try {
        // String hsql = FIXTURE_CONTRACTOR + " from Fixture f where fixtureName = ?";
        // Query q = getSession().createQuery(hsql.toString());
    ...
    public List<Ballast> getAllBallasts() {
        List<Ballast> ballasts = getSession().createCriteria(Ballast.class).list();
    ...
    public Ballast getBallastById(long id) {
        ...
        return (Ballast) getSession().createCriteria(Ballast.class).add(Restrictions.eq("id", id)).uniqueResult();
    ...
    public Bulb getBulbById(long id) {
        ...
        // TODO Auto-generated method stub
        return (Bulb) getSession().createCriteria(Bulb.class).add(Restrictions.eq("id", id)).uniqueResult();
    ...
    public void assignGateway(Fixture fx) {
        ...
        // Gateway gateway = (Gateway) session.get(Gateway.class, fx.getGateway().getId());
        // Floor floor = (Floor) session.get(Floor.class, fx.getFloor().getId());
    ...
    public void updateStats(Fixture fixture) {
        ...
        // Query query = getSession().createQuery(
        // "Update Fixture set " + "lastOccupancySeen = :lastOccupancySeen, " + "lightLevel = :lightLevel, "
        // + "lastConnectivityAt = :lastConnectivityAt, " + "lastStatsRcvdTime = :lastStatsRcvdTime, "
        // + "currApp = :currApp, " + "currentState = :currentState, " + "wattage = :wattage, "
        // + "secGwId = :secGwId, " + "dimmerControl = :dimmerControl, "
        // + "avgTemperature = :avgTemperature, " + "baselinePower = :baselinePower, "
        // + "versionSynced = :versionSynced" + " where id = :id");
        // query.setLong("id", fixture.getId());
    ...
    public void updateRealtimeStats(Fixture fixture) {
        ...
        // + "globalProfileChecksum = :globalProfileChecksum, " + "profileChecksum = :profileChecksum, "
        // + "currentState = :currentState, " + "wattage = :wattage, " + "secGwId = :secGwId, "
...
//        }
//        logger.info("Fixture: " + fixture.getId() + ", updating to state: " + fixture.getState());
//
//        session.update(fixture);
        try {
            Session session = getSession();
            ...
            if (fixt.getState().equals(ServerConstants.FIXTURE_STATE_DELETED_STR)) {
    ...
    public Integer getTotalFixtureCountInSensor(Long floorId) {
        Integer totalFixtureCount = (Integer) getSession().createCriteria(Fixture.class)
                .add(Restrictions.eq("floor.id", floorId)).setProjection(Projections.sum("noOfFixtures"))
                .uniqueResult();
    ...
    public Fixture getFixtureById(Long id) {
        Session session = getSession();
        return (Fixture) session.createCriteria(Fixture.class).add(Restrictions.eq("id", id))
                .createAlias("ballast", "ballast").createAlias("bulb", "bulb").uniqueResult();
        /*
         * Fixture fixture = (Fixture) session.get(Fixture.class, id); fixture.getBallast().getBallastName(); return
         * fixture;
         */

        // List<Fixture> fixtures = getSession().createQuery(FIXTURE_CONTRACTOR + " from Fixture f where f.id = :id")
        // .setLong("id", id).list();
        // if (!ArgumentUtils.isNullOrEmpty(fixtures)) {
    ...
    public Integer getCommType(Long fixtureId) {
        ...
        return null;
        // List<Object> commTypes = getSession().createQuery("Select commType from Fixture where id = :id")
    ...
    public String getIpAddress(Long fixtureId) {
        ...
        return null;
        // List<Object> ipAddresses = getSession().createQuery("Select ipAddress from Fixture where id = :id")
        // .setLong("id", fixtureId).list();
    ...
    public void updateVersion(String version, long id, long gwId) {
        ...
        // + "secGwId = :secGwId, currApp = :currApp, " + "version = :version where id = :id");
        // query.setLong("id", id);
        // query.setTimestamp("lastConnectivityAt", new java.util.Date());
    ...
    public void updateFirmwareVersion(String version, long id, long gwId) {
        ...
        // Query query = getSession().createQuery(
        // "Update Fixture set " + "lastConnectivityAt = :lastConnectivityAt, "
        // + "secGwId = :secGwId, currApp = :currApp, " + "firmwareVersion = :version where id = :id");
        // query.setLong("id", id);
        // query.setTimestamp("lastConnectivityAt", new java.util.Date());
        // query.setString("version", version);
    ...
    public void updateBootInfo(Fixture fx) {
        Session session = getSession();
        session.update(fx);
        fx.setLastConnectivityAt(new Date());
        fx.setLastBootTime(new Date());
        ...
        //
        // TODO: Check with Sreedhar if this required if we do in update way - lalit
    ...
    public void updateFixturePlc(long id, String ipAddr, String version) {
        ...
        // Query query = getSession().createQuery(
        // "Update Fixture set " + "version = :version, ipAddress = :ipAddress where id = :id");
        // query.setLong("id", id);
        // query.setString("version", version);
    ...
    public void adjustLastOccupancyTime(long floorId, int sec) {
        ...
        Query query = getSession().createQuery(
                "Update Fixture set " + "lastOccupancySeen = lastOccupancySeen + :lastOccupancySeen"
                        + " where floor_id = :id");
    ...
    /**
     * Intentional SQL query
     * @param fixtureId
     * @param gwId
     */
    public void updateLastConnectionTime(long fixtureId, long gwId) {
//        Session session = getSession();
//        Fixture fixture = (Fixture) session.get(Fixture.class, fixtureId);
//        fixture.setLastConnectivityAt(new java.util.Date());
//        fixture.setSecGwId(gwId);
         ...
         Query query = getSession().createQuery(
         "Update Fixture set " + "lastConnectivityAt = :lastConnectivityAt, " + "secGwId = :secGwId "
         + " where id = :id");
    ...
    } // end of method updateLastConnectionTime

    @SuppressWarnings("unchecked")
    public List<BallastVoltPower> getAllBallastVoltPowers() {
        List<BallastVoltPower> ballastVoltPowers = getSession().createCriteria(BallastVoltPower.class).list();
    ...
    public List<BallastVoltPower> getAllBallastVoltPowersFromId(long voltPowerMapId) {
    	List<BallastVoltPower> ballastVoltPowers = this.ballastVoltPowersMap.get(voltPowerMapId);
        if (ballastVoltPowers == null) {
            ballastVoltPowers = getSession().createCriteria(BallastVoltPower.class)
                    .add(Restrictions.eq("voltPowerMapId", voltPowerMapId)).list();
            if (ArgumentUtils.isNullOrEmpty(ballastVoltPowers)) {
                // Fetch the default.
                ballastVoltPowers = getSession().createCriteria(BallastVoltPower.class)
    ...
    public void updateGroupId(long id, long groupId) {
        ...
        // Query query = getSession().createQuery("Update Fixture set " + "group_id = :group_id " + "where id = :id");
        // query.setLong("id", id);
    ...
    public void setImageUpgradeStatus(long fixtureId, String status) {
        ...
        // SQLQuery query = getSession().createSQLQuery("update fixture set upgrade_status" +
        // " = :status where id = :id");
        // query.setLong("id", fixtureId);
    ...
    public void updateLastCommandSent(int[] fixtureArr, String cmd) {

        try {
            String queryStr = "update fixture set last_cmd_sent = :cmd, "
                    + "last_cmd_sent_at = :cmdTime, last_cmd_status = 'PENDING' where id in (";
            int noOfFixtures = fixtureArr.length;
            for (int i = 0; i < noOfFixtures; i++) {
                if (i > 0) {
                    queryStr += ", ";
                }
                queryStr += "" + fixtureArr[i];
            }
            queryStr += ")";

            SQLQuery query = getSession().createSQLQuery(queryStr);
            query.setString("cmd", cmd);
            query.setTimestamp("cmdTime", new java.util.Date());
            query.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }

    } // end of method updateLastCommandSent

    public void updateLastCommandStatus(Fixture fixture, String cmd, String status, long gwId) {
        Session session = getSession();
        session.update(fixture);
        fixture.setLastCmdStatus(status);
        fixture.setLastCmdSent(cmd);
        Date date = new java.util.Date();
        fixture.setLastCmdSentAt(date);
        fixture.setLastConnectivityAt(date);
        fixture.setSecGwId(gwId);

        // try {
        // SQLQuery query = getSession().createSQLQuery(
        // "update fixture set last_cmd_status = "
        // + ":cmdStatus, last_cmd_sent = :cmd, last_cmd_sent_at = :cmdTime, last_connectivity_at"
        // + " = :connTime, sec_gw_id = :gwId where id = :id");
        // query.setString("cmdStatus", status);
        // query.setString("cmd", cmd);
        // Date ackDate = new Date();
        // query.setTimestamp("cmdTime", ackDate);
        // query.setTimestamp("connTime", ackDate);
    ...
    } // end of method updateLastCommandStatus
    ...
    public Integer getCommissionStatus(Long fixtureId) {
        ...
        // List<Integer> commTypes = getSession().createQuery("Select commissionStatus from Fixture where id = :id")
        // .setLong("id", fixtureId).list();
        // if (!ArgumentUtils.isNullOrEmpty(commTypes)) {
    ...
    public void updateCommissionStatus(Long fixtureId, int status) {
        ...
        logger.info("Fixture: " + fixtureId + ", existing status: " + commission_status + " new status: " + status);
        commission_status = commission_status | ServerConstants.COMMISSION_STATUS_COMMUNICATION | status;
        Session session = getSession();
        Fixture fixture = (Fixture) session.get(Fixture.class, fixtureId);
...
//        }
    ...
    public void updateCommissionStatus(int[] fixtureIds, int status) {
        try {
            String queryStr = "update Fixture set commission_status = :commission_status " + " where id in (";
     ...
     */
    public void replaceFixture(long fixtureId, String fixtureName, String macAddr, String sensorId, String snapAddr,
            Integer commisionStatus, String state) {
        Session session = getSession();
        Fixture fixture = (Fixture) session.get(Fixture.class, fixtureId);
        fixture.setFixtureName(fixtureName);
        fixture.setMacAddress(macAddr);
        fixture.setSensorId(sensorId);
        fixture.setSnapAddress(snapAddr);
        fixture.setCommissionStatus(commisionStatus);
    ...
    public List<OutageBasePower> getAllOutageBasePowerList() {
        ...
        List<OutageBasePower> outageBasePowerList = getSession().createCriteria(OutageBasePower.class).list();
    ...
    public List<OutageBasePower> getFixtureOutageBasePowerList(long fixtureId) {
        ...
        outageBasePowerList = getSession().createCriteria(OutageBasePower.class)
    ...
    public OutageBasePower getOutageBasePower(long fixtureId, short voltLevel) {
        ...
        List<OutageBasePower> outageBasePowerList = null;
        outageBasePowerList = getSession().createCriteria(OutageBasePower.class)
                .add(Restrictions.eq("fixtureId", fixtureId)).add(Restrictions.eq("voltLevel", voltLevel)).list();
    ...
    public List<Fixture> getAllCommissionedFixtureList() {
        ...
        Session session = getSession();
        List<Fixture> fixtureList = session.createCriteria(Fixture.class)
                .add(Restrictions.eq("state", ServerConstants.FIXTURE_STATE_COMMISSIONED_STR)).list();
        if (!ArgumentUtils.isNullOrEmpty(fixtureList)) {
            return fixtureList;
        } else {
            return null;
    ...
    public Long getFixtureCount(String property, Long pid) {
        ...
        if (property.equals("company")) {
            totalFixtureCount = (Long) getSession().createCriteria(Fixture.class)
                    .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                    .setProjection(Projections.sum("noOfFixtures")).uniqueResult();
        } else {
            totalFixtureCount = (Long) getSession().createCriteria(Fixture.class).add(Restrictions.eq(property, pid))
                    .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                    .setProjection(Projections.sum("noOfFixtures")).uniqueResult();
        }
	...
	public FixtureList loadFixtureList(String property, Long pid, String order, String orderWay, Boolean bSearch,
        ...
        if (property != null && pid != null) {
            oRowCount = getSession().createCriteria(Fixture.class, "fx")
                    .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
            ...
            oCriteria = getSession().createCriteria(Fixture.class, "fx")
                    .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                    .createAlias("gateway", "gw", CriteriaSpecification.LEFT_JOIN)
                    .createAlias("area", "ar", CriteriaSpecification.LEFT_JOIN)
                    .setFetchMode("gw", FetchMode.JOIN)
            ...
            if (bSearch) {
                if (searchField.equals("id")) {
                    oRowCount.add(Restrictions.eq("id", Long.parseLong(searchString)));
                    oCriteria.add(Restrictions.eq("id", Long.parseLong(searchString)));
                }else if (searchField.equals("name")) {
                    oRowCount.add(Restrictions.like("fx.fixtureName", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.fixtureName", "%" + searchString + "%"));
                }else if (searchField.equals("snapaddress")) {
                    oRowCount.add(Restrictions.like("fx.snapAddress", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.snapAddress", "%" + searchString + "%"));
                }else if (searchField.equals("state")) {
                    oRowCount.add(Restrictions.like("fx.state", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.state", "%" + searchString + "%"));
                }else if (searchField.equals("currentprofile")) {
                    oRowCount.add(Restrictions.like("fx.currentProfile", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.currentProfile", "%" + searchString + "%"));
                }else if (searchField.equals("upgradestatus")) {
                    oRowCount.add(Restrictions.like("fx.upgradeStatus", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.upgradeStatus", "%" + searchString + "%"));
                }else if (searchField.equals("currapp")) {
                    oRowCount.add(Restrictions.like("fx.currApp", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.currApp", "%" + searchString + "%"));
                }else if (searchField.equals("version")) {
                    oRowCount.add(Restrictions.like("fx.version", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("fx.version", "%" + searchString + "%"));
                }else if (searchField.equals("gateway")) {
                    oRowCount.add(Restrictions.like("gw.gatewayName", "%" + searchString + "%"));
                    oCriteria.add(Restrictions.like("gw.gatewayName", "%" + searchString + "%"));
                }
            }
            if (limit > 0) {
    ...
    public List<Fixture> getFixtureOutFixtureList(String property, Long pid) {
        Criteria oCriteria = getSession().createCriteria(Fixture.class, "fx")
                .add(Restrictions.ne("state", ServerConstants.FIXTURE_STATE_DELETED_STR))
                .createAlias("eventsAndFaults", "ef", CriteriaSpecification.LEFT_JOIN)
                .setFetchMode("ef", FetchMode.JOIN)
                .add(Restrictions.eq("ef.eventType", EventsAndFault.FIXTURE_OUTAGE_EVENT_STR));
	...
	public Map<Integer, Object[]> getRecentFixtureDetails() {
        ...
        try {
            ...
            String hsql = "SELECT f.id,"
                    ...
                    + "pr.min_level, "
                    + "pr.on_level " 
                    + " from fixture f, profile_handler ph, profile pr" 
                    + " where f.state =  'COMMISSIONED' "
                    + " and ph.id = f.profile_handler_id "
                    ...
                    + " and pr.id = getProfileByFixtureGroupId( f.id, " + dayOfWeek + ", 0, " + currentMinute + ")"
                    ;
