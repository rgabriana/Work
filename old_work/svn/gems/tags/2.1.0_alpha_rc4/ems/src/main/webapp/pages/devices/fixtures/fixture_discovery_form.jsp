...
<spring:url value="/services/org/fixture/getdiscoverystatus" var="getDiscoveryStatusUrl" scope="request" />
<spring:url value="/services/org/gateway/du/updatenoofsensors/" var="updateNoOfSensorsUrl" scope="request" />
<spring:url value="/services/org/fixture/startnetworkdiscovery/" var="startNetworkDiscoveryUrl" scope="request" />
<spring:url value="/services/org/fixture/getcountbygateway/" var="getCountByGatewayUrl" scope="request" />
<spring:url value="/services/org/fixture/cancelnetworkdiscovery" var="cancelNetworkDiscoveryUrl" scope="request" />
...
<style>
	#fx-discovery-wrapper #fixture-discovery-form{border-bottom: 1px solid #DDDDDD; padding: 10px 20px 5px;}
...
fixturesCountMap = {}; //Map having gatewayid as key and no. of fixtures as value
...
$(document).ready(function() {
	...
	<c:forEach var="fxentry" items="${fixturesCountMap}" varStatus="status">
		fixturesCountMap["${fxentry.key}"] = "${fxentry.value}";
    </c:forEach>
    
 // Hide close button on discovery dialog box to force user through the cancel flow .
...
function loadGatewayData(){
	...
	<c:forEach items="${gateways}" var="gateway">
		$("#fdGatewayCombo").append(new Option("${gateway.gatewayName}", "${gateway.id}"));
		gw_cnt++;
	</c:forEach>
...
function onGatewaySelect(){ 
	...
	$("#noOfDiscovered").val(fixturesCountMap[gatewayId]);
}
...
function startDiscovery(bStatus){
	if(bStatus == 2 || bStatus == 3 || bStatus == 4) {
		displayFxDiscoveryMessage("Starting Discovery. Please wait...", COLOR_SUCCESS);
	} else {
...
function getUpdatedDiscoveryStatus(){
	loadAllFixtures(); // Ensuring that the discoverd fixtures count is fetched first before calling Discovery status
		...
		success: function(data){
			...
			switch(bStatus) {
				case DISC_STATUS_SUCCESS:
					displayFxDiscoveryMessage("All fixtures are discovered.", COLOR_SUCCESS);
...
function loadAllFixtures(){
	...
	$.ajax({
		...
		contentType: "application/json; charset=utf-8",
		success: function(data){
			var resNoOfFixtures = (1 * data.status);
			$("#noOfDiscovered").val(resNoOfFixtures);
			fixturesCountMap[gatewayId] = resNoOfFixtures;
			
			var noOfInstalled = 1 * $("#noOfInstalled").val();

			if(resNoOfFixtures >= noOfInstalled) {
				displayFxDiscoveryMessage("All fixtures are discovered.", COLOR_SUCCESS);
				showCompleteDiscovery();
			}
		}
...
function exitWindow(){
	stopDiscoveryTimer();
	$("#fixtureDiscoveryDialog").dialog("close");
}
...
<div id="fx-discovery-wrapper" style="height:100%; width:100%;">
		<form id="fixture-discovery-form">
			...
			<div class="fieldWrapper">
				<div class="fieldlabel">Number of installed sensors:</div>
				<div class="fieldInput"><input class="text" id="noOfInstalled" name="noOfInstalled"/></div>
				<br style="clear:both;"/>
			</div>
			<div style="height:3px;"></div>
			<div class="fieldWrapper">
				<div class="fieldlabel">Number of discovered sensors:</div>
