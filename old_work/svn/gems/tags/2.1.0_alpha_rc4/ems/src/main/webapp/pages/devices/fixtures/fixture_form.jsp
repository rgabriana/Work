...
// 	$("#updateButton").button();
	...
	function switchFixtureImage(){
		displayRestoreLabelMessage('<spring:message code="fixtureForm.message.restore.waiting"/>', COLOR_DEFAULT);
		$.ajax({
			type: 'POST',
			url: "${switchImagefixtureUrl}",
			data: "<fixture><id>${fixture.id}</id></fixture>",
			success: function(data){
				displayRestoreLabelMessage(data.msg, COLOR_SUCCESS);
 			},
			dataType:"json",
			contentType: "application/xml; charset=utf-8",
		});
	...
	function loadAllProfiles(){
		...
		if(FX_CURRENT_PROFILE_BEFORE_UPDATE == "Custom"){
			//alert("if Custom ");
			var modifiedCurrentProfile = document.getElementById("currentProfile");
			modifiedCurrentProfile.add(new Option("Custom","99"), null);
			PROFILE_DATA["Custom"] = 99;
...
<div id="fixture-dialog-form" >
	...
	<div style="padding-bottom: 5px;">
		<button id="updateButton" onclick="javascript:return saveFixture();" style="float:left;" ><spring:message code="fixtureForm.label.applyBtn"/></button>&nbsp;
				...
				<td width=33%>
					<table cellspacing="0" cellpadding="0">
						<tr>
							...
							<td style="width:300px; padding: 2px 15px;">
	...
	<c:if test="${fn:startsWith(fixture.version, '1') == false}">
		<div class="upperdiv" style="padding-left:5px;">		    
			<table cellpadding="0" cellspacing="0">
				<tr>
					<td>
						<button id="restoreButton" onclick="javascript:return confirmRestore();"><spring:message code="fixtureForm.label.restoreBtn"/></button>
						Restore will boot the fixture with alternate version. &nbsp;		
					</td>
					<td>
						<div id="restore_message" style="font-size: 14px; font-weight: bold; padding: 5px 0 0 10px;"></div>
					</td>
				</tr>
			</table>
		</div>
