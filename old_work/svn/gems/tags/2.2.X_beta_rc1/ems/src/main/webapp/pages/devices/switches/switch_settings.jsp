...
function applySceneToSwitch(sceneName , sceneId){
	
	// This is a global variable and used at some places. do not delete.
	SCENE_ID = sceneId
	resetStatusIcon();
	if(sceneName=="auto"){
		setStatusIconAsOn(sceneName);
		setDimmerState(101);
		...
		setStatusIconAsOn(SCENE_ID);
		setSceneToSwitch(SCENE_ID);
	}
...
function applyDimToSwitch()
{
	 //Makes the dim	 
	 $.ajax({
			type: "POST",
			url: "${applySwitchUrl}"+ SWITCH_ID +"/"+ "action/dimdown/argument" +"/10/"+"?ts="+new Date().getTime(),
			success: function(data){
				if(data != null){
 ...
 //For 2.2 Sensors only
 function applyBrightenToSwitch()
 {
 	 //Makes the brighten 	 
 	 $.ajax({
 			type: "POST",
 			url: "${applySwitchUrl}"+ SWITCH_ID +"/"+ "action/dimup/argument" +"/10/"+"?ts="+new Date().getTime(),
 			success: function(data){
 				if(data != null){
 ...
 {	 	 
	 $.ajax({
			type: "POST",
			url: "${applySwitchUrl}"+ SWITCH_ID +"/"+ "action/auto/argument" +"/0/"+"?ts="+new Date().getTime(),
			success: function(data){
				if(data != null){
...
<tr >
	...
	<button id="autoButton" onclick="setDimmerState(101);">Auto</button>
	</td>
		...
		<td height=auto valign="top">
			...
			<div style="background-color: #999; border-radius: 6px 6px 0 0; color: white; font-weight: bold; padding: 8px;">Scenes</div>
				<c:forEach items='${scenes}' var='scene'>
							...
							<div id="${scene.id}" class="sws-list-wrapper" onclick="applySceneToSwitch('${scene.name}','${scene.id}')">
