...
public class FirmwareUpgradeDao extends BaseDaoHibernate {
	...
	public List<ImageUpgradeDeviceStatus> loadDeviceStatus(Date fromDate,Date toDate) {
		...
		String hsql = "from ImageUpgradeDeviceStatus where startTime > '"+  DateUtil.formatDate(fromDate,"yyyy-MM-dd HH:mm:ss") + "' AND startTime < '" + DateUtil.formatDate(toDate,"yyyy-MM-dd HH:mm:ss") + "'";
        Query q = getSession().createQuery(hsql.toString());              
        oList = q.list();
        return oList;
	}
