...
public class WdsResponsePacket {
    ...
    public void processResponse() {
        ...
        if (oWds == null) {
            logger.error(snapAddr + ": There is no EWS, ignoring the node pkt - " + ServerUtil.getLogPacket(pkt));
            pkt = null;
            return;
        }
        ...
        case ServerConstants.ACK_TO_MSG:
            ...
            if ((pkt[msgStartPos] & 0xFF) == ServerConstants.ISP_INIT_ACK_OPCODE) {		
                ...
                logger.info("(EWS Commissioned): EWS packet Ack for apply switch param.");
                break;
        	  	default:
                break;
        	  	}
