...
public class SwitchFixtureDao extends BaseDaoHibernate {
	...
	public SwitchFixtures loadSwitchFixturebySwitchIdFixtureId(Long sid, Long fid) {
        try {
            List<SwitchFixtures> results = null;
            String hsql = SWITCHFIX_CONTRACTOR + " from SwitchFixtures sf where switchId = ? and fixtureId=?";
            Query q = getSession().createQuery(hsql.toString());
            q.setParameter(0, sid);
            q.setParameter(1, fid);
            results = q.list();
            if (results != null && !results.isEmpty()) {
                return (SwitchFixtures)results.get(0);
            }
        } catch (HibernateException hbe) {
            throw SessionFactoryUtils.convertHibernateAccessException(hbe);
        }
